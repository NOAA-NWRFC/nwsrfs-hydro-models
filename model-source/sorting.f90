module sorting
  ...
  function  rrev(x)
      ! reverse a double precision vector 
      implicit none

      double precision, dimension(:), intent(in):: x
      double precision, dimension(size(x)):: rrev
      ...
      do
        ...
        temp    = rrev(head)           !    otherwise, swap them
        rrev(head) = rrev(tail)
        rrev(tail) = temp
  ...
  function  irev(x)
      ...
      do
        if (head >= tail)  EXIT           !    if head crosses tail, exit
        temp    = irev(head)           !    otherwise, swap them
        irev(head) = irev(tail)
        irev(tail) = temp
        head    = head + 1                !    move forward
        tail    = tail - 1                !    move backward
