...
CGW      SUBROUTINE FKA7(P,C,QB,NDT,COTIME,IB)
      SUBROUTINE FKA7(P,C,QB,QC,NDT,INFL_ST,OUTFL_ST,STOR_ST)
C
C.......................................................................
C
...
CGW      3.5 QC    - OUTPUT - ROUTED (ATTENUATED) OUTFLOW
...
CGW      DIMENSION P(1),C(1),QB(1)
      INTEGER NDT, IB
      REAL QC(NDT),QB(NDT)
CGW   ADDING ARRAYS TO TRACK INFLOW, OUTFLOW, STORAGE STATE
      REAL, DIMENSION(NDT) :: INFL_ST,OUTFL_ST,STOR_ST
      DIMENSION P(*),C(*)
CGW   Adding FTPY help
CGW Cf2py intent(out) QC,INFL_ST,OUTFL_ST,STOR_ST
C
...
CGW        WRITE(MESSAGESTRING, 681) 'DTYPIN from param file =', DTYPIN
CGW	call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)
      END IF 
!CP   WRITE(*,681) 'DTYPIN from param file =', DTYPIN
CFC   CALL FDCODE(DTYPIN,STDUNT,INDIMS,MSNG,NUPDT,TSCALE,NEXTRA,IER)
   
CGW      CALL getDimensionAndUnitInFortran(DTYPIN, INDIMS, IUNIT)
...
CGW      WRITE(MESSAGESTRING,602)IBOS,IBOS1,NPOS,IBOS4,IBOS14,
CGW     1  IBK,NPKQ,IBK1,ITA,XITA,X2,Y1,S2ODT,MEANQ      
CGW      call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)
CGW  602 FORMAT(1H0,10X,47HINITIAL VALUES FOR IBOS,IBOS1,NPOS,IBOS4,IBOS14
CGW     1   ,45H,IBK,NPKQ,IBK1,ITA,XITA,X2,Y1,S2ODT,MEANQ ARE/11X,9I10,
CGW     2    G10.4/11X,3G10.4,2X,L4)
...
CGW      WRITE(MESSAGESTRING,611) (QB(I),I=1,MAXDEBUG)
CGW      call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)
      
CGW  611 FORMAT(1H0,10X,'INFLOW VALUES TO K OPERATION (QB ARRAY)'/
CGW     1  (1X,10G10.4))
      IEOS1=IBOS4-1
      DO 622 I=IBOS1,IEOS1
CFC      WRITE(IODBUG,612)NPOS,(P(I),I=IBOS1,IEOS1)
CGW        WRITE(MESSAGESTRING,612)NPOS,P(I)
CGW        call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)
  622 CONTINUE    
CGW  612 FORMAT(1H0,10X,'THE O VS 2*S/DT+O TABLE FOLLOWS - THE NUMBER OF ',
CGW     1  'PAIRS OF VALUES IS ',I3/(11X,8G12.4))
C
      DO 12 I=1,NPKQ
...
CGW      WRITE(MESSAGESTRING,613)NPOS4,P(I)
CGW      call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)
  623 CONTINUE
CGW  613 FORMAT(1H0,10X,'THE O VS 2*S/(DT/4)+O TABLE FOLLOWS - THE ',
CGW     1  'NUMBER OF PAIRS OF VALUES IS ',I3/(11X,8G12.3))
CFC      END IF
C.......................................................................
C
...
CGW        WRITE(MESSAGESTRING,614)VALUE
CGW        call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)
      END IF       
CGW  614 FORMAT(11X,'IN DO 10 LOOP, VALUE = ',G10.4)
C
      IF(MEANQ)GO TO 9
C
      IF(CONK)GO TO 56
...
CGW        WRITE(MESSAGESTRING,603)X1,X2,VALUE,Y2,XK1,XK2
CGW        call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)
      END IF   
CGW  603 FORMAT(11X,40HIN DO 10 LOOP, X1,X2,VALUE,Y2,XK1,XK2 = ,6G10.4)
C
...
CFC      IF(LBUG)WRITE(IODBUG,605)NPOS4,S2ODT,DX
      IF(FEWSDEBUG .GE. 1) THEN
CGW        WRITE(MESSAGESTRING,605)NPOS4,S2ODT,DX
CGW        call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)
      END IF      
CGW  605 FORMAT(11X,41HABOUT TO ENTER DO 5 LOOP - TIME INTERVAL ,
CGW     1   06H= DT/4/11X,17HNPOS4,S2ODT,DX = ,I10,2G10.4)
C
      DXOVR4=DX/4.
      XITAO2=XITA/2.
C
...
CGW        WRITE(MESSAGESTRING,609)Y1,Y2,XK14,XK24,X2
CGW        call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)
      END IF   
CGW  609 FORMAT(11X,45HK LT DT/8 IN DO 5 LOOP - SET Y2=MIN(X2,VALUE)/
CGW     1   11X,21HY1,Y2,XK14,XK24,X2 = ,5G10.4)
      Y2=X2
      IF(VALUE.LT.Y2)Y2=VALUE
C
    6 S2ODT=VALUE - Y2
...
C
      Y1=Y2
C
CGW   STORE INFLOW, OUTFLOW, STORAGE STATE FOR TIMESTEP
      INFL_ST(I)=X2
      OUTFL_ST(I)=Y2
...
CFC      IF(LBUG)WRITE(IODBUG,608)C(2),C(3),C(4)
      IF(FEWSDEBUG .GE. 1) THEN
CGW        WRITE(MESSAGESTRING,608)C(2),C(3),C(4)
CGW        call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)
      END IF      
CGW  608 FORMAT(11X,36HAFTER DO 10 LOOP - C(2),C(3),C(4) = ,
CGW    1   3G10.4)
C
...
CGW         WRITE(MESSAGESTRING,601)(QB(I),I=1,MAXDEBUG)
CGW         call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)
      END IF      
CGW  601 FORMAT(11X,14HIN FKA7 - QB =/(10X,14F8.2))
C
...
CGW  610 FORMAT(1H0,10X,46H**WARNING**  IN SUBROUTINE FKA7, THE VALUE OF
CGW     1   ,26HS20DT HAS GONE BELOW -0.5 ,I10,07H TIMES./
CGW     2  24X,56HTHIS INDICATES A POSSIBLE ERROR IN THE LAG/K PARAMETERS.)
CFC      CALL WARN
C
      RETURN
      END
