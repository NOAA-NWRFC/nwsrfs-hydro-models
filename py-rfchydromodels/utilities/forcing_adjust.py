...
def adj_climo_map_pet(aorc_climo, px_adj, p_redist, std, shift, upper_limit, lower_limit):
    ...
    for site in sites:
        ...
        working_line=pd.concat([working_line.iloc[-1:].shift(-365, freq='d'),working_line,working_line.iloc[:1].shift(365, freq='d')], axis=0)
        # Shift the dataframe by the specified number of days.  Also add interpolated entries to the 15th of each month using the shifted dates. Remove the extra Sep/Oct values added in the previous line
        working_line_shift=pd.concat([pd.DataFrame(index=working_line.index),working_line.shift(shift, freq='d')], axis=0).sort_index().interpolate(method='time').loc['2098-10-15':'2099-9-15']
        # Only keep the interpolated dates on the 15th, remove any potential duplicate rows from the shift
        working_line_shift=working_line_shift.loc[working_line_shift.index.day==15].drop_duplicates().dropna()     
        
        # Enforce supplemental monthly data limits##
        
        # Check to see if any of the updated climo values violate the set thresholds.  If they do, then set to threshold
        upper_limit_exceeded=working_line_shift[site+'_redistributed'] > upper_limit[site]
        working_line_shift.loc[upper_limit_exceeded,[site+'_redistributed']]=upper_limit.loc[upper_limit_exceeded,[site]].values
        lower_limit_exceeded=working_line_shift[site+'_redistributed'] < lower_limit[site]
        working_line_shift.loc[lower_limit_exceeded,[site+'_redistributed']]=lower_limit.loc[lower_limit_exceeded,[site]].values
...
##Tool for MAP and PET adjustments
#note that this approach is a slight variation to the MAP/PET approach.  Adjustment are divided into two.  Adjustments are made 
#independently partitioned to the largest 6 months and independently partioned to the 6 smallest months 
def adj_climo_mat_ptps(aorc_climo, px_adj,p_redist,std,shift,upper_limit,lower_limit):
    
    #use 1/2 of a normal distribution to control the weighting.  Use two curve to account for the separation of the 6 smallest/largest
    x = np.linspace(1, 6, 6)
    weight=stats.norm.pdf(x, 1, std)
    ...
    for site in aorc_climo.columns.array:  
        ...
        #get the total temperature to redistribute above the median and below median
        below_median_sum=working_line.iloc[0:6,working_line.columns.get_loc(site+'_Deviation')].multiply(p_redist).sum()
        above_median_sum=working_line.iloc[6:12,working_line.columns.get_loc(site+'_Deviation')].multiply(p_redist).sum()
        ...
        #Create a column of the updated climo which is: the untouched_mthly_climo +PLUS+ total_climo_to_be_redistributed*weighting_distribution
        working_line[site+'_redistributed']=working_line.loc[:,[site]].median().values+working_line[site+'_remaining_orig']+working_line[site+'_distribute']*working_line[site+'_weighting']
        #Resort the dataframe by date
        working_line.sort_index(inplace=True)
        ...
        #Check to see if any of the updated climo values violate the set thresholds.  If they do, then set to threshold
        upper_limit_exceeded=working_line_shift[site+'_redistributed'] > upper_limit[site+'_redistributed']
        working_line_shift.loc[upper_limit_exceeded,[site+'_redistributed']]=upper_limit.loc[upper_limit_exceeded,[site+'_redistributed']].values
        lower_limit_exceeded=working_line_shift[site+'_redistributed'] < lower_limit[site+'_redistributed']
        working_line_shift.loc[lower_limit_exceeded,[site+'_redistributed']]=lower_limit.loc[lower_limit_exceeded,[site+'_redistributed']].values
