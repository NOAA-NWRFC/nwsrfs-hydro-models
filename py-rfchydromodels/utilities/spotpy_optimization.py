...
class ModelWrapper:
    ...
    def __init__(self,
        ...
        self.model = Model(forcings,pars,obs)
        self.model.run()
        self.sim_flow_daily = self.model.sim_flow_cfs.resample('D').mean()
        self.obs_flow_daily = obs.resample('D').mean()['flow_cfs']

    def get_objectives(self, objectives=None):
    ...
    def rmse(self):
        return spotpy.objectivefunctions.rmse(self.sim_flow_daily,self.obs_flow_daily)

    def mae(self):
        return spotpy.objectivefunctions.mae(self.sim_flow_daily,self.obs_flow_daily)

    def log(self):
        return np.sum((np.log(self.sim_flow_daily + 1) - np.log(self.obs_flow_daily + 1)) ** 2)
...
class spot_setup(object):
    ...
    def get_objectives(self,sim,obs):
