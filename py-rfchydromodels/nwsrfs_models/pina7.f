...
CGW     	WRITE(MESSAGESTRING,*) 'ENTER PINA7'
...
CGW 1100 IF(FEWSDEBUG .GE. 1) THEN
CGW      	WRITE(MESSAGESTRING,603) IP
CGW ADDED SINGLE LINE BELOW
 1100 CONTINUE
CGW	call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)       
CFC	 WRITE (IOBUG,603) IP
CGW  603 FORMAT('USED SPACES IN THE P ARRAY ', I5)
...
CGW	    call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)	    
CGW  602 FORMAT(' FOR ONE QUARTER OF THE ORIGINAL TIME INTERVAL ',
CGW     1   ' THE NUMBER OF PAIRS OF O AND 2*S/DT+O VALUES = ',I3)
CFC            CALL PROS7 (P(IBOS1),NPOS,IODBUG)
        ENDIF
CFC         WRITE (IODBUG,604) (P(I),I=1,IP)
        do 644 I=1,IP
CGW        WRITE(MESSAGESTRING,604) P(I)
CGW	 call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)	 
  644    continue 
CGW  604 FORMAT (' THE P ARRAY CONTAINS: ' / (1X,10G10.4))
CGW	 ENDIF
