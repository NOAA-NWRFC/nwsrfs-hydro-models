...
class sacsnowuh_udp(object):
    ...
    def get_bounds(self):
        ...
        for pname in self.limits['name']:
            lower.append(self.limits[self.limits['name'] == pname]['lower'].to_numpy()[0])
            upper.append(self.limits[self.limits['name'] == pname]['upper'].to_numpy()[0])
