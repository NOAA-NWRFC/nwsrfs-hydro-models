...
cCP   CALL FPRBUG(SNAME,1,24,IBUG)
      IF (FEWSDEBUG.GT.3) THEN
         WRITE(MESSAGESTRING, *) 'Enter EX24 -'
...
C
cCP   WRITE(IODBUG,900) LPO,LCO
      WRITE(MESSAGESTRING,900) LPO,LCO
      call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)
  900 FORMAT(1H0,'CONTENTS OF THE PO AND C ARRAYS FOR API-CONT',
     -5X,'NUMBER OF VALUES--PO=',I3,2X,'CO=',I3)
C
cCP   WRITE(IODBUG,901) (PO(I),I=1,LPO)
      WRITE(MESSAGESTRING, 901) (PO(I),I=1,LPO)
      call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)
   
  901 FORMAT(1H0,15F8.3)
cCP   WRITE(IODBUG,901) (CO(I),I=1,7)
      WRITE(MESSAGESTRING, 901) (CO(I),I=1,7)
      call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)

cCP   WRITE(IODBUG,920)
      WRITE(MESSAGESTRING,920)
      call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)
  920 FORMAT(1H0,'CONTROL VARIABLES, POINTERS, AND TIME INTERVALS')
C
cCP   WRITE(IODBUG,921) ITP,IVOPT,IPRINT,LRSPM,LRGPM,IFRZE,NOFRZE,
      WRITE(MESSAGESTRING,921) ITP,IVOPT,IPRINT,LRSPM,LRGPM,IFRZE,
     - NOFRZE,IR,IRS,LRG,LSUMS,IPRSAC
      call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)
  921 FORMAT(1H0,12I10)
C
cCP   WRITE(IODBUG,921) LPE,LSC,ITSC,LWE,ITWE,LTA,ITTA,
      WRITE(MESSAGESTRING,921) LPE,LSC,ITSC,LWE,ITWE,LTA,ITTA,
     -  LAI,ITAI,LAPI,ITAPI
      call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)
C
cCP   WRITE(IODBUG,921) LFRZE,ITFI,LAPIC,ITAPIC,LAETI,ITAETI,
      WRITE(MESSAGESTRING,921) LFRZE,ITFI,LAPIC,ITAPIC,LAETI,ITAETI,
     -  LFRS,ITFEI
      call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)
C
cCP   WRITE(IODBUG,921) IDA,IHR,LDA,LHR,LDACPD,LHRCPD,IDADAT,
      WRITE(MESSAGESTRING,921) IDA,IHR,LDA,LHR,LDACPD,LHRCPD,IDADAT,
     -  MAINUM,IFFG,IFILLC,NCSTOR
      call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)
C   
cCP   IF ((IVOPT.EQ.1).AND.(NDT35.GT.0)) WRITE(IODBUG,923)
      IF ((IVOPT.EQ.1).AND.(NDT35.GT.0)) WRITE(MESSAGESTRING,923)
     -  NDT35,(IDT35(I),VAL35(I),I=1,NDT35)
      call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)
  923 FORMAT(1H0,'MOD135 DEBUG',I5,5(I7,F7.2))
C
      IF (NACV.EQ.0) GO TO 100
cCP   WRITE(IODBUG,924) NACV
      WRITE(MESSAGESTRING,924) NACV
      call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)
  924 FORMAT(1H0,'FCOAPI COMMON BLOCK CONTENTS -- NACV=',I3)
C  
      DO 90 N=1,NACV
cCP     WRITE(IODBUG,925) JHAPI(N),(APICO(I,N),I=1,4),BFX(N),
        WRITE(MESSAGESTRING,925) JHAPI(N),(APICO(I,N),I=1,4),BFX(N),
     -  (FGCO(I,N),I=1,2)
        call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)
...
C*************************************************************
C
cCP   IF (IBUG.EQ.1) IPRINT=1
      IF (FEWSDEBUG.GE.1) IPRINT=1
...
C
cCP   103 IOUT=IPR
cCP   IF (IBUG.EQ.1) IOUT=IODBUG
cCP   IF((IBUG.EQ.0).AND.(MAINUM.GT.1)) WRITE (IOUT,902)
C
cCP   902 FORMAT(1H1)
C
 103  CONTINUE
cCP   WRITE(IOUT,903) (PO(I),I=2,6),MONTH,IYEAR,TZCODE,UNIT(METRIC+1)
      WRITE(MESSAGESTRING,903) (PO(I),I=2,6),MONTH,IYEAR,TZCODE,
     >                          UNIT(METRIC+1)
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
  903 FORMAT(1H0,'DETAILED API-CONT OUTPUT FOR',1X,5A4,5X,I2,'/',
     -I4,3X,'TIME ZONE=',A4,10X,'UNITS ARE',1X,A4)
C
cCP   WRITE(IOUT,904) VAR1(IVOPT+1)
      WRITE(MESSAGESTRING,904) VAR1(IVOPT+1)
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
...
cCP   CALL CHCO24(IDA,KPHR,NOUTZ,NOUTDS,I,IFRZE,IPRINT,IOUT,APIX,SMIX,
cCP   -  IBUG)
      CALL CHCO24(IDA,KPHR,NOUTZ,NOUTDS,I,IFRZE,IPRINT,APIX,SMIX)
  136 CONTINUE
C
C***************************************
...
C
      ...
      KHR=IHR
      KOFF=KDA-IDADAT
CP    IF (MAINUM.NE.1) GO TO 150
c      IF (IFFG.EQ.0) GO TO 150
C     CALCULATE THE COMPUTATIONAL PERIOD AT OR JUST BEFORE LSTCMPDY
c      KDAFFG=LDACPD
c      KHRFFG=(LHRCPD/ITP)*ITP
c      IF (KHRFFG.GT.0) GO TO 150
c      KDAFFG=KDAFFG-1
c      KHRFFG=24
c  150 IC=1
      IC=1
      ICAEI=1
cCP   IF (IBUG.EQ.0) GO TO 200
      IF (FEWSDEBUG.EQ.0) GO TO 200
cCP   WRITE(IODBUG,922)
      WRITE(MESSAGESTRING,922)
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
  922 FORMAT(1H0,'INITIAL TIMING VARIABLES')
  
cCP   WRITE(IODBUG,921) IDA,IHR,IDADAT,KDA,KHR,KOFF,
      WRITE(MESSAGESTRING,921) IDA,IHR,IDADAT,KDA,KHR,KOFF,
...
C
  220 CALL APIC24(P,RS,FRS,AI,RG,R,PE,AESC,TA,TAVG,WE,AIADJ,KHR,MONTH,
     -            IDAY,IHOUR,IVOPT,IFRZE,LWE,LSC,IPRINT,ITP)
cCP       -IDAY,IHOUR,IVOPT,IFRZE,LWE,LSC,IPRINT,IOUT,ITP,IBUG)
C
C**************************************************
C     CHECK FOR CHANGES TO CARRYOVER
C**************************************************
C
CP    IF (MAINUM.NE.1) GO TO 223
      IF (NM.EQ.NACV) GO TO 223
      JH=JH+ITP
      DO 221 I=1,NACV
      IF (JH.NE.JHAPI(I)) GO TO 221
      NM=NM+1
cCP   CALL CHCO24(KDA,KHR,NOUTZ,NOUTDS,I,IFRZE,IPRINT,IOUT,APIX,SMIX,
cCP   -  IBUG)
      CALL CHCO24(KDA,KHR,NOUTZ,NOUTDS,I,IFRZE,IPRINT,APIX,SMIX)
  221 CONTINUE
C
C*******************************
...
CP    IF (MAINUM.NE.1) GO TO 248
c      IF (IFFG.EQ.0) GO TO 248
...
C
  249 CTEMP(1)=API
      ...
      CTEMP(6)=FI
      CTEMP(7)=FEI
cCP   CALL FCWTCO(KDA,KHR,CTEMP,LCO)
      IC=IC+1
C
C**********************************************
...
CP    IF (MAINUM.NE.1) GO TO 310
c     IF (IFFG.NE.0) GO TO 350
  310 CO(1)=API
      CO(2)=SMI
      CO(3)=BFSC
      CO(4)=BFI
...
C**********************
C
cCP   390 IF (ITRACE.LT.1) GO TO 399
 390  IF (FEWSDEBUG.EQ.0) GO TO 399
C
cCP   WRITE (IODBUG,905) SNAME
      WRITE(MESSAGESTRING,905) 
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
  905 FORMAT(1H0,'**EXIT',1X,' EX24')
C
  399 RETURN
      END
