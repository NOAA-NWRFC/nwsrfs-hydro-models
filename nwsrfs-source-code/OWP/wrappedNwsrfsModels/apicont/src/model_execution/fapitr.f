...
cCP   COMMON/FDBUG/IODBUG,ITRACE,IDBALL,NDEBUG,IDEBUG(20)
cCP   COMMON/WHERE/ISEG(2),IOPNUM,OPNAME(2)
      COMMON/FAPICO/NAPICS,APICOS(12,10)
      COMMON/FCOAPI/NACV,JHAPI(10),APICO(4,10),BFX(10),FGCO(2,10)
cCP   COMMON/FCTIME/IDARUN,IHRRUN,LDARUN,LHRRUN,LDACPD,LHRCPD,NOW(5),
cCP   1 LOCAL,NOUTZ,NOUTDS,NLSTZ,IDA,IHR,LDA,LHR,IDADAT
C
...
C
C     DATA STATEMENTS
cCP   DATA SNAME/4HFAPI,4HTR  /
      CHARACTER*8 OPNAME
      DATA JBUG,KBUG/4HMODS,4HAPIC/
      DATA BLANK/4H    /
C.......................................
C     TRACE LEVEL=1, DEBUG SWITCH=IBUG.
cCP   CALL FPRBUG(SNAME,1,1,IBUG)
cCP   IF(IFBUG(JBUG).EQ.1) IBUG=1
cCP   IF(IFBUG(KBUG).EQ.1) IBUG=1
      IF (FEWSDEBUG.GT.3) THEN
         WRITE(MESSAGESTRING, *) 'Enter FAPITR'
...
cCP   90 IF(IBUG.EQ.0) GO TO 100
 90   IF (FEWSDEBUG.EQ.0) GO TO 100
cCP   WRITE(IODBUG,900) NAPICS
      WRITE(MESSAGESTRING,900) NAPICS
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
  900 FORMAT(1H0,46HFAPITR DEBUG--CONTENTS OF FAPICO COMMON BLOCK.,2X,
     1  7HNAPICS=,I5)
C  
      DO 91 N=1,NAPICS
cCP   WRITE(IODBUG,901)(APICOS(I,N),I=1,12)
      WRITE(MESSAGESTRING,901)(APICOS(I,N),I=1,12)
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING) 
...
C     RETRIEVE SMIX.
      LPM=PO(26)
      SMIX=PO(LPM+6)
cCP   IF(IBUG.EQ.1) WRITE(IODBUG,902)IDA,IHR,LDA,LHR,IDT,JH1,JH2,SMIX
      IF (FEWSDEBUG.GT.0) THEN
      WRITE(MESSAGESTRING,902)IDA,IHR,LDA,LHR,IDT,JH1,JH2,SMIX
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
...
cCP   IF(IBUG.EQ.0) RETURN
      IF (FEWSDEBUG.EQ.0) RETURN
cCP   WRITE(IODBUG,903) NACV
      WRITE(MESSAGESTRING,903) NACV               
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
  903 FORMAT(1H0,46HFAPITR DEBUG--CONTENTS OF FCOAPI COMMON BLOCK.,2X,
     1  5HNACV=,I5)
      DO 110 N=1,NACV
cCP     WRITE(IODBUG,904)JHAPI(N),(APICO(I,N),I=1,4),BFX(N),
        WRITE(MESSAGESTRING,904)JHAPI(N),(APICO(I,N),I=1,4),BFX(N),
     1  (FGCO(I,N),I=1,2)
        call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
