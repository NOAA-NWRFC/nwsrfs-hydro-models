...
C
     ...
     &         4HL3/T,4HL3/T,4HL3  /
      DATA BLANK/4H    /
C
C      DATA TSIO/1.01,1.01,1.01,1.01,1.01,0.01,0.01,0.01,1.01,1.01,
C UPDATED TSIO DATA STATEMENT POS 9 and 10 (TRIBQL1 and TRIBQL2) are inputs 0.01
       DATA TSIO/1.01,1.01,1.01,1.01,1.01,0.01,0.01,0.01,0.01,0.01,
     & 1.01,1.01,1.01/
C
...
cc      IF(ID.GE.6 .AND. ID.LE.10) MSGVAL=1
      MSGVAL=1.01-TSIO(ID)
C LC
C COMMENT OUT CALL TO CHEKTS, FDCODE AND CHECK OF DIMENSIONS
C      CALL CHEKTS(TSID(1,ID),DTYPE(ID),IDT(ID),ICHK,DCODE,
C     & MSGVAL,0,IERCK)
C      IF (IBUG.GE.2) WRITE(IODBUG,1666) ICHK
C 1666 FORMAT('  VALUE OF ICHK = ',I10)
C
C      IF (IERCK.NE.0) THEN
...
C
C      IX = ID
C      CALL FDCODE(DTYPE(ID),UCODE,DCODE,IX,IX,X,IX,IERD)
C
C      IF (ID.EQ.9 .OR. ID.EQ.10) THEN
C      IF (UCODE.EQ.UC(ID) .OR.UCODE.EQ.UC(8)) GO TO 320
C      ENDIF
C
C      IF (UCODE.NE.UC(ID)) THEN
C        CALL STRN51(93,1,UCODE,1)
C        TSOK(ID) = .FALSE.
C        GO TO 10
C      END IF
C
