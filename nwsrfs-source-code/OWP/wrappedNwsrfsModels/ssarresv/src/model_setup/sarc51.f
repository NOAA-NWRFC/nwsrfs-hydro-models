...
C@PROCESS LVL(77)
C
      SUBROUTINE SARC51(WK,IUSEW,LEFTW,NCSAR)
C
C DESC READ AND STORE CARRYOVER VALUES
C FOR SAR SUB-SECTIONS OF OPERATION 51
C-------------------------------------------------------------------
C  ARGS:
...
C     NCSAR - NUMBER OF WORDS NEEDED TO STORE CO INFO
C-------------------------------------------------------------------
C
...
C    ================================= RCS keyword statements ==========
      ...
      DATA             RCSKW1,RCSKW2 /                                 '
     .$Source: /fs/hseb/ob72/rfc/ofs/src/fcinit_ssarresv/RCS/sarc51.f,v $
     . $',                                                             '
     .$Id: sarc51.f,v 1.1 1996/03/21 14:35:56 page Exp $
     . $' /
C    ===================================================================
C
C
      DATA KEYWDS/
     &   4HQ-IN,4HST  ,4HPOOL,4H    ,4HSTOR,4HAGE ,4H    ,4H    ,
...
C
      IF(IFMSNG(VAL(2)).EQ.1) THEN
        CALL TERP26(VAL(3),VAL(2),STORGE,ELSTOR,NSE,IFLAG,FEWSDEBUG)
        GO TO 350
      ENDIF
C
...
C
      IF(IFMSNG(VAL(3)).EQ.1) THEN
        CALL TERP26(VAL(2),VAL(3),ELSTOR,STORGE,NSE,IFLAG,FEWSDEBUG)
        GO TO 350
      ENDIF
C
