...
C@PROCESS LVL(77)
C
      SUBROUTINE UPB51(WK,IUSEW,LEFTW,NPUPB,NTUPB,NCUPB,OKUPB)
C
...
C   NPUPB - NO. OF WORDS NEEDED TO HOLD UPERBKWR PARM INFO
C   NTUPB - NO. OF WORDS NEEDED TO HOLD UPERBKWR TS INFO
C   NCUPB - NO. OF WORDS NEEDED TO HOLD UPERBKWR CARRYOVER INFO
C   NRUPB - NO. OF WORDS NEEDED TO HOLD UPERBKWR REGULATION CODE INFO
C   OKUPB  - LOGICAL VARIABLE TELLING WHETHER ANY MISTAKES OCCURRED IN
C            UPERBKWR  SECTION INPUT.
C
C-----------------------------------------------------------------------
C
...
C
     ...
     .LKYUNT(4),ENDUPB(2),UNITC(4),DIM(4),WK(*),IKYENG(4),
     . LKYENG(4),IG(4)
C
      LOGICAL OKUPB ,PRINT1,PRINT2,USET
...
C    ================================= RCS keyword statements ==========
      ...
      DATA             RCSKW1,RCSKW2 /                                 '
     .$Source: /fs/hseb/ob72/rfc/ofs/src/fcinit_ssarresv/RCS/upb51.f,v $
     . $',                                                             '
     .$Id: upb51.f,v 1.1 1996/03/21 14:41:39 page Exp $
     . $' /
C    ===================================================================
C
C
...
C
      ...
      IERR = 0
C
C  SET OFFSET FOR START OF INFO IN UPERBKWR SECTION
C
      NGOFF = IUSEW
C
      DO 3 I =1,4
...
C
C--------------------------------------------------------------------
C  NOW PROCESS INPUT UP TO 'ENDUPERB ', LOOKING FOR, IN ORDER, KEYWORDS
...
C
      ...
      NCARD = 0
C
  100 IF (NCARD .GE. LAUPB ) GO TO 900
      NUMFLD = 0
      CALL UFLD51(NUMFLD,IERF)
      IF (NCARD .GE. LAUPB ) GO TO 900
...
C
C  SET POINTER FOR START OF UPERBKWR PARM INFO AND REFILL POSITION IN
C  WORK ARRAY WITH THIS VALUE.
C
      PUPBL = IUSEW + 1.01
      LOCX = NGOFF + 1
      CALL RFIL51(WK,LOCX,PUPBL)
C
      CALL SABP51(WK,IUSEW,LEFTW,NPUPB)
C
C  NOW REFILL THE WORD IN WORK WITH THE NUMBER OF WORDS NEEDED TO STORE
C  THE UPERBKWR PARM INFO.
C
      PUPBN = NPUPB + 0.01
      LOCX = NGOFF + 2
      CALL RFIL51(WK,LOCX,PUPBN)
C
      GO TO 100
C
C-----------------------------------------------------------------------
...
C
      TUPBL = IUSEW + 1.01
      LOCX = NGOFF + 3
      CALL RFIL51(WK,LOCX,TUPBL)
C
      CALL SART51(WK,IUSEW,LEFTW,NTUPB)
C
C  SET THE NUMBER OF WORDS NEEDED TO STORE THE TIME SERIES INFO
C
      TUPBN = NTUPB + 0.01
      LOCX = NGOFF + 4
      CALL RFIL51(WK,LOCX,TUPBN)
      GO TO 100
C
C------------------------------------------------------------------
...
606   FORMAT(/10X,'**ERROR** IN UPB51 NO PARMS DEFINED SO CANNOT',
...
C
      CUPBL = IUSEW + 1.01
      LOCX = NGOFF + 5
      CALL RFIL51(WK,LOCX,CUPBL)
C
      CALL SABC51(WK,IUSEW,LEFTW,NCUPB)
C
C  RESET THE NUMBER OF WORDS TO STORE CARRYOVER
C
      CUPBN = NCUPB + 0.01
      LOCX = NGOFF + 6
      CALL RFIL51(WK,LOCX,CUPBN)
C
      GO TO 100
C
C--------------------------------------------------------------------
...
C
C  SET POINTER FOR START OF UPERBKWR REGULATION CODE INFO AND REFILL
C  POSITION IN WORK ARRAY WITH THIS VALUE.
C
      RUPBL = IUSEW + 1.01
      LOCX = NGOFF + 7
      CALL RFIL51(WK,LOCX,RUPBL)
C
      CALL SABP51(WK,IUSEW,LEFTW,NRUPB)
C
C  NOW REFILL THE WORD IN WORK WITH THE NUMBER OF WORDS NEEDED TO STORE
C  THE UPERBKWR REGULATION CODE INFO.
C
      RUPBN = NRUPB + 0.01
      LOCX = NGOFF + 8
      CALL RFIL51(WK,LOCX,RUPBN)
C
      GO TO 100
C
C--------------------------------------------------------------
...
C
 ...
 1000 CONTINUE
      ...
      OKUPB  = .FALSE.
C
C  POSITION TO START OF UPERBKWR SECTION INPUT AND PRINT INPUT WITH
C  LINE NUMBERS.
C
        WRITE(MESSAGESTRING,770)
	call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)
 770    FORMAT(1H1,' *** UPERBKWR SECTION INPUT FOR THIS OPERATION ',
     .  'DEFINITION ***'//)
C
      IGSTRT = LUPB +1
      CALL POSN26(MUNI51,IGSTRT)
