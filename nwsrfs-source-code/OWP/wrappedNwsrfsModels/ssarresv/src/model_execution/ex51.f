...
C
C LC  SET FEWSDEBUG FOR DEBUG PURPOSES
        
      
      LOC=PO(12)
      NTS=PO(LOC)
   
C      write(*,*)'number of time series = ',NTS
      PINDEX=PO(12)
      do i=1,NTS
C         write(*,*)'array location =',IDPT(i)
C	 WRITE(*,*)'IN OR OUT  = ',PO(PINDEX+5)
	 PINDEX=PINDEX+5
      enddo
...
C
 ...
 1610 FORMAT('   ** INST. INFLOW TIME-SERIES AT PERIOD START:')
     ...
     + 1,8, formatstr,LDQI1,LDQI1+NUM-1,1,D)
     
 1440 write(MESSAGESTRING,1620)
      call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)
 1620 FORMAT('   ** INST. INFLOW TIME-SERIES AT PERIOD END:')
      write(formatstr,*)'%10.1f'
      call logonedimensionarrayfromfortran(DEBUG_LEVEL, NULLLINE,
     + 1,8,formatstr,LDQI2,LDQI2+NUM-1,1,D)      
...
1300  CONTINUE
      ...
      WRITE(MESSAGESTRING,1630) RESTYP(IRTYP),
     1 (D(LDOEL+I-1),I=1,NPEL)
      call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)
 1630 FORMAT(/'   ** OBSERVED POOL ELEVATION TIME-SERIES FOR ',A8,
     & 'BACKWATER RESERVOIR :'//,(8F10.2))     
      WRITE(MESSAGESTRING,810) NERRD,ELVMIN,AUNIT(METRIC+1)
C THE ERRD IS NOT WRITTEN OUT; NEED TO LOG USING CHECK OF MAXLINES
C     &(ERRD(I),I=1,NERRD)
...
C
      ...
      D(LOCERR(I))=ERRD(I)
1900  CONTINUE
      IF (FEWSDEBUG.LT.2 .OR. LDOEL.LE.0) GO TO 2000
      WRITE(MESSAGESTRING,1640)
      call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)      
1640  FORMAT('   ** POOL ELEVATION TIME SERIES RESTORED :')
      write(formatstr,*)'%10.2f'
      call logonedimensionarrayfromfortran(DEBUG_LEVEL, NULLLINE,
     + 1,8, formatstr,LDOEL,LDOEL+NPEL-1,1,D)
2000  CONTINUE
C
C-------------------------------------------------------------------
