...
C
      NPA(L+L1) = NVAL*4+27
C
      IF (FEWSDEBUG.LT.2 .OR. LUSE.LE.71) GO TO 156
      IRTYP=ISTYP+1      
      WRITE(MESSAGESTRING,1590) RESTYP(IRTYP)
      call logfromfortran(DEBUG_LEVEL,MESSAGESTRING) 
 1590 FORMAT(//,' *** FCN ARRAY FOR ',A8,'BACKWATER RESERVOIR ',
     & 'IN XWKS51 ***')
      ...
      IE = I+7
      IFCN2 = IE-71      
      WRITE(MESSAGESTRING,1605) IFCN1,IFCN2
      call logfromfortran(DEBUG_LEVEL,MESSAGESTRING) 
      WRITE(MESSAGESTRING,1610) (WK(L+J),J=I,IE)
      call logfromfortran(DEBUG_LEVEL,MESSAGESTRING) 
  155 CONTINUE
 156  CONTINUE
      LPP = LPP + NVAL*3 + 1
...
C  STORE MAXIMUM TRIBUTARY FLOW TO SHUT DOWN RESERVOIR RELEASE
      ...
      LPP = LPP + 1
C
      IF (FEWSDEBUG.LT.2 .OR. LUSE.LE.0) GO TO 190
      IRTYP = ISTYP+1
      WRITE(MESSAGESTRING,1600) RESTYP(IRTYP)
      call logfromfortran(DEBUG_LEVEL,MESSAGESTRING) 
...
C
      NPA(L+1) = LUSE
C
      IF (FEWSDEBUG.LT.2 .OR. LUSE.LE.0) GO TO 390
      WRITE(MESSAGESTRING,1601) 
      call logfromfortran(DEBUG_LEVEL,MESSAGESTRING) 
...
C dws     statement to avoid compiler warnings ... 2006-01-23
      ...
      NTTS = PO(NUPO12)
      NTS = (NTTS-2)/NRES-5
C LC change all references to NDD*NTIM24 to NDD - NDD now holds total number of data values for each ts
C      NDO = NTS*NDD*NTIM24
ccav      NDO = NTS*NDD
      NDO = NTS*NTIMESTEPS
      DO 500 II=1,NRES
C
...
C
 600  CONTINUE
C
      IF(FEWSDEBUG.LT.2) GO TO 5006
      DO 5005 I=1,NRES
         IZ=RRC(1,I)
         IF (IZ.LE.7) GOTO 5005
         WRITE(MESSAGESTRING,5010) (RRC(M,I),M=1,7)
	 call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)    
5010     FORMAT(//5X,'RRC =',7F10.1)
         WRITE(MESSAGESTRING,5015) (RRC(M,I),M=8,IZ)
	 call logfromfortran(DEBUG_LEVEL,MESSAGESTRING) 
...
C STORE POINTER FOR TIME SERIES WORK SPACE
      LOCWS(NRES+3) = LRCW+1
C
      IF (FEWSDEBUG.LT.2 .OR. NRRCT.LE.0) GO TO 9000
      NST = NPA(25)
      NND = NST+NRRCT
 ...
 1620 FORMAT(//,' *** WORK ARRAY FOR REGULATION OPTION ***')
      ...
      WRITE(MESSAGESTRING,1605) I,IE
      call logfromfortran(DEBUG_LEVEL,MESSAGESTRING) 
      WRITE(MESSAGESTRING,1611) (NPA(J),J=I,IE)
      call logfromfortran(DEBUG_LEVEL,MESSAGESTRING) 
...
C
 9000 CONTINUE
      LWK = PO(16)
      KWS = NRES+2
      IF (FEWSDEBUG.LT.2) GO TO 850
      WRITE(MESSAGESTRING,1690) LUSE,LWK
      call logfromfortran(DEBUG_LEVEL,MESSAGESTRING) 
 1690 FORMAT(//1X,'NO. OF WORDS OF WORK SPACE USED= ',I8,
     & 5X,'NO. OF WORDS AVAILABLE, PO(16) = ',I8,
     & /1X,'START LOCATIONS FOR REQUIRED WORK SPACE:')
      IF(NRES.EQ.1) THEN
         WRITE(MESSAGESTRING,1691) (LOCWS(K),K=1,KWS),NST
	 call logfromfortran(DEBUG_LEVEL,MESSAGESTRING) 
 1691    FORMAT(5X,'TRIBU-LAKE-PARM, TIME-SERIES, CARRYOVER,',
     & ' REGULATION-INFORMATION = ',5I8)
      ENDIF
      IF(NRES.EQ.2) THEN
         WRITE(MESSAGESTRING,1692) (LOCWS(K),K=1,KWS),NST
	 call logfromfortran(DEBUG_LEVEL,MESSAGESTRING) 
 1692 FORMAT(
     & 5X,'UPPER-LAKE-PARM, LOWER-LAKE-PARM, TIME-SERIES,',
     & ' CARRYOVER, REGULATION-INFORMATION =',5I8)
      ENDIF
  850 CONTINUE
C
      IF (FEWSDEBUG.GE.1) THEN
