...
CC      ENDIF 
       IF(FEWSDEBUG.LT.2 .OR. KE.LE.0) GO TO 320
       WRITE(MESSAGESTRING,1610) E1,E2
...
CC     5    ICS(2*IPC-1),ICS(2*IPC),E2 
CC      ENDIF 
      IF(FEWSDEBUG.LT.2) GO TO 425
      ITER = 20-N+1
      WRITE(MESSAGESTRING,1611) E1,E2,Q1T,ITER
      call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)
...
C
CC  KSH CHANGE START
      IF (FEWSDEBUG.LT.2) GO TO 1060
      IF(ISHUT.NE.1) GO TO 891
      WRITE(MESSAGESTRING,892) IS2
      call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)
 ...
 891  IIR = IS(25)
      ...
      IF(NPA(IIR).LE.7 .OR. JULHRX.LT.IRRC3) THEN
        WRITE(MESSAGESTRING,893) IS2
	call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)
 893    FORMAT(/5X,I5,' HOUR REGULATION CODE NOT SPECIFIED,',
     &  ' FREEFLOW OPTION IS USED!')
        GO TO 1060
      ENDIF    
      KDA = JULHRX/24+1
      KHR = JULHRX-(KDA-1)*24
      WRITE(MESSAGESTRING,895) IS2
      call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)
 895  FORMAT(/5X,I5,' HOUR REGULATION CODE BRACKETED BY:',
     & '   TIME (HR)  DATA VALUE   DATA CODE')      
      ...
      ICN = NPA(IISC+2)+1
      JHR = NPA(IISC)/10
      IF ( ICN .GT. 7 ) GO TO 1060 !CP it prints out a strange string if icn > 7
      CALL MDYH1(KDA,KHR,KMON,KDAY,KYEAR,KHOUR,NLSTZ,NOUTDS,TZCS)
      IF(IC.EQ.1)
     & WRITE(MESSAGESTRING,897) KMON,KDAY,KYEAR,KHOUR,JULHRX,
     &  JHR,WK(IISC+1),IRRC(1,ICN),IRRC(2,ICN)
       call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)
 897   FORMAT(5X,6HDATE= ,I2.2,'/',I2.2,'/',I4.4,'/',I2.2,
     & 6X,8HJULHOUR=,I10,I11,F12.2,4X,2A4)
      IF(IC.EQ.2)
     & WRITE(MESSAGESTRING,898) JHR,WK(IISC+1),IRRC(1,ICN),IRRC(2,ICN)
       call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)
...
C      ENDIF
      WRITE(MESSAGESTRING,1614) IRRC(1,ICN),IRRC(2,ICN),VALUE,QC
      call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)
