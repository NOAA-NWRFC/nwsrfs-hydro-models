...
C---------------------------------------------------------------------
C
ccav      COMMON/FPROG/MAINUM,VERS,VDATE(2),PNAME(5),NDD
cca      COMMON/FDBUG/IODBUG,ITRACE,IDBALL,NDEBUG,IDEBUG(20)
      COMMON/LTS51/
...
C LC - comment out below; don't use NDD
C      LWQO1 = LOCWS(NRES+3)+(NRES-IRES)*NTS*NDD*NTIM24
C      LWQO2 = LWQO1+NDD*NTIM24
C      LWQM  = LWQO2+NDD*NTIM24
C      LWEL  = LWQM+NDD*NTIM24
C      LWST  = LWEL+NDD*NTIM24
C      IF(NTS.EQ.10) GO TO 450
C      LWBQI1 = LWST+NDD*NTIM24
C      LWBQI2 = LWBQI1+NDD*NTIM24
C      LWBQIM = LWBQI2+NDD*NTIM24

 
      LWQO1 = LOCWS(NRES+3)+(NRES-IRES)*NTS*NTIMESTEPS
 
      LWQO2 = LWQO1+NTIMESTEPS

      LWQM  = LWQO2+NTIMESTEPS

      LWEL  = LWQM+NTIMESTEPS 

      LWST  = LWEL+NTIMESTEPS
      IF(NTS.EQ.10) GO TO 450

      LWBQI1 = LWST+NTIMESTEPS

      LWBQI2 = LWBQI1+NTIMESTEPS

      LWBQIM = LWBQI2+NTIMESTEPS
 450  CONTINUE
C
      IF(FEWSDEBUG.LE.1 .OR. NS2.NE.2) GO TO 1000
      WRITE(MESSAGESTRING,*)'WRITING P ARRAY INDICES'
      call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)
      WRITE(MESSAGESTRING,900) 
     & LPQI1,LPQI2,LPQO1,LPQO2,LPQM,LPEL,LPST,LPOQO,LPOQM,LPOEL,
     & LPQL1,LPQL2,LPBQI1,LPBQI2,LPBQIM
      call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)
      WRITE(MESSAGESTRING,*)'WRITING D ARRAY INDICES'
      call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)
      WRITE(MESSAGESTRING,900) 
     & LDQI1,LDQI2,LDQO1,LDQO2,LDQM,LDEL,LDST,LDOQO,LDOQM,LDOEL,
     &  LDQL1,LDQL2,LDBQI1,LDBQI2,LDBQIM
      call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)
      WRITE(MESSAGESTRING,*)'WRITING I and WK ARRAY INDICES'
      WRITE(MESSAGESTRING,900) 
     & LIQI1,LIQI2,LIQO1,LIQO2,LIQM,LIEL,LIST,LIOQO,LIOQM,LIOEL,
     &  LIQL1,LIQL2,LIBQI1,LIBQI2,LIBQIM,
