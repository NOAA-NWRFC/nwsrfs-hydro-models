...
C     SUBROUTINE PIN57 (P,LEFTP,IUSEP,C,LEFTC,IUSEC)
       SUBROUTINE PIN57 (P,C, FILENAME)
...
C23456789012345678901234567890123456789012345678901234567890123456789012
      ...
      CHARACTER *72 CTITLE
      CHARACTER * 8 ADJID,DIVID,RFIID,RFOID,OLID,CDID
      CHARACTER * 9 CEID,PEID,MATID,NATID
      CHARACTER * 4 ADJCD,DIVCD,RFICD,RFOCD,OLCD,CDCD
      CHARACTER * 5 CECD,PECD,MATCD,NATCD
      CHARACTER *1024 FILENAME

      INTEGER OPTION
      INTEGER MAXLENGTH
      INTEGER COUNT, ERRFLAG,NUMBEROFELEMENTS
C	COMMON/TSDATA/qmape(200),qmat(200),qnat(200),ttime(200)
...
C     CHARACTER *512 MESSAGESTRING(20)
	include 'flogm'
C	DATA (MESSAGESTRING(J) (1:512),J=1,20) /' '/
...
C     CHECK TRACE LEVEL
CSR      CALL FPRBUG('PIN57   ',1,57,FEWSDEBUG)
...
C	open the file right here

CSR	Initialize the output ids to NONE so we know above if they weren't specified
	ADJID='NONE'
	DIVID='NONE'
	RFIID='NONE'
	RFOID='NONE'
	...
	IF(FEWSDEBUG .GE. 1) THEN
      	WRITE(MESSAGESTRING,801)CTITLE
 ...
 801  FORMAT('PIN57:  CTITLE: ',A72)
	...
	IF(FEWSDEBUG .GE. 1) THEN
      WRITE(MESSAGESTRING,802) OPTION
	call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)
	END IF
 802  FORMAT('PIN57:  OPTION: ',I1)
	...
	IF(FEWSDEBUG .GE. 1) THEN
	WRITE(MESSAGESTRING,803) MATID,MATCD,NATID,NATCD
	 call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)
	END IF
 ...
 806  FORMAT('PIN57:  RFIID,RFICD: ',2A8,/,
	...
	IF(FEWSDEBUG .GE. 1) THEN
      WRITE(MESSAGESTRING,807) LAT,AREA,EFF,MFLOW
	call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)
	END IF
 807  FORMAT('PIN57:    LAT: ',F8.2,/,
	...
	IF(FEWSDEBUG .GE. 1) THEN
      WRITE(MESSAGESTRING,808) K(16),K(46),K(75),K(106),
     + K(136),K(167)
	call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)
	END IF
 808  FORMAT('PIN57: JAN-JUN COEFFICIENTS: ',F4.2,1X,
	...
	IF(FEWSDEBUG .GE. 1) THEN
      WRITE(MESSAGESTRING,809) K(197),K(228),K(259),
     + K(289),K(320),K(350)
	call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)
	END IF
 809  FORMAT('PIN57: JUL-DEC COEFFICIENTS: ',F4.2,1X,
	...
	IF(FEWSDEBUG .GE. 1) THEN
      WRITE(MESSAGESTRING,810) RFIN,RFOUT,RFSTOR
	call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)
	END IF
...
CSR      CALL CHEKTS (NATID,NATCD,24,1,'L3  ',0,1,IERFLG)
CSR     IF (IERFLG.NE.0) IERROR = 1

CSR      CALL CHEKTS (ADJID,ADJCD,24,1,'L3  ',0,1,IERFLG)
CSR      IF (IERFLG.NE.0) IERROR = 1

CSR      CALL CHEKTS (DIVID,DIVCD,24,1,'L3  ',0,1,IERFLG)
CSR    IF (IERFLG.NE.0) IERROR = 1

CSR      CALL CHEKTS (RFIID,RFICD,24,1,'L3  ',0,1,IERFLG)
CSR      IF (IERFLG.NE.0) IERROR = 1

CSR      CALL CHEKTS (RFOID,RFOCD,24,1,'L3  ',0,1,IERFLG)
CSR      IF (IERFLG.NE.0) IERROR = 1

CSR      CALL CHEKTS (OLID,OLCD,24,1,'L3  ',0,1,IERFLG)
CSR      IF (IERFLG.NE.0) IERROR = 1

CSR      CALL CHEKTS (CDID,CDCD,24,1,'L3  ',0,1,IERFLG)
CSR      IF (IERFLG.NE.0) IERROR = 1

CSR      CALL CHEKTS (CEID,CECD,24,1,'L   ',0,1,IERFLG)
CSR      IF (IERFLG.NE.0) IERROR = 1

CSR      IF (OPTION.EQ.0) THEN
CSR         CALL CHEKTS (MATID,MATCD,6,1,'TEMP',0,1,IERFLG)
CSR         IF (IERFLG.NE.0) IERROR = 1
CSR      ELSE
CSR         CALL CHEKTS (PEID,PECD,24,1,'L   ',0,1,IERFLG)
CSR         IF (IERFLG.NE.0) IERROR = 1
CSR      ENDIF
...
C     EFF MUST BE BETWEEN 0.00 AND 1.00
      ...
      IF (EFF.GT.1) THEN
         WRITE(MESSAGESTRING,FMT='(10X,''**ERROR** THE IRRIGATION 
     + EFFICIENCY'','' MUST BE BETWEEN 0.00 AND 1.00'')')
...
C     RFIN MUST BE LESS THAN (1-EFF)
      ...
      IF (ANINT(RFIN).GT.ANINT(1-EFF)) THEN
         WRITE(MESSAGESTRING,FMT='(10X,''**ERROR** THE RETURN FLOW '',
...
C  DEBUG OUTPUT FOR EMPIRICAL COEFFICIENT COMPUTATIONS
	...
	IF(FEWSDEBUG .GE. 1) THEN
         DO I = 1,365
            WRITE (MESSAGESTRING,1001) I,K(I)
	call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)
...
C  DEBUG OUTPUT FOR DAYLIGHT HOUR COMPUTATIONS
         ...
         IF (FEWSDEBUG.EQ.1) THEN
            WRITE (MESSAGESTRING,1002) I,H(I)
...
C  DEBUG OUTPUT FOR ANNUAL DAYLIGHT HOURS
      ...
      IF (FEWSDEBUG.EQ.1) THEN
         WRITE (MESSAGESTRING,1003) HYEAR
...
CSR      CALL CHECKP (IUSEP,LEFTP,IERFLG)
      IF (IERFLG.NE.0) IERROR = 1

      IUSEC = 1

CSR      CALL CHECKC (IUSEC,LEFTC,IERFLG)
      IF (IERFLG.NE.0) IERROR = 1
...
C        CARD 9 READ: RETURN FLOW PARAMETERS
 ...
 990  IF (IERROR.GE.1) THEN
      ...
      IF (ITRACE.GE.1) WRITE(MESSAGESTRING,999)
