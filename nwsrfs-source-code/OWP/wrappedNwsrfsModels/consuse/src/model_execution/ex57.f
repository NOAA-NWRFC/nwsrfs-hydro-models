...
C  DEBUG OUTPUT FOR FIRST DATA VALUE LOCATION
       ...
       IF (FEWSDEBUG.GE.1) THEN 
          WRITE(MESSAGESTRING,1001) KDA,KHR,IDADAT,ITS
...
C  DEBUG OUTPUT FOR LAST DATA VALUE LOCATION
      ...
      IF (FEWSDEBUG.GE.1) THEN
         WRITE(MESSAGESTRING,1002) LDA,LTS
         call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)
      END IF
...
C  DO LOOP TO CALCULATE CROP ET FOR EACH DAY OF THE RUN
	  ...
	  call getDimensionAndUnitInFortran('SQME ', IDIMI, IUNIT)
...
C  SCS BLANEY CRIDDLE ET ESTIMATION EQUATION
            ...
            IF (FEWSDEBUG.GE.1) THEN
	       WRITE (MESSAGESTRING,*) 'DAY IS' ,DAY
	       call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)
	    ENDIF
	    K = P(DAY+57)
...
C  DEBUG OUTPUT FOR CROP ET COMPUTATIONS USING TEMPERATURE INPUT
	...
	IF (FEWSDEBUG.GE.1) THEN
	    ...
	    WRITE (MESSAGESTRING,1004) ILM,ILD,ILY,DAY,T(W),T(X),
     +                  T(Y),T(Z),MATC,MATF,PERC,K,CE(I)
	    call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)
...
C  DEBUG OUTPUT FOR CROP ET COMPUTATIONS USING POTENTIAL ET INPUT
	...
	IF (FEWSDEBUG.GE.1) THEN
	   CALL MDYH1(KDA,KHR,ILM,ILD,ILY,ILH,NOUTZ,NOUTDS,NTZCD)
	   WRITE (MESSAGESTRING,1007) ILM,ILD,ILY,DAY,K,PE(I),CE(I)
	   call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)
 1007      FORMAT (' EX57: CROP ET: OPTION 1: ',I2,I3,2I5,F5.2,2F6.2)
	END IF
...
C  DEBUG OUTPUT FOR THE FLOW COMPUTATIONS
	...
	IF (FEWSDEBUG.GE.1) THEN
	   CALL MDYH1(KDA,KHR,ILM,ILD,ILY,ILH,NOUTZ,NOUTDS,NTZCD)
	   WRITE(MESSAGESTRING,1010)ILM,ILD,ILY,CE(I),QCD(I),QDIV(I),
     +     QSUM,QNAT(I),QADJ(I),QRFIN(I),QRFOUT(I),QOL(I),CTEMP(1)
	   call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)
