             ...
             size_t *out_buf_len,size_t *grib_length,size_t *off,unsigned char *pds_flag)
{
  short sign;
  size_t pds_len;
  int dval,yr,cent,hr,min,p1mod256,p2mod256;
  size_t n;
...
/***************************************/
/* P1 */
/* set this to number of "256" hours */
/* taken from NCEP w3lib routine w3fi68 */
  p1mod256 = (grib_lbl[16]/256);
  p1mod256 = p1mod256 % 256; 
  setBits(out_buf,p1mod256,*off+144,8); 
/* P2 */
/* set to the remainder of hours */
  p2mod256 = grib_lbl[16] % 256;
  setBits(out_buf,p2mod256,*off+152,8);
}
else
{
...
/* time range */
  ...
  switch (grib_lbl[18]) {
    ...
    case 4:
    case 10:
       /* case 10 - this case is specific for > 255 hour case for precip grids */
    case 51:
    case 113:
    case 114:
    case 115:
...
{static char rcs_id1[] = "$Source: /fs/hseb/ob9d/ohd/pproc_lib/src/PackGrib/RCS/packgrib_.c,v $";
 static char rcs_id2[] = "$Id: packgrib_.c,v 1.2 2012/03/09 14:50:57 pst Exp $";}
/*  ===================================================  */
