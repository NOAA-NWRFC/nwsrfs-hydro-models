...
CP 03/09SUBROUTINE FROST2_1(PX,TA,WE,AESC,SH,DT,iprint,iout,iopt)
C lc 5/09 added passing of frzd from frzind1 to argument list
      SUBROUTINE FROST2_1(PX,TA,WE,AESC,SH,DT,iopt,FRZD)
...
CP 03/09     REAL TSINT(10),SWINT(10),SWHINT(10)
CP 03/09     REAL DSINT(10)/0.05,0.1,0.2,0.5,1.0,1.5,2.0,0.,0.,0./
CP 03/09     REAL DSINTW(10)/0.05,0.1,0.2,0.5,1.0,1.5,2.0,0.,0.,0./
      REAL TSTMP(10),DSMOD(10),SWTMP(10),SWHTMP(10)
CP 03/09     SAVE DSINT,DSINTW
...
cv07       ENDIF 
       ...
       IF(ISTART .EQ. 0) THEN
        WRITE(MESSAGESTRING,'(A9,6F)') ' FRZPAR= ',(FGPM(I),I=1,6)
        call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)

        WRITE(MESSAGESTRING,'(A9,F)') '   SMAX= ',smax
        call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)

        IF ( NDSL .GT. 0 ) THEN
          WRITE(MESSAGESTRING, *)
     >    ' DESIRED SOIL LAYERS TO OUTPUT TEMPERATURE AND MOISTURE: '
          call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
   
          WRITE(MESSAGESTRING, *) '   ',(DSINT(I),I=1,NDSL)
          call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
        END IF
CP 03/09       WRITE(*,*) ' FRZPAR:',(FGPM(I),I=1,6),' SMAX=',smax
CP 03/09       WRITE(*,*) ' DESIRED SOIL LAYERS TO OUTPUT TEMPERATURE:'
CP 03/09       WRITE(*,*) '   ',(DSINT(I),I=1,NINT)
CP 03/09       WRITE(*,*) ' DESIRED SOIL LAYERS TO OUTPUT SOIL MOISTURE:'
CP 03/09       WRITE(*,*) '   ',(DSINTW(I),I=1,NINTW)
       endif 
...
cv07       IF(ISTART .EQ. 0) THEN
        WRITE(MESSAGESTRING,*)' SAC-SMA TIME STEP=',DT,'days '
        call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
   
        WRITE(MESSAGESTRING,*)' FROZEN GROUND TIME STEP=',
     >                          DTFRZ/3600,'hr'
        call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)

        WRITE(MESSAGESTRING,*)' NUMBER FROZEN GROUND LOOPS=',IDT
        call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
cv07       endif 
...
cc      ZBOT = FGPM(7+NSOIL+1)
      DO I=1,NSOIL
       ZSOIL(I)=FGPM(I+7)
      ENDDO       
cv07       write(*,'(a6,i3,2i2,3f5.0,f3.0,10f7.3)') 'NFUNC=',nloop,
cv07     +    nupl,nsac,(fgpm(i),i=4,7),(zsoil(i),i=1,fgpm(7))
...
CP 03/09IF(IPRINT .NE. 0) THEN
        WRITE(MESSAGESTRING,*) '    INITIAL SOIL LAYER STATES: '
        call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)

        WRITE(MESSAGESTRING,'(A20,7F7.3)') ' LAYER DEPTH (M):   ',
     +      ZSOIL(1),(ZSOIL(1)-ZSOIL(I),I=2,NSOIL)
        call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)

        WRITE(MESSAGESTRING,'(A20,7F7.2)') ' TOTAL MOISTURE:    ',
     +      (SMC(I),I=1,NSOIL)
        call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
   
        WRITE(MESSAGESTRING,'(A20,7F7.2)') ' UNFROZEN MOISTURE: ',
     +      (SH2O(I),I=1,NSOIL)
        call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)

        WRITE(MESSAGESTRING,'(A20,5F7.2)') ' TEMPERATURE:       ',
     +      (TSOIL(I),I=1,NSOIL) 
        call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)   

        WRITE(MESSAGESTRING,'(A,F7.1)') '   INITIAL FROST INDEX(MM)=',
     +	    FGCO(1)
        call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
...
C  UPPER ZONE STORAGES
      ...
      NUP=2
      IEXIT=0
CP 03/09CALL SAC2FRZ1(DWT,DWF,SMC,SH2O,NUP,NUPL,ZSOIL,SMAX,IOUT,IEXIT)
      CALL SAC2FRZ1(DWT,DWF,SMC,SH2O,NUP,NUPL,ZSOIL,SMAX,IEXIT)

C  LOWER ZONE STORAGES
      DWT=RTLZ*(LZTWC-LZTWC0)
      DWF=RTLZ*(LZFSC+LZFPC-LZFSC0-LZFPC0)
      NUP=NUPL+1
CP 03/09CALL SAC2FRZ1(DWT,DWF,SMC,SH2O,NUP,NSAC,ZSOIL,SMAX,IOUT,IEXIT)
      CALL SAC2FRZ1(DWT,DWF,SMC,SH2O,NUP,NSAC,ZSOIL,SMAX,IEXIT)

cVK 06/07 for soil moisture version only
...
C initialize frzd to -999.0
       FRZD=-999.0
      IF(ISFRZ .NE. 0 .OR. ITFRZ .NE. 0) THEN   
       IEXIT=0
cv07       CALL FRZ2SAC1(PX,ZSOIL,SMC,SH2O,NUPL,NSAC,SWLT,RTUZ,RTLZ,
       CALL FRZ2SAC1(PX,ZSOIL,NUPL,NSAC,SWLT,RTUZ,RTLZ,IEXIT)
CP 03/09     +     IOUT,IEXIT)
...
C  FROST DEPTH CALCULATION
       FRZD=FRZIND1(SMC,SH2O,ZSOIL,NSOIL,NUP,NSAC,RTUZ,RTLZ,SWLT)
      ENDIF
...
cVK 06/07
      if(ivers .gt. 1) then
       CALL SOIL_INT1(TSTMP,NMOD,DSMOD,DSINT,NDSL,TSINT)
       CALL SOIL_INT1(SWHTMP,NMOD,DSMOD,DSINT,NDSL,SWHINT)
CP 03/09  CALL SOIL_INT1(TSTMP,NMOD,DSMOD,DSINT,NINT,TSINT)
CP 03/09  CALL SOIL_INT1(SWHTMP,NMOD,DSMOD,DSINTW,NINTW,SWHINT)
      endif

      CALL SOIL_INT1(SWTMP,NMOD,DSMOD,DSINT,NDSL,SWINT)   
CP 03/09  CALL SOIL_INT1(SWTMP,NMOD,DSMOD,DSINTW,NINTW,SWINT)
   
cVK 06/07 test only
cVK 06/07 test only      write(*,*) (swint(i),swhint(i)i=1,nintw),(tsint(i),i=1,nint)

cVK  06/07  Recalculate soil moisture states into mm per layer
cVK  06/07      DO I=1,NINTW
cVK  06/07       IF(I .EQ. 1) THEN
cVK  06/07        SWINT(I)=SWINT(I)*DSINTW(I)*1000.
cVK 06/07
cVK  06/07        if(ivers .gt. 1) SWHINT(I)=SWHINT(I)*DSINTW(I)*1000
        
cVK  06/07       ELSE	
cVK  06/07        SWINT(I)=SWINT(I)*(DSINTW(I)-DSINTW(I-1))*1000.
cVK 06/07
cVK  06/07        if(ivers .gt. 1) SWHINT(I)=SWHINT(I)*(DSINTW(I)-
cVK  06/07     +                             DSINTW(I-1))*1000
cVK  06/07       ENDIF
cVK  06/07      ENDDO 	
