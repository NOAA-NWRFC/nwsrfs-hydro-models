...
C
CP 03/09 SUBROUTINE FLAND1(PAR,DT,PXV,EDMND,TA,WE,AESC,SH,TCI,
CP 03/09   1  IPRINT,IOUT,IDAY,IHOUR,MONTH,IYEAR,ICALL
CP 03/09   +  ,RUZICE,RLZICE,RUZPERC)
      SUBROUTINE FLAND1(PAR,DT,PXV,EDMND,TA,WE,AESC,SH,TCI, 
     1   FGIX, FRZD, IDAY,IHOUR,MONTH,IYEAR,ICALL,
     1   RUZICE,RLZICE,RUZPERC)
...
CVK      COMMON/FPMFG1/FGPM(10)
      COMMON/FPMFG1/itta,FGPM(15),ivers,ifrze,ixrc,nsoillayer      
...
C lc removed no longer needed
C     SAVE IIPREV,FRSTPREV,FRZPREV
C lc commented out no longer needed for calculating frzd at bottom of routine
...
c
      IF ( FEWSDEBUG .GE. 5 ) THEN
       write(MESSAGESTRING,*) 'Cst:',uztwc,uzfwc,lztwc,lzfsc,lzfpc
       call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
       write(MESSAGESTRING,*) 'Hst:',uztwh,uzfwh,lztwh,lzfsh,lzfph
       call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
       write(MESSAGESTRING,*) 'Cpv:',uztwc0,uzfwc0,lztwc0,lzfsc0,lzfpc0
       call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
      END IF
C.......................................
...
cVK 06/07      IF( IVERS .LE. 2) CALL FGFR1(DEFR,FR,UZDEFR,FI)
      
C lC commented out logic below
...
CVK
CVK      IF (IFRZE.GT.0) CALL FROST1(PXV,SSUR,SDRO,TA,LWE,WE,ISC,AESC,DT,
CVK     1  IBUG)
CVK  ERIC'S VERSION OF FROST INDEX
cVK 06/07      IF(IFRZE .GT. 0) THEN      
cVK 06/07       IF (IVERS.EQ.1) CALL FROST1(PXV,SSUR,SDRO,TA,WE,AESC,DT,
cVK 06/07     1 IPRINT,icall)
...
CP 03/09  CALL FROST2_1(PXV,TA,WE,AESC,SH,DT,iprint,iout,icall)
          CALL FROST2_1(PXV,TA,WE,AESC,SH,DT,icall,FRZD)      
       ENDIF
cVK 06/07      ENDIF      
C.......................................
C     PRINT DETAILED ACCOUNTING VALUES IF REQUESTED.
c
CP 2/25/09  We want to write out the frozen depth result
cCP      IF (IPRINT.EQ.1) THEN
c
CVK  OLD FROZEN GROUND VERSION
cVK 06/07       IF(IVERS .LE. 1) THEN
c Generate daily output for defined period
c        if(iyear .eq. 1965 .and. month .lt. 5 .and. 
c     +        iyear .eq. 1965 .and. month .gt. 2) then                        
c        ssperc=ssperc+sperc
c        sroimp=sroimp+roimp
c        ssdro=ssdro+sdro
c        sssur=ssssur+ssur
c        ssif=ssif+sif
c        sbfs=sbfs+bfs
c        sbfp=sbfp+bfp
c        stci=stci+tci
c        sedmnd=sedmnd+edmnd
c        stet=stet+tet
c        spxv=spxv+pxv
c        ssurf=sroimp+ssdro+sssur+ssif
c        sgrnd=sbfs+sbfp
c        if(ihour .eq. 24) then
c         WRITE (IOUT,977) iyear,month,IDAY,IHOUR,UZFWC+UZTWC,
c     +   LZTWC+LZFSC+LZFPC,spxv,ssperc,ssurf,sif,sgrnd,stci,stet

cVK 06/07        WRITE (IOUT,977) iyear,month,IDAY,IHOUR,UZTWC,UZFWC,LZTWC,
cVK 06/07     1  LZFSC,LZFPC,ADIMC,SPERC,ROIMP,SDRO,SSUR,SIF,BFS,BFP,TCI,EDMND,
cVK 06/07     2  TET,PXV,uztwh,uzfwh,lztwh,lzfsh,lzfph,we,sh,aesc,ta,fgco(1)
...
C        DO II=FGPM(7),1,-1
C         IF(TSOIL(II) .LE. 0.) GOTO 6543
C        ENDDO
cCP 3/1/09 the java code is using -999.000 (MISSING) to represent no value for
c SFGD (FORZEN DEPTH)   FRZD=0.
C        FRZD=-999.
C        if(fgco(1) .lt. 0.) frzd=tsoil(8)
C        GOTO 6542
C6543    IF(II .EQ. 1) THEN
C         DZ1=-0.5*FGPM(8)
C        ELSE
C         DZ1=0.5*(FGPM(7+II-1)-FGPM(7+II))
C        ENDIF
C        IF(TSOIL(II) .EQ. 0.) THEN
C         FRZD=-(DZ1+FGPM(7+II))*100.
C        ELSE 
C         DZ2=FGPM(7+II)-FGPM(7+II+1)
C         IF(II .NE. FGPM(7)) DZ2=0.5*DZ2
C         FRZD=(-TSOIL(II)*(DZ1+DZ2)/(TSOIL(II+1)-TSOIL(II))-
C     +            (DZ1+FGPM(7+II)))*100.
C        ENDIF
c6542    CONTINUE
C        IF(II .LT. IIPREV .AND. FGCO(1) .LE. FRSTPREV)
C     +    FRZD=FRZPREV
c
cCP 2/25/09 Write out frozen ground index
c
        FGIX = FGCO(1)
...
c        sta=sta+ta*0.25
c        do iii=1,fgpm(7)
c         stx(iii)=stx(iii)+tsoil(iii)*0.25
c        enddo 
c        if(ihour .eq. 24) then
c         WRITE (IOUT,977) iyear,month,IDAY,IHOUR,UZFWC+UZTWC,
c     +   LZTWC+LZFSC+LZFPC,spxv,ssperc,ssurf,sif,sgrnd,stci,stet,
c     +   FGCO(1),UZFWC+UZTWC-fgco(2)-fgco(3),
c     +   LZTWC+LZFSC+LZFPC-fgco(4)-fgco(5)-fgco(6),WE,SH,
c     3   TSOIL(7),TSOIL(8),FRZD,STA,(stx(II),II=1,FGPM(7))
c  977 FORMAT (1H ,i5,3I3,3F6.1,5F6.2,F5.1,F6.1,F5.1,F6.1,
c     +        2F5.1,3F6.1,20F6.1)        

        IF ( FEWSDEBUG .GE. 5 ) THEN
CP 03/09 WRITE (IOUT,905) iyear,month,IDAY,IHOUR,UZTWC,UZFWC,
         WRITE(MESSAGESTRING,905)
     1   iyear,month,IDAY,IHOUR,UZTWC,UZFWC,
...
cc     3  (TSOIL(II),II=1,FGPM(7)),TSOIL(7),TSOIL(8),FRZD,
     3  (TSINT(II),II=1,NDSL),TSOIL(7),TSOIL(8),FRZD,
     4  (SWINT(II),II=1,NDSL),(SWHINT(II),II=1,NDSL)
         call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
        END IF
c         ssperc=0.
c         sroimp=0.
...
cVK 06/07       ENDIF
  905 FORMAT (1H ,i5,3I3,F7.2,F7.3,F7.2,F7.3,2F7.2,7F7.3,2F8.3,F7.3,
     +        F9.4,5f7.2,2f6.1,F6.3,F9.4,f7.2,8f7.2,20F7.3)
...
cc  260 RSUM(1)=RSUM(1)+TCI
cc      RSUM(2)=RSUM(2)+ROIMP
cc      RSUM(3)=RSUM(3)+SDRO
cc      RSUM(4)=RSUM(4)+SSUR
cc      RSUM(5)=RSUM(5)+SIF
cc      RSUM(6)=RSUM(6)+BFS
cc      RSUM(7)=RSUM(7)+BFP
