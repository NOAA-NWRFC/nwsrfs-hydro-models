...
C    1NOW(5),LOCAL,NOUTZ,NOUTDS,NLSTZ,IDA,IHR,LDA,LHR,IDADAT
...
CVR   IF((KDA.EQ.ICDAY(IC)).AND.(KHR.EQ.ICHOUR(IC))) GO TO 116
      GO TO 115
C
C     INTERMEDIATE CARRYOVER BEING SAVED
C
CV116 CALL FCWTCO(KDA,KHR,R,NC)
C
      IC=IC+1
C
...
C
  119 IF(FEWSDEBUG.EQ.0) GO TO 121
      WRITE(MESSAGESTRING,901)(P(I),I=2,6),NQLST,NC
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
      WRITE(MESSAGESTRING,902)(P(I),I=1,NQLST)
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
