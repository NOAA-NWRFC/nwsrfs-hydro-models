...
//	int	i=0, jul_day_min = 0, jul_day_max = 0, jul_day_find = 0;
	...
	if ( getFewsDebugFlag() > 10 )
	{
	   /*AVCP PrintDebug( 30, routine,*/
	   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	   "%s %f for %d/%d/%d from _data (%d)", routine, prev_ptr->getData(),
	...
	int n_dist)
{
	...
	if( !_ts_data_head ){
		...
		if( !tsdata ){
			/*AVCP PrintWarning( 1, routine,*/
		        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
			"%s %s Unable to allocate memory for TSData object.", 
			warnStr, routine );
		...
		if ( getFewsDebugFlag() > 10 )
		{
		   /*AVCP PrintDebug( 10, routine,*/
		   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,  
		   "%s Initiated the Linked list with: \"%s\".", 
		   routine, "TSDistData" ); //AVCP (char*)tsdata );
		}
		// Construct the distribution array on the TSDistData object...
		if( tsdata->setDistribution( dist, n_dist ) ) {
			/*AVCP PrintWarning( 1, routine,*/
                        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		        "%s %s Troubles setting distribution.", 
			warnStr, routine );
			return( STATUS_FAILURE );
		}	
		return( STATUS_SUCCESS );
		...
		for( 	i=0, ptr = _ts_data_head; 
			...
			if( ptr->getDate() == date ){
                                if ( getFewsDebugFlag() > 10 )
				{
				   /*AVCP PrintDebug( 30, routine,*/
				  logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
				   "%s Setting %f for %s at %d", routine, 
				   value, (char*)date, i );
				}
	...
	if( found ){
		   ...
		   /*AVCP PrintDebug( 10, routine,*/
		  logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		  "%s Updated data value at: %s to %f.", routine, 
		  (char*)date, value );
	       }
	       return( STATUS_SUCCESS );
	}
	...
	if( !tsdata ){
		/*AVCP PrintWarning( 1, routine,*/
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		"%s %s Unable to allocate memory for TSData object.", warnStr,
		routine );
		return( STATUS_FAILURE );
	}
		...
		for( 	i=0, ptr = _ts_data_head; 
			...
			if( ptr->getDate() < date && !ptr->getNext() ){
			        ...
			        if ( getFewsDebugFlag() > 10 )
				{
				   /*AVCP PrintDebug( 10, routine,*/
				  logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
				   "%s Inserting %s before %s.", routine,
				   (char*)tsdata->getDate(), 
				   (char*)ptr->getDate() );
				}
				tsdata->setNext( ptr );
			        ...
			        if ( getFewsDebugFlag() > 10 )
				{
				   /*AVCP PrintDebug( 10, routine,*/
				  logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
				   "%s Inserting %s between %s and %s.",
				   routine, (char*)tsdata->getDate(), 
				   (char*)ptr->getDate(),
			        ...
			        if ( getFewsDebugFlag() > 10 )
				{
				   /*AVCP PrintDebug( 10, routine,*/
				  logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
			           "%s %s is less than %s and %s, continuing "
				   "search...", routine, 
				   (char*)tsdata->getDate(), 
				   (char*)ptr->getDate(),
				   (char*)ptr->getNext()->getDate() );
		...
		for( 	i=_ndata, ptr = _ts_data_tail; 
			...
			if( ptr->getDate() < date && !ptr->getNext() ){
			        ...
			        if ( getFewsDebugFlag() > 10 )
				{
				   /*AVCP PrintDebug( 10, routine, */
				  logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
				  "%s Inserting %s between %s and %s.", routine,
				  (char*)tsdata->getDate(), 
				  (char*)ptr->getDate(),
				  (char*)ptr->getNext()->getDate() );
				}
			        ...
			        if ( getFewsDebugFlag() > 10 )
				{
				   /*AVCP PrintDebug( 10, routine, */
				  logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
			          "%s %s is less than %s, continuing search...",
				  routine, (char*)tsdata->getDate(), 
				  (char*)ptr->getDate() );
				}
			}
	...
	if( tsdata->setDistribution( dist, n_dist ) ) {
		/*AVCP PrintWarning( 1, routine,*/
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	        "%s %s Troubles setting distribution.", warnStr, routine );
		return( STATUS_FAILURE );
	}	
	...
	if( !found ){
		/*AVCP PrintWarning( 1, routine, */
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		"%s %s With DistributedTS we should always insert the data!",
	        warnStr, routine );
		delete tsdata;
		return( STATUS_FAILURE );
	}
...
{	char	routine[] = "HourTS::HourTS(char*)";
	...
	/*AVCP PrintWarning( 1, routine, */
        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
        "%s (%s) - Constructing from file (%s) not enabled", 
	warnStr, routine, input );
	init();
}
...
// Notes:	(1)	Need to spend time on this
//
extern TSDate _curDate;  //bug #562 5/29/12
HourTS::HourTS ( const HourTS& ts ) : TS( ts )
{
	...
	if( allocateDataSpace() ){
		/*AVCP PrintWarning( 1, routine,*/
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		"%s (%s) - Troubles allocating data space in the copy "
		"constructor.", warnStr, routine );
	}
	else {
		int		row=0, col=0;
		TSDate		t( _date1, TSDate::DATE_FAST );

		//bug #562 5/29/12 for( ; t<=_date2; t.addHour( _data_interval_mult ) ){
		for( ; t<=_curDate; t.addHour( _data_interval_mult ) ){
			if( getDataPosition( t, &row, &col, NULL ) ){
				/*AVCP PrintWarning( 1, routine,*/
			        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
				"%s (%s) - Troubles getting data position "
				"for %s.", warnStr, routine, t.toString() );
				continue;
			}
...
HourTS& HourTS::operator= ( const HourTS& ts )
{
	...
	if( &ts	== this ){
	        ...
	        if ( getFewsDebugFlag() > 10 )
		{
		   /*AVCP PrintDebug( 10, routine,*/
		   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		   "(%s) - Self Declaration in \"%s\".", routine );
		}
		return( *this );
	}
	//
	...
	if( freeDataSpace() ){
		/*AVCP PrintWarning( 1, routine,*/
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		"%s (%s) - Troubles deleting data space.", warnStr, routine );
		return( *this );
	}
	...
	if( ts._data ){
		if( allocateDataSpace() ){
			/*AVCP PrintWarning( 1, routine,*/
		        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
			"%s (%s) - Troubles allocating data space.", warnStr,
			routine );
			return( *this );
		}
...
{	char	routine[]="(HourTS.allocateDataSpace) -";
	...
	if( !nmonths ){
		/*AVCP PrintWarning( 1, routine,*/
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		"%s %s TS has 0 months POR, maybe Dates haven't been set yet",
	        warnStr, routine );
		return( STATUS_FAILURE );
	}
	...
	if( _data ) {
		/*AVCP PrintWarning( 1, routine,*/
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		"%s %s Memory already allocated for data space.",
		"Re-allocating.", warnStr, routine );
		// Free the old memory...
		freeDataSpace();
	}
	...
	if( !_data ) {
		/*AVCP PrintWarning( 1, routine,*/
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		"%s %s Trouble allocating memory for %d double*'s", warnStr,
		routine, nmonths );
		return( STATUS_FAILURE );
	}
	...
	for (	i = 0; i < nmonths;
		...
		if ( _data_interval_mult <= 24 ) {
			...
			/*AVCP PrintWarning ( 1, routine,*/
		        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
			"%s %s Only know how to handle 1-24 hour data, "
			"not %d-hour", warnStr, routine, _data_interval_mult );
		}
		...
		if( !_data[i] ) {
			/*AVCP PrintWarning( 1, routine,*/
		        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
			"%s %s Unable to allocate memory for %d doubles at "
			"row %d", warnStr, routine, nvals, i );
			return( STATUS_FAILURE );
		}
        ...
        if ( getFewsDebugFlag() > 10 )
	{
	   /*AVCP PrintDebug( 10, routine,*/
	   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	   "%s Successfully allocated %d months of memory", routine,
	   nmonths );
	}
	return 0;
}
...
{	char  	routine[] = "HourTS::formatOutput(int,long)";	
	...
	/*AVCP PrintWarning( 1, routine,*/
        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	"%s (%s) - Routine is not enabled", warnStr, routine );
...
int HourTS::freeDataSpace( void )
{
	...
	if( !_data ){
		/*AVCP PrintStatus( 10, routine,*/
	   if ( getFewsDebugFlag() > 10 )
	   {
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	        "(%s) - Data Array hasn't been allocated yet, no need "
		"to freeDataSpace.", routine );
	   }
	   return( STATUS_SUCCESS );
	}
...
{	char	routine[] = "(HourTS.getDataPoint) -";
	...
	if(	(date < _date1) || (date > _date2) ) {
		...
		/*AVCP PrintWarning( 1, routine,*/
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		"%s %s %d/%d/%d 1245 not within POR (%d/%d/%d - %d/%d/%d)",
		warnStr, routine, date.getMonth(), date.getDay(), 
		date.getYear(), _date1.getMonth(), _date1.getDay(), 
		_date1.getYear(), _date2.getMonth(), _date2.getDay(), 
		_date2.getYear() );
	...
	if ( getDataPosition(date, &row, &column, NULL) ) {
		/*AVCP PrintWarning( 1, routine,*/
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		"%s %s %d/%d/%d:%d Unable to get data position",
		warnStr, routine, date.getMonth(), date.getDay(), 
        ...
        if ( getFewsDebugFlag() > 10 )
	{
	   /*AVCP PrintDebug( 30, routine,*/
	   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
 	   "%s %f for %d/%d/%d from _data[%d][%d]", 
	   routine, _data[row][column], date.getMonth(), date.getDay(),
	   date.getYear(), row, column );
	}
...
{	char	routine[] = "(HourTS.getDataPointer) -";
	...
	if(	(date < _date1) || (date > _date2) ) {
		/*AVCP PrintWarning( 1, routine,*/
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		"%s %s %d/%d/%d 1313 not within POR (%d/%d/%d - %d/%d/%d)",
		warnStr, routine, date.getMonth(), date.getDay(), 
		date.getYear(),
        ...
        if ( getFewsDebugFlag() > 10 )
	{
	   /*AVCP PrintDebug( 10, routine,*/
	   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	   "%s Returning data for %d/%d/%d from _data[%d][%d]", routine,
...
{	char	routine[] = "(HourTS.getDataPosition) -", tz[8], tz1[8];
	...
	if ( !tz[0] || !strcmp(tz,tz1) ) {
		...
		tzshift = 0;
	        ...
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		"%s %s Do not know how to shift time zones yet "
		"(\"%s\" to \"%s\"", warnStr, routine, tz1, tz );
		tzshift = 0;
	}
	...
	*column	= 0;
	if(	(date < _date1) || (date > _date2) ) {
		...
		/*AVCP PrintWarning( 1, routine,*/
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		"%s %s %d/%d/%d 1400 not within POR (%d/%d/%d - %d/%d/%d)",
		warnStr, routine, date.getMonth(), date.getDay(), 
		date.getYear(),
...
{	char	routine[] = "(HourTS.getDataValue) -";
	...
	if(	(date < _date1) || (date > _date2) ) {
		...
		/*AVCP PrintWarning( 1, routine,*/
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		"%s %s %d/%d/%d 1465 not within POR (%d/%d/%d - %d/%d/%d)",
		warnStr, routine, 
        ...
        if ( getFewsDebugFlag() > 10 )
	{
	   /*AVCP PrintDebug( 30, routine,*/
	   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	   "%s %f for %d/%d/%d from _data[%d][%d]", routine, _data[row][column],
	   date.getMonth(), date.getDay(), date.getYear(), row, column );
	}
	return( _data[row][column] );
}
...
int HourTS::printSample( ostream& out )
{
	...
	/*AVCP PrintWarning( 1, routine,*/
	logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	"%s (%s) - This is a virtual function, should be defined lower down",
        warnStr, routine );
...
{	char		routine[]="(HourTS.refresh) -";
	...
	if ( !_dirty ) {
	        if ( getFewsDebugFlag() > 10 )
		{
		   /*AVCP PrintDebug ( 10, routine,*/
		   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		   "%s Time series is not dirty.  Not recomputing limits",
		   routine );
		}
		return 0;
	}
        ...
        if ( getFewsDebugFlag() > 10 )
	{
	   /*AVCP PrintDebug( 10, routine,*/
	   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
       	   "%s Time Series is dirty. Recomputing limits", routine );
	}
...
{	char	routine[]="HourTS.setDataValue";
	...
	if( 	(date < _date1) || (date > _date2) ) {
		/*AVCP PrintWarning( 1, routine,*/
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		"%s (%s) - Date %d/%d/%d %02d:%02d:%02d.%02d is outside bounds",
		warnStr, routine, date.getMonth(), date.getDay(), 
		date.getYear(), date.getHour(),
        ...
        if ( getFewsDebugFlag() > 10 ) 
	{
	   /*AVCP PrintDebug( 30, routine,*/
	   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	   "(%s) - Setting %f for %d/%d/%d %02d at %d,%d", 
	   routine, value, date.getMonth(), date.getDay(), date.getYear(), 
	   date.getHour(), row, column );
	}
...
{	
	...
	/*AVCP PrintWarning( 1, routine,*/
	logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	"%s (%s) - This function is virtual, redefine in lower classes",
        warnStr, routine );
...
{	char	routine[]="IrregularTS::IrregularTS";
	...
	{
	   /*AVCP PrintDebug( 50, routine,*/
	   logMessageWithArgsAndExitOnError( DEBUG_LEVEL, "(%s) - Constructing",
	   routine );
	}
...
int IrregularTS::allocateDataSpace( void )
{
	...
	/*AVCP PrintWarning( 2, routine,*/
	logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	"%s (%s) - Memory allocation is dynamic for IrregularTS.",
        warnStr, routine );
...
{	char	routine[] = "(IrregularTS.enforceDataGaps) -";
	...
	for (	;
		...
		data_value = getDataValue(t);
                if ( getFewsDebugFlag() > 10 )
		{
		   /*AVCP PrintDebug ( 2, routine,*/
		   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		   "%s Processing date %s in period %s to %s", routine,
		...
		if( t.notEquals(start) ) {
			...
			if( prevDate.lessThan(t) && !isDataMissing(data_value)){
				...
				if ( getFewsDebugFlag() > 10 )
				{
				  ...
				  logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
				   "%s Set MISSING value at: %s", routine,
				   (char *)prevDate);
				}
			}	
			prevDate = t;
	...
	if ( getFewsDebugFlag() > 10 )
	{
	   ...
	   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	   "%s Set %d missing values after checking %d values",
 	   routine, datacount, missingcount );
	}
	return 0;	
}
//------------------------------------------------------------------------------
...
int IrregularTS::freeDataSpace( void )
{
	...
	if( !_ts_data_head ){
	   ...
	   {
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	        "%s (%s) - Data Array is not allocated, no "
		"need to freeDataSpace.", warnStr, routine );
	   }
	   return( STATUS_SUCCESS );
	}
...
{	char	routine[] = "(IrregularTS::getDataDate) -";
	...
	if( increment != 1 ){
		/*AVCP PrintWarning( 1, routine,*/
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		"%s %s Support for non-one increment %d is not yet included",
		warnStr, routine, increment );
	}
	...
	for( ptr = _ts_data_head; ptr != NULL; ptr = ptr->getNext() ){
		if( ptr->getDate() == date0 ){
			if( ptr->getNext() == NULL ){
				...
				/*AVCP PrintWarning( 2, routine,*/
			        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		                "%s %s Given the last date (%s), will return a"
			        " date one hour later.", warnStr, routine,
				(char*)date0 );
				t = date0;
				t.addHour( 1 );
			}
				...
				if ( getFewsDebugFlag() > 10 )
				{
				   /*AVCP PrintDebug( 10, routine,*/
				  logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
				   "%s \"%s\" is next date after \"%s\".",
				   routine, (char*)t, (char*)date0 );
				}
			}
			found = 1;
			break;
		...
		if( 	ptr->getDate() < date0 && 
			...
			if ( getFewsDebugFlag() > 10 )
			{
		           /*AVCP PrintDebug( 10, routine,*/
			   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
                           "%s Date (%s) is between this date (%s) and the next"
			   " date (%s), using closest", routine,
			...
			if( 	fabs( date0_double - date_double ) <= 
				...
				if ( getFewsDebugFlag() > 10 )
				{
			           /*AVCP PrintDebug( 10, routine,*/
				   logMessageWithArgsAndExitOnError(DEBUG_LEVEL,
				   "%s is closer to %s than %s", routine,
				   (char*)date0, (char*)ptr->getDate(), 
				   (char*)ptr->getNext()->getDate() );
				}
				t = ptr->getNext()->getDate();
				...
				if( ptr->getNext()->getNext() != NULL ){
				  ...
				  {	     
				        /*AVCP PrintDebug( 10, routine,*/
				        logMessageWithArgsAndExitOnError( 
				        DEBUG_LEVEL, "%s %s is closer to %s "
					"than %s but there is a NULL, "
					"incrementing %s by 1 hour.",
					routine, (char*)date0, 
					(char*)ptr->getNext()->getDate(),
					(char*)ptr->getDate(),
//cfan					ptr->getNext()->getDate() );
	...
	if( found == 0 ) {
		...
		if ( getFewsDebugFlag() > 10 )
		{
		   /*AVCP PrintDebug( 10, routine,*/
		   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		   "%s Can't find the date after %s, returning the last "
		   "date + 1 hour: %s.", routine,
		   (char*)date0, (char*)t );
		}
	}
...
{	char	routine[] = "(IrregularTS::getDataPoint) -";
	...
	if(	(date < _date1) || (date > _date2) ) {
		...
		/*AVCP PrintWarning( 1, routine,*/
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		"%s %s %d/%d/%d not within POR (%d/%d/%d - %d/%d/%d)",
		warnStr, routine,date.getMonth(), date.getDay(), date.getYear(),
		_date1.getMonth(), _date1.getDay(), _date1.getYear(),
		_date2.getMonth(), _date2.getDay(), _date2.getYear() );
		data_point.setData ( _missing );
		return data_point;
	...
	if ( getFewsDebugFlag() > 10 )
	{
	   /*AVCP PrintDebug( 30, routine,*/
	   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	   "%s %f for %d/%d/%d from _data (%d)", routine,
	   ptr->getData(), date.getMonth(), date.getDay(), date.getYear(), i );
	}
...
{	char	routine[] = "IrregularTS::getDataPointer";
	...
	/*AVCP PrintWarning( 1, routine, */
        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	"%s (%s) - The routine is not implemented.", warnStr, routine );
	return( NULL );
}
//------------------------------------------------------------------------------
...
{	char	routine[] = "IrregularTS::getDataPosition";
	...
	/*AVCP PrintWarning( 2, routine,*/
        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	"%s (%s) - is not used with IrregularTS.", warnStr, routine );
...
double IrregularTS::getDataValue( TSDate &date )
{
	...
	if ( getFewsDebugFlag() > 10 )
	{
	   /*AVCP PrintDebug( 30, routine,*/
	   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	   "%s %f for %d/%d/%d from _data (%d)", routine, ptr->getData(),
	   date.getMonth(), date.getDay(), date.getYear(), i );
	}
...
{	char		routine[]="(IrregularTS.refresh) -";
	...
	if ( !_dirty ) {
		if ( getFewsDebugFlag() > 10 )
		{
	           /*AVCP PrintDebug ( 10, routine,*/
		   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		   "%s Time series is not dirty.  Not refreshing", 
		   routine );
		}
		return 0;
	}
...
int IrregularTS::setDataValue( TSDate &date, double value )
{
	...
	if( found ){
	        if ( getFewsDebugFlag() > 10 )
		{
		   /*AVCP PrintDebug( 10, routine,*/
		   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		   "%s Updated data value at: %s to %f.",
		   routine, (char*)date, value );
		}
		return( STATUS_SUCCESS );
	}
	//
		...
		for( 	i=0, ptr = _ts_data_head; 
			...
			if( ptr->getDate() < date && !ptr->getNext() ){
			        ...
			        if ( getFewsDebugFlag() > 10 )
				{
				   /*AVCP PrintDebug( 10, routine,*/
				   logMessageWithArgsAndExitOnError(
				   DEBUG_LEVEL, "%s Inserting %s after %s.", 
				   (char*)tsdata->getDate(), 
				   (char*)ptr->getDate() );
				}
				ptr->setNext( tsdata );
			        ...
			        if ( getFewsDebugFlag() > 10 )
				{
				   /*AVCP PrintDebug( 10, routine,*/
				   logMessageWithArgsAndExitOnError(DEBUG_LEVEL,
			           "%s is less than %s and %s, continuing "
				   "search...", routine,
				   (char*)tsdata->getDate(), 
				   (char*)ptr->getDate(),
				   (char*)ptr->getNext()->getDate() );
	...
	if( !found ){
		/*AVCP PrintWarning( 1, routine,*/
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		"%s %s With IrregularTS we should always insert the data!",
	        warnStr, routine );
		delete tsdata;
		return( STATUS_FAILURE );
	}
...
{	char routine[]="(TSData::setDataFlag) -";
	...
	if( flag == NULL ){
		/*AVCP PrintWarning( 1, routine,*/
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		"%s %s Attempting to set a NULL data flag.",
	        warnStr, routine );
		return 1;
	}
	...
	if( !_data_flag ){
		/*AVCP PrintWarning( 1, routine,*/
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		"%s %s Unable to allocate memory for %d char.",
		warnStr, routine, strlen( flag ) + 1 );
		return 1;
	}
...
{	char routine[]="(TSData::setUnits) -";
	...
	if( units == NULL ){
		/*AVCP PrintWarning( 1, routine,*/
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		"%s %s Attempting to set a NULL units string.",
	        warnStr, routine );
		return 1;
	}
	...
	if( !_units ){
		/*AVCP PrintWarning( 1, routine,*/
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		"%s %s Unable to allocate memory for %d char.",
		warnStr, routine, strlen( units ) + 1 );
		return 1;
	}
...
{	char routine[] = "(TSDateIterator(TS*)) -";
	...
	if ( ts ) {
		...
		if ( !_date1 ) {
			/*AVCP PrintWarning ( 2, routine,*/
		        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
			"%s %s Unable to set start date in constructor",
		        warnStr, routine );
		}
		if ( !_date2 ) {
			/*AVCP PrintWarning ( 2, routine,*/
		        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
			"%s %s Unable to set end date in constructor",
		        warnStr, routine );
		}
	}
	else {	
	   /*AVCP PrintWarning ( 2, routine,*/
	   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	   "%s %s NULL time series for iterator", warnStr, routine );
	}
...
{	char routine[] = "(TSDateIterator(TS*,TSDate,TSDate)) -";
	...
	if ( !ts ) {
		/*AVCP PrintWarning ( 2, routine,*/
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	        "%s %s Null time series for iterator",
	        warnStr, routine );
	}
	else {	_ts = ts;
...
int TSDate::operator= ( char* string )
{
	...
	if( !nlist1 || !list1 ){
		/*AVCP PrintWarning( 2, routine,*/
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		"%s %s Troubles breaking initial string \"%s\".", 
		warnStr, routine, string );
		return( STATUS_FAILURE );
	}
	...
	if( strstr( list1[0], "/" ) ){
		...
		if( nlist2 != 3 ){
			/*AVCP PrintWarning( 1, routine,*/
		        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		        "%s %s Month/Day/Year string \"%s\" not in anticipated "
			"format.", warnStr, routine, list1[0] );
		...
		if( !value ){
			/*AVCP PrintWarning( 1, routine,*/
		        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
			"%s %s Month portion of HECDSS string \"%s\" is not"
		        " valid.", warnStr, routine, list1[0] );
			list1 = FreeStringList( list1 );
			return( STATUS_FAILURE );
		}
		...
		if( !IsInteger( tmp ) ){
			/*AVCP PrintWarning( 1, routine,*/
		        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
			"%s %s Year portion  of string \"%s\" is not valid.",
			warnStr, routine, list1[0] );
			list1 = FreeStringList( list1 );
			return( STATUS_FAILURE );
		}
		value = atoi( tmp );
		...
		if( strlen( list1[0] ) != 8 ){
			/*AVCP PrintWarning( 1, routine,*/
		        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		        "%s %s YearDayMonth string \"%s\" is not in "
			"anticipated format.", warnStr, routine, list1[0] );
			list1 = FreeStringList( list1 );
			return( STATUS_FAILURE );
		}
	...
	if( nlist1 >= 2 ){
		...
		if( nlist2 != 3 ){
			/*AVCP PrintWarning( 2, routine,*/
		        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
			"%s %s Hour:Min:Sec string \"%s\" not in anticipated "
			"format.", warnStr, routine, list2[1] );
			list1 = FreeStringList( list1 );
			list2 = FreeStringList( list2 );
			return( STATUS_FAILURE );
		}
		...
		if ( getFewsDebugFlag() > 10 )
		{
	           /*AVCP PrintDebug( 10, routine, */
		   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		   "%s Date \"%s\" only will set Month, Day, and year.", 
		   routine, string );
		}
	}
	...
	if( nlist1 >= 3 ){
		...
		if( IsInteger( list1[2] ) ){
			/*AVCP PrintWarning( 1, routine,*/
		        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
			"%s %s TZ string \"%s\" not in anticipated format.",
		        warnStr, routine, string );
			list1 = FreeStringList( list1 );
			return( STATUS_FAILURE );
		}
		...
		if ( getFewsDebugFlag() > 10 )
		{
	           /*AVCP PrintDebug( 10, routine,*/
		   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		   "%s Date \"%s\" does not contain TZ, using Zulu.",
		   routine, string );
		}
		setTimeZone( "Z" );
	}
	...
	if ( getFewsDebugFlag() > 10 )
	{
	   /*AVCP PrintDebug( 10, routine,*/
	   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	   "%s Converted \"%s\" to %d/%d/%d %02d:%02d:%02d.%02d %s.", routine,
	   string, _month, _day, _year, _hour, _minute, 
	   _second, _hsecond, _tz );
	}
...
{      	char routine[] = "(TSDate::addInterval) -";
	...
	if( interval == TS::INTERVAL_SECOND ) {
		...
		/*AVCP PrintWarning( 1, routine,*/
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		"%s %s Incrementing by Weekly not Implemented yet",
	        warnStr, routine );
                return ( 1 );
        }
                ...
                /*AVCP PrintWarning ( 2, routine,*/
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		"%s %s Interval %d is unsupported", 
		warnStr, routine, interval );
...
{	char routine[] = "TSDate.formatTimeString";
	...
	/*AVCP PrintWarning ( 1, routine,*/
        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
        "%s (%s) - NOT IMPLEMENTED", warnStr, routine );
	return "";
}
					...
					int index )
{
	...
	if ( getFewsDebugFlag() > 10 )
	{
	   /*AVCP PrintDebug( 30, routine,*/
	   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	   "(%s) - Date for %d is %d/%d/%d %d", routine,
...
int TSDate::getNumIntervals( TSDate t1, TSDate t2, int base, int mult )
{
	...
	if( t2 < t1 ) {
		/*AVCP PrintWarning( 1, routine, */
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		"%s %s End Date (%d/%d/%d) is before Start date (%d/%d/%d)",
		warnStr, routine,
        ...
        if ( getFewsDebugFlag() > 10 )
	{
	   /*AVCP PrintDebug( 30, routine, */
	   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	   "%s %d Intervals (%d,%d) between %d/%d/%d and %d/%d/%d", routine,
 	   intervals, base, mult,
	   t1.getMonth(), t1.getDay(), t1.getYear(),
	   t2.getMonth(), t2.getDay(), t2.getYear() );
	}
...
{	char	routine[]="TSDate::setDay";
	...
	if( _behavior_flag & DATE_STRICT ){
                if( _day > NumDaysInMonth( _month, _year ) || _day < 1 ) {
                        /*AVCP PrintWarning( 2, routine,*/
		        /*logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
                        "%s (%s) - day has been set to %d for year %d", 
			warnStr, routine, _day, _year );
			*/
			if ( getFewsDebugFlag() > 0 )
			{
		  	
		   	   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		  	   "(%s) - day has been set to %d for year %d", 
			   routine, _day, _year );
			}
                }
	}
...
{	char	routine[]="TSDate::setHSecond";
	...
	if( _behavior_flag & DATE_STRICT ){
                if( _hsecond > 99 || _hsecond < 0 ) {
                        /*AVCP PrintWarning( 1, routine,*/
		        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
                        "%s (%s) - hsecond has been set to %d", 
			warnStr, routine, _hsecond );
                }
	}
	return 0;
}
...
{	char	routine[]="TSDate::setHour";
	...
	if( _behavior_flag & DATE_STRICT ){
                if( _hour > 23 || _hour < 0 ) {
		//3/11/2010 changed to debug
                        /*AVCP PrintWarning( 2, routine,*/
		        /*logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
                        "%s (%s) - hour has been set to %d", 
			warnStr, routine, _hour );
			*/
			if ( getFewsDebugFlag() >= 1 )
			{
		  	
		   	logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		  	 "(%s) - hour has been set to %d", 
			 routine, _hour );
			}
                }
	}
...
{	char	routine[]="TSDate::setMinute";
	...
	if( _behavior_flag & DATE_STRICT ){
                if( _minute > 59 || _minute < 0 ) {
                        /*AVCP PrintWarning( 2, routine,*/
		        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
                        "%s (%s) - minute has been set to %d", 
			warnStr, routine, _minute );
                }
	}
...
{	char	routine[]="TSDate::setMonth";
	...
	if( _behavior_flag & DATE_STRICT ){
                if( _month > 12 || _month < 1 ) {
                        /*AVCP PrintWarning( 2, routine,*/
		        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
                        "%s (%s) - month has been set to %d", 
			warnStr, routine, _month );
                }
	}
...
{	char	routine[]="TSDate::setSecond";
	...
	if( _behavior_flag & DATE_STRICT ){
                if( _second > 59 || _second < 0 ) {
                        /*AVCP PrintWarning( 2, routine,*/
		        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
                        "%s (%s) - second has been set to %d", 
			warnStr, routine, _second );
                }
	}
...
{	char	routine[]="TSDate::setTimeZone";
	...
	if( !_tz ){
		/*AVCP PrintWarning( 2, routine,*/
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		"%s (%s) - Unable to allocate memory for %s", 
		warnStr, routine, zone );
		return( 1 );
	}
...
{	char	routine[]="TSDate::setWeekday";
	...
	if( _behavior_flag & DATE_STRICT ){
                if( _weekday < 0 ) {
                        /*AVCP PrintWarning( 2, routine, */
		        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
                        "%s (%s) - Weekday has been set to %d", 
			warnStr, routine, _weekday );
                }
	}
...
{	char	routine[]="TSDate::setYear";
	...
	if( _behavior_flag & DATE_STRICT ){
                if( _year < 0 ) {
                        /*AVCP PrintWarning( 2, routine,*/
		        /*logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
                        "%s (%s) - year has been set to %d", 
			warnStr, routine, _year );
			*/
			if ( getFewsDebugFlag() >= 1 )
			{
		  	
		   	logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		  	"(%s) - year has been set to %d", 
			routine, _year );
			}
                }
	}
...
{	
	...
	if( dist == NULL ) {
	        if ( getFewsDebugFlag() > 10 )
		{
		   /*AVCP PrintDebug( 10, routine, */
		   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		   "%s Distribution is not set, using default uniform "
		   "distribution for %d values.", routine, n_dist );
		}
		for( i = 0; i < _n_dist; i++ ) {
...
{	char	routine[] = "TSIdent::TSIdent(copy)";
        ...
        if ( getFewsDebugFlag() > 10 )
	{
	   /*AVCP PrintDebug ( 50, routine,*/
	   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	   "(%s) - Enter copy constructor", routine );
	}
	init();
...
{	char	routine[] = "(TSIdent::init) -";
	...
	if ( getFewsDebugFlag() > 10 )
	{
	   /*AVCP PrintDebug ( dl, routine,*/
	   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	   "%s Initializing TSIdent...", routine );
	}
	...
	if ( setAlias ("") ) {
		/*AVCP PrintWarning( 1, routine,*/
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		"%s %s Unable to initialize alias", warnStr, routine );
		++error_count;
	}
	...
	if ( setFullIdentifier ("") ) {
		/*AVCP PrintWarning( 1, routine,*/
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		"%s %s Unable to initialize full identifier",
	        warnStr, routine );
		++error_count;
	}
	...
	if ( setMainLocation("") ){
		/*AVCP PrintWarning( 1, routine,*/
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		"%s %s Unable to initialize main location",
	        warnStr, routine );
		//++error_count;
	}
	...
	if ( setSubLocation("") ){
		/*AVCP PrintWarning( 1, routine,*/
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		"%s %s Unable to initialize sub-location" ,
		warnStr, routine );
		++error_count;
	}
	...
	if ( setMainSource("") ){
		/*AVCP PrintWarning( 1, routine,*/
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		"%s %s Unable to initialize main source", warnStr, routine );
		++error_count;
	}
	if ( setSubSource("") ){
		/*AVCP PrintWarning( 1, routine,*/
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	         "%s %s Unable to initialize sub-source", warnStr, routine );
		++error_count;
	}
	...
	if ( setType("") ){
		/*AVCP PrintWarning( 1, routine,*/
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	        "%s %s Unable to initialize type", warnStr, routine );
		++error_count;
	}
	...
	if ( setInterval("") ){
		/*AVCP PrintWarning( 1, routine,*/
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	        "%s %s Unable to initialize interval", warnStr, routine );
		++error_count;
	}
	...
	if ( setScenario("") ){
		/*AVCP PrintWarning( 1, routine,*/
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	        "%s %s Unable to initialize scenario", warnStr, routine );
		++error_count;
	}
        if ( getFewsDebugFlag() > 10 ) 
	{
	   /*AVCP PrintDebug ( dl, routine,*/
	    logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	    "%s ...done initializing TSIdent.", routine );
	}
...
{	char	routine[]="(TSIdent::parseIdentifier(char*,flag)) -";
	...
	if ( getFewsDebugFlag() > 10 )
	{
	   /*AVCP PrintDebug ( dl, routine,*/
	   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	   "%s Declare TSIdent within this routine...", routine );
	}
	...
	if ( getFewsDebugFlag() > 10 )
	{
	   /*AVCP PrintDebug ( dl, routine,*/
	   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	   "%s ...done declaring TSIdent", routine );
	}
	...
	for ( i = 0; i < nlist1; i++ ) {
	        if ( getFewsDebugFlag() > 10 )
		{
		   /*AVCP PrintDebug ( dl, routine,*/
		   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		   "%s TS ID list[%d]:  \"%s\"", routine, i, list[i] );
		}
	}
	...
	if ( getFewsDebugFlag() > 10 )
	{
	   /*AVCP PrintDebug ( dl, routine,*/
	   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	   "%s Full TS ID:  \"%s\"", routine, identifier );
	}
	...
	*/
	if ( getFewsDebugFlag() > 10 )
	{
	   /*AVCP PrintDebug ( dl, routine,*/
	   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	   "%s %s After split: fullloc=\"%s\" fullsrc=\"%s\" type=\"%s\" "
	   " int=\"%s\" scen=\"%s\"", warnStr, routine,
        ...
        if ( getFewsDebugFlag() > 10 )
	{
	   /*AVCP PrintDebug ( dl, routine, */
	   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	   "%s Returning local TSIdent...", routine );
	}
	return tsident;
}
//------------------------------------------------------------------------------
...
{	char	routine[]="TSIdent.setAlias";
        ...
        if ( getFewsDebugFlag() > 10 )
	{
	   /*AVCP PrintDebug ( dl, routine,*/
	   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	   "(%s) - Trying to set alias to \"%s\"", routine, alias );
	}
	...
	if( !_alias ){
		/*AVCP PrintWarning( 1, routine,*/
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		"%s (%s) - Unable to Allocate memory for alias \"%s\"", 
		warnStr, routine, alias );
                return 1;
	}
...
{	char	routine[] = "TSIdent::setFullIdentifier";
	...
	if( !_identifier ) {
	       /*AVCP PrintWarning( 1, routine,*/
	       logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	       "%s (%s) - Unable to allocate memory for full identifier \"%s\"",
	       warnStr, routine, full_identifier );
               return( STATUS_FAILURE );
	}
...
{	char	routine[] = "TSIdent::setFullLocation";
        ...
        if ( getFewsDebugFlag() > 10 )
	{
	   /*AVCP PrintDebug ( dl, routine,*/
	   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	   "(%s) - Resetting full location to \"%s\"",
	   routine, full_location );
	}
	...
	if( !_full_location ) {
		/*AVCP  PrintWarning( 1, routine,*/
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		"%s (%s) - Unable to allocate memory for full location \"%s\"",
		warnStr, routine, full_location );
                return( STATUS_FAILURE );
	}
...
{	char	routine[] = "TSIdent::setFullSource";
        ...
        if ( getFewsDebugFlag() > 10 )
	{
	   /*AVCP PrintDebug ( dl, routine,*/
	   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	   "(%s) - Setting full source to \"%s\"", routine, full_source );
	}
	...
	if( !_full_source ) {
		/*AVCP PrintWarning( 1, routine,*/
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		"%s (%s) - Unable to allocate memory for full source \"%s\"",
		warnStr, routine, full_source );
                return( STATUS_FAILURE );
	}
...
{	char	routine[] = "(TSIdent::setIdentifier(void)) -";
        ...
        if ( getFewsDebugFlag() > 10 )
	{
	   /*AVCP PrintDebug ( dl, routine,*/
	   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	   "%s Setting full identifier from parts...", routine );
	}
	...
	if ( getFewsDebugFlag() > 10 )
	{
	   /*AVCP PrintDebug ( dl, routine,*/
	   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	   "%s ...successfully called getIdentifierFromParts...", routine );
	}
...
{	char	routine[] = "(TSIdent::setIdentifier) -";
        ...
        if ( getFewsDebugFlag() > 10 )
	{
	   /*AVCP PrintDebug ( dl, routine, */
	   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	   "%s Trying to set identifier to \"%s\"", routine, identifier );
	}
	...
	if ( !identifier[0] ) {
		...
		if ( getFewsDebugFlag() > 10 )
		{
	           /*AVCP PrintDebug ( dl, routine,*/
		   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		   "%s identifier string is empty, not processing!", routine );
		}
		return 1;
	}
	...
	if ( getFewsDebugFlag() > 10 )
	{
	   /*AVCP PrintDebug ( dl, routine,*/
	   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	   "%s Done declaring temp TSIdent.", routine );
	   /*AVCP PrintDebug ( dl, routine,*/
	   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	   "%s Parsing identifier...", routine );
	}
	...
	if ( getFewsDebugFlag() > 10 ) 
	{
	   /*AVCP PrintDebug ( dl, routine,*/
	   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	   "%s ...back from parsing identifier", routine );

	// Now copy the temporary copy into this instance...
	   /*AVCP PrintDebug ( dl, routine,*/
	   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	   "%s Setting the individual parts...", routine );
	}
	...
	if ( getFewsDebugFlag() > 10 ) 
	{
	   /*AVCP PrintDebug ( dl, routine,*/
	   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	   "%s ... done setting the individual parts", routine );
	}
...
{	char		interval_base_string[256],
	...
	if ( getFewsDebugFlag() > 10 )
	{
	   /*AVCP PrintDebug ( dl, routine,*/
	   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	   "(%s) - Setting interval to \"%s\"...", routine, interval_string );
	}
...
{	char	routine[] = "(TSIdent.setInterval) -";
	...
	if ( interval_mult <= 0 ) {
		/*AVCP PrintWarning ( 2, routine,*/
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		"%s %s Interval multiplier (%d) must be greater than zero",
		warnStr, routine, interval_mult );
		return 1;
	}
	if (	(interval_base != TS::INTERVAL_SECOND) &&
		...
		/*AVCP PrintWarning ( 2, routine,*/
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		"%s %s Base interval (%d) is not recognized", 
		warnStr, routine, interval_base );
		return 1;
	}
...
{	char	routine[] = "(TSIdent::setIntervalString) -";
	...
	if( !_interval_string ) {
		/*AVCP PrintWarning( 1, routine, */
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		"%s %s Unable to allocate memory for interval string \"%s\"",
		warnStr, routine, interval_string );
                return( STATUS_FAILURE );
	}
...
{	char	routine[] = "TSIdent.setLocation(void)";
	...
	if ( getFewsDebugFlag() > 10 )
	{
	   /*AVCP PrintDebug ( dl, routine,*/
	   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	   "(%s) - Resetting full location from parts...", routine );
	}
...
{	char	routine[] = "TSIdent::setLocation(char*)";
	...
	if ( getFewsDebugFlag() > 10 )
	{
	   /*AVCP PrintDebug ( dl, routine,*/
	   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	   "(%s) - Trying to set location to \"%s\"", routine, location );
	}
...
{	char	routine[] = "TSIdent::setMainLocation";
	...
	if ( getFewsDebugFlag() > 10 )
	{
	   /*AVCP PrintDebug ( dl, routine,*/
	   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	   "(%s) - Trying to set main location to \"%s\"", 
	   routine, main_location );
	}
	...
	if( !_main_location ) {
		/*AVCP PrintWarning( 1, routine,*/
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		"%s (%s) - Unable to allocate memory for main location \"%s\"",
		warnStr, routine, main_location );
                return 1;
	}
...
{	char	routine[] = "TSIdent.setMainSource";
        ...
        if ( getFewsDebugFlag() > 10 ) 
	{
	   /*AVCP PrintDebug ( dl, routine,*/
	   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	   "(%s) - Setting main source to \"%s\"", routine, main_source );
	}
	...
	if( !_main_source ) {
		/*AVCP PrintWarning( 1, routine,*/
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		"%s (%s) - Unable to allocate memory for main source \"%s\"",
		warnStr, routine, main_source );
		return 1;
	}
...
{	char	routine[] = "TSIdent.setScenario";
	...
	if ( getFewsDebugFlag() > 10 ) 
	{
	   /*AVCP PrintDebug ( dl, routine,*/
	   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	   "(%s) - Setting scenario to \"%s\"...", routine, scenario );
	}
	if( _scenario ) {
		delete [] _scenario;
	}
	...
	if( !_scenario ){
		/*AVCP PrintWarning( 1, routine,*/
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		"%s (%s) - Unable to allocate memory for scenario \"%s\"",
	        warnStr, routine, scenario );
		return 1;
	}
...
{	char	routine[] = "TSIdent.setSource";
        ...
        if ( getFewsDebugFlag() > 10 )
	{
	   ...
	   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	   "(%s) Resetting full source from its parts", routine );
	}
...
{	char	routine[] = "TSIdent.setSource(char*,char*)";
	...
	if ( getFewsDebugFlag() > 10 )
	{
	   /*AVCP PrintDebug ( dl, routine,*/
	   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	   "(%s) - Setting source using \"%s\" and \"%s\"", routine,
...
{	char	routine[] = "TSIdent.setSource(char*)";
	...
	if ( getFewsDebugFlag() > 10 )
	{
	   /*AVCP PrintDebug ( dl, routine, */
	   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	   "(%s) - Setting source using \"%s\"", routine, source );
	}
	if ( _behavior_mask & NO_SUB_SOURCE ) {
...
{	char	routine[]="TSIdent.setSubLocation";
	...
	if ( getFewsDebugFlag() > 10 )
	{
	   /*AVCP PrintDebug ( dl, routine,*/
	   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	   "(%s) - Trying to set sub-location to \"%s\"", 
	   routine, sub_location );
	}
	if( _sub_location ){
        	delete [] _sub_location;
	}
	...
	if( !_sub_location ){
		/*AVCP PrintWarning( 1, routine,*/
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		"%s (%s) - Unable to Allocate memory for sub-location \"%s\"",
		warnStr, routine, sub_location );
                return 1;
	}
...
{	char	routine[]="TSIdent.setSubSource";
	...
	if ( getFewsDebugFlag() > 10 )
	{
	   /*AVCP PrintDebug ( dl, routine, */
	   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	   "(%s) - Setting sub-source to \"%s\"", routine, sub_source );
	}
	if( _sub_source ){
        	delete [] _sub_source;
	}
	...
	if( !_sub_source ){
		/*AVCP PrintWarning( 1, routine,*/
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		"%s (%s) - Unable to Allocate memory for sub-source \"%s\"",
		warnStr, routine, sub_source );
                return 1;
	}
...
{	char 	routine[] = "TSIdent.setType";
	...
	if ( getFewsDebugFlag() > 10 ) 
	{
	   /*AVCP PrintDebug ( dl, routine,*/
	   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	   "(%s) - Setting type to \"%s\"...", routine, type );
	}
	if( _type ) {
        	delete [] _type;
	}
	...
	if( !_type ){
		/*AVCP PrintWarning( 1, routine,*/
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		"%s (%s) - Unable to allocate memory for data type \"%s\"", 
		warnStr, routine, type );
                return 1;
	}
...
{	char		digit[250], routine[] = "TSInterval.parseInterval";
	...
	if( digit_count == 0 ){
		...
		if ( getFewsDebugFlag() > 10 )
		{
		   /*AVCP PrintDebug( dl, routine,*/
		   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		   "(%s) - %d Hourly", routine,	tsinterval.getMultiplier() );
		}
		return tsinterval;
	}
	...
	if ( getFewsDebugFlag() > 10 )
	{
	   /*AVCP PrintDebug ( dl, routine, */
	   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	   "(%s) - Multiplier: %d", routine, tsinterval.getMultiplier() );
	}
	...
	if(	!ESPstrncasecmp( &interval_string[digit_count], "minute", 6) ) {
	        ...
	        if ( getFewsDebugFlag() > 10 )
		{
	           /*AVCP PrintWarning( 2, routine,*/
	           logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		   "(%s) - Unrecognized Interval \"%s\", returning HOURLY", 
 		   routine, &interval_string[digit_count] );
		}
		tsinterval.setBase ( TS::INTERVAL_HOUR );
	}
	...
	if ( getFewsDebugFlag() > 10 )
	{
	   /*AVCP PrintDebug( dl, routine,*/
	   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	   "(%s) - Base: %d (%s), Mult: %d", routine, tsinterval.getBase (),
	   tsinterval.getBaseString (),	tsinterval.getMultiplier () ); 
	}
...
{	char	routine[]="TSInterval.setBaseString";
	...
	if ( getFewsDebugFlag() > 10 )
	{
	   /*AVCP PrintDebug ( dl, routine, */
	   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	   "(%s) - Trying to set sub-location to \"%s\"", routine,
	...
	if( !_interval_base_string ){
		/*AVCP PrintWarning( 1, routine,*/
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		"%s (%s) - Unable to Allocate memory for sub-location \"%s\"",
		warnStr, routine, interval_base_string );
                return 1;
	}
...
int TSList::addTSToList( TS* time_series )
{
	...
	if( _ts_list == NULL ) {
		/*AVCP PrintWarning( 1, routine,*/
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	        "%s (%s) - Cannot allocate %d TS pointers.",
		warnStr, routine, _num_ts+1 );
		return( STATUS_FAILURE );
	}
...
int TSList::addTSToList( TS* time_series, char* io_set )
{
	...
	if( _io_flags == NULL ) {
		/*AVCP PrintWarning( 2, routine,*/
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	        "%s (%s) - Could not allocate %d ints.", warnStr, routine,
		_num_ts );
		return( STATUS_FAILURE );
	}
...
TS* TSList::getTSFromList( char* ident )
{
	...
	if( list == NULL || nlist == 0 ) {
		/*AVCP PrintWarning( 1, routine,*/
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	        "%s (%s) - TS id is malformed.", warnStr, routine );
		return( (TS*)NULL );
	}
	if( nlist == 3 ) {
		...
		if( ts_ident->setLocation( list[0] ) ||
			...
			/*AVCP PrintWarning( 1, routine,*/
		        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		        "%s (%s) - TSIdent %s is malformed.", warnStr,
			routine, ident );
			delete ts_ident;
			list = FreeStringList( list );
			return( (TS*) NULL );
		 ...
		 if ( getFewsDebugFlag() > 10 )
		 {
		    /*AVCP PrintDebug( 5, routine,*/
		    logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		    "(%s) - Could not find time series with "
 		    "identifier %s in list.", routine, 
		    ts_ident->getIdentifier() );
		 }
		delete ts_ident;
		list = FreeStringList( list );
		return( (TS*) NULL );
	...
	// If we get here we are dealing with an alias
		 ...
		 if ( getFewsDebugFlag() > 10 )
		 {
		    ...
		    logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		    "(%s) - Could not find time series with "
		    "alias %s in list.", routine, list[0] );
		 }
		 list = FreeStringList( list );
		 return( (TS*) NULL );
	}
		 ...
		 /*AVCP PrintWarning( 1, routine,*/
	         logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	         "%s (%s) - Could not find time series using identifier %s.", 
		 warnStr, routine, ident );
		 list = FreeStringList( list );
		 return( (TS*) NULL );
	}
}
...
int TSTimeZone::calculateOffsetHours( char* new_code )
{
	...
	if( new_code == NULL ){
		/*AVCP PrintWarning( 2, routine,*/
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		"%s (%s) - Incoming Zone is NULL, cannot continue.",
	        warnStr, routine );
		return 0;
	}
...
int TSTimeZone::calculateOffsetHours( char* from_tz, char* to_tz )
{
	...
	if( !from_tz || !to_tz ){
		/*AVCP PrintWarning( 2, routine,*/
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		"%s (%s) - Incoming arguments are NULL, cannot continue.",
	        warnStr, routine );
		return( 0 );
	}
	...
	if ( getFewsDebugFlag() > 10 )
	{
	   /*AVCP PrintDebug( dl, routine,*/
	   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	   "(%s) - Offset from \"%s\" -> \"%s\" is %d.", 
	   routine, from_tz, to_tz, diff );
	}
	...
	while( TimeZoneData[i].tz[0] != '\0' ){
		if( 	TimeZoneData[i].ntz == zone && 
		        ...
		        if ( getFewsDebugFlag() > 10 )
			{
			   /*AVCP PrintDebug( dl, routine,*/
			   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
			   "(%s) - Code for zone %d (%d) is \"%s\".",
			   routine, zone, ds_flag, TimeZoneData[i].tz );
			}
...
//cfan		strcpy( code, "Z" ); 
		...
		/*AVCP PrintWarning( 2, routine,*/
		logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		"%s (%s) - Zone %d (%d) is not found in look-up list, "
		"returning %s", warnStr, routine,
...
int TSTimeZone::getDefinedDSFlag( char* code )
{
	...
	if( code == NULL ){
		/*AVCP PrintWarning( 2, routine,*/
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		"%s (%s) - Incoming Code is NULL, cannot continue.",
	        warnStr, routine );
		return( 0 );
	}
...
char* TSTimeZone::getDefinedDescription( char* string )
{
	...
	/*AVCP PrintWarning( 1, routine,*/
        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
        "%s (%s) - Not enabled yet", warnStr, routine );
	return( (char*)NULL );
}
//------------------------------------------------------------------------------
	...
	while( TimeZoneData[i].tz[0] != '\0' ){
		if( !strcmp( TimeZoneData[i].tz, code ) ){
		        if ( getFewsDebugFlag() > 10 )
			{
			   /*AVCP PrintDebug( dl, routine,*/
			   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
			   "(%s) - Zone Number for \"%s\" ( %s ) is %d.",
			   routine, code, TimeZoneData[i].desc, 
			   TimeZoneData[i].ntz );
			}
...
int TSTimeZone::reset( int flag )
{
	...
	if( flag == RESET_WITH_CODE ){
		...
		/*AVCP PrintWarning( 2, routine,*/
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		"%s (%s) - Unrecognized reset flag %d.", 
		warnStr, routine, flag );
		return( 1 );
	}
}
//------------------------------------------------------------------------------
...
//cfan	char		tsfile[256];
	...
	if ( !tsid_string0 ) {
		/*AVCP PrintWarning ( 1, routine,*/
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	        "%s (%s) - Time series identifier is NULL", warnStr, routine );
		return (char *)NULL;
	}
	else if ( !tsid_string0[0] ) {
		/*AVCP PrintWarning ( 1, routine,*/
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		"%s (%s) - Time series identifier is empty",
	        warnStr, routine );
		return (char *)NULL;
	}
	...
	if ( !tsdatapath0 ) {
		/*AVCP PrintWarning ( 1, routine,*/
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		"%s (%s) - Time series path is NULL.  Using \".\"",
	        warnStr, routine );
		strcpy ( tsdatapath, "." );
	}
		...
		/*AVCP PrintWarning ( 1, routine,*/
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		"%s (%s) - Time series path is empty.  Using \".\"",
	        warnStr, routine );
		strcpy ( tsdatapath, "." );
	}
	...
	if ( getFewsDebugFlag() > 10 )
	{
	   /*AVCP PrintDebug ( 1, routine,*/
	   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	   "(%s) - Trying to find time series for ID \"%s\"",
	   routine, tsid_string );
	}
	...
	if ( tsdatadirs == (char **)NULL ) {
	        if ( getFewsDebugFlag() > 10 )
		{
		   /*AVCP PrintDebug ( 1, routine,*/
		   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		   "(%s) - Trying to initialize global path data...", routine );
		}
/*
...
*/
			...
			if ( !ntsdatadirs ) {
				...
				/*AVCP PrintWarning ( 1, routine,*/
			        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
				"%s (%s) - DSS_TSDATA_PATH is empty.  "
				"Using \".\"", warnStr, routine );
				tsdatadirs = AddToStringList ( tsdatadirs, ".",
				&ntsdatadirs );
			}
					...
					/*AVCP PrintDebug ( 10, routine,*/
				        logMessageWithArgsAndExitOnError( 
				        DEBUG_LEVEL, "(%s) - tsdatadir[%d] ="
				       	"\"%s\"", routine, i, tsdatadirs[i] );
				    }
				}
			}
/*
...
*/
	...
	if ( scenario[0] ) {
	        ...
	        if ( getFewsDebugFlag() > 10 )
		{
		   /*AVCP PrintDebug ( 10, routine,*/
		   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		   "(%s) - Trying TS file from scenario:  \"%s\"", 
		   routine, scenario );
		}
		if ( FileReadable(scenario) ) {
		        ...
		        if ( getFewsDebugFlag() > 10 )
			{
			   /*AVCP PrintDebug ( 10, routine,*/
			   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
			   "(%s) - Found TS file from scenario:  \"%s\"", 
			   routine, scenario );
			}
...
//cfan			return tsfile;
		...
		for ( i = 0; i < nfulltsdatadirs; i++ ) {
		        if ( getFewsDebugFlag() > 10 )
			{
			   /*AVCP PrintDebug ( 10, routine,*/
			   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
			   "(%s) - Trying TS file from path:  \"%s\"", 
			   routine, fulltsdatadirs[i]);
			}
...
//cfan				return tsfile;
	...
	/*AVCP PrintWarning ( 1, routine,*/
	logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	"%s (%s) - Unable to find TS file for \"%s\"", 
	warnStr, routine, tsid_string );
...
{	char	routine[]="(TSUtil.getDataLimits) -";
	...
	if ( !ts ) {
		/*AVCP PrintWarning ( 2, routine,*/
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	        "%s %s NULL time series", warnStr, routine );
		TSLimits tslimits0;
		return tslimits0;
	}
...
*/
	...
	if( found == 0 ){
		/*AVCP PrintWarning( 1, routine,*/
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		"%s %s Problems Finding Limits, whole POR missing!",
	        warnStr, routine );
		TSLimits limits0;
		return limits0;
	}
	...
	if ( getFewsDebugFlag() > 10 )
	{
	   /*AVCP PrintDebug( 10, routine,*/
	   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	   "%s Found limits to be: %f (%s)-> %f (%s)", routine,
	   min, (char *)min_date, max, (char *)max_date );
	   /*AVCP PrintDebug( 10, routine,*/
	   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	   "%s Found non-missing data dates to be: %s -> %s", routine,
...
{	char		rtn[] = "TSUtil.getPeriodFromTS";
	...
	TSLimits	limits;
        ...
        if ( getFewsDebugFlag() > 10 )
	{	   
	   /*AVCP PrintDebug ( 10, rtn,*/
	   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	   "(%s) - Checking %d TS to find POR limits", rtn, nstats );
	}

	// Check for NULL list...

	if ( nstats == 0 ) {
		/*AVCP PrintWarning ( 2, rtn,*/
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	        "%s (%s) - Zero time series list", warnStr, rtn );
		return limits;
	}
	...
	if ( !ts[0] ) {
		/*AVCP PrintWarning ( 2, rtn,*/
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		"%s (%s) - Null first time series", warnStr, rtn );
		return limits;
	}
	...
	for (i=1; i<nstats; i++) {
		...
		if ( flag == MAX_POR ) {
		   ...
		   /*AVCP PrintWarning ( 2, rtn,*/
		        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		        "%s (%s) - Unknown flag option %u.", warnStr, rtn,
			flag );
			return limits;
		}
	}
	...
	if ( flag == MIN_POR ) {
	        if ( getFewsDebugFlag() > 10 ) 
		{
		   /*AVCP PrintDebug ( 10, rtn,*/
		   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		   "(%s) - Found minimum POR limits to be %s to %s",
		   rtn, (char*)start, (char*)end );
		}
	}
	        ...
	        if ( getFewsDebugFlag() > 10 )
		{
		   /*AVCP PrintDebug ( 10, rtn,*/
		   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		   "(%s) - Found maximum POR limits to be %s to %s", 
		   rtn, (char*)start, (char*)end );
		}
	}
...
TSDate TSUtil::getTSDateFromString( char* date_string )
{
	...
	if( strlen( date_string ) == 8 ){
	    ...
	    {
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		"%s (%s) - Invalid Date String %s, returning empty date", 
		warnStr, routine, date_string );
	    }
	...
	if ( getFewsDebugFlag() > 10 )
	{
	   /*AVCP PrintDebug( 10, routine, */
	   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	   "(%s) - Converted %s to: %d/%d %02d:%02d", routine,
...
{	char	routine[] = "TS.TS(char*)";
	...
	/*AVCP PrintWarning ( 1, routine,*/
        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	"%s (%s) - Constructing from file (%s) is not enabled", 
	warnStr, routine, input );
	init();
}
...
{	char	routine[] = "TS::allocateDataSpace";
	...
	/*AVCP PrintWarning( 1, routine, */
        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	"%s (%s) - This is a virtual function, redefine in lower classes",
        warnStr, routine );
...
{	char	routine[]="TS::formatOutput(int,long)";
	...
	/*AVCP PrintWarning( 1, routine,*/
        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	"%s (%s) - This function is virtual, redefine in derived classes",
        warnStr, routine );
...
{	char	routine[]="TS.refresh()";
	...
	/*AVCP PrintWarning( 1, routine,*/
        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	"%s (%s) - This is a virtual function, define in lower classes",
        warnStr, routine );
...
int TS::writePersistentHeader( ostream& in )
{
	...
	/*AVCP PrintWarning( 1, routine,*/
	logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	"%s (%s) - This is a virtual function, should be defined in the "
	"derived class", warnStr, routine );
...
int TS::writePersistentHeader( char *fname )
{
	...
	/*AVCP PrintWarning( 1, routine,*/
	logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	"%s (%s) - This is a virtual function, should be defined in the derived"
	"class", warnStr, routine );
