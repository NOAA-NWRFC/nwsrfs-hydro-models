...
int ConstExpr  ::  verify()
{
		   ...
		   if(  getFewsDebugFlag() > 10 )
	           {
	               ...
	               "(%s) Attaching to the DATE variable...",routine );
	           }
		...
		// It could also be the string representation of a date...
			...
			if( nlist < 2 || nlist > 4 ){
			     logMessageWithArgsAndExitOnError( FATAL_LEVEL, 
	                        "%s (%s) Date \"%s\" is not valid format.",errorStr,
				routine, _constant  );				
			...
			if( !_value ){
			   logMessageWithArgsAndExitOnError( FATAL_LEVEL, 
	                        "%s (%s) Unable to allocate memory for a double.",
				errorStr, routine);
			...
			*_value = date.toDouble() - 1;
		    ...
		    if( getFewsDebugFlag() > 10 )
	            {
                       logMessageWithArgsAndExitOnError( DEBUG_LEVEL, 
	               "(%s) Looking up variable \"%s\".",routine, _constant  );
	            }
		    ...
		    if( _value == NULL ){
		       /* AVCP PrintWarning( 1, routine */
		       logMessageWithArgsAndExitOnError( DEBUG_LEVEL, 
	                  "%s (%s) Unable to lookup value for \"%s\".",
			  warnStr, routine, _constant  );
	...
	{
           logMessageWithArgsAndExitOnError( DEBUG_LEVEL, 
	           "(%s) Evaluating numeric constant %s.",routine, _constant  );
	}
	...
	if( !_value ){
	    logMessageWithArgsAndExitOnError( FATAL_LEVEL, 
	           "%s (%s) Unable to allocate memory for a double.",errorStr, routine );
