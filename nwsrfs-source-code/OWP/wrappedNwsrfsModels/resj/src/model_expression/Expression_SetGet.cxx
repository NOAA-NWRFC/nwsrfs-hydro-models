...
double* Expression :: getConstantPtr( char* id )
{
	...
	if( id == NULL ){
	   /* AVCP PrintWarning( 1, routine */
	    logMessageWithArgsAndExitOnError( DEBUG_LEVEL, 
	       "%s (%s) - Incoming constant string is NULL.",warnStr, routine );
	...
	if( _component_root == NULL ){
	 /* AVCP PrintWarning( 1, routine */
	    logMessageWithArgsAndExitOnError( DEBUG_LEVEL, 
	       "%s (%s) - The Expression component root has not been set yet.",
	       warnStr, routine);	
	...
	if( !list || nlist != 2 ){
	    /* AVCP PrintWarning( 1, routine */
	    logMessageWithArgsAndExitOnError( DEBUG_LEVEL, 
	    "%s (%s) - Malformed constant \"%s\".",warnStr, routine, id );
	...
	if( comp == NULL ){
	   /* AVCP PrintWarning( 1, routine */
	   logMessageWithArgsAndExitOnError( DEBUG_LEVEL, 
	    "%s (%s) - Unable to find component matching \"%s\".",
	    warnStr, routine, list[0] );
	...
	if( pvalue == NULL ){
	   /* AVCP PrintWarning( 1, routine */
	   logMessageWithArgsAndExitOnError( DEBUG_LEVEL, 
	    "%s (%s) - Unable to handle Rule keyword \"%s\".  Nonstandard "
	    "and no constant of same name.",warnStr, routine,  id );
	...
	if( pvalue == NULL ){
	   /* AVCP PrintWarning( 1, routine */
	   logMessageWithArgsAndExitOnError( DEBUG_LEVEL, 
	    "%s (%s) - Unable to find location of \"%s\".",
	    warnStr, routine, id );
