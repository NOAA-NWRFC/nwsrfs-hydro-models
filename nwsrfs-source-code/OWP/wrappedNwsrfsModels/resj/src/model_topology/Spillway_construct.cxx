...
int Spillway::construct ( char** re_list, int n_items )  
{
	...
	for( i = 0; i < n_items; i++ ) {
		...
		if( list == NULL || nlist == 0 ) {
		   ...
		   "%s (%s) - Troubles getting elevation spill data for "
		   "Spillway %s %s.", errorStr, routine, ownerID, methID );
		/* AVCP
		   list = FreeStringList( list );
		   continue;
		   */
		...
		if( !strcasecmp( list[0], "TABLE" ) ) {
			...
			if( sub_list == NULL || nsublist == 0 ) {
				...
				"Does \"ENDTABLE\" exist?", 
				errorStr, routine, methID, ownerID );
				/* AVCP
				list = FreeStringList( list );
				continue;
				*/
			...
			if ( nlist < 2 ) {
				/* AVCP totErrs++;
			        PrintError( routine, */
				logMessageWithArgsAndExitOnError( FATAL_LEVEL,
				"%s (%s) - Keyword ELEV_SPILL required "
				"immediately after %s for  Spillway %s "
				"on %s.", errorStr, routine, 
				list[0], list[1], ownerID );
				/* AVCP
				list = FreeStringList( list );
			...
			if( !strcasecmp( list[1], "ELEV_SPILL" ) ) {
				if ( table_found == 1 ) {
				  if( getFewsDebugFlag() > 0 )
				  {
				   ...
				   "%s (%s) - Duplicate TABLE ELEV_SPILL for "
				   "Spillway %s on %s.  Last definition used.", 
   				   warnStr, routine, methID, ownerID );
				  }
				}
				...
				if( _stor_spill_tbl->populate( new_list, 
					...
					"Spill table for  Spillway %s on %s.",
					errorStr, routine, methID, ownerID );
					/* AVCP
					list = FreeStringList( list );
					sub_list = FreeStringList( sub_list );
		...
		if( !strcasecmp( list[0], "INITIALSPILL" ) ) {
			if( nlist < 2 ) {
				...
				/* AVCP PrintWarning( 1, routine, */
			       ...
			       {
		                logMessageWithArgsAndExitOnError( DEBUG_LEVEL, 
				"%s (%s) - Value required with SPILLWAY "
				"keyword %s for Method \"%s %s\". ", 
				warnStr, routine, list[0], _owner->_id, _id );
			       }
				_myValue = MISSING;
		...
		if ( !WarnedOnce ) {
		   ...
		   if( getFewsDebugFlag() > 0 )
		   {
		    ...
		    " If there are other errors or warnings for SPILLWAY %s, "
		    "this may disappear following their correction.",
	...
	if( _intervals == 0 ) {
		/* AVCP totErrs++;
	           PrintError( routine, */
	           logMessageWithArgsAndExitOnError( FATAL_LEVEL, 
		   "%s (%s) - SPILLWAY keyword \"INTERVALS\" not "
		   "defined for %s on %s.", errorStr, routine, 
		   methID, ownerID );
	}
	if( !table_found ) {
		/* AVCP totErrs++;
	        PrintError( routine, */
	        logMessageWithArgsAndExitOnError( FATAL_LEVEL, 
		"%s (%s) - SPILLWAY table for %s on %s "
		"not successfully read.", errorStr, routine, 
		methID, ownerID );
	} 
	...
	if( _myValue == MISSING ) {
		/* AVCP totErrs++;
	        PrintError( routine, */
	        logMessageWithArgsAndExitOnError( FATAL_LEVEL,
		"%s (%s) - SPILLWAY keyword \"INITIALSPILL\" and "
		"Value required but not defined for %s on %s.", 
		errorStr, routine, methID, ownerID );
	}
