	...
	double** group_val )
{
	...
	if ( _pool_obs ) {
		...
		// missing?
			...
			if( elev_value < _owner->_min_pool ) {
				...
				"MINIMUMPOOL used.", warnStr, routine, _owner->_id, 
	...
	if ( !haveTSvalue ) {
		...
		if ( elev_value == MISSING ) {
			...
			/* AVCP PrintWarning( 1, routine, */
		       logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		       "%s (%s) - SetElevation \"%s\" is "
		       "missing its target value for %s.  Setting it "
			"to current pool.", warnStr, routine, _id, cur_date.toString() );
...
// printf(" !RRR SetElevation group_id=%d res=%s meth=%s pool=%f release=%f \n", _group_id, _owner->getID(), _id, elev_value, release);
	...
	{
	   logMessageWithArgsAndExitOnError( DEBUG_LEVEL, 
	   "(%s) - Setting release on %s to %f.", routine, _owner->_id,
	   _owner->_release );
	}
