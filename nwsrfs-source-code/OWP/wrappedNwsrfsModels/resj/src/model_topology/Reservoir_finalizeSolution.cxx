...
int Reservoir::finalizeSolution( TSDate& cur_date )
{
	...
	if( _storage == MISSING ) {
	   ...
	   "%s (%s) - Must have a carryover storage "
	   "value for %s.", warnStr, routine, _id );
	...
	if ( _release == MISSING ) {
	  if( getFewsDebugFlag() > 0 )
	  {
	   /* AVCP PrintWarning( 1, routine, */
	   logMessageWithArgsAndExitOnError( DEBUG_LEVEL, 
	   "%s (%s) - No rules used on %s "
	   "do pass inflow %s", warnStr, routine, _id, cur_date.toString() );
	  }
	...
	if( !_conserve_mass ) {
		...
		//AV changed due to many warning messages in the log 3/11/2010
		if (_pool > pool_top - 0.1) {
		    /* AVCP PrintWarning( 1, routine, */
		    /*
	            logMessageWithArgsAndExitOnError( DEBUG_LEVEL, 
	            "%s (%s) - %s pool %.2f Hit top of elevation-storage curve %.2f %s "
		    "do pass inflow ", warnStr, routine, _id,  
		    _pool, pool_top, cur_date.toString() );
		    */
		    if( getFewsDebugFlag() > 0 )
        	    {
	   	       logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		       "(%s) - %s pool %.2f Hit top of elevation-storage curve %.2f %s "
		       "do pass inflow ", routine, _id,_pool, pool_top,
		        cur_date.toString() );
		    }
		    // _release = _endInflow - _withdraw;
		    _release = _endInflow;
		...
		if( cur_stor < minStor ) {
			...
			if( (maxRelease - decision) > 0.000001 ) {
				...
				{
	                          logMessageWithArgsAndExitOnError( DEBUG_LEVEL, 
			          "%s (%s) - Unable to meet release (%f) on %s--"
				  "anti-oscillation algorithm reset "
				  "release to %f on %s", warnStr, routine, prescribedR, 
				...
				if( prescribedR - newR > 0.000001 )
				{
				   ...
				   warnStr, routine, prescribedR, cur_date.toString(),
				   newR, _id );
				   }
				}
			}
			...
			if ( _release < 0.0 )
			{
			    ...
			    if( cur_stor < minStor )
			    {
			       ...
			       if( getFewsDebugFlag() > 0 )
			       {
				    ...
				    "%s (%s) - Low inflows and "
			            ...
			            "reduced from %f to %f", warnStr, routine, _id, 
			    ...
			    else
			    {
				   ...
				   "%s (%s) - Unable to meet "
			            ...
			            "%f", warnStr, routine, _id, cur_date.toString(),
				    prescribedW, _withdraw  );
			       }
			    }
...
*/
	...
	if( getFewsDebugFlag() > 9 )
        {
	   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	           "(%s) - TIME \"%s\":", routine, cur_date.toString() );
	   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	           "(%s) - Reservoir \"%s\":", routine, _id );
	   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	           "(%s) - Inflow:  %f", routine, _endInflow );
	   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	           "(%s) - Storage:  %f", routine, _storage );
           logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	           "(%s) - Withdrawal:  %f", routine, _withdraw );
	   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	           "(%s) - Loss:  %f", routine, _loss );
	   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	           "(%s) - Release:  %f", routine, _release );
	   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	           "(%s) - Mean Release:  %f", routine, mean_flow );
	   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	           "(%s) - Pool:  %f", routine, _pool );	  
