		...
		if ( adjPool != MISSING ) {
			...
			if( adjPool < _owner->_min_pool ) {
			   if( getFewsDebugFlag() > 0 )
			   {
				...
				"%s %s Observed pool for "
				"%s on %s is less than the Minimum "
				"pool (%f < %f). Setting to Minimum "
				"pool.", warnStr, routine, _id, 
				_owner->_id, adjPool, _owner->_min_pool );
			   }
		...
		if ( adjPool != MISSING ) {
			...
			if( adjPool < _owner->_min_pool ) {
			  if( getFewsDebugFlag() > 0 ) 
			  {
				...
				"%s on %s is less than the Minimum "
				"pool (%f < %f). Setting to Minimum "
				"pool.", warnStr, routine, _id, _owner->_id, 
				adjPool, _owner->_min_pool );
			  }
			  adjPool = _owner->_min_pool;
			}
...
void Adjust :: setInactiveState()
{
	...
	if( _adjsim ) {
	   ...
	   {
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		        "%s %s Inactivation of \"ADJSIM OFF\" "
			"method \"ADJUST %s %s\" is not allowed.  It should be "
			"associated with the [TRUE] expression only.", warnStr,
			routine, _owner->_id, _id );
	   }
	}	
...
int Adjust :: setCOstring( )
{
	...
	char temp_str[54+4+16+1];	// Labels, values, future space, end
	...
	COtype[6] = '\0';
	sprintf( temp_str, 
		"%-12.12s%-12.12s%6.6s%-12.12s%-12.12s%4.4s%16.16s",
		COtype, _id, "-99999", "ADJUST", _owner->_id, value_str, 
		future_str );
