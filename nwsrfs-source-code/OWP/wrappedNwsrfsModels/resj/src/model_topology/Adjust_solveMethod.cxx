	...
	double** group_val )
{
	...
	if( !_pool_obs && !_release_obs ) {
		/*AVCP PrintStatus( 1, routine,*/
	        ...
	        "(%s) - Adjust method %s has no observed "
		"data - not adjusting reservoir\"%s\" states.", 
	...
	if( _pool_obs != NULL && _release_obs != NULL ) {
		...
		if( pool < _owner->_min_pool ) {
			/*AVCP PrintWarning( 1, routine,*/
		        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
			"%s (%s) - Observed pool for %s on "
			"%s is less than the Minimum pool (%f < %f). "
			"Setting to Minimum pool.", warnStr, routine,
			_id, _owner->_id, pool, _owner->_min_pool );
		...
		if( pool < _owner->_min_pool ) {
			/*AVCP PrintWarning( 1, routine,*/
		        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
			"%s (%s) - Observed pool elevation is "
			"below the minimum pool on %s (%f < %f). "
			"Setting pool to minimum.", warnStr, routine, 
			_owner->_id, pool, _owner->_min_pool );
			pool = _owner->_min_pool;
		}
