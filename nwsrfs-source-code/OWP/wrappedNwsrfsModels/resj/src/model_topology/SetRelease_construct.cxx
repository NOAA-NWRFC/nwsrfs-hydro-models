...
int SetRelease::construct ( char** re_list, int n_items )  
{
	...
	for( i = 0; i < n_items; i++ ) { 	
		...
		if( nlist == 0 || list == NULL ) {
			...
			"%s (%s) - Troubles getting release data "
			"for SetRelease %s %s.", errorStr, routine, _owner->getID(), _id );
			/* AVCP
			if( list ) {
				list = FreeStringList( list );
			}
			...
			if( !strcasecmp( list[1], "OBSERVEDRELEASE" ) ) {
				...
				if( _release_obs == NULL ) {
					/* AVCP totErrs++;
				       ...
				       FATAL_LEVEL, "%s (%s) - "
				       "Troubles setting release timeseries"
				       " \"%s\" on SETRELEASE '%s.'", errorStr,
				       routine, list[2], _id );
					/* AVCP
					list = FreeStringList( list );
					continue;
				...
				if( _release_obs == NULL ) {
					/* AVCP totErrs++;
	                                PrintError( routine, */
	                               logMessageWithArgsAndExitOnError( 
				       FATAL_LEVEL, "%s (%s) - Troubles "
				       "setting release timeseries \"%s\" on "
				       "SETRELEASE '%s.'", errorStr, routine,
				       list[2], _id );
				}
				/* AVCP
				list = FreeStringList( list );
		...
		// If it has not, then use the default - NORMAL
			...
			if ( !strcasecmp( list[1], "TIME" ) ) {
				...
				"\"%s\" for SetRelease %s.", errorStr, routine,
			       	list[0], list[1], _id );
				/* AVCP
				list = FreeStringList( list );
				continue;
		...
		// Check for BLENDTBL
			...
			if( nlist > 1 ) {
				if( !IsInteger( list[1] ) ) {
					...
					"is not an integer.", errorStr, routine, 
					list[0], list[1] );
					/* AVCP
					list = FreeStringList( list );
					continue;
				...
				"keyword %s.", errorStr, routine, _type, 
				list[0] );
				/* AVCP
				list = FreeStringList( list );
				continue;
		...
		// associated with reading the control file line-by-line.
			...
			if( value_list == NULL || n_value == 0 ) {
				...
				" Does \"END%s\" exist?", errorStr, routine, 
				list[0], getID(), list[0] );
				/* AVCP	
				list = FreeStringList( list );
				if ( value_list ) {
	...
	if( rel_found == 0 ) {
		/* AVCP totErrs++;
	        PrintError( routine, */
	        logMessageWithArgsAndExitOnError( FATAL_LEVEL,
		"%s (%s) - SetRelease keyword 'VALUES' not found "
	...
	for( i = 0; i < nrel; i++ ) {
		...
		if( i == 0 ) {
			if( strcasecmp( list[nlist - 1], "endelev" ) ||
				...
				/* AVCP  PrintError( routine, */
	                        logMessageWithArgsAndExitOnError( FATAL_LEVEL, 
				"%s (%s) - First line in SetRelease VALUES table "
				"must begin with 'ELEV' and end with "
				"'ENDELEV' for SetRelease %s.", 
		...
		if( nlist > n_ts + 1 ) {
			...
			if( nlist != n_ts + 1 + ( 24 / _t_mult ) ){ 
				 ...
				 list = FreeStringList( list );
				logMessageWithArgsAndExitOnError( FATAL_LEVEL, 
				"%s (%s) - %s %s SetRelease must have one and "
			...
			if ( dist_tally > 1.00001 || 
				dist_tally < 0.99999 ) {
				 ...
				 list = FreeStringList( list );
				 logMessageWithArgsAndExitOnError( FATAL_LEVEL, 
 				"%s (%s) - Distribution on %s %s SetRelease "
				"does not sum to 1.0.",
				errorStr, routine, _owner->_id, _id );
				/*
				list = FreeStringList( list );
