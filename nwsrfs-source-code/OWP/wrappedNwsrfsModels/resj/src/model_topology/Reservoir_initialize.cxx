...
int Reservoir :: initialize()
{
	...
	if( _loss_ts.allocateDataSpace() ) {
		...
		"%s (%s) - Troubles allocating data space"
		" for loss time series on %s.", warnStr, routine, _id );
	...
	if( _pool_ts->allocateDataSpace() ) {
		/*AVCP PrintWarning( 1, routine,*/
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	        "%s (%s) - Troubles allocating data space for pool time "
		"series on %s.", warnStr, routine, _id );
		return( STATUS_FAILURE );
	}
	...
	if( _withdraw_ts->allocateDataSpace() ) {
		/*AVCP PrintWarning( 1, routine,*/
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	        "%s (%s) - Troubles allocating data space"
		" for withdrawal time series on %s.", warnStr, routine, _id );
		return( STATUS_FAILURE );
	}
	...
	if( _release_ts->allocateDataSpace() ) {
		/*AVCP PrintWarning( 1, routine,*/
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	        "%s (%s) - Troubles allocating data space"
		" for release time series on %s.", warnStr, routine, _id );
		return( STATUS_FAILURE );
	}
	...
	if( _spill_ts->allocateDataSpace() ) {
		/*AVCP PrintWarning( 1, routine,*/
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	        "%s (%s) - Troubles allocating data space"
		" for spill time series on %s.", warnStr, routine, _id );
		return( STATUS_FAILURE );
	}
	/*****_withdraw_ts = new HourTS();
	_withdraw_ts->setDate1( Method :: getForecastDate1() );
	...
	if( _storage_ts.allocateDataSpace() ) {
		/*AVCP PrintWarning( 1, routine,*/
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	        "%s (%s) - Troubles allocating data space"
		" for storage time series on %s.", warnStr, routine, _id );
		return( STATUS_FAILURE );
	}
