...
int Balance :: construct ( char** re_list, int n_items )  
{
	...
	if( n_items <= 1 ) {
		...
		"%s (%s) - Must specify at least one "
		"reservoir for Balance method %s on %s.", 
	...
	for( i = 0; i < n_items; i++ ) {
		...
		if( list == NULL || nlist < 1 ) {
			/*AVCP PrintError( routine,*/
		        logMessageWithArgsAndExitOnError( FATAL_LEVEL,
			"%s (%s) - Troubles constructing "
			"Balance method %s on %s.", errorStr, routine,
			_id, _owner->_id );
			//return( STATUS_FAILURE );
		}
		...
		if( !strcasecmp( list[0], "ACE" ) || !strcasecmp( list[0], 
			...
			if ( alreadyACE ) {
			   if( getFewsDebugFlag() > 0 )
			   {
				...
				warnStr, routine, list[2], _ace, _id );
			   }
			}
			alreadyACE = 1;
			_ace = atof( list[1] );
		...
		if( !strcasecmp( list[0], "RESERVOIR" ) || 
			...
			if( strcasecmp( res->_type, "RESERVOIR" ) ) {
				...
				"a Reservoir.", errorStr, routine, list[0] );
				/*AVCP list = FreeStringList( list );
				totErrs++;*/
			}
				...
				if( !strcasecmp( list[0], "LOWER" ) ) {
					if( nlist != 3 ) {
						/*AVCP PrintError( routine,*/
					       logMessageWithArgsAndExitOnError(
					       FATAL_LEVEL,                  
						"%s (%s) - BALANCE keyword %s "
						"must be followed by either "
						"POOL or STORAGE and a value "
						"for Reservoir %s, method %s.",
					...
					/*AVCP PrintWarning( 1, routine, */
					   ...
					   _upper_stor[res_count] = maxStorage;
					}
				}
					...
					/*AVCP PrintWarning(1, routine, */
					   if( getFewsDebugFlag() > 0 )
					   {
					    logMessageWithArgsAndExitOnError(
						 ...
						 "( %f < %f ).  Setting to %f.",
						 warnStr, routine, _id, 
						 res->_id, _min_rel[res_count], 
						 res->_min_release, 
						 res->_min_release);
					   }
					   _min_rel[res_count] = res->_min_release;
					}
				}
					...
					/*AVCP PrintWarning(1, routine, */
					   if( getFewsDebugFlag() > 0 )
					   {
						  ...
						  "to MINRELEASE.", warnStr, 
						  routine, res->_id, _id);
					   }
					   _max_rel[res_count] = _min_rel[res_count];
					}
				}
			...
			if( _upper_stor[res_count] == MISSING ) {
				/*AVCP PrintError( routine,*/
			        logMessageWithArgsAndExitOnError( FATAL_LEVEL,
				        "%s (%s) - Must specify UPPER POOL "
					"or UPPER STORAGE for reservoir %s in "
					"BALANCE method %s.", errorStr, routine,
					res->_id, _id );
				/*AVCP totErrs++; */
			}
			...
			if( !strcmp( res->_id, _owner->_id ) ) {
				...
				if (_min_rel[res_count] > _max_rel[res_count]) {
					/*AVCP PrintError( routine,*/
						...
						"MAXRELEASE ( %f > %f ) for "
						"reservoir %s in BALANCE "
						"method %s.", errorStr, routine, 						_min_rel[res_count] / ffactor, 
		...
		{
		 logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		 "%s (%s) - Keyword %s not recognized for "
		 "BALANCE method %s.", warnStr, routine, list[0], _id );
		}	
		// Freeing memory 
		if ( list ) {
	...
	if( _n_res < 1 ) {
		/*AVCP PrintError( routine,*/
	        logMessageWithArgsAndExitOnError( FATAL_LEVEL,
		"%s (%s) - Balance method %s must have at "
		"least one reservoir.", errorStr, routine, _id );
		/*AVCP totErrs++; */
	}
	if( _bal_mode == 0 ) {
		/*AVCP PrintError( routine,*/ 
	        logMessageWithArgsAndExitOnError( FATAL_LEVEL,
		"%s (%s) - Balance method %s must have a "
		"VOLUME or PERCENT mode specified.", errorStr, routine, _id );
		/*AVCP totErrs++;*/
	}
	if( _owner_pos == -1 ) {
		/*AVCP PrintError( routine,*/
	        logMessageWithArgsAndExitOnError( FATAL_LEVEL,
	        "%s (%s) - Troubles finding owner in Balance "
		"system.", errorStr, routine );
		/*AVCP totErrs++; */
	}
