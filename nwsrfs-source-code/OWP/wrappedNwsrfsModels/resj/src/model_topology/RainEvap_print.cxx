...
int RainEvap :: print( FILE* fp )
{
	...
	if( fp == NULL ) {
	   ...
	   logMessageWithArgsAndExitOnError( FATAL_LEVEL,
	      "%s (%s) - Cannot print RainEvap info -"
	         ...
	         date.addInterval( _t_int, _t_mult ) )
	{
	   ...
	   { 
		   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		      "(%s) - Date: %s  Precip from ctl file: %f\n", routine,
		      date.toString(), _precip_ctl.getDataValue( date, 1 ) );
	   }
	}
	...
	for( date = _evap_ctl.getDate1(); date <= _evap_ctl.getDate2(); 
	  ...
	  /* AVCP PrintDebug( 10, routine, */
	   ...
	   { 
		logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		   "(%s) - Date: %s  Evap form ctl file: %f\n",routine,
		    date.toString(), _evap_ctl.getDataValue( date, 1 ) );
	   }
	}
