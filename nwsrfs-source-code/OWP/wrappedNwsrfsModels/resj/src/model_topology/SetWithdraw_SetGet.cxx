...
int SetWithdraw :: setCOstring( )
{
	...
	char tsStep_str[5], tblStep_str[5], transferVal_str[8],future_str[9], 
		value_str[17];
	char temp_str[54+16+8+1];	// Labels, values, future space, end
	...
	/*sprintf(value_str, "%4.4s%4.4s%8.8s", tsStep_str, tblStep_str, 
		transferVal_str);*/
	sprintf(value_str, "%4d%4d%.8f",_ts_step, _tbl_step, _myValue);
	...
	COtype[6] = '\0';
	sprintf( temp_str, 
		"%-12.12s%-12.12s%6.6s%-12.12s%-12.12s%16.16s%8.8s",
		COtype, _id, "-99999", "SETWITHDRAW", _owner->_id, value_str,
	       	future_str );
