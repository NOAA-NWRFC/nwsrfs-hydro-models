//------------------------------------------------------------------------------
// SetSum :: solveMethod - Algorithm that solves the SetSum method.
//------------------------------------------------------------------------------
...
// 19 Apr 2001	JRV, RTi	Assign new storage based on new pool following
//				completion of loss method (Ensure full updating
//				of states).
// 24 May 2001	JRV, RTi	Removed update of pool following withdrawal
...
//      to the sub methods (except for Loss methods).  For example, SetSum 
...
int SetSum :: solveMethod( TSDate& cur_date, int isPrimarySolution, 
	double** group_val )
{
	char routine[]="SetSum :: solveMethod";
	double *prev[2], sum = 0.0, pool = MISSING;
	int i;
	...
	// Execute the methods in the group summing the return values from 
	// those methods.
	for( i = 0; i < _group_n; i++ ) {
		if( _group[i]->solveMethod( cur_date, isPrimarySolution,
			...
			/* AVCP PrintWarning( 1, routine,      */
		       ...
		       "%s (%s) - Troubles solving "
			"%s for SetSum Method %s.", warnStr, routine, 
			_group[i]->getID(), _id );
...
/*		
printf( " !!SUM _mon= %2d _day= %2d _hour= %2d group_id= %12s sum= %10f\n", 
         cur_date.getMonth(), cur_date.getDay(), cur_date.getHour(), _group[i]->getID(), sum);
*/
	}
	...
	if( _group_id == RELEASE_METHOD ) {
		...
		if( sum < ((Reservoir*)_owner)->_min_release ) {
			/* AVCP PrintWarning( 1, routine,      */
		       logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		       "%s (%s) - SetSum %s on %s produced "
			"release that is less than the reservoir's "
			"minimum release f < %f). Setting release to "
			"minimum release.", warnStr, routine, 
			_id, _owner->_id, sum, 
			((Reservoir*)_owner)->_min_release );

			sum = ((Reservoir*)_owner)->_min_release;
		}
		((Reservoir*)_owner)->_release = sum;
...
/*  ==============  Statements containing RCS keywords:  */
{static char rcs_id1[] = "$Source: /fs/hseb/ob81/ohd/ofs/src/resj_topology/RCS/SetSum_solveMethod.cxx,v $";
 static char rcs_id2[] = "$Id: SetSum_solveMethod.cxx,v 1.8 2006/10/26 15:34:57 hsu Exp $";}
/*  ===================================================  */
