...
#include "ResJSys.h"
#include "resj_main.h"
void carryovertransfer58 ( float* COLD, float* POLD, float* CNEW,  float* PNEW,
	char* resjFOLD, char* resjFNEW)
...
//AVCP	ResJSys :: setWarningSaveLevel( 1 + *ibug );
	...
	if( resjFOLD == NULL ) {
		...
		*/
	    ...
	    "%s (%s) OLD Resj fs5file not specified", errorStr, routine );
	...
	if( resjFNEW == NULL ) {
	   ...
	   logMessageWithArgsAndExitOnError( FATAL_LEVEL,
	   "%s (%s) NEW Resj fs5file not specified", errorStr, routine );
	...
	if ( ierr > 0 ) {
	   logMessageWithArgsAndExitOnError( FATAL_LEVEL,
	   "%s (%s) Unable to instatiate OLD ResJ", errorStr, routine );
	...
	if ( ierr > 0 ) {
	   logMessageWithArgsAndExitOnError( FATAL_LEVEL,
	   "%s (%s) Unable to instatiate NEW ResJ", errorStr, routine);
           /* AVCP sprintf ( error, "Unable to instatiate NEW ResJ" ); */
	...
	int iusecO = (int)POLD[3];
        ...
        memset(ch_cOLD, 0, (iusecO)*sizeof(float)+1);
        if( ch_cOLD == NULL ) {
                /* AVCP ierr = 1; 
                sprintf( error, "Troubles allocating memory for COLD array." );
		AVCP return ierr; */
	   logMessageWithArgsAndExitOnError( FATAL_LEVEL,
	   "%s (%s) Troubles allocating memory for COLD array.", 
	   errorStr, routine );
	...
	int iusecN = (int)PNEW[3];
        ...
        memset(ch_cNEW , 0,  (iusecN)*sizeof(float)+1) ;
        if( ch_cNEW == NULL ) {
                /* AVCP ierr = 1;
                sprintf( error, "Troubles allocating memory for CNEW array." );*/
		/* AVCP return ierr; */
		logMessageWithArgsAndExitOnError( FATAL_LEVEL,
		"%s (%s) Troubles allocating memory for CNEW array.", 
		errorStr, routine );
	...
	//		memor equal to one floating point value (or 6 
	//		characters, right justified).  This value is equal to
	//		-99999 for the final entry to the carryover array.
	...
	int numCompFloats = 2 * numNameFloats;	//6
	...
	int numNextSize = (numNextFloats * sizeof(float)) + 2; //6
				// Number of characters used to hold the index
				// for the next component

	int IndexCompName = numNameFloats * sizeof(float); //12
	...
	int IndexDefineNext = numCompFloats * sizeof(float); //24
				// Index (in the character array version of the
	...
	int IndexLagKMethName = (numCompFloats + numNextFloats) * sizeof(float)+2;//30
				// Index (in the character array version of the
	...
	int IndexMethName = numNameFloats * sizeof(float);//12
				// Index (in the character array version of the
	...
	int IndexMethType = IndexDefineNext + numNextSize;//24+6
				// Index (in the character array version of the
				// carryover array) pointing to the (non-LagK)
	...
	// process until the next keyword index is not defined (= -99999).
	while (IndexNext != -99999) {
		...
		if ( !strncmp(currOLD, "RESERVOIR", 9) ) {
			...
			// carry-over array (string)
				...
				// system
				/*AVCP sprintf( error, "String \"%s\" not found in "
				...
				*/
				...
				"%s (%s) String \"%s\" not found in "
				"new carry over.", warnStr, routine ,copyOLD);
			      }	
                        	continue;
                	}
			...
			if ( compNEW == NULL ) {
				...
				// warning.
			       ...
			       {
				logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
				"%s (%s) RESERVOIR %s not found in the "
				"new version", warnStr, routine, compName );
			       }
				/*AVCP  PrintWarning( 1, routine, error);*/
				continue;
			}
			...
			if ( ierr > 0 ) {
				...
				/* AVCP sprintf( error, "Carryover Transfer failed for "
					"RESERVOIR %s",compName); */
				logMessageWithArgsAndExitOnError( FATAL_LEVEL,
				"%s (%s) Carryover Transfer failed for "
				"RESERVOIR %s", errorStr, routine, compName );
				
				/* AVCP PrintError( routine, error );
				break; */
			}
		}
		else if ( !strncmp(currOLD, "REACH", 5) ) {
			// Check for modified carryover structure for OLD CO
			// We used to have 3*4 + 3*4 + 6 +3*4 + N*8 characters 
			...
			while (currNEW != NULL && !match) {
				...
				sprintf( tmp_str1, "%-12.12s", &currOLD[24+6] );
				sprintf( tmp_str2, "%-12.12s", &currNEW[24+6] );
			...
			if ( !match ) {
				...
				*/
			       ...
			       {
				logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
  	   		        "%s (%s) Method %s not found in the "
				"new version", warnStr, routine, methName );
			       }
				continue;
			}
			...
			if ( !strncmp(methOLD->getType(), "LAGK", 
				...
				if ( ierr > 0 ) {
					...
					//break;
				  logMessageWithArgsAndExitOnError( FATAL_LEVEL,
				  "%s (%s) Carryover Transfer "
			 	  "failed for LAGK", errorStr, routine );
				}
			...
			if ( compOLD == NULL ) {
				...
				*/
				logMessageWithArgsAndExitOnError( FATAL_LEVEL,
				"%s (%s) NODE %s from old carry over "
				"not found in the old system.", errorStr,
				routine, compName );
			...
			if ( ierr > 0 ) {
				...
				logMessageWithArgsAndExitOnError( FATAL_LEVEL,
				"%s (%s) Carryover Transfer failed for "
				"NODE %s", errorStr, routine, compName );				
			...
			while (currNEW != NULL && !match) {
				...
				sprintf( tmp_str1, "%-12.12s", &currNEW[24+6] );
				typeOK = !strncasecmp(tmp_str1, methType, 12);

				// Check owner ID from carry over
				sprintf(tmp_str1,"%-12.12s",&currNEW[24+6+12]);
			...
			if ( !strncmp(methOLD->getType(), "SETWITHDRAW", 
				...
				if ( ierr > 0 ) {
				  ...
				  logMessageWithArgsAndExitOnError( FATAL_LEVEL,
				  "%s (%s) Carryover Transfer "
				  "failed for SETWITHDRAW %s", errorStr,
				  routine, compName );
				...
				if ( ierr > 0 ) {
				  ...
				  logMessageWithArgsAndExitOnError( FATAL_LEVEL,
				  "%s (%s) Carryover Transfer "
			   	  "failed for SPILLWAY %s", errorStr,
				  routine, compName );
				...
				if ( ierr > 0 ) {
				  ...
				  logMessageWithArgsAndExitOnError( FATAL_LEVEL,
				  "%s (%s) Carryover Transfer "
				  "failed for CALCINFLOW %s", errorStr,
				  routine, compName );
			    ...
			    strlen("LOOKUP3")) )
			{
				...
				*/
				logMessageWithArgsAndExitOnError( FATAL_LEVEL,
				"%s (%s) Carryover Transfer failed for "
			        "LOOKUP3 %s", errorStr, routine, compName );
