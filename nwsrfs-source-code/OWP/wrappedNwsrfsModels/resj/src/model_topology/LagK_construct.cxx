...
int LagK :: construct ( char** lagk_list, int n_items )
{
    ...
    for ( i = 0 ; i < n_items ; i++ )
    {
	    ...
	    "%s %s Troubles getting data for %s %s LagK.",
             ...
             ( !strcasecmp ( "ENDTABLE", list[0] ) && ( nlist > 1 ) ) )
        {
            ...
            /*AVCP PrintError ( routine,*/
	    logMessageWithArgsAndExitOnError( FATAL_LEVEL,
	    "%s %s The '%s' keyword has too many arguments on %s %s.", 
	    errorStr, routine, list[0], _owner->getID (), _id ) ;
            //return ( STATUS_FAILURE ) ;
        }
             ...
             ( !IsInteger( list[1] ) ) )
        {
            ...
            /*AVCP PrintError ( routine,*/ 
	    logMessageWithArgsAndExitOnError( FATAL_LEVEL,
            "%s %s The '%s' keyword requires an integer argument, not '%s', " 
            "inside %s %s.", errorStr, routine, 
	    list[0], list[1], _owner->getID (), _id ) ;
            //return ( STATUS_FAILURE ) ;
        }
               ...
               ( !strcasecmp ( "INITIALLAGGEDINFLOW", list[0] ) ) ) )
        {
            ...
            /*AVCP PrintError ( routine, */
	    logMessageWithArgsAndExitOnError( FATAL_LEVEL,
            "%s %s The '%s' keyword requires a double argument, not '%s', " 
            "inside %s %s.", errorStr, routine, 
	    list[0], list[1], _owner->getID (), _id ) ;
            //return ( STATUS_FAILURE ) ;
        }
             ...
             ( !strcasecmp ( "KFTWLOSS", list[0] ) ) )
        {
            ...
            /*AVCP PrintError ( routine, */
	    logMessageWithArgsAndExitOnError( FATAL_LEVEL,
            "%s %s The '%s' keyword requires a two double arguments, "
            "not '%s %s', inside %s %s.", errorStr, routine, 
	    list[0], list[1], list[2], _owner->getID (), _id ) ;
            //return ( STATUS_FAILURE ) ;
        }
             ...
             ( limitFortWorth > 0 && !strcasecmp ( "KFTWLOSS", list[0] ) ) )
        {
            ...
            /*AVCP PrintError ( routine, */
	    logMessageWithArgsAndExitOnError( FATAL_LEVEL,
            "%s %s The '%s' keyword may not be repeated in a single LAGK "
	    "method. inside %s %s.", errorStr, routine,
	    list[0], _owner->getID (), _id ) ;
            //return ( STATUS_FAILURE ) ;
        }
        ...
        if ( ( limitLagTable > 0 && !strcasecmp ( "LAG", list[0] ) ) )
        {
            ...
            /*AVCP PrintError ( routine,*/
	    logMessageWithArgsAndExitOnError( FATAL_LEVEL, 
            "%s %s The '%s' keyword may not appear after INFLOW_LAG table is "
	    "defined, for %s %s.", errorStr, routine, 
	    list[0], _owner->getID (), _id ) ;
            //return ( STATUS_FAILURE ) ;
        }
        ...
        if ( ( limitKTable > 0 && !strcasecmp ( "K", list[0] ) ) )
        {
            ...
            /*AVCP PrintError ( routine, */
	    logMessageWithArgsAndExitOnError( FATAL_LEVEL,
            "%s %s The '%s' keyword may not appear after OUTFLOW_K table is "
	    "defined, for %s %s.", errorStr, routine, 
	    list[0], _owner->getID (), _id ) ;
            //return ( STATUS_FAILURE ) ;
        }
        ...
        if ( !strcasecmp ( "LAG", list[0] ) )
        {
            ...
            if ( _lag < 0 )
            {
                ...
                /*AVCP PrintError ( routine, */
	        logMessageWithArgsAndExitOnError( FATAL_LEVEL,
                "%s %s The '%s' keyword must be followed by a positive "
		"integer, not ('%s'), inside %s %s.", errorStr, routine,
		list[0], list[1], _owner->getID (), _id ) ;
                //return ( STATUS_FAILURE ) ;
            }
        ...
        else if ( !strcasecmp ( "K", list[0] ) )
        {
                 ...
                 ( _out_k_tbl.populate ( 1, KCOLUMN, valueOfK ) ) )
            {
                ...
                /*AVCP PrintError ( routine, */
	       logMessageWithArgsAndExitOnError( FATAL_LEVEL,
	       "%s %s Troubles filling Outflow-K table on %s %s.", 
	       errorStr, routine, _owner->getID (), _id ) ;
                //return ( STATUS_FAILURE ) ;
            }
            _n_kval = 2 ;
            ...
            !strcasecmp( "INLFOW_LAG", list[1]) ) && warnTableLag )
        {
            ...
            /*AVCP PrintError ( routine,*/
	    logMessageWithArgsAndExitOnError( FATAL_LEVEL,
	    "%s %s 'TABLE' keyword not allowed directly "
        ...
        else if ( !strcasecmp ( "TABLE", list[0] ) && nlist < 2 )
        {
            ...
            /*AVCP PrintError ( routine,*/
	    logMessageWithArgsAndExitOnError( FATAL_LEVEL,
	    "%s %s 'TABLE' keyword found inside "
            "'K'..'ENDK' is missing "
            "the required argument 'OUTFLOW_K', in %s on %s.", 
	    errorStr, routine, _id, _owner->_id ) ;
            //return ( STATUS_FAILURE ) ;
        }
        else if ( !strcasecmp ( "TABLE", list[0] ) && nlist < 2 )
	    ...
	    logMessageWithArgsAndExitOnError( FATAL_LEVEL,
	    "%s %s 'TABLE' keyword found inside "
            "'LAG'..'ENDLAG' is missing the required argument 'INFLOW_LAG', "
	    "in %s on %s.", errorStr, routine, _id, _owner->_id ) ;
            //return ( STATUS_FAILURE ) ;
        }
        else if ( !strcasecmp ( "TABLE", list[0] ) && nlist < 2 )
        {
            ...
            /*AVCP PrintError ( routine,*/
	    logMessageWithArgsAndExitOnError( FATAL_LEVEL,
	    "%s %s 'TABLE' keyword is missing its required"
            "argument, 'OUTFLOW_K' or INFLOW_LAG' %s on %s.", 
	    errorStr, routine, _id, _owner->_id ) ;
            //return ( STATUS_FAILURE ) ;
        }
                  ...
                  !strcasecmp ( "outflow_k", list[1] ) ) 
        {
            ...
            if ( lagk_val == NULL || n_lagk == 0 )
            {
                ...
                /*AVCP PrintError ( routine,*/
	        logMessageWithArgsAndExitOnError( FATAL_LEVEL,
	        "%s %s Troubles getting outflow-K table from control file.\n"
                "Typically this is caused by a missing ENDTABLE"
                "or misformatted file.", errorStr, routine );
                //return ( STATUS_FAILURE ) ;
            }
		...
		"Typically this is caused\nby a syntax error in the table", 
		   ...
		   if( getFewsDebugFlag() > 0 )
		   {
		    ...
		    "%s %s Flow values in OUTFLOW_K table "
                    "must be increasing, on %s %s.  ",
                    warnStr, routine, _owner->getID (), _id ) ;
		   }
                   lagk_val = FreeStringList ( lagk_val ) ;
                   list = FreeStringList ( list ) ;
                }
                  ...
                  !strcasecmp ( "inflow_lag", list[1] ) ) 
        {
            ...
            if ( lagk_val == NULL || n_lagk == 0 )
            {
                ...
                /*AVCP PrintError ( routine,*/
	        logMessageWithArgsAndExitOnError( FATAL_LEVEL,
	        "%s %s Troubles getting Inflow-LAG table from control file.\n"
                "Typically this is caused by a missing ENDTABLE"
                "or misformatted file.", errorStr, routine ) ;
                //return ( STATUS_FAILURE ) ;
            }
            ...
            if ( _in_lag_tbl.populate ( lagk_val, n_lagk, ffactor, 1.0 ) )
            {
                ...
                /*AVCP PrintError ( routine,*/
	        logMessageWithArgsAndExitOnError( FATAL_LEVEL, 
	        "%s %s Troubles filling Inflow-LAG table on %s %s.  \n"
                "Typically this is caused\n"
                "by a syntax error in the table", 
                errorStr, routine, _owner->getID (), _id ) ;
                lagk_val = FreeStringList ( lagk_val ) ;
        ...
        else if ( !strcasecmp ( "TABLE", list[0] ) )
        {
            /*AVCP PrintError ( routine,*/
	    logMessageWithArgsAndExitOnError( FATAL_LEVEL,
	    "%s %s 'TABLE' keyword is outside "
            "'LAG'..'ENDLAG' or 'K'..'ENDK' section, (%s %s) "
            "%s, filling table on %s.", errorStr, routine,
	    list[0], list[1], _owner->getID (), _id ) ;
        }
        else if ( !strcasecmp ( "INITIALOUTFLOW", list[0] ) )
        {
            ...
            if ( nlist < 2 )
            {
                ...
                /*AVCP PrintError ( routine,*/
	        logMessageWithArgsAndExitOnError( FATAL_LEVEL,
	        "%s %s Value required immediately after %s method %s "
                "keyword %s.", errorStr, routine, _type, _id, list[0] ) ;
                //return ( STATUS_FAILURE ) ;
            }
        ...
        else if ( !strcasecmp ( "KFTWLOSS", list[0] ) )
        {
            ...
            if ( nlist < 3 )
            {
                ...
                /*AVCP PrintError ( routine,*/
	        logMessageWithArgsAndExitOnError( FATAL_LEVEL,
	        "%s %s 2 values required immediately after %s method %s "
                "keyword %s.", errorStr, routine, _type, _id, list[0] ) ;
                //return ( STATUS_FAILURE ) ;
            }
        ...
        else if ( requireEndValues )
        {
            ...
            if( nlist > 1 )
            {
                ...
                /*AVCP PrintError ( routine,*/
	        logMessageWithArgsAndExitOnError( FATAL_LEVEL,
	        "%s %s Inside 'VALUES'..'ENDVALUES' section, "
                "Extra values found after '%s', "
                "on %s %s.", errorStr, routine, 
		list[0], _owner->getID (), _id ) ;
                //return ( STATUS_FAILURE ) ;
            }
                  ...
                  (strcasecmp("ZZZZZZZZZ", list[0]) > 0) )
        {
            ...
            /*AVCP PrintError ( routine,*/
	    logMessageWithArgsAndExitOnError( FATAL_LEVEL,
	    "%s %s Unknown keyword found inside LAGK section: '%s' (%s %s).",
	    errorStr, routine, list[0], _type, _id ) ;
            //return ( STATUS_FAILURE ) ;
        }
    }
    ...
    if( requireInflowLag )
    {
        /*AVCP PrintError ( routine,*/
        logMessageWithArgsAndExitOnError( FATAL_LEVEL,
        "%s %s Expected an INFLOW_LAG table.  "
        "The 'LAG' keyword was present, but no constant "
        "lag or variable lag table was defined, on (%s %s).", 
    ...
    if ( requireValues )
    {
        /*AVCP PrintError ( routine,*/
        logMessageWithArgsAndExitOnError( FATAL_LEVEL,
        "%s %s The 'COINFLOW' keyword requires a "
        "by a 'VALUES' keyword (%s %s).", errorStr, routine, _type, _id ) ;
        //return ( STATUS_FAILURE ) ;
    }
    if ( requireEndValues )
    {
        /*AVCP PrintError ( routine, */
        logMessageWithArgsAndExitOnError( FATAL_LEVEL,
        "%s %s The 'ENDVALUES' keyword must appear on "
        "a separate line, following the data in the 'VALUES' section "
        "(%s %s).", errorStr, routine, _type, _id ) ;
        //return ( STATUS_FAILURE ) ;
    }
    if ( requireEndCOInflow )
    {
        /*AVCP PrintError ( routine,*/
        logMessageWithArgsAndExitOnError( FATAL_LEVEL,
        "%s %s The 'ENDCOINFLOW' keyword is missing. All COINFLOW..ENDCOINFLOW "
	"sections must be matched and must surround the 'VALUES'..'ENDVALUES' "
	"section, on (%s %s).", errorStr, routine, _type, _id ) ;
        //return ( STATUS_FAILURE ) ;
    }
    ...
    if ( _co_inflow == NULL )
    {
        /*AVCP PrintError ( routine,*/
        logMessageWithArgsAndExitOnError( FATAL_LEVEL,
        "%s %s Could not allocate %d doubles.", errorStr, routine, 
	_sizeInflowCO ) ;
        //numErrs++ ;
    }
    else
    ...
    if ( value_list != NULL && n_value != 0 )
    {
        ...
        if ( list == NULL || n_value == 0 )
        {
            ...
            /*AVCP PrintError ( routine,*/
	    logMessageWithArgsAndExitOnError( FATAL_LEVEL,
	    "%s %s Troubles getting COINFLOW data from control file.", 
	    errorStr, routine ) ;
            //return ( STATUS_FAILURE ) ;
        }
        if ( _sizeInflowCO == 1 && n_value != 1 && _lag_mode == CONSTANT_LAG )
        {
            ...
            /*AVCP PrintError ( routine,*/
	    logMessageWithArgsAndExitOnError( FATAL_LEVEL,
	    "%s %s Must have 1 carryover inflow value when lag is 0.",
	    errorStr, routine );
            //return ( STATUS_FAILURE ) ;
        }
        if ( _sizeInflowCO == 2 && n_value != 2 && _lag_mode == CONSTANT_LAG )
        {
            ...
            /*AVCP PrintError ( routine,*/
	    logMessageWithArgsAndExitOnError( FATAL_LEVEL,
	    "%s %s Must have 2 carryover inflow values when non-zero lag is "
	    "less than time step.", errorStr, routine ) ;
            //return ( STATUS_FAILURE ) ;
        }
        else if ( n_value != _sizeInflowCO )
        {
            ...
            /*AVCP PrintError ( routine,*/
	    logMessageWithArgsAndExitOnError( FATAL_LEVEL,
	    "%s %s %s %s must specify %d carryover inflow values.", 
	    errorStr, routine, _type, _id, _sizeInflowCO ) ;
            //return ( STATUS_FAILURE ) ;
        }
    ...
    if ( _k_mode == VARIABLE_K )
    {
        if ( makeStorageVSOutflowTable ( ) )
        {
            /*AVCP PrintError ( routine,*/
	    logMessageWithArgsAndExitOnError( FATAL_LEVEL,
	    "%s %s Troubles making Outflow-Storage table.", errorStr, routine );
            //numErrs++ ;
        }
        if ( makeStorageVSOutflowTableQuarter () )
        {
            /*AVCP PrintError ( routine,*/
	    logMessageWithArgsAndExitOnError( FATAL_LEVEL,
	    "%s %s Troubles making Outflow-Storage table at 1/4 timestep.",
	    errorStr, routine );
            //numErrs++ ;
        }
    }
    ...
    if ( _n_kval == 1 && valueOfK == 0 )
    {
        if ( _interp )
        {
	       ...
	       {
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	        "%s %s INITIALOUTFLOW value \"%f\" not consistent with "
                "COINFLOW values \"%f and %f\" when lagged resulting in \"%f\"."
                " Instability may result.  Revising "
                "INITIALOUTFLOW value to \"%f\".",
                warnStr, routine, _outflowCO/ffactor, tmpP/ffactor,
                tmpN/ffactor, LagdQin1/ffactor, LagdQin1/ffactor) ;
	       }
               _outflowCO = LagdQin1 ;
               _owner->_outflow = _outflowCO ;
            }
        ...
        else
        {
	      ...
	      if( getFewsDebugFlag() > 0 )
	      {
	        ...
	        "%s %s INITIALOUTFLOW value \"%f\" not consistent with "
                ...
                "may result.  Revising INITIALOUTFLOW value to \"%f\".", 
		warnStr, routine, _outflowCO/ffactor,
