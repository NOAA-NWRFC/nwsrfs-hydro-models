...
int DataIntervalMult;
	...
	// control file
	if( _evap_obs ) {
                DataIntervalMult = _evap_obs->getDataIntervalMult();
		evap = _evap_obs->getDataValue( cur_date ) * _t_mult/DataIntervalMult;
	/* Take out all the output statements by Raymond.Chui@noaa.gov at 2012-03-14, please do not use any C/C++ output statments
		in the future, because they cause FEWS to halt, just use OHD logMessageWithArgsAndExitOnError( INFO_LEVEL, "....")
	*/
	}
		...
		if( _n_evap ) {
		  ...
		  // We will default to 0
		   ...
		   /* AVCP PrintWarning( 1, routine, */
		      ...
		      "%s (%s) - MISSING data in evap "
		      "timeseries for RainEvap %s on %s.  Defaulting "
		      "to 0.", warnStr, routine, _id, _owner->_id );
...
//	}
	...
	if( _owner->_pool == MISSING ) {
	    /* AVCP PrintWarning( 1, routine, */
	    logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		  "%s (%s) - Previous pool elevation on %s is "
		  "missing - not setting loss.", warnStr, routine, _owner->_id );
	    return( STATUS_SUCCESS );
	}
	...
	if( new_pool < _owner->_min_pool ) {
	    /* AVCP PrintWarning( 1, routine, */
	    logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	       "%s (%s) - RainEvap %s on generated losses "
		"that cause pool on %s to drop below Minimum Pool (%f "
		"< %f). Setting pool to minimum.", warnStr, routine, 
		_id, _owner->_id, new_pool, _owner->_min_pool );
	...
	{  
	   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	      "(%s) - Setting loss on %s to %f for %s.",
	       routine, _owner->_id, loss, cur_date.toString() ); 
	}
