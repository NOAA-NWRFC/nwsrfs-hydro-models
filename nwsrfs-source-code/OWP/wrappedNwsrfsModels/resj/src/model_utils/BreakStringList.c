...
//	    "%s Breaking \"%s\" using \"%s\"", routine, string, delim );
//	}
	pt = string;
	while ( *pt ) {
		/*
		...
		*/
		...
		if ( (flag & DELIM_ALLOW_STRINGS) && instring ) {
		   ...
		   logMessageWithArgsAndExitOnError( DEBUG_LEVEL, 
		   "%s (%s) Quoted string \"%s\" is not closed", warnStr, routine, tempstr );
		}
		list = AddToStringList ( list, tempstr, &nlist2 );
		if ( list == (char **)NULL ) {
	           /* AVCP PrintWarning( 5, routine, */
		   logMessageWithArgsAndExitOnError( DEBUG_LEVEL, 
		   "%s (%s) Unable to break string - can't add to new list",warnStr, routine );
		   return (char **)NULL;
		}
//		if ( getFewsDebugFlag() > 3)
//	        {
...
{static char rcs_id1[] = "$Source: /fs/hseb/ob81/ohd/ofs/src/resj_utils/RCS/BreakStringList.c,v $";
