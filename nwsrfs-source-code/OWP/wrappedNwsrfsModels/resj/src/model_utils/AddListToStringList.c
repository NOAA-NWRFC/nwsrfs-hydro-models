...
{	int	i;
	...
	if ( listtoadd == (char **)NULL ) {
		...
		*/
		/*AVCP PrintWarning ( 5, routine,*/
	        ...
	        "%s (%s) - NULL list to add", warnStr, routine );
	...
	for ( i = 0; listtoadd[i] != (char *)NULL; i++ ) {
		...
		if ( newlist == (char **)NULL ) {
			/*AVCP PrintWarning ( 5, routine,*/
		        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
			"%s (%s) - Unable to add string item %d in list", 
			warnStr, routine, i );
			return newlist;
		}
	}
...
{static char rcs_id1[] = "$Source: /fs/hseb/ob81/ohd/ofs/src/resj_utils/RCS/AddListToStringList.c,v $";
