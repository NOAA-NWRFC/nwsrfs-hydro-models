	...
	*ndmx = 0;
	...
	if ( n <= 0 ) {
		...
		logMessageWithArgsAndExitOnError( DEBUG_LEVEL, 
	        "%s (%s) Number of points <= 0.  Returning max = 0.0",
		warnStr, routine );
...
{	int	i;
	...
	*ndmn = 0;
	...
	if ( n <= 0 ) {
		/* AVCP PrintWarning( 50, routine, */
	       logMessageWithArgsAndExitOnError( DEBUG_LEVEL, 
               "%s (%s) Number of points <= 0.  Returning min = 0.0",
	       warnStr, routine );
               return STATUS_FAILURE;
	}
	*min = x[0];
...
{	int	i;
	...
	*ndto = 0;
	...
	if ( n <= 0 ) {
		/* AVCP PrintWarning( 50, routine, */
		logMessageWithArgsAndExitOnError( DEBUG_LEVEL, 
	        "%s (%s) Number of points <= 0.  Returning ndto = 0.0",
	        warnStr, routine );
		return STATUS_FAILURE;
	}
...
int Normalize (int n, float *data)
{
	...
	if ( sum == 0.0 ) {
		/* AVCP PrintWarning( 50, routine, */
		logMessageWithArgsAndExitOnError( DEBUG_LEVEL, 
	        "%s (%s) Sum of weights is  0.  List cannot be normalized",
		warnStr, routine );
		return STATUS_FAILURE;
	}
	for ( i = 0; i < n; i++)
...
{static char rcs_id1[] = "$Source: /fs/hseb/ob81/ohd/ofs/src/resj_utils/RCS/Stat.c,v $";
