	...
	int n_param )
{
	...
	for( i = 0; i < n_param; i++ ) {
		...
		if( _root->getMethod( sub_list[0], sub_list[1], 
		   ...
		   if( getFewsDebugFlag() > 10 )
		   {
		      logMessageWithArgsAndExitOnError( DEBUG_LEVEL, 
	              "(%s) - Duplicate Method: %s %s %s.",
		      routine, sub_list[0], sub_list[1], sub_list[2] );
	           }
		...
		{
		   logMessageWithArgsAndExitOnError( DEBUG_LEVEL, 
		   "(%s) - Searching tree for component \"%s\".",
		   routine, sub_list[1] );
	        }
		...
		if( comp == NULL ) {
		   ...
		   "%s (%s) - PARAMETERS line \"%s\": "
		   "Unable to find \"%s\" in Component tree. "
		   "Check Topology.", errorStr, routine,
		   param_list[i], sub_list[1] );
	           /*AVCP		
		   if ( sub_list) {
		      sub_list = FreeStringList( sub_list );
		   }
		...
		if( !strcmp( sub_list[0], "ADJUST" ) ) {
		   ...
		   logMessageWithArgsAndExitOnError( DEBUG_LEVEL, 
		    "%s (%s) - The \"%s\" method is not supported yet.",
		    warnStr, routine, sub_list[0] );
		...
		if( !method ){
			...
			/* AVCP PrintError( routine, */
			logMessageWithArgsAndExitOnError( FATAL_LEVEL, 
		      "%s (%s) - Unable to allocate memory for method \"%s\".",
		...
		if(strlen(sub_list[2]) > 12 ) {
			...
			/* AVCP PrintError( routine, */
			logMessageWithArgsAndExitOnError( FATAL_LEVEL, 
		        "%s (%s) - Method ID \"%s\" is too long ( > 12 chars).",
	  	        errorStr, routine, sub_list[2] );
		...
		if( method->setID( sub_list[2] ) ){
			...
			/* AVCP PrintError( routine, */
			logMessageWithArgsAndExitOnError( FATAL_LEVEL, 
		        "%s (%s) - Troubles setting Identifier \"%s\" for %s "
		        "method.", errorStr, routine, 
			sub_list[2], sub_list[0] );
		...
		if( comp->addMethod( NULL, method ) ){
			...
			/* AVCP PrintError( routine, */
			logMessageWithArgsAndExitOnError( FATAL_LEVEL, 
		        "%s (%s) - Troubles adding the method "
			"\"%s\" to Component \"%s\".", 
			errorStr, routine, method->getID(), comp->getID() );
			/*AVCP
			if ( method != NULL ) {
				delete method;
				method = NULL;
	...
	for( i=0; i<nlist; i++ ) {
		/* AVCP PrintDebug( 20, routine, "Rule Line[%d]: %s", i, list[i]
		);*/
                if( getFewsDebugFlag() > 10 )
		{
		   logMessageWithArgsAndExitOnError( DEBUG_LEVEL, 
		   "(%s) - Rule Line[%d]: %s", routine, i, list[i] );
	        }
		...
		if( !strncmp( list[i], "[", 1 ) ) {
			...
			if( expr_str[0] != LBRACE || 
				...
				/* AVCP PrintError( routine, */
				logMessageWithArgsAndExitOnError( FATAL_LEVEL, 
		                "%s (%s) - Conditional expression \"%s\" is "
				"malformed.", errorStr, routine, expr_str );	
		...
		// Handle the RULE Rule
			...
			if( strlen( expr_str ) == 0 ){
				...
				/* AVCP PrintError( routine, */
				logMessageWithArgsAndExitOnError( FATAL_LEVEL, 
		                 "%s (%s) - Rule line \"%s\" is not within any "
		     		"expressions.", errorStr, routine, list[1] );	
...
// printf( "\n !!str previous string= %s, nspil= %d. nrel_spil= %d\n", res_name, nspil, nrel_spil);
		        ...
		        {
		           logMessageWithArgsAndExitOnError( DEBUG_LEVEL, 
		           "(%s) - Using expression \"%s\" for rule \"%s\".",
			   routine, expr_str, list[i] );
	                }
			...
			if( comp == NULL ){
				...
				/* AVCP PrintError( routine, */
				logMessageWithArgsAndExitOnError( FATAL_LEVEL, 
		                "%s (%s) - Unable to find \"%s\" in Component "
				"tree.", errorStr, routine, sub_list[1] );	
			        /*AVCP 	
				if ( sub_list) {
					sub_list = FreeStringList( sub_list );
				}
			...
			if( method == NULL ){
				//totErrs++;
				/* AVCP PrintError( routine, "RULE- %s: " */
				logMessageWithArgsAndExitOnError( FATAL_LEVEL, 
		                "%s (%s) - RULE- %s: Unable to find \"%s\" "
				"method \"%s\" ",
			      	errorStr, routine, rule, sub_list[0],
			        sub_list[2] );	
				/*AVCP
				if ( sub_list) {
					sub_list = FreeStringList( sub_list );
			...
			if( !expr ){
			   ...
			   method = NULL;
			   logMessageWithArgsAndExitOnError( FATAL_LEVEL, 
		           "%s (%s) - Expression \"%s\" could not be parsed!",
		            errorStr, routine, expr_str );
			  //continue;
			}
			...
			if( expr->verify() ){
				...
				/* AVCP PrintError( routine, */
				logMessageWithArgsAndExitOnError( FATAL_LEVEL, 
		               "%s (%s) - Unable to verify constant values for "
				"\"%s\".", errorStr, routine, expr_str );
			...
			if( comp->addExpression( expr, sub_list[0],
				...
				/* AVCP PrintError( routine, */
				logMessageWithArgsAndExitOnError( FATAL_LEVEL, 
		                "%s (%s) - Troubles adding the expression "
				"\"%s\" at postion %d to Component %s.",
	 		        errorStr, routine, expr_str, 
				pos, comp->getID() );
				//continue;
			}
	...
	if( nspil >= 1 && nrel_spil <= 1 ) {
		...
		/* AVCP PrintError( routine, */
		logMessageWithArgsAndExitOnError( FATAL_LEVEL, 
		"%s (%s) - SPILLWAY method can not be used alone, "
		"must be used with SETRELEASE method. "
		"You can overwrite with SETSUM, SETMAX "
		"or SETMIN method for reservoir %12s",
		errorStr, routine, res_name  );
