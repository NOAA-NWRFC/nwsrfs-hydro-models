...
/*AVCP void execute58( char* fname, float* CO, float* PO, float* D, 
		...
		int* st_hr, int* end_da, int* end_hr, int* ifillc, int* iusec )
{
 ...
 long int hostidnum;
	...
	ResJSys resjsys;
	/*AVCP ResJSys::setIPR_IODEBUG (*ipr, *iodebug);*/
	...
	logMessageWithArgsAndExitOnError( DEBUG_LEVEL, 
		"input parameter file = %s ", ctl_file);

//cew split if on open into two pieces
	fp = fopen( ctl_file, "r" );
	if( fp == NULL ) {
		/*AVCP *ierr = 1;*/
		sprintf( error, "Troubles opening file: %s", ctl_file );
		/* AVCP PrintError( routine, */
		logMessageWithArgsAndExitOnError( FATAL_LEVEL, 
	        "%s %s %s", errorStr, routine, error );
		//AVCP return;
...
//hdh	strcat(tmp_file,"/.resj_tmp_file");
        ...
        strftime(timestamp,sizeof timestamp,".%Y.%H.%M.%S",mytm);  
	...
	strcat(tmp_file,timestamp);
	
	tfp = fopen( tmp_file, "w+" );
	if( tfp == NULL ) {
		/*AVCP *ierr = 1;*/
		sprintf( error, "Troubles opening temporary file: %s", 
			tmp_file );
		/* AVCP PrintError( routine, */
		logMessageWithArgsAndExitOnError( FATAL_LEVEL, 
		"%s %s %s", errorStr, routine, error );
	...
	char_co[ (*iusec)*sizeof(float) ] = '\0';
        ...
        {
           logMessageWithArgsAndExitOnError(DEBUG_LEVEL,"Begin CO:\n%s\n\n",char_co);
        }
	...
	while( fgets( string, 256, fp ) != NULL ) {
		...
		if( !strcasecmp( list[0], "UNITS" ) ) {
			if( !strcasecmp( list[1], "ENGLISH" ) ) {
				lfactor = 0.3048;   /* Meter (M)              */
				vfactor = 1233.5;   /* Cubic meter (CM)       */
				ffactor = 0.028317; /* Cubic meter second(CMS)*/
			}
		}
		...
		if( Node_found ) {
			...
			if( strstr( list[0], "DISCHARGE" ) )
			{
			        ...
			        !strcasecmp( list[0], "DISCHARGE" ) )
			    {
			        ...
			        // the index
			        strncpy(tmpval, &value[12+12+6], 8); 
			        tmpval[8] = '\0';
			        conv_val = atof( tmpval ) / ffactor;
			        whichCO[0] = 1;
			    }
			    else if(!strcasecmp( list[0], "PREVIOUSDISCHARGE" ))
			    {
			        ...
			        strncpy(tmpval, &value[12+12+6+8], 8); 
			        tmpval[8] = '\0';
			        conv_val = atof( tmpval ) / ffactor;
			        whichCO[1] = 1;
			    }
			...
			else if( strstr( list[0], "INFLOW" ) && NewCO )
			{
			        ...
			        !strcasecmp( list[0], "INFLOW" ) )
			    {
			        ...
			        strncpy(tmpval, &value[12+12+6+8+8], 8); 
			        tmpval[8] = '\0';
			        conv_val = atof( tmpval ) / ffactor;
			        whichCO[2] = 2;
			    }
			    else if( !strcasecmp( list[0], "PREVIOUSINFLOW" ) )
			    {
			        ...
			        // INITIALINFLOW
			        strncpy(tmpval, &value[12+12+6+8+8+8], 8); 
			        tmpval[8] = '\0';
			        conv_val = atof( tmpval ) / ffactor;
			        whichCO[3] = 3;
			    }
			...
			else if( strstr( list[0], "DIVERSION" ) && NewCO )
			{
			        ...
			        !strcasecmp( list[0], "DIVERSION" ) )
			    {
			        ...
			        strncpy(tmpval, &value[12+12+6+8+8+8+8], 8); 
			        tmpval[8] = '\0';
			    ...
			    else if( !strcasecmp( list[0], "PREVIOUSDIVERSION" ) )
			    {
			        ...
			        // INITIALINFLOW, PREVIOUSINFLOW,
			        // INITIALDIVERSION
			        strncpy(tmpval, &value[12+12+6+8+8+8+8+8], 8); 
			        tmpval[8] = '\0';
			        // NOTE: It is possible that the carryover value
			        // begins with "-999".  When the array was
			        // written, the "-999" was replaced with "MISS"
		...
		if( LagK_found ) {
			if( val_found == 1 ) {
				...
				initialOutflow = 1;
				strncpy( tmpval, 
					&value[42+NewCO*12+sizeInflowCO*8], 8);
				tmpval[8] = '\0';
				conv_val = atof( tmpval ) / ffactor;
				fprintf(tfp, "%s  %f\n", list[0], conv_val);
				continue;
				...
				initialStorage = 1;
				strncpy( tmpval, 
					&value[50+NewCO*12+sizeInflowCO*8], 8);
				tmpval[8] = '\0';
				conv_val = atof( tmpval ) / ffactor;
				fprintf(tfp, "%s  %f\n", list[0], conv_val);
				continue;
				...
				strncpy( tmpval, 
					&value[58+NewCO*12+sizeInflowCO*8], 8);
				tmpval[8] = '\0';
				conv_val = atof( tmpval ) / ffactor;
				fprintf(tfp, "%s  %f\n", list[0], conv_val);
				continue;
			...
			while (value != NULL && !surematch) {
				...
				sprintf( tmp_str, "%-12.12s", &value[24+6] );
		...
		if( Lookup3_found )
		{
		    ...
		    else if( !strcasecmp( list[0], "BLENDTBL" ) )
		    {
		        ...
		        strncpy(tmp_str0, &value[12+12+6+12+12+4], 4);
		        tmp_str0[4] = '\0';
		        // And the value after the BLENDTBL step
		        strncpy(tmp_str1, &value[12+12+6+12+12+4+4], 4);
		        tmp_str1[4] = '\0';
		...
		else if( !strcasecmp( list[0], "LOOKUP3" ) )
		{
		    ...
		    while (value != NULL && !surematch)
		    {
		        ...
		        // carry over
		        sprintf( tmp_str, "%-12.12s", &value[24+6+12] );
		...
		if( SetR_W_E_found ) {
			if( !strcasecmp( list[0], "BLENDTS" ) ) {
				...
				// index 
				strncpy(tmpval, &value[12+12+6+NewCO*24], 4);
				tmpval[4] = '\0';
				...
				strncpy(tmpval, &value[12+12+6+NewCO*24+4], 4); 
				tmpval[4] = '\0';
				conv_int = atoi( tmpval );
				fprintf(tfp, "%s  %s  %d\n", list[0], list[1], 
					conv_int);
				...
				// BLENDTBL step.
				strncpy(tmpval, &value[12+12+6+NewCO*24+4+4], 8); 
				tmpval[8] = '\0';
				conv_val = atof( tmpval ) / ffactor;
				fprintf(tfp, "%s %f\n", list[0], conv_val);
			}
		...
		if( Spillway_found ) {
			if( !strcasecmp( list[0], "INITIALSPILL" ) ) {
				...
				strncpy(tmpval, &value[12+12+6+12+12], 8);
				tmpval[8] = '\0';
				conv_val = atof( tmpval ) / ffactor;
				fprintf(tfp, "%s  %f\n", list[0], conv_val);
				whichCO[0] = 1;
		...
		if( CalcInflow_found ) {
			if( !strcasecmp( list[0], "REMAININGVOL" ) ) {
				// copy the value following two identifiers, the
				// index, and the other two identifiers.
				strncpy(tmpval, &value[12+12+6+12+12], 16);
				tmpval[16] = '\0';
...
*/
	...
	for( i = 0, totalts = 0, ints = 0; i < strlen( char_po ); i+=20 ) {
		if( char_po[i+18] == 'I' && char_po[i+19] == 'N' ) {
			...
			{
			   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
			   "%s out_ts_index[%d]= %d; d_index[%d]: %d",
			   routine, totalts-ints,out_ts_index[totalts-ints],
			   totalts, d_index[totalts] );
			}
		}
		totalts++;
	}
	...
	{
           logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	   "%s OUTPUT TIME SERIES (%d): tot:%d, ints:%d\n", routine, n_outts,
	   totalts, ints );
	}
	...
	for( i = 0; i < n_outts; i++ ) {
		...
		if( outts == NULL ) {
			...
			idate.addInterval( TS :: INTERVAL_HOUR, t_step ), 
				...
				if ( getFewsDebugFlag() > 3 && j <= 100)
				{
				   logMessageWithArgsAndExitOnError(DEBUG_LEVEL,
				   "IN EXECUTE58 TEST %s outputTS_D[%d][%d] =%f\n", routine,
				   out_ts_index[i],j, D[out_ts_index[i]+j]);
				}
			}
		}
	}
	...
	if( *ifillc == 1 && *num_co > 0 ) {
		...
		/* AVCP we don't need use the logic below to write CO
        ...
        if(getFewsDebugFlag() >= 3)
        {
          char *end_char_co =  (char*)malloc( (*iusec)*sizeof(float)+1 );

          memcpy( end_char_co, CO, (*iusec)*sizeof(float) );
          end_char_co[ (*iusec)*sizeof(float) ] = '\0';
          logMessageWithArgsAndExitOnError(DEBUG_LEVEL, "END CHAR CO IN EXE: %s", end_char_co);
        }
	...
	/*AVCP sprintf( error, "Successfully ran ResJ.");
	length = strlen( error ); 
	ResJ_fwrite( &length, error, ipr );*/

        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
        "%s Successfully ran ResJ.", routine );
