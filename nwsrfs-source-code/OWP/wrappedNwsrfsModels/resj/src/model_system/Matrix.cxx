...
//     double Matrix value
// Calls:
//     logMessageWithArgsAndExitOnError( int, char*, char* )
// Errors:
//     None
// Warnings:
...
double Matrix :: getElement( int row, int col )
{
    ...
    if( row < 0 || col < 0 || row >= _n_rows || col >= _n_cols )
    {
        ...
        logMessageWithArgsAndExitOnError( DEBUG_LEVEL, 
            "%s (%s) - Cannot get matrix element at "
            "position [%d][%d]  - out of range.", warnStr, routine, row, col );
...
int Matrix :: setMatrix ( int rows, int cols, double value )
{
    ...
    if( rows < 1 || cols < 1 )
    {
       /* AVCP PrintWarning( 1, routine */
       logMessageWithArgsAndExitOnError( DEBUG_LEVEL, 
        "%s (%s) - Cannot set matrix with the number "
        "rows and/or columns < 1.", warnStr, routine );
        return 1;
    } 
    ...
    if( allocateDataSpace() )
    {
       /* AVCP PrintWarning( 1, routine */
       logMessageWithArgsAndExitOnError( DEBUG_LEVEL, 
       "%s (%s) - Cannot allocate %d doubles.", warnStr, routine, rows*cols );
...
int Matrix :: setMatrix( int rows, int cols, double* values )
{
    ...
    if( rows < 1 || cols < 1 )
    {
       /* AVCP PrintWarning( 1, routine */
       logMessageWithArgsAndExitOnError( DEBUG_LEVEL, 
       "%s (%s) - Cannot set matrix with the number rows "
       "and/or columns < 1.", warnStr, routine );
    ...
    if( values == NULL )
    {
        /* AVCP PrintWarning( 1, routine */
        logMessageWithArgsAndExitOnError( DEBUG_LEVEL, 
        "%s (%s) - Cannot set matrix with NULL data.", warnStr, routine );
    ...
    if( allocateDataSpace() )
    {
        /* AVCP PrintWarning( 1, routine */
        logMessageWithArgsAndExitOnError( DEBUG_LEVEL, 
        "%s (%s) - Cannot allocate %d double.", warnStr, routine, rows*cols );
