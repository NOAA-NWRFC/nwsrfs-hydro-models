...
C
     ...
     .          QIMT(NUMDAY),IERD(NUMDAY),REX(NUMDAY)
! bug 280     .          QIMT(31),IERD(31),REX(31)
C
...
!CP   1600 FORMAT('   *** ENTER XU1826 ***')
      IF ( FEWSDEBUG.GE.4 ) THEN
         WRITE(MESSAGESTRING,'(A23)') '   *** ENTER XU1826 ***'
...
!CP   IF (IBUG.GE.2) WRITE(IODBUG,1610) (D(LTSPOO+I-1),I=1,NTSPOO)
      IF ( FEWSDEBUG.GE.4 ) THEN
         NPRINT=NTSPOO
...
C
  ...
  140 CONTINUE
!CP   IF (IBUG.GE.1) WRITE(IODBUG,1603) (W(LPTADL+I-1),I=1,LOBSA)
      IF ( FEWSDEBUG.GE.4 ) THEN
         NPRINT=LOBSA
	 IF (NPRINT.GT.100) NPRINT=100
...
C
      ...
      NMISS = 0
  100 CONTINUE
!CP   IF (IBUG.GE.2) WRITE(IODBUG,1620) (W(LOCWK+I-1),I=1,NTSPOO)
      IF ( FEWSDEBUG.GE.4 ) THEN
         NPRINT=NTSPOO
	 IF (NPRINT.GT.100) NPRINT=100
         ...
         call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
 1620    FORMAT(/'   ** FILLED ELEVATION TIME-SERIES :')
         WRITE(FORMATSTR, *) '%10.3f'
	 call logonedimensionarrayfromfortran(DEBUG_LEVEL,NULLLINE,1,8,
     >        FORMATSTR,LOCWK,LOCWK+NPRINT-1,1,W)
      END IF
C
...
!CP   WRITE(IPR,611) IDAY
      WRITE(MESSAGESTRING, 611) IDAY
...
C
  ...
  200 CONTINUE
!CP   IF (IBUG.GE.1) 
!CP  .WRITE(IODBUG,1630) (QIMT(I),I=1,NDAY)
      IF ( FEWSDEBUG.GE.4 ) THEN 
         NPRINT=NDAY
	 IF(NPRINT.GT.100) NPRINT=100
         ...
         call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
 1630    FORMAT(/'   ** MEAN DAILY COMPUTED INFLOWS :') 
         WRITE(FORMATSTR, *) '%10.1f'
	 call logonedimensionarrayfromfortran(DEBUG_LEVEL,NULLLINE,1,8,
     >        FORMATSTR,1,NPRINT,1,QIMT)
      END IF
C
...
C        CALL UMEMOV(D(LTQIM),W(LBFQIM),NUM)
  ...
  560        W(LBFQIM+I-1) = D(LTQIM+I-1)*NTIM24
      ENDIF
C
!CP   IF (IBUG.GE.2) THEN
      IF ( FEWSDEBUG.GE.4 ) THEN
         NPRINT=NDAY
	 IF (NPRINT.GT.100) NPRINT=100
	 
!CP      WRITE(IODBUG,1625) (D(LTSQ+I-1),I=1,NDAY)
         WRITE(MESSAGESTRING, 1625) 
         call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
 1625 FORMAT(/'   ** OBSERVED DAILY MEAN DISCHARGES :') 
         WRITE(FORMATSTR, *) '%10.1f'
	 call logonedimensionarrayfromfortran(DEBUG_LEVEL,NULLLINE,1,8,
     >        FORMATSTR,LTSQ,LTSQ+NPRINT-1,1,D)

!CP      WRITE(IODBUG,1640) (W(LBFQI+I-1),I=1,NUM)
         NPRINT=NUM
	 IF (NPRINT.GT.100) NPRINT=100
         WRITE(MESSAGESTRING, 1640) 
         call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
 1640 FORMAT(/'   ** INST. ADJUSTED INFLOWS :') 
         WRITE(FORMATSTR, *) '%10.1f'
         call logonedimensionarrayfromfortran(DEBUG_LEVEL,NULLLINE,1,8,
     >        FORMATSTR,LBFQI,LBFQI+NPRINT-1,1,W)

!CP      WRITE(IODBUG,1650) (W(LBFQIM+I-1),I=1,NUM)
         WRITE(MESSAGESTRING, 1650) 
         call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
 1650 FORMAT(/'   ** MEAN ADJUSTED INFLOWS :') 
         WRITE(FORMATSTR, *) '%10.1f'
         call logonedimensionarrayfromfortran(DEBUG_LEVEL,NULLLINE,1,8,
     >        FORMATSTR,LBFQIM,LBFQIM+NPRINT-1,1,W)
      ENDIF
C
...
!CP   1699 FORMAT('    *** EXIT XU1826 ***')
      IF ( FEWSDEBUG.GE.4 ) THEN
         WRITE(MESSAGESTRING, '(A23)') '    *** EXIT XU1826 ***'
	 call logfromfortran(DEBUG_LEVEL, MESSAGESTRING) 
      END IF
      RETURN
      END
