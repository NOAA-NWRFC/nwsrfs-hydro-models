...
!CP   1600 FORMAT('    *** ENTER XMAX26 ***')
      IF ( FEWSDEBUG.GE.4 ) THEN
         WRITE(MESSAGESTRING, '(A23)') '    *** ENTER XMAX26 ***'
...
C
      ...
      MAX = MAX0(MAX,NEVSQ)
C
!CP   IF (IBUG .GE. 2) WRITE(IODBUG,1610) I,NEVSQ,MAX
      IF ( FEWSDEBUG.GE.3 ) THEN
         WRITE(MESSAGESTRING, 1610) I,NEVSQ,MAX
	 call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
      END IF
...
!CP   1699 FORMAT('       *** EXIT XMAX26 ***')
      IF ( FEWSDEBUG.GE.4 ) THEN 
         WRITE(MESSAGESTRING, '(A23)') '    *** EXIT XMAX26 ***'
	 call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
      END IF
      RETURN
      END
