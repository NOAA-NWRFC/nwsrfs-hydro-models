...
!CP   1600 FORMAT('   *** ENTER XADJ26 ***')
      IF ( FEWSDEBUG.GE.4 ) THEN
         WRITE(MESSAGESTRING, '(A23)') '   *** ENTER XADJ26 ***'
	 call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
      END IF

      INCOMP=0
!BUG 280       DO 50 I=1,31
      DO 50 I=1,NUMDAY
  50  IERD(I)=0
!CP   IF (IBUG.LE.1) GO TO 60
      IF ( FEWSDEBUG.LT.5 ) GO TO 60
      IF (LOBSQO.EQ.0) GO TO 55
!CP   WRITE(IODBUG,1620)
      WRITE(MESSAGESTRING, 1620)
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
 1620 FORMAT(/10X,'** OBSERVED INSTANTAEOUS DISCHARGES **')
!CP   WRITE(IODBUG,1650) (D(LOBSQO+I-1),I=IBSQ,IESQ)
!1650 FORMAT(1X,8F10.1)
     ...
     >     FORMATSTR,LOBSQO+IBSQ-1, LOBSQO+NPRINT-1,1,D)
       

!CP   55  WRITE(IODBUG,1640)
  55  WRITE(MESSAGESTRING, 1640)
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
 1640 FORMAT(/10X,'** MEAN DAILY DISCHARGES **')
!CP   WRITE(IODBUG,1650) (QMD(I),I=IBQM,IEQM)
      NPRINT=IEQM
      IF ( NPRINT.GT.100 ) NPRINT=100
      
      WRITE(FORMATSTR, *) '%10.1f'
      call logonedimensionarrayfromfortran(DEBUG_LEVEL,NULLLINE,1,8, 
     >     FORMATSTR,IBQM,NPRINT,1,QMD)
...
C
 ...
 95   NB=NE+1
      NE=NB+NP-1
 100  CONTINUE
!CP   IF (IBUG.LE.1) GO TO 96
      IF ( FEWSDEBUG.LT.5 ) GO TO 96
!CP   WRITE(IODBUG,1690) ITER,IERMX,SQMD(IERMX),QMD(IERMX)
      WRITE(MESSAGESTRING, 1690) ITER,IERMX,SQMD(IERMX),QMD(IERMX)
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
 1690 FORMAT(/5X,' ITER= ',I2,
     & '  MAX. DAILY OUTFLOW DIFF. OCCURED AT DAY ',I3,
     & '  ADJUSTED= ',F10.1,'  OBSERVED= ',F10.1)
 
!CP   WRITE(IODBUG,1630)
      WRITE(MESSAGESTRING, 1630)
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
 1630 FORMAT(10X,'** ADJUSTED INSTANTAEOUS DISCHARGES **')
 
!CP   WRITE(IODBUG,1650) (QS(I),I=IBSQ,IESQ)
      NPRINT=IESQ
      IF ( NPRINT.GT.100 ) NPRINT=100

      WRITE(FORMATSTR, *) '%10.1f'
      call logonedimensionarrayfromfortran(DEBUG_LEVEL,NULLLINE,1,8,
     >     FORMATSTR,IBSQ,NPRINT,1,QS)
      

!CP   WRITE(IODBUG,1660)
      WRITE(MESSAGESTRING, 1660)
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
 1660 FORMAT(10X,'** MEAN ADJUSTED DAILY DISCHARGES **')
 
!CP   WRITE(IODBUG,1650) (SQMD(I),I=IBQM,IEQM)
      NPRINT=IEQM
      IF ( NPRINT.GT.100 ) NPRINT=100
      
      WRITE(FORMATSTR, *) '%10.1f'
      call logonedimensionarrayfromfortran(DEBUG_LEVEL,NULLLINE,1,8,
     >     FORMATSTR,IBQM,NPRINT,1,SQMD)
      

!CP   WRITE(IODBUG,1640)
...
C
      ...
      ITER=ITER+1
      GO TO 70
!CP   160  WRITE(IPR,905)
 160  WRITE(MESSAGESTRING, 905)
...
C
 ...
 585  INCOMP=1
      IERD(I)=1
!CP   WRITE(IPR,906) I,SQMDI,QMDI
      WRITE(MESSAGESTRING, 906) I,SQMDI,QMDI
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
...
!CP   1699 FORMAT('    *** EXIT XADJ26 ***')
      IF ( FEWSDEBUG.GE.4 ) THEN
         WRITE(MESSAGESTRING,'(A23)') '    *** EXIT XADJ26 ***'
         call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
      END IF
C
      RETURN
