...
!CP   1600 FORMAT('   *** ENTER XPTR26 ***')
      IF ( FEWSDEBUG.GE.4 ) THEN
         WRITE(MESSAGESTRING, '(A23)') '   *** ENTER XPTR26 ***'
...
C
 9000 CONTINUE
!CP   IF (IBUG.GE.2) WRITE(IODBUG,1680) SUNUM,IBASE,LEVEL,IORD,LOCPM,
      IF ( FEWSDEBUG.GE.3 ) THEN
         WRITE(MESSAGESTRING, 1680) SUNUM,IBASE,LEVEL,IORD,LOCPM,
     .                              LOCTS,LOCCO
         call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
...
!CP   1699 FORMAT('    *** EXIT XPTR26 ***')
      IF ( FEWSDEBUG.GE.4 ) THEN
         WRITE(MESSAGESTRING, '(A23)') '    *** EXIT XPTR26 ***'
	 call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
      END IF
      RETURN
      END
