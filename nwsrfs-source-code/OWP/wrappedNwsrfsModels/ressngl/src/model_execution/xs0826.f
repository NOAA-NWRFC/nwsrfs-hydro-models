C MODULE XS0826
C
      SUBROUTINE XS0826(SUNUM,PO,W,D,LOCWS,LOCOWS,IDPT,NHRYR)
C--------------------------------------------------------------------
C
...
C with NTIMESTEPS
     ...
     .          ELRES(20),RELNO(20),DNOVAL(2),NLAGN(2),NHRYR(1),
...
!CP   1600 FORMAT('   *** ENTER XS0826 ***')
      IF ( FEWSDEBUG.GE.4 ) THEN
         WRITE(MESSAGESTRING, '(A23)') '   *** ENTER XS0826 ***'
...
C
      ...
      IF (FEWSDEBUG.GE.4) THEN
        WRITE(MESSAGESTRING, *)'IDARUN=',IDARUN,' NS1=',NS1,' NTIM24=',
     >                        NTIM24
        call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
      END IF
...
!CP   IF (IBUG.LT.2) GO TO 510
      IF ( FEWSDEBUG.LT.3 ) GO TO 510
      IF(NLAGK.LE.1) THEN 
!CP     WRITE(IODBUG,1652)
        WRITE(MESSAGESTRING, 1652)
        call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
        GO TO 510
      ENDIF
 1652 FORMAT(/'    *** CALLING EX7 FOR LAG/K OPERATION ***')
!CP   IF(J.EQ.1) WRITE(IODBUG,1650)
      IF(J.EQ.1) THEN
        WRITE(MESSAGESTRING, 1650)
        call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
      END IF
 1650 FORMAT(/'    *** CALLING EX7 FOR K OPERATION ***')
!CP   IF(J.EQ.2) WRITE(IODBUG,1651)
      IF(J.EQ.2) THEN
        WRITE(MESSAGESTRING, 1651)
        call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
      END IF
...
C      LQTLX = LOCOWS(12)+(IG-1)*NDD*NTIM24+NS2+NTLAG-1
      ...
      W(LQTLX) = QTLX
C
C      LOCSTG(IG) = LOCOWS(12) + (IG+5)*NDD*NTIM24
      LOCSTG(IG) = LOCOWS(12) + (IG+5)*NTIMESTEPS
      
C      LOCWK1(IG) = LOCOUT + NDD*NTIM24
      LOCWK1(IG) = LOCOUT + NTIMESTEPS
C      LOCWK2(IG) = LOCWK1(IG) + NDD*NTIM24
      LOCWK2(IG) = LOCWK1(IG) + NTIMESTEPS
C
      JSUM = (IG-1)*2+1
      DSVALU(JSUM) = RCCO(2)
      DSVALU(JSUM+1) = QLKUP
...
C      IF(MAINUM.EQ.2) GO TO 2015
      IF(IFMSNG(FRLOOP).EQ.1) GO TO 2015
      IF(NS2.NE.1 .OR. IFCST.NE.0) GO TO 2015
!CP   WRITE(IPR,9003) RTCVID
      WRITE(MESSAGESTRING, 9003) RTCVID
...
C IF FLOW CONTROLLED, NO SPACE ARE RESERVED FOR CARRYOVER IN OLD METHOD
      ...
      ENDIF
 4950 CONTINUE
!CP   IF(IBUG.LE.1) GO TO 5000
      IF ( FEWSDEBUG.LT.3 ) GO TO 5000
!CP   WRITE(IODBUG,1660) LOCRCO,RCCO
      WRITE(MESSAGESTRING, 1660) LOCRCO,RCCO
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
!CP   WRITE(IODBUG,1665) DSVALU
      WRITE(MESSAGESTRING, 1665) DSVALU
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
!CP   WRITE(IODBUG,1670) IG,ISTAGE(IG),ITRIB(IG),QOUTX(1),QADDX,
      WRITE(MESSAGESTRING, 1670) IG,ISTAGE(IG),ITRIB(IG),QOUTX(1),QADDX,
     & STGX,QLKUP
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
...
!CP   1699 FORMAT('    *** EXIT XS0826 ***')
      IF ( FEWSDEBUG.GE.4 ) THEN
         WRITE(MESSAGESTRING,'(A23)') '    *** EXIT XS0826 ***'
	 call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
      END IF
C
ccc      ibug=0
