...
!CP   600 FORMAT(' *** ENTER X17I26 ***')
      IF ( FEWSDEBUG.GE.4 ) THEN
         WRITE(MESSAGESTRING, '(A23)') ' *** ENTER X17I26 ***'
...
C
!CP   IF (IBUG .GE. 2) WRITE(IODBUG,601) NBLEND,DIFFS,QDIFF,NSTEPS,IDTQ
      IF ( FEWSDEBUG.GE.3 ) THEN
         WRITE(MESSAGESTRING, 601) NBLEND,DIFFS,QDIFF,NSTEPS,IDTQ
	 call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
...
C
      IQTYPE = 1
      ISUTYP = 2
!CP   IF (IBUG .GE. 2) WRITE(IODBUG,602)
!CP   IF (IBUG .GE. 2) WRITE(IODBUG,650) (D(LOBSQO+I-1),I=1,NPVAL)
      IF ( FEWSDEBUG.GE.4 ) THEN
         NPRINT=NPVAL
  ...
  602    FORMAT(/10X,'** OBSERVED INST. DISCHARGES **')
     ...
     >        FORMATSTR,LOBSQO,LOBSQO+NPRINT-1,1,D)
...
C
  900 CONTINUE
!CP   IF (IBUG .GE. 2) WRITE(IODBUG,603)
!CP   IF (IBUG .GE. 2) WRITE(IODBUG,650) (W(LOCQO+I-1),I=1,NRUN)
!CP   IF (IBUG.GE.1) WRITE(IODBUG,699)
!CP   699 FORMAT('   ***EXIT X17I26 ***')
      IF ( FEWSDEBUG.GE.4 ) THEN
         NPRINT=NRUN
         IF ( NPRINT.GT.100 ) NPRINT=100
  ...
  603    FORMAT(/10X,'** ADJUSTED INST. DISCHARGES **')
         WRITE(FORMATSTR, *) '%10.0f'
         call logonedimensionarrayfromfortran(DEBUG_LEVEL,NULLLINE,1,8,
     >        FORMATSTR,LOCQO,LOCQO+NPRINT-1,1,W)
         WRITE(MESSAGESTRING, '(A21)') '   ***EXIT X17I26 ***'
	 call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
      END IF
C
      RETURN
