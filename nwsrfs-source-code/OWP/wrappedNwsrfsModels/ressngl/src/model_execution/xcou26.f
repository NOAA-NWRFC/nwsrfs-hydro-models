...
C
      ...
      CHARACTER*5 DIM, UCODE
C
!CP   IF (IBUG.GE.1) WRITE(IODBUG,1600)
!CP   1600 FORMAT('   *** ENTER XCOU26 ***')
      IF ( FEWSDEBUG.GE.4 ) THEN
         WRITE(MESSAGESTRING, '(A23)') '   *** ENTER XCOU26 ***'
...
C
!CP   IF (IBUG.GE.2) WRITE(IODBUG,1610) IBASE,LEVEL
      IF ( FEWSDEBUG.GE.3 ) THEN
        WRITE(MESSAGESTRING, 1610) IBASE,LEVEL
	call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
...
C  END OF CHANGE OF 10/11/89
!CP   IF (IBUG.GE.2) WRITE(IODBUG,1620) IOFFCO,IOFFT
      IF ( FEWSDEBUG.GE.3 ) THEN
         WRITE(MESSAGESTRING, 1620) IOFFCO,IOFFT
	 call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
...
!CP   CALL FDCODE(PO(LOCTS+2),X,DIM,IX,IX,X,IX,IX)
      CALL getDimensionAndUnitInFortran(PO(LOCTS+2), DIM, UCODE)
      IQO=0
      IF(TRIM(DIM).EQ."L3/T") IQO=1
      COLAST=QO2
C
...
C
      ...
      IDADAT = IDA
      LOCIN = LOCOWS(12)
C      LOCOUT = LOCIN + NDD*NTIM24
      LOCOUT = LOCIN + NTIMESTEPS
C
      NCLGK=0
      LCLKX=LOCCLK
      DO IG=1,NGAGES
...
C
 ...
 1520 CONTINUE
!CP   IF(IBUG.LT.2 .OR. NS2.GE.2) GO TO 188
!CP   WRITE(IODBUG,1611) IG
      IF ( FEWSDEBUG.LT.3 .OR. NS2.GE.2) GO TO 188
      WRITE(MESSAGESTRING, 1611) IG
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
 1611 FORMAT(1X,'LOCAL INFLOW FOR GAGE ',I1,':')
!CP   WRITE(IODBUG,1612) (D(IDPT(LOCTSD)+IOFFTS+IUM-1),IUM=1,NUM)
!1612 FORMAT(1X,8F10.1)
     ...
     >     FORMATSTR,IDPT(LOCTSD)+IOFFTS,IDPT(LOCTSD)+IOFFTS+NPRINT-1,
     >     1,D)
C
  188 CONTINUE
C
...
C      LQTLX = LOCOWS(12)+(IG-1)*NDD*NTIM24+NS2+NTLAG-1
      LQTLX = LOCOWS(12)+(IG-1)*NTIMESTEPS+NS2+NTLAG-1
      W(LQTLX) = QTLX
C      LORC = LOCSTQ+(IG-1)*3+2
C      LOCWK1 = LOCOUT + NDD*NTIM24
      LOCWK1 = LOCOUT + NTIMESTEPS
C      LOCWK2 = LOCWK1 + NDD*NTIM24
      LOCWK2 = LOCWK1 + NTIMESTEPS
C
      STGX=0.0
C      IF (W(LORC).EQ.BLANK.AND.W(LORC+1).EQ.BLANK) GO TO 1840
     ...
     .           IDAT,IHRT,IX,IER,0)
      FRLOOP=FLSAVE
C      LSTGX = LOCOWS(12) + (IG+5)*NDD*NTIM24+NS2+NTLAG-1
      LSTGX = LOCOWS(12) + (IG+5)*NTIMESTEPS+NS2+NTLAG-1
      W(LSTGX) = STGX
      GO TO 1850
 1840 CONTINUE
      IF(IOLDC.EQ.1) THEN
      ...
      W(LOCRCO+3) = 0.0
!CP   1850 IF (IBUG.GE.2) WRITE(IODBUG,1670) IG,QOUTX(1),QADDX,STGX
 1850 IF ( FEWSDEBUG.GE.3 ) THEN
         WRITE(MESSAGESTRING, 1670) IG,QOUTX(1),QADDX,STGX
	 call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
...
C
      IOFMSG = LOCOWS(6) - 1
C      IOFDEV = LOCOWS(6) + NDD*NTIM24 - 1
      IOFDEV = LOCOWS(6) + NTIMESTEPS - 1
...
!CP comment out reference to MAINUM;
!   ALWAYS DO; DON'T CARE IF NOT MAINUM.EQ.1 (FORECAST MODE)
!CP   IF (ADJUST .AND. .NOT.ADJRUN .and. mainum .ne. 2) GO TO 9000
      IF (ADJUST .AND. .NOT.ADJRUN) GO TO 9000
C
...
C
!CP   IF (IBUG.GE.2) WRITE(IODBUG,1690) NS2,(W(LOCWS(4)+L-1),L=1,NUMCOV)
      IF ( FEWSDEBUG.GE.3 ) THEN
         WRITE(MESSAGESTRING, 1690) NS2 
         call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
 1690    FORMAT('  >>> SAVING CARRYOVER FOR PERIOD NO. ',I3) 
     ...
     >        FORMATSTR,LOCWS(4),LOCWS(4)+NUMCOV-1,1,W)
      END IF
C      CALL FCWTCO(ICDAY(IPOS),ICHOUR(IPOS),W(LOCWS(4)),NUMCOV)
      LASTCO = LASTCO + 1
C
 9000 CONTINUE
!CP   IF (IBUG.GE.1) WRITE(IODBUG,1699)
!CP   1699 FORMAT('    *** EXIT XCOU26 ***')
      IF ( FEWSDEBUG.GE.4 ) THEN
         WRITE(MESSAGESTRING, '(A23)') '    *** EXIT XCOU26 ***'
	 call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
      END IF
      RETURN
      END
