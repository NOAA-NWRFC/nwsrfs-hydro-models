...
!CP   600 FORMAT('  *** ENTER X17N26 ***')
      IF ( FEWSDEBUG.GE.5 ) THEN
         WRITE(MESSAGESTRING, '(A23)') '  *** ENTER X17N26 ***'
...
C
   10 CONTINUE
C
!CP   IF (IBUG .GE. 2) WRITE(IODBUG,650) ITYPE,DIFRA1,DIFRA2,PDDIFF,
!CP   IF (IBUG .GE. 2) WRITE(IODBUG,655) (W(LOCW+K-1),K=NST,NEND)
      IF ( FEWSDEBUG.GE.5 ) THEN
         WRITE(MESSAGESTRING, 650) ITYPE,DIFRA1,DIFRA2,PDDIFF,
  ...
  650 FORMAT('  ITYPE    DIFRA1    DIFRA2  PD  DIFF  IPD  NMISS  NFILL',
	 ...
	 IF (NPRINT.GT.100) NPRINT=100
         WRITE(MESSAGESTRING, 655) 
         call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
  655    FORMAT('  BEFORE INTERPOLATION ') 
         WRITE(FORMATSTR, *) '%12.3f'
         call logonedimensionarrayfromfortran(DEBUG_LEVEL,NULLLINE,1,6,
     >        FORMATSTR,LOCW+NST-1,LOCW+NPRINT-1,1,W)
...
C
  100 CONTINUE
C
!CP   IF (IBUG .GE. 2) WRITE(IODBUG,690) (W(LOCW+K-1),K=NST,NEND)
      IF ( FEWSDEBUG.GE.5 ) THEN
         NPRINT=NEND
         IF ( NPRINT.GT.100 ) NPRINT=100
         WRITE(MESSAGESTRING, 690) 
         call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
  690    FORMAT('  AFTER INTERPOLATION ') 
         WRITE(FORMATSTR, *) '%12.3f'
         call logonedimensionarrayfromfortran(DEBUG_LEVEL,NULLLINE,1,6,
     >        FORMATSTR,LOCW+NST-1,LOCW+NPRINT-1,1,W)
...
!CP   699 FORMAT('    *** EXIT X17N26 ***')
      IF ( FEWSDEBUG.GE.5 ) THEN
         WRITE(MESSAGESTRING, '(A23)') '    *** EXIT X17N26 ***'
	 call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
      END IF
      RETURN
      END
