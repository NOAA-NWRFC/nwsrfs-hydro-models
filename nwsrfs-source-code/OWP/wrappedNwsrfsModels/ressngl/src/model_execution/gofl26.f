...
!CP   WRITE(IODBUG,40)  IFCST,FCST,NS2,GOFLSH,NUMLG,NUMSM,NLGDN,NSMDN,
      WRITE(MESSAGESTRING, 40)  IFCST,FCST,NS2,GOFLSH,NUMLG,NUMSM,NLGDN,
     $NSMDN,NGATE,NFUTUR,BGNGAT,       GATMAX,QGENMX,SLUICE,       S1,
...
C WRITE TRACE AND DEBUG IF REQUIRED.
C
!CP   190 IF(IBUG-1)260,240,200
!CP   200 WRITE(IODBUG,40)IFCST,FCST,NS2,GOFLSH,NUMLG,NUMSM,NLGDN,NSMDN,
  190 IF ( FEWSDEBUG.LT.4 ) GO TO 260
      WRITE(MESSAGESTRNG, 40)  IFCST,FCST,NS2,GOFLSH,NUMLG,NUMSM,NLGDN,
     $NSMDN,NGATE,NFUTUR,BGNGAT,       GATMAX,QGENMX,SLUICE,       S1,
     $ELEV1,ELEV2,NUM,NTERP,IBUG
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
   
!CP   WRITE(IODBUG,210)(SIGELV(I),I=1,NSIGEL)
      WRITE(MESSAGESTRNG, 210) 
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
  210 FORMAT(1H0,27H SIGNIFICANT ELEVATIONS ARE) 

      WRITE(FORMATSTR, *) '%12.3f'
      call logonedimensionarrayfromfortran(DEBUG_LEVEL,NULLLINE,1,10,
     >     FORMATSTR,1,NSIGEL,1,SIGELV)
      

!CP   WRITE(IODBUG,220)IFLAG,NSE,(STOR(I),ELEV(I),I=1,NSE)
      WRITE(MESSAGESTRNG, 220)IFLAG,NSE !,(STOR(I),ELEV(I),I=1,NSE)
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
  220 FORMAT(1H0,9H IFLAG IS,I6,43H.  NO OF POINTS ON ELEV. - STORAGE CU
     ...
     >     FORMATSTR,1,NSE,1,STOR,ELEV)
      ...
      JEND=NS2+NFUTUR-1
!CP   WRITE(IODBUG,230)(QIMHYD(I),I=NS2,JEND)
      WRITE(MESSAGESTRNG, 230) 
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
  230 FORMAT(1H0,111HFUTURE MEAN TIME INTERVAL INFLOWS INCLUDING
     $PRESENT TIME INTERVAL FOR DECISION ON OPENING FLOOD GATE ARE) 

      WRITE(FORMATSTR, *) '%12.3f'
      call logonedimensionarrayfromfortran(DEBUG_LEVEL,NULLLINE,1,10,
     >     FORMATSTR,NS2,JEND,1,QIMHYD)
      

!CP   240 WRITE(IODBUG,250)
 240  WRITE(MESSAGESTRNG, 250)
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
