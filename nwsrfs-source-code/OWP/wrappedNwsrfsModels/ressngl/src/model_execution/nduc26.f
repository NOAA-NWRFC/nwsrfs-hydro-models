...
C
    1 CALL SURC26(ELVSUR,QISUR,STOR,ELEV)
      IF(SURCHG)  GO TO 3
!CP   IF(IBUG.NE.2) GO TO 600
      IF ( FEWSDEBUG.LT.3 ) GO TO 600
!CP   WRITE(IODBUG,2)
      WRITE(MESSAGESTRING, 2)
...
C
!CP   3 IF(IBUG-1)50,10,20
!CP   10 WRITE(IODBUG,30)
!CP      GO TO 50
!CP   20 WRITE(IODBUG,30)
    3 IF ( FEWSDEBUG.LT.4 ) GO TO 50
      WRITE(MESSAGESTRING, 30)
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
   30 FORMAT(1H0,10X,17H** NDUC26 ENTERED)

!CP   WRITE(IODBUG,40) IFCST,FCST,SURCHG,FRACT,QIBAC1,QI1,QI2,
      WRITE(MESSAGESTRING, 40) IFCST,FCST,SURCHG,FRACT,QIBAC1,QI1,QI2,
...
c
C 09/11/08 To fix the math different in ob83 and ob90. The issue is when 
C HUPPEX=-999.0, it does not cancel out (-999.0 + 999.0).
C Kwang said that if less than -900 that means missing value so we don't need
C to do "HUPPEX+999.0"
      HUPPEX=HUPPER
C     IF(HUPPEX.LT.-900.0) HUPPEX=RULELX+HUPPEX+999.0
      IF(HUPPEX.LT.-900.0) HUPPEX=RULELX
      HCHECX=HCHECK
C     IF(HCHECX.LT.-900.0) HCHECX=RULELX+HCHECX+999.0
      IF(HCHECX.LT.-900.0) HCHECX=RULELX
      HLOWEX=HLOWER
C     IF(HLOWEX.LT.-900.0) HLOWEX=RULELX+HLOWEX+999.0
      IF(HLOWEX.LT.-900.0) HLOWEX=RULELX
C
...
C
!CP   IF(IBUG.NE.2) GO TO 115
      IF ( FEWSDEBUG.LT.3 ) GO TO 115
!CP   WRITE(IODBUG,110) QQO1,PREVQ2,QQO2,SS1,SS2,ELRES1,ELRES2
      WRITE(MESSAGESTRING, 110) QQO1,PREVQ2,QQO2,SS1,SS2,ELRES1,ELRES2
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
  110 FORMAT(1H0,'QQO1,PREVQ2,QQO2,SS1,SS2,ELRES1,ELRES2',
     & /1X,5F13.0,2F12.3)

!CP   WRITE(IODBUG,111) J90,I,I10,ISFRPD,IFRACT,FRACT,FRPD,SFRPD,QIBGN,
      WRITE(MESSAGESTRING, 111)J90,I,I10,ISFRPD,IFRACT,FRACT,FRPD,SFRPD,
     $QIBGN,QIEND,QIMSUR,QI1,QI2,QQI1,QQI2,S1,SS1,SS2
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING) 
...
C
  ...
  122 QQO2=QQO22
      SS2=SS1+QQIM-(QQO1+QQO22)*0.5*FRPD
      CALL NTER26(SS2,ELRES2,STOR,ELEV,NSE,IFLAG,NTERP,IBUG)
!CP   IF(IBUG.NE.2) GO TO 130
      IF ( FEWSDEBUG.LT.3 ) GO TO 130
!CP   WRITE(IODBUG,123) QQO2,PREVQ2,JJ
      WRITE(MESSAGESTRING, 123) QQO2,PREVQ2,JJ
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING) 
...
C WRITE TRACE AND DEBUG IF REQUIRED.
C
!CP   IF(IBUG-1)600,580,380
!CP   380 WRITE(IODBUG,390)
      IF ( FEWSDEBUG.LT.4 ) GO TO 600
      WRITE(MESSAGESTRING, 390)
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
...
!CP   WRITE(IODBUG,430) (ELVFAL(I),I=1,NUMVAL)
      WRITE(MESSAGESTRING, 430) 
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
  430 FORMAT(1H0,72H ELEVATION VALUES USED IN DECISION TABLE TO DEFINE E
     $VACUATION OPTION ARE) 

      WRITE(FORMATSTR, *) '%12.3f'
      call logonedimensionarrayfromfortran(DEBUG_LEVEL,NULLLINE,1,10,
     >     FORMATSTR,1,NUMVAL,1,ELVFAL)
      

  440 IF(NCON.EQ.1.OR.NCON.EQ.3) GO TO 460
!CP   WRITE(IODBUG,450) (PEAKQ(I),I=1,NUMVAL)
      WRITE(MESSAGESTRING, 450)
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
  450 FORMAT(1H0,75H PEAK OUTFLOW VALUES USED IN DECISION TABLE TO DEFIN
     $E EVACUATION OPTION ARE) 

      WRITE(FORMATSTR, *) '%12.3f'
      call logonedimensionarrayfromfortran(DEBUG_LEVEL,NULLLINE,1,10,
     >     FORMATSTR,1,NUMVAL,1,PEAKQ)
      

  460 IF(NCON.NE.3.OR.NCON.NE.5) GO TO 480
!CP   WRITE(IODBUG,470)(PEAKEL(I),I=1,NUMVAL)
      WRITE(MESSAGESTRING, 470) 
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
  470 FORMAT(1H0,77H PEAK ELEVATION VALUES USED IN DECISION TABLE TO DEF
     $INE EVACUATION OPTION ARE) 

      WRITE(FORMATSTR, *) '%12.3f'
      call logonedimensionarrayfromfortran(DEBUG_LEVEL,NULLLINE,1,10,
     >     FORMATSTR,1,NUMVAL,1,PEAKEL)
      

!CP   480 WRITE(IODBUG,490)(OPTEV(I),I=1,NUMVAL)
  480 WRITE(MESSAGESTRING, 490) 
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
  490 FORMAT(1H0,37H OPTION VALUES FOR DECISION TABLE ARE) 

      WRITE(FORMATSTR, *) '%12.3f'
      call logonedimensionarrayfromfortran(DEBUG_LEVEL,NULLLINE,1,10,
     >    FORMATSTR,1,NUMVAL,1,OPTEV)
      

      IF(ISURG.NE.1)  GO TO 580
      IF(NGAT.LE.1 .OR. NELEV.LE.1) GO TO 545
      DO 530 J=1,NGAT
!CP   530 WRITE(IODBUG,540)GATE(J),(EVACEL(K),EVACQ(J,K),K=1,NELEV)
  530 WRITE(MESSAGESTRING, 540)GATE(J),(EVACEL(K),EVACQ(J,K),K=1,NELEV)
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
...
!CP   550 WRITE(IODBUG,560)SCQI(J),(SCEL(K),SCQO(K,J),K=1,NSCEL)
  550 WRITE(MESSAGESTRING, 560)SCQI(J),(SCEL(K),SCQO(K,J),K=1,NSCEL)
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
...
!CP   WRITE(IODBUG,570)(ELEV(I),STOR(I),I=1,NSE)
      WRITE(MESSAGESTRING, 570) !(ELEV(I),STOR(I),I=1,NSE)
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
  570 FORMAT(1H0,' ALTERNATING VALUES OF ELEVATION AND STORAGE FOR THE',
     ...
     >     FORMATSTR,1,NSE,1,ELEV,STOR)
      

!CP  580 WRITE(IODBUG,590)
  580 WRITE(MESSAGESTRING, 590)
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING) 
