...
C
!CP   IF (ITRACE .GE. 1) WRITE(IODBUG,600)
!CP   600 FORMAT('   *** ENTER X17M26 ***')
      IF ( ITRACE.GE.1 ) THEN
         WRITE(MESSAGESTRING, '(A23)') '   *** ENTER X17M26 ***'
...
C
  900 CONTINUE
      IF ( ITRACE.GE.1 ) THEN
         WRITE(MESSAGESTRING, '(A23)') '    *** EXIT X17M26 ***'
	 call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
      END IF
!CP   IF (ITRACE .GE. 1) WRITE(IODBUG,699)
!CP   699 FORMAT('    *** EXIT X17M26 ***')
      RETURN
      END
