...
!CP   WRITE(IODBUG,50) IFCST,FCST,NS2,IUNC,CREST,STOCR,SFILL,SPILMX,
      WRITE(MESSAGESTRING, 50) IFCST,FCST,NS2,IUNC,CREST,STOCR,SFILL,
     $SPILMX,QI1,QI2,QIM,QO1,QO2,QOM,QOK1,QOK2,S1,S2,ELEV1,ELEV2,
...
!CP   320 IF(IBUG-1)410,390,330
!CP   330 WRITE(IODBUG,50) IFCST,FCST,NS2,IUNC,CREST,STOCR,SFILL,SPILMX,

  320 IF ( FEWSDEBUG.LT.4 ) GO TO 410
      WRITE(MESSAGESTRING, 50) IFCST,FCST,NS2,IUNC,CREST,STOCR,SFILL,
     $SPILMX,QI1,QI2,QIM,QO1,       QO2,       QOM,QOK1,QOK2,S1,      S2
     $,ELEV1,ELEV2,TESTPK,NTOTPK,NTERPQ,QGENMX,SLUICE,NQG,PCTERG,NTERP,
     $IBUG,NTRIAL
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
...
!CP   WRITE(IODBUG,370) TESTPK,NUMPKO,(PEAKO(I),PKPOS(I),I=1,NUMPKO)
      WRITE(MESSAGESTRING, 370) TESTPK,NUMPKO !,
!     $                          (PEAKO(I),PKPOS(I),I=1,NUMPKO)
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
  370 FORMAT(1H0,70H NO. OF PEAK DISCHARGES BETWEEN REGULAR TIME INTERVA
     ...
     $EAK DISCHARGE AND POSITION IN INSTANTANEOUS OUTFLOW ARRAY ARE)
C     $/(1X,6(F12.3,F4.1,4X)))

      WRITE(FORMATSTR, *) '%12.3f'
      call logonedimensionarrayfromfortran(DEBUG_LEVEL,NULLLINE,2,6,
     >     FORMATSTR,1,NUMPKO,1,PEAKO,PKPOS)
      

      IF(NQG.EQ.0) GO TO 390
!CP   WRITE(IODBUG,380) (TOTALQ(I),QG(I),I=1,NQG)
      WRITE(MESSAGESTRING, 380) !(TOTALQ(I),QG(I),I=1,NQG)
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
 380  FORMAT(1H0,75H ALTERNATING VALUES OF TOTAL DISCHARGE AND MAXIMUM G
     $ENERATION DISCHARGE ARE) !/(1X,10F12.3))

      WRITE(FORMATSTR, *) '%12.3'
      call logonedimensionarrayfromfortran(DEBUG_LEVEL,NULLLINE,2,5,
     >     FORMATSTR,1,NQG,1,TOTALQ,QG)
      
   
!CP   390 WRITE(IODBUG,400)
 390  WRITE(MESSAGESTRING, 400)
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
