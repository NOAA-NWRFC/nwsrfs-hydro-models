...
C
      SUBROUTINE ERUL26(RULEL,DATE,RULELV,NHRYR)
C
...
!CP   INCLUDE 'common/fdbug'
      ...
      INCLUDE 'flogm'
      DIMENSION RULEL(1),DATE(1),RULELV(1),NHRYR(1)
C
...
C
!CP   IF(IBUG.GE.1) WRITE(IODBUG,30)
  ...
  150 A=RULELV(J)-RULELV(J-1)
      ...
      DO 290 IS2=1,NUM
      JHR = JHR+MINODX
      !BUG1199 11/28/13 NHRYR() DEFINE IN COMMON BLOCKS RESV26
      !IADD = IS2/NTIM24
      !IDSET = IDARUN + IADD
      !NDAYR = 365 + LEAP26(IDSET,IHR)
      !JHRYR=NDAYR*24
      !IF(JHR.GT.JHRYR) JHR=MOD(JHR,JHRYR)
      IF(JHR.GT.NHRYR(IS2)) JHR=MOD(JHR,NHRYR(IS2))
      !END BUG1199 11/28/13
      DO 220 I=2,NRUL
      J = I
      JHR2=KHRRUX+(DATE(J)-1)*24
  ...
  290 CONTINUE
!CP   IF(IBUG.LT.2) GO TO 600
      IF ( FEWSDEBUG.LT.3 ) GO TO 600
      RULEL1=BGNRCE
      IF(NS2.GE.2) RULEL1=RULEL(NS2-1)
      RULEL2=RULEL(NS2)
!CP   WRITE(IODBUG,510)NUM,JULDAT,NDAYR,KHRBGN,KHRRUL,RULEL1,RULEL2
      WRITE(MESSAGESTRING, 510)NUM,JULDAT,NDAYR,KHRBGN,
     $                         KHRRUL,RULEL1,RULEL2
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
  510 FORMAT(1H0,' NUM,JULDAT,NDAYR,KHRBGN,KHRRUL,RULEL1,RULEL2',
     & /1X,5I6,2F8.2)

!CP   WRITE(IODBUG,520) (DATE(I),I=1,NRUL)
  ...
  520 FORMAT(1H0,' DATES AND RULE CURVE ELEVATION:') 
     ...
     >     FORMATSTR,1,NRUL,1,DATE)
      
   
!CP   WRITE(IODBUG,525) (RULELV(I),I=1,NRUL)
      WRITE(FORMATSTR, *) '%8.2f'
      call logonedimensionarrayfromfortran(DEBUG_LEVEL,NULLLINE,1,10,
     >      FORMATSTR,1,NRUL,1,RULELV)
      

!CP   WRITE(IODBUG,530) (RULEL(I),I=1,NUM)
...
CC110 FORMAT(1H0,33H ADJUSTED RULE CURVE ELEVATION IS,F12.3, 26H   UNADJ
...
CC220 FORMAT(1H0,82H ALTERNATING VALUES OF DATES AND RULE CURVE ELEVATIO
...
CC 260  RETURN
!CP   IF(IBUG.GE.1) WRITE(IODBUG,550)
      IF ( FEWSDEBUG.GE.4 ) THEN
