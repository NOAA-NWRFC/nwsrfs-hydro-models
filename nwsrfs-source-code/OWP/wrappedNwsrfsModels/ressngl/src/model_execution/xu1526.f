...
!CP   1600 FORMAT('   *** ENTER XU1526 ***')
      IF ( FEWSDEBUG.GE.4 ) THEN
         WRITE(MESSAGESTRING, '(A23)') '   *** ENTER XU1526 ***'
...
C  ARRAY.
C
C      LOCOBS = 3*NDD*NTIM24 + LOBSTO
      LOCOBS = 3*NTIMESTEPS + LOBSTO
      BGNSTL = W(LOCOBS)
C
   10 CONTINUE
C
...
!CP   1699 FORMAT('    *** EXIT XU1526 ***')
      IF ( FEWSDEBUG.GE.4 ) THEN
         WRITE(MESSAGESTRING, '(A23)') '    *** EXIT XU1526 ***'
	 call logfromfortran(DEBUG_LEVEL, MESSAGESTRING) 
      END IF
C
      RETURN
