...
!CP   WRITE(IODBUG,40) IFCST,FCST,NRUN,NUM,NDAYRN,JULDAY,KHRBGN,
      WRITE(MESSAGESTRING, 40) IFCST,FCST,NRUN,NUM,NDAYRN,JULDAY,KHRBGN,
...
C
!CP   IF(IBUG.NE.2) GO TO 480
      IF ( FEWSDEBUG.LT.3 ) GO TO 480
!CP   WRITE(IODBUG,470) FLOUTB,FLOUT
      WRITE(MESSAGESTRING, 470) FLOUTB,FLOUT
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
...
C WRITE DEBUG IF REQUIRED.
C
!CP   IF(IBUG.NE.2) GO TO 630
!CP   WRITE(IODBUG,610) JTIMRL

      IF ( FEWSDEBUG.LT.3 ) GO TO 630
      WRITE(MESSAGESTRING, 610) JTIMRL
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
  610 FORMAT(1H0,38H POOL IS BROUGHT BACK TO RULE CURVE AT,I6, 9H EACH D
     $AY)
  
!CP   WRITE(IODBUG,620) JBGN,KSTART,KEND,KEND24,NUM,KTIMBG,KTIME,JULIAN,
      WRITE(MESSAGESTRING, 620) JBGN,KSTART,KEND,KEND24,NUM,KTIMBG,
     $KTIME,JULIAN,QOMIN,STOR1,SUMOUT,VOLUME,KSTART,KEND 
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
  620 FORMAT(1H0,' JBGN,KSTART,KEND,KEND24,NUM,KTIMBG,KTIME,JULIAN,',
     $ 'QOMIN,STOR1,SUMOUT,VOLUME'/1X,8I6,4F10.0,
     & /1X,' (QOMHYD(K),K=',I5,1H,I5,1H)) 
      NPRINT=KEND
      IF ( NPRINT.GT.100 ) NPRINT=100
      
      WRITE(FORMATSTR, *) '%10.0f'
      call logonedimensionarrayfromfortran(DEBUG_LEVEL,NULLLINE,1,8,
     >     FORMATSTR,KSTART,NPRINT,1,QOMHYD)
...
C WRITE DEBUG IF REQUIRED.
C
!CP   IF(IBUG.NE.2) GO TO 810
!CP   WRITE(IODBUG,790) JTIMRL,JDARUL

      IF ( FEWSDEBUG.LT.3 ) GO TO 810
      WRITE(MESSAGESTRING, 790) JTIMRL,JDARUL
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
  790 FORMAT(1H0,43H POOL IS BROUGHT BACK TO RULE CURVE AT TIME,I6,14H O
     $N DAY NUMBER,I6)
  
!CP   WRITE(IODBUG,800) JDAWEK,KDAWEK,LDAWEK,JBGN,JSTART,JEND,JENDWK,
      WRITE(MESSAGESTRING, 800) JDAWEK,KDAWEK,LDAWEK,JBGN,JSTART,JEND
     $JENDWK,KSTART,KEND,KEND24,NUM,KTIMBG,KTIME,FLOUT,QMNWEK,QMNDAY,
     $QOMIN,STOR1,VOLSAV,VOLUME,KSTART,KEND 
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
...
C WRITE TRACE AND DEBUG IF REQUIRED.
C
!CP   920 IF(IBUG-1)1160,1140,930
!CP   930 WRITE(IODBUG,940)
  920 IF ( FEWSDEBUG.LT.4 ) GO TO 1160
      WRITE(MESSAGESTRING, 940)
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
...
!CP   WRITE(IODBUG,970) (QOMHYD(I),I=1,NS2)
...
!CP   WRITE(IODBUG,951) (STOHYD(I),I=1,NS2)
      WRITE(FORMATSTR, *) '%10.0f'
      call logonedimensionarrayfromfortran(DEBUG_LEVEL,NULLLINE,1,8,
     >     FORMATSTR,1,NPRINT,1,STOHYD)

      IF(IPOWER.NE.1) GO TO 1140
      NPRINT=NUM
      IF ( NPRINT.GT.100 ) NPRINT=100
      
!CP   WRITE(IODBUG,950) (QIMHYD(I),I=1,NUM)
      WRITE(MESSAGESTRING, 950) 
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
...
!CP   WRITE(IODBUG,951) (SQIM(I),I=1,NUM)
      WRITE(FORMATSTR, *) '%10.0f'
      call logonedimensionarrayfromfortran(DEBUG_LEVEL,NULLLINE,1,8,
     >     FORMATSTR,1,NPRINT,1,SQIM)

!CP   IF(IFCST.EQ.1) WRITE(IODBUG,960)(QOMOBS(I),I=1,NRUN)
      IF(IFCST.EQ.1) THEN
         NPRINT=NRUN
         IF ( NPRINT.GT.100 ) NPRINT=100
 
         WRITE(MESSAGESTRING, 960) 
         call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
  960    FORMAT(1H0,37H  VALUES OF MEAN OBSERVED OUTFLOW ARE)
         WRITE(FORMATSTR, *) '%10.0f'
         call logonedimensionarrayfromfortran(DEBUG_LEVEL,NULLLINE,1,8,
     >        FORMATSTR,1,NPRINT,1,QOMOBS)
      END IF
!CP   WRITE(IODBUG,980) (RULEL(I),I=1,NUM)
  ...
  980 FORMAT(1H0,' RULE CURVE ELEVATIONS (RULEL) AND STORAGES',
     & ' (RULSTO):')
      ...
      call logonedimensionarrayfromfortran(DEBUG_LEVEL,NULLLINE,1,8,
     >     FORMATSTR,1,NPRINT,1,RULEL) 

!CP   WRITE(IODBUG,951) (RULSTO(I),I=1,NUM)
      WRITE(FORMATSTR, *) '%10.0f'
      call logonedimensionarrayfromfortran(DEBUG_LEVEL,NULLLINE,1,8,
     >     FORMATSTR,1,NPRINT,1,RULSTO) 

      IF(DATEUP(1).NE.-999.0) THEN
!CP     WRITE(IODBUG,990) (DATEUP(I),I=1,NELVUP)
        WRITE(MESSAGESTRING, 990) 
        call logfromfortran(DEBUG_LEVEL, MESSAGESTRING) 
  990 FORMAT(1H0,' DATE (DATEUP) AND UPPER LIMITING ELEVATION',
     & ' (ELEVUP):') 
        NPRINT=NELVUP
        IF ( NPRINT.GT.100 ) NPRINT=100
        WRITE(FORMATSTR, *) '%8.0f'
        call logonedimensionarrayfromfortran(DEBUG_LEVEL,NULLLINE,1,8,
     >       FORMATSTR,1,NPRINT,1,DATEUP) 

!CP     WRITE(IODBUG,1001) (ELEVUP(I),I=1,NELVUP)
        WRITE(FORMATSTR, *) '%8.3f'
        call logonedimensionarrayfromfortran(DEBUG_LEVEL,NULLLINE,1,8,
     >       FORMATSTR,1,NPRINT,1)
      ENDIF
      IF(ELVUPR.LT.-500.0 .OR. DATEUP(1).NE.-999.0) THEN
!CP     & WRITE(IODBUG,1020) (STOMAX(I),I=1,NUM)
        NPRINT=NUM
        IF ( NPRINT.GT.100 ) NPRINT=100

        WRITE(MESSAGESTRING, 1020) 
        call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
 1020   FORMAT(1H0,' PERIOD UPPER STORAGE LIMIT (STOMAX):') 
        WRITE(FORMATSTR, *) '%10.0f'
        call logonedimensionarrayfromfortran(DEBUG_LEVEL,NULLLINE,1,8,
     >       FORMATSTR,1,NPRINT,1,STOMAX)
      END IF
      IF(DATELR(1).NE.-999.0) THEN
        NPRINT=NELVLR
        IF ( NPRINT.GT.100 ) NPRINT=100
!CP     WRITE(IODBUG,1000) (DATELR(I),I=1,NELVLR)
 ...
 1000   FORMAT(1H0,' DATE (DATELR) AND LOWER LIMITING ELEVATION',
     & ' (ELEVLR):') 
        WRITE(FORMATSTR, *) '%8.0f'
        call logonedimensionarrayfromfortran(DEBUG_LEVEL,NULLLINE,1,8,
     >       FORMATSTR,1,NPRINT,1,DATELR)

!CP     WRITE(IODBUG,1001) (ELEVLR(I),I=1,NELVLR)
        WRITE(FORMATSTR, *) '%8.3f'
        call logonedimensionarrayfromfortran(DEBUG_LEVEL,NULLLINE,1,8,
     >       FORMATSTR,1,NPRINT,1,ELEVLR)
      ENDIF
      IF(ELVLWR.LT.-500.0 .OR. DATELR(1).NE.-999.0) THEN
        NPRINT=NUM
        IF ( NPRINT.GT.100 ) NPRINT=100

!CP     & WRITE(IODBUG,1050) (STOMIN(I),I=1,NUM)
        WRITE(MESSAGESTRING, 1050) 
        call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
 1050   FORMAT(1H0,' PERIOD LOWER STORAGE LIMIT (STOMIN):')
        WRITE(FORMATSTR, *) '%10.0f'
        call logonedimensionarrayfromfortran(DEBUG_LEVEL,NULLLINE,1,8,
     >       FORMATSTR,1,NPRINT,1,STOMIN)
...
!CP     WRITE(IODBUG,1070) (ELVGEN(I),I=1,NELVGN)
        WRITE(MESSAGESTRING, 1070) 
        call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
 1070   FORMAT(1H0,' ELEVATION (ELEVGN) AND MAXIMUM GENERATION ',
     $ ' DISCHARGE (GENMXQ):') 
        WRITE(FORMATSTR, *) '%10.3f'
        call logonedimensionarrayfromfortran(DEBUG_LEVEL,NULLLINE,1,8,
     >       FORMATSTR,1,NPRINT,1,ELVGEN) 

!CP     WRITE(IODBUG,951) (GENMXQ(I),I=1,NELVGN) 
        WRITE(FORMATSTR, *) '%10.0f'
        call logonedimensionarrayfromfortran(DEBUG_LEVEL,NULLLINE,1,8,
     >       FORMATSTR,1,NPRINT,1,GENMXQ)
      ENDIF
...
!CP      WRITE(IODBUG,1080) (DATMIN(I),I=1,NDATMN)
         WRITE(MESSAGESTRING, 1080) 
         call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
 1080  FORMAT(1H0,' DATE (DATMIN) AND REQUIRED MINIMUM OUTFLOW (QMIN):')
         WRITE(FORMATSTR, *) '%10.0f'
         call logonedimensionarrayfromfortran(DEBUG_LEVEL,NULLLINE,1,8,
     >        FORMATSTR,1,NPRINT,1,DATMIN)

!CP      WRITE(IODBUG,951) (QMIN(I),I=1,NDATMN)
         WRITE(FORMATSTR, *) '%10.0f'
         call logonedimensionarrayfromfortran(DEBUG_LEVEL,NULLLINE,1,8,
     >        FORMATSTR,1,NPRINT,1,QMIN)
      ENDIF
     
!CP   WRITE(IODBUG,1090) MIDNIT,(FRACMD(I),I=1,NTIM24)
      WRITE(MESSAGESTRING, 1090) MIDNIT 
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
      WRITE(FORMATSTR, *) '%6.3f'
      call logonedimensionarrayfromfortran(DEBUG_LEVEL,NULLLINE,1,8,
     >     FORMATSTR,1,NTIM24,1,FRACMD)

!CP   WRITE(IODBUG,1090) JTIMRL,(FRACRL(I),I=1,NTIM24)
      WRITE(MESSAGESTRING, 1090) JTIMRL
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
 1090 FORMAT(1H0,'PERIOD FRACTIONS OF 24-HR VOLUME WITH',
     & ' LAST PERIOD ENDING AT ',I2,' HOUR:') 
      WRITE(FORMATSTR, *) '%6.3f'
      call logonedimensionarrayfromfortran(DEBUG_LEVEL,NULLLINE,1,8,
     >     FORMATSTR,1,NTIM24,1,FRACRL)

      IF(JDARUL.EQ.-999) GO TO 1140
!CP   WRITE(IODBUG,1100)(I,FRACDA(I),I=1,7)
      WRITE(MESSAGESTRING, 1100)(I,FRACDA(I),I=1,7)
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
 1100 FORMAT(1H0,' DAILY FRACTION OF WEEKLY VOLUME FOR MIDNIGHT TO',
     & ' MIDNIGHT:'/(1X,7(I2,F6.3,2X)))
 
!CP   WRITE(IODBUG,1110) JTIMRL,(I,FRAC24(I),I=1,7)
      WRITE(MESSAGESTRING, 1110) JTIMRL,(I,FRAC24(I),I=1,7)
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
 1110 FORMAT(1H0,'DAILY FRACTION OF WEEKLY VOLUME WHEN POOL IS BROUGHT',
     $ ' BACK TO RULE CURVE AT ',I2,' HOUR:'/(1X,7(I2,F6.3,2X)))
 
!CP   1140 WRITE(IODBUG,1150)
 1140 WRITE(MESSAGESTRING, 1150)
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
