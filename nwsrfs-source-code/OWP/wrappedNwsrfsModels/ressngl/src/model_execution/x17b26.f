...
!CP   600 FORMAT('   *** ENTER X17B26 ***')
      IF ( FEWSDEBUG.GE.4 ) THEN
         WRITE(MESSAGESTRING, '(A23)') '   *** ENTER X17B26 ***'
...
C
 10   M=NSTEPS-NBLND
C
!CP   IF (IBUG .GE. 2) WRITE(IODBUG,610) IB,LAST,FWDBCK(IFB+1),DQ
      IF ( FEWSDEBUG.GE.3 ) THEN
         WRITE(MESSAGESTRING, 610) IB,LAST,FWDBCK(IFB+1),DQ
	 call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
...
C
!CP   IF (IBUG .GE. 2) WRITE(IODBUG,620) Q(I)
      IF ( FEWSDEBUG.GE.3 ) THEN
         WRITE(MESSAGESTRING, 620) Q(I)
         call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
...
C
!CP   IF (IBUG .GE. 2) WRITE(IODBUG,630) Q(I)
      IF ( FEWSDEBUG.GE.3 ) THEN
         WRITE(MESSAGESTRING, 630) Q(I)
         call logfromfortran(DEBUG_LEVEL, MESSAGESTRING) 
...
!CP   699 FORMAT('    *** EXIT X17B26 ***')
      IF ( FEWSDEBUG.GE.4 ) THEN
         WRITE(MESSAGESTRING, '(A23)') '    *** EXIT X17B26 ***'
         call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
      END IF
C
      RETURN
