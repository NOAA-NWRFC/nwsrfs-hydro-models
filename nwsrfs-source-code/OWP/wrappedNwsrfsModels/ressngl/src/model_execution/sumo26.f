...
!CP   WRITE(IODBUG,40) JSTART,JEND,SUMOUT,STOR1,FLOUT,
      WRITE(MESSAGESTRING, 40) JSTART,JEND,SUMOUT,STOR1,FLOUT,
...
!CP   90 WRITE(IODBUG,40) JSTART,JEND,SUMOUT,STOR1,FLOUT,
      WRITE(MESSAGESTRING, 40) JSTART,JEND,SUMOUT,STOR1,FLOUT,
     $QMNDAY,QMNWEK,ADJSTO,BGNSUM
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
   
!CP   WRITE(IODBUG,100) (RULSTO(I),I=JSTART,JEND)
      WRITE(MESSAGESTRING, 100) 
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
  100 FORMAT(1H0,' PERIOD RULECURVE STORAGES:') 
      NPRINT=JEND
      IF ( NPRINT.GT.100 ) NPRINT=100
      
      WRITE(FORMATSTR, *) '%10.0f'
      call logonedimensionarrayfromfortran(DEBUG_LEVEL,NULLLINE,1,8,
     >     FORMATSTR,JSTART,NPRINT,1,RULSTO)

!CP   WRITE(IODBUG,110) (QIMHYD(I),I=JSTART,JEND)
      WRITE(MESSAGESTRING, 110) 
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
  110 FORMAT(1H0,' PERIOD MEAN AND SUMMATION OF MEAN INFLOWS:')
      WRITE(FORMATSTR, *) '%10.0f'
      call logonedimensionarrayfromfortran(DEBUG_LEVEL,NULLLINE,1,8,
     >     FORMATSTR,JSTART,NPRINT,1,QIMHYD) 
   
!CP   WRITE(IODBUG,111) (SQIM(I),I=JSTART,JEND)
      WRITE(FORMATSTR, *) '%10.0f'
      call logonedimensionarrayfromfortran(DEBUG_LEVEL,NULLLINE,1,8,
     >     FORMATSTR,JSTART,NPRINT,1,SQIM)
