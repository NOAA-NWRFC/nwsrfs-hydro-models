...
!CP   1600 FORMAT('   *** ENTER XFMQ26 ***')
      IF ( FEWSDEBUG.GE.4 ) THEN
         WRITE(MESSAGESTRING, '(A23)') '    *** ENTER XFMQ26 ***'
...
C
   25 CONTINUE
C
!CP   IF (IBUG.GE.2) WRITE(IODBUG,1610) (PO(LOKELV+I-1),I=1,NMQVAL)
!CP   IF (IBUG.GE.2) WRITE(IODBUG,1611) (W(LOCMQ+I-1),I=1,NMQVAL)
	 ...
	 IF ( NPRINT.GT.100 ) NPRINT=100
         ...
         call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
 1610    FORMAT(/'   ** ELEVATION VS. MAX DISCHARGE CURVE :') 
         WRITE(FORMATSTR, *) '%12.3f'
         call logonedimensionarrayfromfortran(DEBUG_LEVEL,NULLLINE,1,6,
     >        FORMATSTR,LOKELV,LOKELV+NPRINT-1,1,PO)

         WRITE(FORMATSTR, *) '%12.3f'
         call logonedimensionarrayfromfortran(DEBUG_LEVEL,NULLLINE,1,6,
     >        FORMATSTR,LOCMQ,LOCMQ+NPRINT-1,1,W)
      END IF
C
      GO TO 9000
C
...
!CP   1699 FORMAT('    *** EXIT XFMQ26 ***')
      IF ( FEWSDEBUG.GE.4 ) THEN
         WRITE(MESSAGESTRING, '(A23)') '    *** EXIT XFMQ26 ***'
         call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
      END IF
      RETURN
      END
