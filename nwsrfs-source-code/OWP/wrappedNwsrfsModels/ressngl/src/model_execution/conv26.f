...
!CP   WRITE(IODBUG,40) KSTART,KEND,KEND24,LSTART,JOPT,JOBS,
      WRITE(MESSAGESTRING, 40) KSTART,KEND,KEND24,LSTART,JOPT,JOBS,
     $JULIAN,NDAYR,NTIM24,QGENMX,QGENDS,OUTMIN
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
   40 FORMAT(1H0,' KSTART,KEND,KEND24,LSTART,JOPT,JOBS,',
     $'JULIAN,NDAYR,NTIM24,QGENMX,QGENDS,OUTMIN',/1X,9I5,3F10.0)
C
...
CR1 *** REVISION NO. 1 *** END ***
  433 VOL=VOL+QOMADJ(J)
      PRDS=JEND-JSTART+1
      QOFMN=VOL/PRDS
!CP   IF(IBUG.EQ.2) WRITE(IODBUG,434) QOMEAN(I),QOFMN
      IF ( FEWSDEBUG.GE.4 ) THEN
         WRITE(MESSAGESTRING, 434) QOMEAN(I),QOFMN
	 call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
      END IF
...
C WRITE TRACE AND DEBUG IF REQUIRED.
C
!CP   IF(IBUG-1)560,540,450
!CP   450 WRITE(IODBUG,460)
      IF ( FEWSDEBUG.LT.4 ) GO TO 560
...
!CP   IF(OUTMIN.EQ.-999.0) WRITE(IODBUG,461)QOMIN
      IF (OUTMIN.EQ.-999.0) THEN
         WRITE(MESSAGESTRING, 461)QOMIN
	 call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
      END IF
  461 FORMAT(1H0,42H MINIMUM OUTFLOW FOR LAST TIME INTERVAL IS,F8.0)
   
!CP   WRITE(IODBUG,470) (QOMEAN(I),I=1,NQMEAN)
      WRITE(MESSAGESTRING, 470) 
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
  470 FORMAT(1H0,24H 24-HR MEAN OUTFLOWS ARE) 
      WRITE(FORMATSTR, *) '%10.0f'
      call logonedimensionarrayfromfortran(DEBUG_LEVEL,NULLLINE,1,8,
     >     FORMATSTR,1,NQMEAN,1,QOMEAN)
   
!CP   WRITE(IODBUG,480) (QOMBAC(I),I=1,NTIM24)
      WRITE(MESSAGESTRING, 480) 
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
  480 FORMAT(1H0,42H BACK VALUES OF TIME INTERVAL OUTFLOWS ARE)
      WRITE(FORMATSTR, *) '%12.3f'
      call logonedimensionarrayfromfortran(DEBUG_LEVEL,NULLLINE,1,10,
     >     FORMATSTR,1,NTIM24,1,QOMBAC) 
...
!CP   WRITE(IODBUG,500) (QOMSIM(I),I=1,KEND)
      WRITE(MESSAGESTRING, 500)
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
  500 FORMAT(1H0,' PERIOD MEAN OUTFLOWS FROM PERIOD NO. 1') 
      WRITE(FORMATSTR, *) '%10.0f'
      call logonedimensionarrayfromfortran(DEBUG_LEVEL,NULLLINE,1,8,
     >     FORMATSTR,1,KEND,1,QOMSIM)
   
C$  510 WRITE(IODBUG,520) (QOMADJ(I),I=1,KEND)
C$  520 FORMAT(1H0,33H ADJUSTED MEAN OUTFLOW VALUES ARE/(1X,8F10.0))
   
!CP   WRITE(IODBUG,530) (FRACTN(I),I=1,NTIM24)
  ...
  530 FORMAT(1H0,57H FRACTIONS OF 24-HR VOLUME FOR TIME INTERVAL OUTFLOW
     $S ARE) 
      ...
      call logonedimensionarrayfromfortran(DEBUG_LEVEL,NULLLINE,1,10,
     >     FORMATSTR,1,NTIM24,1,FRACTN) 
!CP   540 WRITE(IODBUG,550)
      WRITE(MESSAGESTRING, 550)
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
