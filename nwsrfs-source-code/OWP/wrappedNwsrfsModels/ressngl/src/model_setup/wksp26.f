...
C
      call numberoftimesteps(itimestep,NTIMESTEPS)
...
C  PERIOD IN THE RUN.
C
C      NWKSP = NWKSP + 4*NDD*NTIM24
      NWKSP = NWKSP + 4*NTIMESTEPS
C
C-----------------------------------
...
C      NWKSP = NWKSP + 2*NDD*NTIM24
      NWKSP = NWKSP + 2*NTIMESTEPS
C
C---------------------------------------------
...
C
C      NWKSP = NWKSP + NDD*NTIM24
      NWKSP = NWKSP + NTIMESTEPS
C
C--------------------------------------------
...
C
C LC the second reference to NDD does not use *NTIM24
C still replaced with NTIMESTEPS
C      NWKSP = NWKSP + NDD*NTIM24 + 2*NSE + NDD + NUMCO
      NWKSP = NWKSP + NTIMESTEPS + 2*NSE + NTIMESTEPS + NUMCO
C
C-------------------------------------------------------------------
...
C
C      NWKSP = NWKSP + 3*NDD*NTIM24
      NWKSP = NWKSP + 3*NTIMESTEPS
C
C---------------------------------------------------
...
C
C      NWKSP = NWKSP + 8*NDD*NTIM24
      NWKSP = NWKSP + 8*NTIMESTEPS
C------------------------------------------------
...
C
!CP   IF (IBUG.GE.2) WRITE(IODBUG,1690) NWKSP,
      IF ( FEWSDEBUG.GE.3 ) THEN
         WRITE(MESSAGESTRING, 1690) NWKSP, (LOCOWS(K),K=1,13)
         call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
 1690 FORMAT(1H0,' NO. OF WORDS NEEDED FOR WORK SPACE = ',I8,
     ./5X,'       OPTIONAL WORK SPACE = ',13I5)
      END IF
C
!CP   IF (IBUG.GE.1) WRITE(IODBUG,901)
      IF ( FEWSDEBUG.GE.4 ) THEN
