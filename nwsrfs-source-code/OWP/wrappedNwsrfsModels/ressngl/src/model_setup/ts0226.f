...
C
!  175 CONTINUE
!      CALL FDCODE(DTYPE,TRIM(UCODE),X,IX,IX,X,IX,IERD)
!      IF (TRIM(UCODE).EQ.UCMS.OR.UCODE.EQ.UCMSD) GO TO 180
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!      
      CALL getDimensionAndUnitInFortran(DTYPE, DCODE, UCODE)
      IF (TRIM(UCODE).EQ."CMS".OR.TRIM(UCODE).EQ."CMSD") GO TO 180
C
...
C
  180 CONTINUE
      IF (TRIM(DCODE).EQ."L3".OR.TRIM(DCODE).EQ."L3/T") GO TO 185
!CP      IF (TRIM(DCODE).EQ.DIML3.OR.DCODE.EQ.DIMLT) GO TO 185
C
      CALL STER26(46,1)
      GO TO 10
C
