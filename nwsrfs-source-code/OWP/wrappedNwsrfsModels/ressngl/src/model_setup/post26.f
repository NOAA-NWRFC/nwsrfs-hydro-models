...
C
C
!CP   IF (IBUG.GE.1) WRITE (IODBUG,*) 'ENTER POST26'
      IF ( FEWSDEBUG.GE.4 ) THEN
         WRITE(MESSAGESTRING,'(A12)') 'ENTER POST26'
...
C LC - move call of WKSP26 to outside PIN
C it depends on NTIMESTEPS which is available 
C after PIN(); set P(16) with results
C      CALL WKSP26(PO,NWKSP)
C
...
!CP      WRITE(IPR,699)
  ...
  699 FORMAT ('0**NOTE** THE FOLLOWING ERRORS AND/OR WARNINGS WERE ',
     * 'GENERATED DURING POST-INPUT PROCESSING OF OPERATION RES-SNGL:')
         ...
         ENDIF
C
!CP   IF (IBUG.GE.1) WRITE(IODBUG,*) 'EXIT POST26'
      IF ( FEWSDEBUG.GE.4 ) THEN
         WRITE(MESSAGESTRING,'(A11)') 'EXIT POST26'
	 call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
      END IF
C
      RETURN
