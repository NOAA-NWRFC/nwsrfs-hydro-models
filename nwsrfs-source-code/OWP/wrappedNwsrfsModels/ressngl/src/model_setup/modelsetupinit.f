      SUBROUTINE modelsetupinit(PATH)
      ...
      CHARACTER*200 PATH, PATH_FILENAME
      LOGICAL  file_exists

C Initialize common ionum      
      DATA IN/5/
      DATA IPR/6/
      DATA IPU/7/,MUNI26/89/
C Initialize common suky26 
...
C Initialize common mult26
      DATA MULTNM/4HRULE,4HCURV,4HE   ,4HELVS,4HQ   ,4H    ,
     2            4HHEAD,4HVSQ ,4H    ,4HHTWV,4HSQ  ,4H    ,
     3            4HCRES,4HT   ,4H    ,4HSPIL,4HLWAY,4H    ,
     4            12*4H    /
     
C Initialize common comn26
      DATA CONVL/1.0/, CONVQ/1.0/, CONVLT/1.0/,CONVST/1.0/       
       
      PATH_FILENAME=TRIM(PATH)
C
C Open file with operation definition (params.txt)
C
      OPEN(IN, ERR=100, ACCESS='SEQUENTIAL', FORM='FORMATTED',
     >     FILE=PATH_FILENAME) 

C Open a temporary file used by pin to transfer original operation 
C definition to code assumes in unit 89
C
      OPEN(MUNI26,ERR=100,FORM='FORMATTED', STATUS='SCRATCH')  
      RETURN

 100  WRITE(MESSAGESTRING, '(A19)') 'ERROR opening file '
