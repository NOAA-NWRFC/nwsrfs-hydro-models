...
int ResJSys :: parseParameters( char** param_list, int n_items )
{
	...
	if( !param_list || !n_items ){
	   ...
	   */
	   ...
	   "%s %s Empty parameters list.", warnStr, routine);
	...
	for( i = 0; i< n_items; i++ ) {
		...
		if( !strcasecmp( list[0], "UNITS" ) && i != 0 ) {
			...
			*/
			logMessageWithArgsAndExitOnError( FATAL_LEVEL, 
	                "%s %s If explicitly defined, UNITS "
			"must be first item in PARAMETERS.",
			errorStr, routine );			
		...
		if( !strcasecmp( list[0], "UNITS" ) ) {
			if( !strcasecmp( list[1], "ENGLISH" ) ) {
				...
				*/
				logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		                "%s %s UNITS assignment "
			        "\"%s\" irregular.  Set to default, "
				"METRIC.", warnStr, routine, list[1] );				
				Method :: setUnitType( SI );
			}
			list = FreeStringList( list );
			continue;
		...
		if( !strcasecmp( list[0], "RESERVOIR" ) ||
			...
			if ( list[1] == NULL ) {
				...
				*/
				logMessageWithArgsAndExitOnError( FATAL_LEVEL, 
		                "%s %s No identifier supplied "
				"with keyword \"%s\" in PARAMETERS.",
				errorStr, routine, list[0] );
				
				/*AVCP list = FreeStringList( list );						                           continue; */
			}
			...
			if ( sub_list == NULL ) {
				...
				*/
				logMessageWithArgsAndExitOnError( FATAL_LEVEL, 
		                "%s %s Could not create "
				"sub-list beginning with line \"%s\". "
				" Perhaps no 'END%s'?",
				errorStr, routine, param_list[i], list[0] );
			...
			//printf("get component parameter %s\n", list[1]);
			comp = _root->getComponentPtr( list[1] );
			if( comp == NULL ) {
				//totErrs++;
				/* AVCP PrintError( routine, "Cannot find " */
				logMessageWithArgsAndExitOnError( FATAL_LEVEL,
				"%s %s Cannot find component for \"%s\".",
			        errorStr, routine, list[0] );
				
				/*AVCP sub_list = FreeStringList( sub_list );
				list = FreeStringList( list );
				continue;*/
			}
		...
		if( !list || nlist != 3 ) {
			...
			logMessageWithArgsAndExitOnError( FATAL_LEVEL, 
		        "%s %s Expecting 3 parts to Parameter Line \"%s\".", 
		...
		if( !strcasecmp( list[0], "CONSTANT" ) ) {
		   ...
		   {
		       logMessageWithArgsAndExitOnError( DEBUG_LEVEL, 
		       "%s Skipping over CONSTANT line: \"%s\".", 
		       routine, param_list[i] );
	           }
		...
		if ( endFound == 0 ) {
			...
			// a message.
			//totErrs++;
		/* AVCP PrintError( routine, "Parameter keyword does not " */
			logMessageWithArgsAndExitOnError( FATAL_LEVEL, 
		        "%s %s Parameter keyword does not "
			"have matching end keyword (%s).",
			errorStr, routine, temp );
	...
	for( i = 0; i< method_listSize; i++ ) {
		...
		if ( sub_list == NULL ) {
			//totErrs++;
	       	/* AVCP PrintError( routine, "Could not create sub-list "*/
			logMessageWithArgsAndExitOnError( FATAL_LEVEL, 
		        "%s %s Could not create sub-list "
	       		"beginning with line \"%s\".  Perhaps "
      			"no 'END%s'?", errorStr, routine, 
			method_list[i], list[0] );
			
			/*AVCP i = method_listSize;
			list = FreeStringList( list );
			continue;*/
		}
		...
		if( !method ){
		   ...
		   logMessageWithArgsAndExitOnError( DEBUG_LEVEL, 
	           "%s %s %s %s %s was not initialized in parseRules "
		   "section...skipping.",
		   warnStr, routine, list[0], list[1], list[2]);
		...
		if( method->construct( sub_list, count ) ) {
			...
			list = FreeStringList( list );
			sub_list = FreeStringList( sub_list );
		/* AVCP PrintError( routine, "Troubles constructing %s.", */
			logMessageWithArgsAndExitOnError( FATAL_LEVEL, 
		        "%s %s Troubles constructing %s.",
			errorStr, routine, method_list[i - count -1] );
	...
	while( i < nMS && !totErrs )
	{
		...
		/* AVCP PrintError( routine, "Lookup3 method owned by %s is " */
			logMessageWithArgsAndExitOnError( FATAL_LEVEL, 
		        "%s Lookup3 method owned by %s is "
	                "located within subtree of MaxStage %s %s.", 
			errorStr, routine, ucp->getID(), method->getID() );
			...
			/* AVCP  PrintError( routine,  */
			logMessageWithArgsAndExitOnError( FATAL_LEVEL, 
		        "%s %s Lookup3 method transfers to "
	                "Component %s, located within subtree of MaxStage "
	                "%s %s.", errorStr, routine, Lookup3TOCOMP[k], 
			ucp->getID(), method->getID());	            
	            }
	        }
	    }
