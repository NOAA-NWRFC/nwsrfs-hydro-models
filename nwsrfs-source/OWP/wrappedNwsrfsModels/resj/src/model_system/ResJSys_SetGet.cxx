...
int ResJSys :: addCOString( char* string )
{
	...
	if( string == NULL ) {
	   ...
	   logMessageWithArgsAndExitOnError( DEBUG_LEVEL, 
		"%s (%s) - Cannot add null string to carryover "
		"CO array for FORTRAN interface.", warnStr, routine );		
	...
	*/
	...
	if( _co_list == NULL ) {
		/* AVCP PrintWarning( 1, routine */
		logMessageWithArgsAndExitOnError( DEBUG_LEVEL, 
		"%s (%s) - Troubles allocating %d chars.",  
		warnStr, routine, length + 1);
		return( STATUS_FAILURE );
	}
	strcat( _co_list, string );
	_co_list[ length ] = '\0';

	// The incoming string always has a -99999 specified in the
	...
	if( _co_size > 0 ) {
		char* ptr = strstr( _co_list, "-99999" ); 
		if( ptr == NULL ) {
		    /* AVCP PrintWarning( 1, routine */
		    logMessageWithArgsAndExitOnError( DEBUG_LEVEL, 
		     "%s (%s) - Troubles inserting position"
		     "of next Component in CO array.", warnStr, routine);
	
			return( STATUS_FAILURE );
		}
		char pos[7];
		sprintf( pos, "%6d", prev_len );
		strncpy( ptr, pos, 6 );
	}
...
int ResJSys :: addPOString( char* string )
{
	...
	if( string == NULL ) {
	        /* AVCP PrintWarning( 1, routine */
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL, 
		"%s (%s) - Cannot add null string to time "
		"series PO array for FORTRAN interface.", warnStr, routine );
...
char* ResJSys :: getDebugString( int* size )
{
	...
	if( !size ){
	   // AVCP PrintWarning( 1, routine 
	   logMessageWithArgsAndExitOnError( DEBUG_LEVEL, 
	     "%s (%s) - Incoming Size variable is NULL.", warnStr, routine );
	   return( "" );
	}
	(*size)		= _debug_str_len;
	return( _debug_str );
...
int ResJSys :: setCODates( int* hr, int* da, int* mo, int* yr, int num_co )
{
	...
	if( !hr || !da || !mo ) {
	   /* AVCP PrintWarning( 1, routine */
	   logMessageWithArgsAndExitOnError( DEBUG_LEVEL, 
	   "%s (%s) - Cannot set carryover save dates -"
	   " missing hour, day, or month information.", warnStr, routine );			
	   return( STATUS_FAILURE );
	}
	...
	if( _co_dates == NULL ) {
	   /* AVCP PrintWarning( 1, routine */
	   logMessageWithArgsAndExitOnError( DEBUG_LEVEL, 
	   "%s (%s) - Could not allocate %d TSDates.", 
	   warnStr, routine, num_co );	     
	   return( STATUS_FAILURE );
	}
	if( _co_array_str == NULL ) {
	   /* AVCP PrintWarning( 1, routine */
	   logMessageWithArgsAndExitOnError( DEBUG_LEVEL, 
	   "%s (%s) - Could not allocate %d char*s.",
	   warnStr, routine, num_co );
...
TS** ResJSys :: getResJOutput( char** list, int nlist )
{
	...
	if( ts_list == NULL ) {	
	   /* AVCP PrintWarning( 1, routine */
	   logMessageWithArgsAndExitOnError( DEBUG_LEVEL, 
	   "%s (%s) - Could not allocate %d TS*.", warnStr, routine, nlist );	
	   return( NULL );
	}
	for( i = 0; i < nlist; i++ ) {	
		...
		if( ts_list[i] == NULL ) {
		   /* AVCP PrintWarning( 1, routine */
		   logMessageWithArgsAndExitOnError( DEBUG_LEVEL, 
	           "%s (%s) - Could not get output time series %s.",
		   warnStr, routine, list[i] );			
		   return( (TS**)NULL );
		}
	}
	return( ts_list );
