...
int ResJSys::run( char* fname )
{
	...
	if( fp == NULL ){
	   logMessageWithArgsAndExitOnError( FATAL_LEVEL, 
	   "%s %s File \"%s\" is not available for reading.",
	   errorStr, routine, _control_file );
	   /* AVCP
	         PrintError( routine,
...
// printf("\n 1!!!!! lfactor= %10f  ffactor= %10f \n", Component::_lfactor, Component::_ffactor);
	...
	if( ts_list == NULL || nts_list == 0 ){
		...
		logMessageWithArgsAndExitOnError( FATAL_LEVEL, 
	         "%s %s Troubles getting TIMESERIES/ENDTIMESERIES lines.",
		 errorStr, routine );
		/* AVCP
		      PrintError( routine,
	...
	if( topo_list == NULL || ntopo_list == 0 ){
		/*AVCP numErrs++;*/
		logMessageWithArgsAndExitOnError( FATAL_LEVEL, 
	        "%s %s Troubles getting TOPOLOGY/ENDTOPOLOGY lines.",
	        errorStr, routine );
		 /* AVCP
		        PrintError( routine,
	...
	if( rule_list == NULL || nrule_list == 0 ){
		...
		"Troubles getting RULES/ENDRULES lines." );*/
 	        ...
 	        logMessageWithArgsAndExitOnError( FATAL_LEVEL, 
	        "%s %s Troubles getting RULES/ENDRULES lines.",
	        errorStr, routine );
	}
	...
	if( param_list == NULL || nparam_list == 0 ){
		...
		*/
		logMessageWithArgsAndExitOnError( FATAL_LEVEL, 
	        "%s %s Troubles getting PARAMETERS/ENDPARAMETERS lines.",
		errorStr, routine );
	...
	if( !_root ){
	   /* AVCP
		...
		"Topology tree is NULL!" );
	      ...
	      */
	   logMessageWithArgsAndExitOnError( DEBUG_LEVEL, 
	   "%s %s Topology tree is NULL!", warnStr, routine );
	   return( STATUS_FAILURE );
	}
	if( _root->checkMethods() ){
        ...
        logMessageWithArgsAndExitOnError( DEBUG_LEVEL, 
        "%s Checked syntax of \"%s\".", routine, _control_file );
	...
	char* fname, int nts, char** tsids, float** tsdata )
{
	...
	if( !fname || !tsids || !tsdata ){
	   /* AVCP
	      ...
	      */
	   logMessageWithArgsAndExitOnError( DEBUG_LEVEL, 
	   "%s %s Incoming arguments are NULL.", warnStr, routine);	
	   return( STATUS_FAILURE );
	}
	...
	"Running simulation from %s - %s.", _t1.toString(), _t2.toString() );*/
        ...
        logMessageWithArgsAndExitOnError( DEBUG_LEVEL, 
        "%s Running simulation from %s - %s.", 
        routine, _t1.toString(), _t2.toString() );
	...
	for( i=0; i<nts; i++ ){
		...
		if( !tspt ){
		    /*AVCP		    
		          ...
		          */
		     logMessageWithArgsAndExitOnError( DEBUG_LEVEL, 
	             "%s %s time series %s does not "
		     "appear in the TIMESERIES section of the control file.",
     		     warnStr, routine, tsids[i] );
		...
		if( tspt->allocateDataSpace() ) {
		   ...
		   logMessageWithArgsAndExitOnError( DEBUG_LEVEL, 
	           "%s %s Troubles allocating memory.", warnStr, routine );		
		   return( STATUS_FAILURE );
		}
		...
		{
		   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	           "%s nid (%s), base(%d), mult(%d)\n",routine, 
		   id.getIdentifier(), base, mult); 
		}
		...
		for( j=0, t=_t1; t <= _t2; j++, t.addInterval( base, mult ) ){
			if( tspt->setDataValue( t, tsdata[i][j] ) ){
			   ...
			   logMessageWithArgsAndExitOnError( DEBUG_LEVEL, 
	                   "%s %s Troubles setting data value to %f at %s.", 
			   warnStr, routine, tsdata[i][j], t.toString() );
...
char* AppendLine( char* line, char* new_line, int flag )
{
	...
	if( new_line == NULL ){
	    /* AVCP
	       ...
	       */
	   logMessageWithArgsAndExitOnError( DEBUG_LEVEL, 
	   "%s (%s) - Line to append is NULL.", warnStr, routine );
