...
int ResJSys :: parseTopology( char** topo_list, int n_items )
{
	...
	if( comp_list == NULL ){
	   ...
	   logMessageWithArgsAndExitOnError( FATAL_LEVEL, 
           "%s (%s) Unable to allocate memory for %d Component(s)",
           errorStr, routine, n_items );
		
		/*AVCP delete [] comp_list;
	...
	for( i = 0; i< n_items; i++ ) {
		...
		if( !first_line_found ) {
			if( nlist != 2 ){
			    ...
			    /* AVCP PrintError( routine, */
			    logMessageWithArgsAndExitOnError( FATAL_LEVEL, 
		            "%s (%s) - First line of Topology section \"%s\" "
			    "is malformed.", errorStr, routine, topo_list[i] );
		...
		if( !strcmp( "RESERVOIR", list[0] ) ){
			...
			"Number of components is now erroneous as "
			"count equals total topology lines.", 
			errorStr, routine, topo_list[i], list[0]  );
		...
		if(strlen(list[1]) > 12 ) {
			...
			PrintError( routine, */
			logMessageWithArgsAndExitOnError( FATAL_LEVEL, 
		        "%s (%s) - Component ID \"%s\" is too long ( > 12 "
			"chars.", errorStr, routine, list[1] );
		...
		if( comp->setID( list[1] ) ) {
			...
			PrintError( routine, */
			logMessageWithArgsAndExitOnError( FATAL_LEVEL, 
		        "%s (%s) - Troubles setting compnent ID to \"%s\".", 
			...
			if(!strcmp( "BELOW", list[j] ) ) {
				...
				if( found ){
					if( comp->addSon( comp_list[k] ) ){
					    ...
					    FATAL_LEVEL, "%s (%s) - Topology "
					    "line \"%s\": Topological linkage not "
					    "set between %s and %s.", 
				            errorStr, routine, topo_list[i], 
					    comp->getID(), 
					    comp_list[k]->getID());
					...
					logMessageWithArgsAndExitOnError( 
				        ...
				        "has not been defined yet.", errorStr,
				       	routine, topo_list[i],list[j+1] );  
	...
	if( n_comp == 0 ){
		...
		/* AVCP PrintError( routine, */
		logMessageWithArgsAndExitOnError( FATAL_LEVEL, 
	        "%s (%s) - Topology section has no valid items.", 
	        errorStr, routine );  
