...
int Table :: populate( char** list, int nlist, double lfactor, double vfactor )
{
	...
	if( !list || !nlist ){
	   logMessageWithArgsAndExitOnError( DEBUG_LEVEL, 
	       "%s Incoming arguments are NULL or zero.",routine );
	...
	for( int i=0; i<nlist; i++ ){
		...
		if( !sublist || nsublist != 2 ){
		   logMessageWithArgsAndExitOnError( FATAL_LEVEL, 
	              "%s Table row \"%s\"\n does "
		      "not have exactly 2 fields.",routine, list[i] );
...
int Table :: populate( double *c1, double *c2, int size )
{
	...
	if( !c1 || !c2 ){
	   logMessageWithArgsAndExitOnError( DEBUG_LEVEL, 
	       "%s Incomining arrays are NULL.",routine);
		return( STATUS_FAILURE );
	}
...
int Table :: populate( int row, int col, double value )
{
	...
	if( row < 0 || row >= _rows ){
	   logMessageWithArgsAndExitOnError( DEBUG_LEVEL, 
	       "%s Row value %d is not valid.",routine, row );
	...
	if( col < 0 || col > 1 ){
	   logMessageWithArgsAndExitOnError( DEBUG_LEVEL, 
	       "%s Column value %d is not valid..",routine, col );
