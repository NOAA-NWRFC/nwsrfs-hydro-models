...
{	int	jday;
	...
	if (	GetJulianDay1900FromDate( month, day, year, &jday) ) {
		...
		"%s (%s) Unable to convert date (%d/%d/%d:%d) to Julian hour",
		warnStr, routine, month, day, year, hour );
		return STATUS_FAILURE;
	}
...
{static char rcs_id1[] = "$Source: /fs/hseb/ob81/ohd/ofs/src/resj_utils/RCS/time-date.c,v $";
