...
{	FILE	*fp;
	...
	*/
	...
	if ( !found ) {
		...
		*/
		/*AVCP if ( GetDef ( "rfs_sys_dir", sysdir ) ) {
		...
		*/
		/*AVCP sprintf ( dufile, "%s/DATAUNIT", sysdir );*/
	        sprintf ( dufile, "%s", getDataUnitFileName() );
		/*
		...
		*/
		if ( !(fp = fopen(dufile,"r")) ) {
			...
			/*AVCP PrintWarning ( 5, routine, message ); */
			
                        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
                        "%s (%s) - %s\n", warnStr, routine, message);
...
{static char rcs_id1[] = "$Source: /fs/hseb/ob81/ohd/ofs/src/resj_utils/RCS/GetUnitsFormat.c,v $";
