...
int Reservoir :: setCOstring( )
{
	...
	char temp_str[30+64+80+1];	// Labels, values, future space, end

        value_str[0] = '\0';
	// Get current values (values at end of the current time step)
	snprintf( rel_str, 9, "%f", _release );
        strncat( value_str, rel_str, 8 );
	
	snprintf( pool_str, 9, "%f", _pool );
	strncat( value_str, pool_str, 8 );
	
	snprintf( with_str, 9, "%f", _withdraw );
	strncat( value_str, with_str, 8 );
	
	snprintf( inflow_str, 9, "%f", _inflowCO );
        strncat( value_str, inflow_str, 8 );

	// Get previous-type values (values at end of the previous of time step)
	snprintf( prevRel_str, 9, "%f", _prevReleaseCO );
        strncat( value_str, prevRel_str, 8 );
	
	snprintf( prevPool_str, 9, "%f", _prevPoolCO );
        strncat( value_str, prevPool_str, 8 );
	
	snprintf( prevWithdraw_str, 9, "%f", _prevWithdrawCO );
        strncat( value_str, prevWithdraw_str, 8 );
	
	snprintf( prevInflow_str, 9, "%f", _prevInflowCO );
        strncat( value_str, prevInflow_str, 8 );

        value_str[64] = '\0';

	// Prepare value portion of the string
/*LX6	sprintf(value_str, "%8.8s%8.8s%8.8s%8.8s%8.8s%8.8s%8.8s%8.8s",
...
*/
	...
	sprintf( temp_str, 
		"%-12.12s%-12.12s%6.6s%64.64s%80.80s",
		_type, _id, "-99999", value_str, future_str );

	// Write the string to the system CO string
	if( ResJSys::addCOString( temp_str ) ) {
		/* AVCP PrintWarning( 1, routine,      */
		        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		        "%s (%s) - Troubles adding carryover data to "
			"CO array." , warnStr, routine );
...
//	are shifted in preparation for the next time step.
	...
	char rel_str[9], pool_str[9], with_str[9], inflow_str[9], 
		prevInflow_str[9], prevPool_str[9], prevRel_str[9], 
		prevWithdraw_str[9], future_str[81], value_str[65];
	char temp_str[54+64+80+1];	// Orig, enhanced, future space
	...
	snprintf( rel_str, 9, "%f", _startRelease );
        strncat( value_str, rel_str, 8 );
	
	snprintf( pool_str, 9, "%f", _startPool );
        strncat( value_str, pool_str, 8 );
	
	snprintf( with_str, 9, "%f", _startWithdraw );
        strncat( value_str, with_str, 8 );

	snprintf( inflow_str, 9, "%f", _startInflow );
        strncat( value_str, inflow_str, 8 );
	...
	***************/
	...
	// Get previous-type values (values at end of the previous of time step)
	snprintf( prevRel_str, 9, "%f", _prevRelease );
        strncat( value_str, prevRel_str, 8 );
	
	snprintf( prevPool_str, 9, "%f", _prevPool );
        strncat( value_str, prevPool_str, 8 );
	
	snprintf( prevWithdraw_str, 9, "%f", _prevWithdraw );
        strncat( value_str, prevWithdraw_str, 8 );
	
	snprintf( prevInflow_str, 9, "%f", _prevInflow );
        strncat( value_str, prevInflow_str, 8 );
