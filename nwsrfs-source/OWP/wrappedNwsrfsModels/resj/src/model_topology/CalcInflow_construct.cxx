...
int CalcInflow::construct ( char** re_list, int n_items )  
{
	...
	for( i = 0; i < n_items; i++ ) {
		...
		if( nlist == 0 || list == NULL ) {
			/*AVCP PrintError( routine,*/
			 ...
			 "%s (%s) - Troubles getting data for %s %s %s.", 
			 errorStr, routine, _type, _owner->getID(), _id );
			/*AVCP totErrs++;
			  continue;*/
		}
		...
		if( !strcasecmp( list[0], "TSINPUT" ) ) {
			...
			if( !strcasecmp( list[1], "POOL" ) ) {
				...
				"\"%s\" for %s %s %s.", errorStr, routine,
				list[0],list[1], _type, _owner->_id, _id );
				/*AVCP totErrs++;
				continue;*/
			}
			continue;
		...
		if( !strcasecmp( list[0], "TSOUTPUT" ) ) {
			...
			if( !strcasecmp( list[1], "INFLOW" ) ) {
				...
				if( j == 15 ) {
					...
					/*AVCP PrintError( routine,*/
				        logMessageWithArgsAndExitOnError(
					FATAL_LEVEL, "%s (%s) - Exceeded the "
					"limit of 15 output timeseries "
					"from Component %s while working "
					"with %s %s.", errorStr, routine,
				...
				if ( !tempTS ) {
					/*AVCP PrintError( routine, */
					logMessageWithArgsAndExitOnError(
				        FATAL_LEVEL, "%s (%s) - Unable to "
					"find TSOUTPUT '%s' in time series "
		...
		// Check for USEFORSIM
 		...
 		// Handle keyword TABLE
			...
			if( sub_list == NULL || nsublist == 0 ) {
				...
				/*AVCP PrintError( routine, */
				logMessageWithArgsAndExitOnError( FATAL_LEVEL,
				"%s (%s) - Troubles forming sub_list for "
				"table values for %s %s %s.  Does "
				"\"ENDTABLE\" exist?", errorStr, routine, _type,				_owner->_id, _id );
				/*AVCP continue;*/
			}
			...
			if ( nlist < 2 ) {
				/*PrintError( routine,*/
				logMessageWithArgsAndExitOnError( FATAL_LEVEL,
				"%s (%s) - Keyword MAXCHANGE required "
			...
			// Check for keyword "MAXCHANGE"
				...
				if ( table_found == 1 ) {
                                   ...
                                   if( getFewsDebugFlag() > 0 )
				   {
					...
					"Last definition used.", warnStr,
					routine, list[0], list[1], _type, 
					_owner->_id, _id );
				   }
				}
				...
				logMessageWithArgsAndExitOnError( FATAL_LEVEL,
				"%s (%s) - Keyword MAXCHANGE required "
				"immediately after %s for %s %s %s.", errorStr, 
				routine, list[0], _type, _owner->_id, _id );
			}
 		...
 		// Handle keyword MININFLOW
			...
			if( _minInflow == MISSING ) {
				...
				/*AVCP PrintError( routine,*/
				logMessageWithArgsAndExitOnError( FATAL_LEVEL,
				"%s (%s) - %s cannot handle special cases based"
				" on MISSING or near-MISSING values. "
				"\"%s\" causes this problem for %s %s %s.",
			        errorStr, routine, _type, re_list[i],
			      	_owner->_id, _id );
			}
			continue; 
		}
	...
	if( _pool_obs == NULL ) {
		...
		/*AVCP PrintError( routine,*/
	        logMessageWithArgsAndExitOnError( FATAL_LEVEL,
	        "%s (%s) - Pool observation timeseries required "
		"(but not parameterized) for %s %s %s.", errorStr, routine, 
		_type, _owner->getID(), _id );
	}
...
int CalcInflow::buildTables( char** infl, int ninfl, double ffactor )
{
	...
	for( i = 0; i < ninfl; i++ ) {
		...
		if( nlist == 0 || list == NULL ) {
			...
			warnStr, routine, _type, _owner->_id, _id );
			return( STATUS_FAILURE );
		}
		else if( nlist != 3 ) {
			list = FreeStringList( list );
			...
			"values on CALCINFLOW method \"%s\" line \"%s\".",
		        errorStr, routine, _id, infl[i] );
		...
		if( i != 0 ) {
			...
			if( LastInflow <= lastRowVal ) {
				...
				/*AVCP PrintError( routine,*/
			       logMessageWithArgsAndExitOnError( FATAL_LEVEL,
			       "%s (%s) - MAXCHANGE table must "
			       "contain ascending values in the first column."
			       " (%s %s %s)", errorStr, routine, _type, 
				_owner->_id, _id );
				/* AVCP return(STATUS_FAILURE);*/
			}
		}
