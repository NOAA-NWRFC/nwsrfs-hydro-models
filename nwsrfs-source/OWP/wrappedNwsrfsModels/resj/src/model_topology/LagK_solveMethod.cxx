...
Line20:
        ...
        if ( getFewsDebugFlag() > 9 )
	{
	   ...
	   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
           "(%s) - KNT+1,TR,X1,X2,Y1,Y2 = %d,%lf,%lf,%lf,%lf,%lf\n", 
           routine, knt+1, tr, x1, x2, y1, y2 );
	}
    }
    ...
    if ( knt >= 20 ) 
    {
        ...
        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
        "%s (%s) - RESJ-ComputeSegmentRouting - Did Not Converge .",
        warnStr, routine );
    }
}
...
double LagK :: CalculateFortWorthLoss ( double qli, double qprev, double qk )
{
    ...
    /*AVCP PrintDebug ( 10, routine, */
       logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
       "(%s) - Enter CalculateFortWorthLoss tlrc=%lf, qbntl=%lf, qli=%lf, "
       "qprev=%lf, qk=%lf\n", routine, _transLossCoef, _transLossLevel, qli, 
       qprev, qk ) ;
    }
    if ( qli < qprev )
    {
    ...
    /*AVCP PrintDebug (10, routine,*/
      logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
      "(%s) - Exit CalculateFortWorthLoss q=%lf, qtl=%lf\n", 
      routine, q, qtl);
    }
    return q;
}
                               ...
                               double** group_val )
{
    ...
    /*AVCP PrintDebug ( 10, routine,*/
      logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
      "%s x1, x2, value, y2, xk1, xk2, y1  = "
      "x1=%.2lf, %.2lf, %.2lf, y2=%.2lf, %.2lf, %.2lf,y1= %.2lf\n", 
      routine, x1, x2, value, y2, xk1, xk2, y1 ) ;
    }
    ...
    else if ( ( xk1 > xita / 2.0 ) || ( xk2 > xita / 2.0 ) )
    {
	    ...
	    if ( getFewsDebugFlag() > 9 )
	    {
               /*AVCP PrintDebug ( 10, routine,*/
	       logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	       "%s j, x14, x2, value, y2, s2Odt -- "
               "%d, %.2lf, %.2lf, %.2lf, %.2lf, %.2lf\n", 
               routine, j, x14, x2, value, y2, s2Odt ) ;
	    }
            y1 = y2;
        }
...
Line295:
        ...
        if ( !add )
        {
	        ...
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	        "%s (%s) Bad interpolation j=%d, .", warnStr, routine,
		cur_date.toString ( ) ) ;
            }
            smallTable.populate ( 0, FLOWCOLUMN, 
        ...
        double** group_val )
{
        ...
        if ( getFewsDebugFlag() > 9 )
	{
           /*AVCP PrintError ( routine, */
	   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	   "(%s) - Using LagK within a ComboMethod is not currently supported.",
	   routine ) ;
	}
    ...
    if ( getFewsDebugFlag() > 9 )
    {
       ...
       logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
       "(%s) - Current date: %s  QI1:  %f  QI2:  %f", routine,
       cur_date.toString ( ) , LagdQin1, LagdQin2 ) ;
       /*AVCP PrintDebug ( 5, routine,*/
       logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
       "(%s) - Setting outflow on %s to %f.", routine,
       _owner->_id, Qout2 );
    }
