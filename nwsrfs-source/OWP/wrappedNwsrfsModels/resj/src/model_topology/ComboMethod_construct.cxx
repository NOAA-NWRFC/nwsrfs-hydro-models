...
int ComboMethod :: construct( char** meth_list, int nmeth )
{
	...
	for( i = 0; i < nmeth; i++ ) {
		...
		if( !sub_list || nsub_list != 3 ) {
			...
			"%s %s Troubles building group "
			"member info for %s.", warnStr, routine, _id );
		...
		if( !method ) {
			...
			/*AVCP PrintWarning( 1, routine,*/
		        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		        "%s %s Could not get Method "
			"pointer for %s.", warnStr, routine, sub_list[2] );
			return( STATUS_FAILURE );
		}
		...
		if( i == 0 ) {
			...
			/*PrintWarning( 1, routine, */
		      ...
		      {
		        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
			"%s %s Method %s (type %d) not of compatible type with "
			"rest of group (type %d) for %s - not adding.", 
			warnStr, routine, method->getID(), 
			method->getGroupID(), group_type, _id );
		      }
			sub_list = FreeStringList( sub_list ); 
			continue;
		}
	...
	if( _group_n < 1 ) {
		/*AVCP PrintWarning( 1, routine,*/
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	        "%s %s Must have at least one reservoir "
		"in order to Balance using %s on %s.", 
