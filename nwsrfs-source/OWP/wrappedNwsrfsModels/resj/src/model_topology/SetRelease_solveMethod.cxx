	...
	double** group_val )
{
	...
	if ( _release_obs ) {
		// An observed release timeseries exists.  Is its value missing?
			...
			/* AVCP PrintDebug( 5, routine */
	                ...
	                {
	                    logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
			    "(%s) - Setting outflow for %s to %f "
			    "on %s.", routine, _owner->_id, rel_value, 
			    cur_date.toString() );
			}
			return( STATUS_SUCCESS );
		}
	...
	// Unknown _mode value
		...
		/* AVCP PrintWarning( 1, routine, */
		logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		"%s (%s) - Unknown SetRelease INTERPOLATE "
		"value '%d' in solution! No interpolation done.", 
		warnStr, routine, _mode );
	...
	if( target_value == MISSING ) {
		...
		/* AVCP PrintWarning( 1, routine, */
		logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		"%s (%s) - SetRelease %s unable to find "
		"target value for %s.  Last release (%s) used.",
		warnStr, routine, _id,
	   ...
	   if( _ts_step != 1 ) {
		...
		if ( _mode != INTERPOLATE_ELEV && _mode != INTERPOLATE_ALL ) {
			...
			if( prev_pool == MISSING ) {
				...
				"if it had. ", warnStr, routine, getID(), 
				two_prior.toString() );
				// blend_flag = 2;
				_ts_step = 1;
			}
	...
	if( rel_value == MISSING ) {
		...
		/* AVCP PrintWarning( 1, routine, */
		logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		"%s (%s) - Value returned from a SetRelease"
		" time series is MISSING - Setting to minimum "
		"release.", warnStr, routine );
		rel_value = _owner->_min_release;
	}
...
// printf(" !RRR SetRelease group_id=%d res=%s meth=%s pool=%f release=%f \n", _group_id, _owner->getID(), _id, pool, rel_value);
	...
	{
	   logMessageWithArgsAndExitOnError( DEBUG_LEVEL, 
	   "(%s) - Setting outflow for %s to %f on %s.", routine,
