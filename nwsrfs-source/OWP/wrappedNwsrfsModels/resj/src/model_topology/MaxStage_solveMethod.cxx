...
//				max stage.
// 29 Oct 2008  Wen K., ASM  rcid was defined to be 8 characters long, but fills
// with 8
// //              characters + end of string character which exceed the limit
// and cause
// //              error in the fortran call later.  Changed to 9 character
// long.
//------------------------------------------------------------------------------
		...
		if ( _max_discharge != MISSING) {
			...
			if (err_flag == -999) {
				/*AVCP PrintError(routine,*/
			        ...
			        "%s %s Failure translating stage to discharge "
				"using a Rating Curve ID. This is most likely "
				"because the calibration program is being "
				"run - Rating Curve ID's are not supported in "
				"calibration mode.", errorStr, routine );
...
*/
	...
	if( inFromOwnRes.allocateDataSpace() ) {
		/*AVCP PrintWarning( 1, routine,*/
	        ...
	        "%s %s Troubles allocating data space for "
		"outflow time series on %s.", warnStr, routine, _id );
		return( STATUS_FAILURE );
	}
...
//	while( difference > _criterion && iter <= _max_iter ) {
		...
		********************/
			    ...
			    //if ( getFewsDebugFlag() > 3 )
  			    //{ CP 4/28/10 changed DEBUG_LEVEL to DEBUG_LEVEL
			    //to match with legacy code
			        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
				"%s %s MaxStage "
				"method %s - D/S control %s require "
				"future date %s exceeds end run date %s "
				"at timestep %s.  Returning last release "
				"(%f)", routine, _owner->getID(), 
				_id, _dcp->getID(), t2.toString(), 
...
// printf("kwz---debug1 buildSubTree\n");
		if( sub_root == NULL ) {
			...
			/*AVCP PrintError( routine,*/
		        logMessageWithArgsAndExitOnError( FATAL_LEVEL,
			"%s %s Troubles building sub-topology MaxStage method "
			"%s on %s - cannot execute MaxStage.", 
		...
		if( sub_root->buildSubTree_specialTieIn( ) ) {
			...
			logMessageWithArgsAndExitOnError( FATAL_LEVEL,
			"%s %s Troubles building sub-topology MaxStage method "
			"%s on %s due to \"ToComp\" SetWithdraw complexities - "
			"cannot execute MaxStage.", errorStr, routine, 
			_id, _owner->_id );
			//return( STATUS_FAILURE );
		}
		...
		if( _next_ds_comp == NULL ) {
			...
			/*AVCP PrintError( routine,*/
		        logMessageWithArgsAndExitOnError( FATAL_LEVEL,
		        "%s %s Troubles setting next downstream "
			"component for MaxStage %s on %s.  Cannot "
			"execute MaxStage.", errorStr, routine, _id, 
			_owner->_id );
			//return( STATUS_FAILURE );
		}
		...
		if( _inflow_pos == -1 ) {
			...
			/*AVCP PrintError( routine,*/
		        logMessageWithArgsAndExitOnError( FATAL_LEVEL,
		        "%s %s Troubles establishing next "
		...
		if( t2 > getForecastDate2() ) {
			...
			/*AVCP PrintWarning( 1, routine,*/
		    //if ( getFewsDebugFlag() >= 3 )  
	            //{CP 4/28/10 change DEBUG_LEVEL to DEBUG_LEVEL to
		    //match with legacy code
		       logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		       "%s %s MaxStage method %s - D/S control %s requires "
		       "future date %s exceeds end run date %s "
		       "at timestep %s.  Returning last release "
		       "(%f)",routine, _owner->getID(), 
		       _id, _dcp->getID(), t2.toString(), 
...
*/
		...
		if( t1 > t2 ) {
			...
			/*AVCP PrintError( routine,*/
		        logMessageWithArgsAndExitOnError( FATAL_LEVEL,
		        "%s %s Start date is after end date for "
			"MaxStage %s on %s.", errorStr, routine,
			_id, _owner->_id );
			//return( STATUS_FAILURE );
		}
...
*/
	while( difference > _criterion && iter <= _max_iter ) {
		...
		if( solve.run( sub_root, t1, t2, _t_int, _t_mult, 
			...
			/*AVCP	 PrintError( routine, */
			logMessageWithArgsAndExitOnError( FATAL_LEVEL,
			"%s %s Troubles solving sub-topology "
...
*/
	...
	if( iter >= _max_iter && !MinTooHigh) {
		/*AVCP PrintWarning( 1, routine,*/
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	        "%s %s MaxStage %s failed to converge "
		"within specified tolerance of %f percent within the "
		"maximum iterations of %d (%s).", warnStr, routine, _id,
	       	_criterion, _max_iter, cur_date.toString() );
	}
...
/*  ==============  Statements containing RCS keywords:  */
{static char rcs_id1[] = "$Source: /fs/hseb/ob83/ohd/ofs/src/resj_topology/RCS/MaxStage_solveMethod.cxx,v $";
 static char rcs_id2[] = "$Id: MaxStage_solveMethod.cxx,v 1.8 2007/10/09 13:39:20 hsu Exp $";}
/*  ===================================================  */
