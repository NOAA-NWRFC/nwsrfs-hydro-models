...
*/
	...
	for( i=0; i<_n_const; i++ ){
		if( !strcmp( _const_id_list[i], id ) ){
			...
			/*AVCP PrintDebug( 10, routine,*/
			   ...
			   "%s Constant \"%s\" already set to %f. Updating "
			   "to %f.", routine, id, _const_val_list[i], value );
			}
	...
	if( !_const_val_list ){
		/*AVCP PrintWarning( 1, routine,*/
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		"%s %s Unable to allocate memory for the %dth double.", 
		warnStr, routine, _n_const );
		return( STATUS_FAILURE );
	}
	...
	/*AVCP PrintDebug( 10, routine,*/
	  logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	  "%s Set \"%s\"=%f at list position %d.", routine,
	...
	for( i = 0; i < _n_meth; i++ ) {
		...
		!strcasecmp( meth_id, _method_list[i]->getID() ) ) {
			if( _method_list[i] == NULL ) {
			   ...
			   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		             "%s %s Cannot add expression to NULL method.",
			        warnStr, routine );
			...
			if( _expr_list[i] != NULL ) {
			    ...
			    {
			        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
				"%s %s Previous expression %s for method %s "
				"will be overwritten with %s.", warnStr,
				routine, _expr_list[i]->toString(), meth_id,
				expr->toString() );
			    }
			}
			_expr_list[i] = expr;
...
int Component :: addMethod( Expression* expr, Method* meth )
{
	...
	if( _n_meth == 100 ) {
		/*AVCP PrintWarning( 1, routine,*/
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		"%s %s Exceeding max number of expressions and methods.",
		warnStr, routine );
		return( STATUS_FAILURE );
	}	
	...
	if( meth == NULL ) {
		/*AVCP PrintWarning( 1, routine, */
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	        "%s %s Cannot add NULL or method to list.", warnStr, routine );
		return( STATUS_FAILURE );
	}
...
int Component :: addSon( Component* son )
{
	...
	if( son == NULL ) {
	   ...
	   {
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		"%s %s Cannot add NULL pointer to son list.", warnStr, routine);
	   }
	   return( 2 );
	}
	...
	if( _son_list == NULL ){
		/*AVCP PrintWarning( 1, routine,*/
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		"%s %s Unable to allocate memory for %dth Component*.", 
		warnStr, routine, _n_son );
		return( STATUS_FAILURE );
	}
	...
	if( _son_list[_n_son-1]->_father == NULL ) {
		...
		/*AVCP PrintWarning( 1, routine,*/
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	        "%s %s Illegal topology - Component "
		"%s cannot have multiple downstream links.", 
		warnStr, routine, _son_list[_n_son-1]->_id );
		return( STATUS_FAILURE );
	}
	//son->print(stdout);
	return( STATUS_SUCCESS );
...
Component* Component :: getComponentPtr( char* ident )
{
	...
	if( ident == NULL ){
		/*PrintWarning( 1, routine,*/
	     ...
	     {
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		"%s %s Incoming Identifier is NULL.", warnStr, routine );
	     }
	     return( NULL );
	}
...
Method* Component :: getMethod( char* meth_type, char* comp_id, char* meth_id )
{
	...
	if( comp == NULL ){
		/*AVCP PrintWarning( 1, routine, */
	     ...
	     {
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		"%s %s Component \"%s\" does not exist in the topology tree.",
		warnStr, routine, comp_id );
	     }
  	     return( NULL );
	}
	...
	if ( getFewsDebugFlag() > 15 )
	{
	   /*AVCP PrintDebug( 2, routine, */
	   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	   "%s Troubles getting %s %s on %s", routine, 
	   meth_type, meth_id, comp_id );
	}
	return( (Method*)NULL );
}
...
HourTS* Component :: getOutputTS( char* type )
{
	...
	if( type == NULL || !strcmp( type, "" ) ) {
		/*AVCP PrintWarning( 2, routine,*/ 
	     ...
	     {
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	        "%s %s Cannot get OutputTS - NULL data type.", warnStr,
		routine);
	     }
	     return( NULL );
	}
...
double* Component :: getValuePtr( char* id )
{
	...
	if( value_ptr != NULL ){
	        if ( getFewsDebugFlag() > 15 )
		{
		   /*AVCP PrintDebug( 10, routine,*/
		   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		   "%s Returning pointer to internal value for \"%s\".", 
		   routine, id );
		}
		return( value_ptr );
	}
	...
	for( i=0; i<_n_const; i++ ){
		if( !strcmp( _const_id_list[i], id ) ){
		        if ( getFewsDebugFlag() > 9 )
			{
			   /*AVCP PrintDebug( 15, routine,*/
			   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
			   "%s Found \"%s\" at position %d.", routine, id, i );
			}
...
int Component :: setID( char* ident )
{
	...
	if( ident == NULL ){
		/*AVCP PrintWarning( 1, routine,*/ 
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	        "%s %s NULL identifier passed in.", warnStr, routine );
		return( STATUS_FAILURE );
	}
	if( !strcmp( ident, "" ) ) {
		/*AVCP PrintWarning( 2, routine, */
	     ...
	     {
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	        "%s %s Blank component identifier.", warnStr, routine );
	     }
   	     return(STATUS_SUCCESS);
	}
	strcpy( _id, ident );
...
int Component :: resetInflowTS( HourTS* in ) 
{
	...
	if( in == NULL && _n_son == 0 ) {
		/*AVCP PrintWarning( 2, routine, */
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	        "%s %s Cannot set most upstream Component"
		" \"%s\" inflow to NULL.", warnStr, routine, _id );
		return( STATUS_FAILURE );
	}
...
int Component :: setInflowTS( HourTS* in ) 
{
	...
	if( in == NULL ) {
		/*AVCP PrintWarning( 1, routine, */
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	        "%s %s Cannot set inflow timeseries to NULL.", 
		warnStr, routine );
		return( STATUS_FAILURE );
	}
	_in_count++;
	...
	if( _inflow_ts == NULL ) {
		/*AVCP PrintWarning( 1, routine,*/
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	        "%s %s Could not allocate %d HourTS pointers.",
		warnStr, routine, _in_count );
		return( STATUS_FAILURE );
	}
	_inflow_ts[ _in_count-1 ] = in;
...
int Component :: setSolutionOrder( int lastNumber )
{
	...
	for( i=0; i<_n_son; i++ ){
		...
		//printf("last number = %d, ID = %s\n", lastNumber, _son_list[i]->getID());
	}
