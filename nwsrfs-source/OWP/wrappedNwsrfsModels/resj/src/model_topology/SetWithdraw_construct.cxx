...
int SetWithdraw::construct ( char** re_list, int n_items )  
{
	...
	for( i = 0; i < n_items; i++ ) {
		...
		if( nlist == 0 || list == NULL ) {
		   ...
		   "%s %s Troubles getting withdraw data "
                   "for SetWithdraw %s %s.", 
		...
		if( !strcasecmp( list[0], "TSINPUT" ) ) {
			...
			if( !strcasecmp( list[1], "OBSERVEDWITHDRAW" ) ) {
				...
				"and timeseries identifier required "
				"immediately after SETWITHDRAW method "
				"keyword %s.", warnStr, routine, list[0] );
			       }
		...
		// If it has not, then use the default - NORMAL
			...
			if ( !strcasecmp( list[1], "TIME" ) ) {
				...
				/* AVCP   PrintError( routine, */
	                        logMessageWithArgsAndExitOnError( FATAL_LEVEL, 
				"%s %s Unrecognized %s keyword "
				"\"%s\" for SetWithdraw %s.", errorStr, 
				routine, list[0], list[1], _id );
				/* AVCP
				list = FreeStringList( list );
				continue;
				*/
			...
			if( nlist > 2 ) {
				if( !IsInteger( list[2] ) ) {
					/* AVCP totErrs++;
	                                PrintError( routine, */
	                                logMessageWithArgsAndExitOnError( 
				        FATAL_LEVEL,"%s %s %s value '%s' "
					"is not an integer.", errorStr, 
					routine, list[0], list[2] );
					/* AVCP
					list = FreeStringList( list );
					continue;
					*/
		...
		// Check for BLENDTS
			...
			if( nlist > 1 ) {
				...
				/* AVCP totErrs++;
                                PrintError( routine, */
				logMessageWithArgsAndExitOnError( 
			        FATAL_LEVEL, "%s %s Value required "
		...
		// associated with reading the control file line-by-line.
			...
			if( _receivingComp == NULL ) {
				/* AVCP totErrs++;
                                PrintError( routine, */
				logMessageWithArgsAndExitOnError( FATAL_LEVEL,
				"%s %s SetWithdraw %s ToComp %s not found in "
				"Component tree.", errorStr, routine, 
				_id, list[1] );
				/* AVCP
				list = FreeStringList( list );
				continue;
			...
			if( !strcasecmp( list[2], "INSTANTANEOUS" ) ) {
				...
				if ( ownNum > CompNum ) {
					/* AVCP totErrs++;
                                        PrintError( routine, */
				        logMessageWithArgsAndExitOnError( 
				        FATAL_LEVEL, "%s %s %s transfer to "
					"Component %s from SetWithdrawal %s "
					"method on %s is unacceptable due to "
					"solution order of components.",
					errorStr, routine, list[2], list[1], 
					_id, _owner->_id);
					/* AVCP
					list = FreeStringList( list );
					continue;
				...
				/* AVCP totErrs++;
                                PrintError( routine, */
				logMessageWithArgsAndExitOnError( FATAL_LEVEL,
				"%s %s Keyword \"%s\" unacceptable for "
			...
			if( _receivingComp->setInflowTS( &_Comp_ts) ) {
				/* AVCP totErrs++;
                                PrintError( routine, */
				logMessageWithArgsAndExitOnError( FATAL_LEVEL,
				"%s %s Troubles setting withdrawal to "
				"Component timeseries \"%s\" on %s for "
				"SetWithdraw %s", errorStr, routine, ts_id, 
			...
			{
		        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,  
			"%s %s '%s' is an unrecognized (or misplaced) keyword."
			"If there are other errors or warnings for SetWithdraw"
			"%s, this may disappear following their correction.",
	...
	if( with_found == 0 ) {
		/* AVCP totErrs++;
                PrintError( routine, */
		logMessageWithArgsAndExitOnError( FATAL_LEVEL,
		"%s %s SetWithdraw keyword 'VALUES' not found "
	...
	for( i = 0; i < nwith; i++ ) {
		...
		if( i == 0 ) {
			if( strcasecmp( list[nlist - 1], "endelev" ) ||
				...
				/* AVCP     PrintError( routine, */
				logMessageWithArgsAndExitOnError( FATAL_LEVEL, 
				"%s %s First line in SetWithdraw VALUES "
			...
			if ( n_ts <= 1 ) {
				/* AVCP     PrintError( routine, */
				logMessageWithArgsAndExitOnError( FATAL_LEVEL, 
				"%s %s At least 2 elevation points must be "
			...
			for( k = 1; k < nlist - 1; k++ ) {
				...
				if ( atof( list[k] ) <= lastval ) {
				   ...
				   logMessageWithArgsAndExitOnError( 
				   FATAL_LEVEL, 
				   "%s %s SetWithdraw (%s) ELEV values "
				   "must be in ascending order. (%s >= %s)", 
				  errorStr, routine, _id, list[k-1], list[k] );
				   /* AVCP
					list = FreeStringList( list );
					return( STATUS_FAILURE );
				   */
		...
		if( nlist > n_ts + 1 ) {
			...
			if( nlist != n_ts + 1 + ( 24 / _t_mult ) ){ 
				/* AVCP     PrintError( routine, */
			       logMessageWithArgsAndExitOnError( FATAL_LEVEL, 
			       "%s %s %s %s SetWithdraw must have one and only"
			       "one distribution value for each timestep.", 
			       errorStr, routine, _owner->_id, _id ); 
				/* AVCP
				list = FreeStringList( list );
				return( STATUS_FAILURE ); 
				*/
			...
			if ( dist_tally > 1.00001 || 
				...
				/* AVCP     PrintError( routine, */
				logMessageWithArgsAndExitOnError( FATAL_LEVEL, 
				"%s %s Distribution on %s %s SetWithdraw "
				"does not sum to 1.0.",
				errorStr, routine, _owner->_id, _id );
				/* AVCP
				list = FreeStringList( list );
			...
			/* AVCP     PrintError( routine, */
			logMessageWithArgsAndExitOnError( FATAL_LEVEL,  
			"%s %s incorrect number of value on SETWITHDRAW "
			"method \"%s\" line \"%s\".", 
			errorStr, routine, _id, withd[i] );
			/* AVCP
			list = FreeStringList( list );
			return(STATUS_FAILURE);
			*/
