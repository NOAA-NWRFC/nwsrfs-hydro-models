...
int Lookup3::construct ( char** re_list, int n_items )  
{
    ...
    if( _ownerType == CM_REACH )
    {
	    ...
	    "%s %s LOOKUP3 %s cannot function on REACH component '%s'.", 
	    errorStr, routine, _id, _owner->getID() );
    ...
    for( i = 0; i < n_items; i++ )
    {
        ...
        if( nlist == 0 || list == NULL )
        {
            /*AVCP PrintError( routine, */
	    logMessageWithArgsAndExitOnError( FATAL_LEVEL,
	    "%s %s Troubles getting data for Lookup3 %s %s.", 
        ...
        else if( !strncasecmp( list[0], "IN", 2 ) )
        {
            ...
            else if( !strcasecmp( list[0], "INITIALTRANSFER" ) )
            {
                ...
                if( toCompBad )
                {
                    ...
                    /*AVCP PrintError( routine,*/
		    logMessageWithArgsAndExitOnError( FATAL_LEVEL,
		    "%s %s TOCOMP must be successfully "
                    "processed before INITIALTRANSFER.  (%s %s %s)",
                    errorStr, routine, getType(), _owner->_id, _id );
                }
                else
                {
	...
	/*AVCP totErrs++;*/
        /*AVCP PrintError( routine,*/
        logMessageWithArgsAndExitOnError( FATAL_LEVEL,
        "%s %s Keyword VALUES not found. (%s %s %s)", errorStr, routine,
    ...
    if( _colVar == INDEX_TS && _colIndex == NULL )
    {
        ...
        /*AVCP PrintError( routine,*/
        logMessageWithArgsAndExitOnError( FATAL_LEVEL,
        "%s %s Columns indexed using a non-parameterized "
	"time series.  Please review \"INPUT_TS\" and \"COLUMNVAR\" "
	"keywords. (%s %s %s)", errorStr, routine, getType(), _owner->_id, _id);
    }
    if( _rowVar == INDEX_TS && _rowIndex == NULL )
    {
        ...
        /* PrintError( routine, */
	logMessageWithArgsAndExitOnError( FATAL_LEVEL,
        "%s %s Rows indexed using a non-parameterized "
    ...
    if( _tableVar == TVAR_RELEASE && _fromComp )
    {
        ...
        /*AVCP PrintError( routine,*/
        logMessageWithArgsAndExitOnError( FATAL_LEVEL,
        "%s %s Negative values not allowed in table when "
        "TABLEVAR is RELEASE. (%s %s %s)", errorStr, routine,
	getType(), _owner->_id, _id );
    }
    ...
    if( _receivingComp )
    {
        ...
        if( _toCompMode && _myValue == MISSING )
        {
            ...
            /*AVCP PrintError( routine,*/
	   logMessageWithArgsAndExitOnError( FATAL_LEVEL,
	   "%s %s Lookup3 keyword 'INITIALTRANSFER' not found, "
           "but required with ToComp NextStep parameterization for %s "
           "on %s.", errorStr, routine, _id, _owner->_id );
        }
        ...
        if( _tableVar == TVAR_AUGMENTATION || _tableVar == TVAR_RELEASE )
        {
            ...
            /*AVCP PrintError( routine, */
	    logMessageWithArgsAndExitOnError( FATAL_LEVEL,
	    "TABLEVAR AUGMENTATION and RELEASE not "
            "compatibile with TOCOMP functionality.  Please consider using "
            "WITHDRAWAL or DIVERSION instead for %s %s %s.", errorStr, routine,
	   ...
	   {
	    logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	    "%s %s Negative table values suggest water "
            ...
            warnStr, routine, getType(), _owner->getID(), _id, getType() );
	   }
        }
        ...
        if( _myValue < 0 )
        {
	       ...
	       {
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	        "%s %s INITIALTRANSFERs value is negative "
                "(%f).  Replacing with 0 in %s %s %s.", warnStr, routine,
		_myValue, getType(), _owner->getID(), _id );
	       }
...
int Lookup3::buildTable( char** strTbl, int nlines )
{
    ...
    if( nlist == 0 || list == NULL )
    {
        ...
        /*AVCP PrintError( routine, */
        logMessageWithArgsAndExitOnError( FATAL_LEVEL,
        "%s %s Troubles getting data for %s '%s'.", errorStr, routine,
        getType(), _id );
        //return( STATUS_FAILURE );
    }
    n_col = nlist;
    ...
    if( _colVar == INDEX_DATE )
    {
        ...
        for( i = 0; i < n_col; i++ )
        {
            ...
            if( month == 2 && day == 29 )
            {
                ...
                /*AVCP PrintError( routine,*/
	        logMessageWithArgsAndExitOnError( FATAL_LEVEL,
	        "%s %s Index date \"%s\" invalid--No leap years. "
                "(%s %s %s).", errorStr, routine, 
		list[i], getType(), _owner->getID(), _id );
            }
            else if( month > 12 || month < 1 || day > 31 || day < 1 )
            {
                ...
                /*AVCP PrintError( routine,*/
	        logMessageWithArgsAndExitOnError( FATAL_LEVEL,
	        "%s %s Column index date \"%s\" invalid "
                "(%s %s %s).", errorStr, routine, 
		getType(), getType(), _owner->getID(), _id );
            }
            else
            {
        ...
        if( totErrs )
        {
            ...
            /*AVCP PrintError( routine,*/
	    logMessageWithArgsAndExitOnError( FATAL_LEVEL,
	    "%s %s Trouble with column index dates. (%s %s %s).",
            errorStr, routine, getType(), _owner->getID(), _id );
            //return( STATUS_FAILURE );
        }
    ...
    else
    {
        ...
        if( _colVar == INDEX_TS && strcmp( _colOrigUnits, "\0" ) )
        {
		    ...
		    {
		    logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		    "%s %s Unable to convert units %s to %s "
                    "for %s-type time series (%s %s %s).", errorStr, routine,
		    _colOrigUnits, tsUnits, id.getType(), getType(), 
		    _owner->getID(), _id );
		    }
                }
            }
        ...
        for( i = 0; i < n_col; i++ )
        {
            if( !IsDouble( list[0] ) )
            {
                ...
                /*AVCP PrintError( routine,*/
	        logMessageWithArgsAndExitOnError( FATAL_LEVEL,
	        "%s %s Non-date column indexes must be "
                "numerical values for %s %s %s.", errorStr, routine, 
		getType(), _owner->getID(), _id );
            }
            else
            {
    ...
    if( _rowVar == INDEX_TS && strcmp( _rowOrigUnits, "\0" ) )
    {
        ...
        if( _colOrigUnits[0] != '*' )
        {
                ...
                "" ) )
            {
                ...
                /*AVCP PrintError( routine, */
		logMessageWithArgsAndExitOnError( FATAL_LEVEL,
	        "%s %s Unable to convert units %s to %s for "
                "%s-type time series (%s %s %s).", errorStr, routine, 
		_rowOrigUnits, tsUnits, id.getType(), getType(), 
		_owner->getID(), _id );
            }
        }
    }
    ...
    for( i=1; i<nlines; i++ )
    {
        ...
        if( nlist != n_col + 1 )
        {
            ...
            /*AVCP PrintError( routine,*/
	    logMessageWithArgsAndExitOnError( FATAL_LEVEL,
	    "%s %s Number of columns (%d) and number of data "
            "fields on row %d (%d) do not agree. (%s %s %s)", errorStr, routine,
	    n_col, i, nlist - 1, getType(), _owner->getID(),_id );
            //return( STATUS_FAILURE );
        }
        ...
        if( _rowVar == INDEX_DATE )
        {
	        ...
	        logMessageWithArgsAndExitOnError( FATAL_LEVEL,
	        "%s %s Index date \"%s\" invalid--No leap years. "
                "(%s %s %s).", errorStr, routine, list[0], getType(),
	       	_owner->getID(), _id );
            }
            else if( month > 12 || month < 1 || day > 31 || day < 1 )
            {
                ...
                /*AVCP PrintError( routine, */
		logMessageWithArgsAndExitOnError( FATAL_LEVEL,
	        "%s %s Row index date \"%s\" invalid "
                "(%s %s %s).", errorStr, routine, getType(), 
		getType(), _owner->getID(), _id );
            }
            else
            {
        ...
        else
        {
            ...
            if( !IsDouble( list[0] ) )
            {
                ...
                /*AVCP PrintError( routine, */
		logMessageWithArgsAndExitOnError( FATAL_LEVEL,
	        "%s %s Non-date row indexes must be "
                "numerical values for %s %s %s.", errorStr, routine, getType(),
                _owner->getID(), _id );
            }
            else
            {
    ...
    else
    {
        ...
        for( i = 1; i< n_col; i++ )
        {
            if( colI[i] <= colI[i-1] )
            {
                ...
                /*AVCP PrintError( routine,*/
	        logMessageWithArgsAndExitOnError( FATAL_LEVEL,
	        "%s %s Column indexes must be in ascending (or "
                "chronological) order. (%s %s %s)", errorStr, routine, 
		getType(), _owner->getID(), _id );
            }
        }
        for( i = 1; i< n_row; i++ )
        {
            if( rowI[i] <= rowI[i-1] )
            {
                ...
                /*AVCP PrintError( routine,*/
	        logMessageWithArgsAndExitOnError( FATAL_LEVEL,
	        "%s %s Row indexes must be in ascending (or "
                "chronological) order. (%s %s %s)", errorStr, routine,
		getType(), _owner->getID(), _id );
            }
        }
    ...
    if( _table.populate( rowI, n_row, colI, n_col, data, n_col * n_row ) )
    {
        ...
        /*AVCP PrintError( routine,*/
        logMessageWithArgsAndExitOnError( FATAL_LEVEL,
        "%s %s Problems creating table.  Review nearby "
        "warnings. chronological) order. (%s %s %s)", errorStr, routine,
	getType(), _owner->getID(), _id );
        //return( STATUS_FAILURE );
    }
...
int Lookup3::key_BLENDTBL_BLENDTS( char **list, int nlist, int type )
{
    ...
    if( nlist > 1 )
    {
        ...
        if( !IsInteger( list[1] ) )
        {
            ...
            /*AVCP PrintError( routine,*/
	    logMessageWithArgsAndExitOnError( FATAL_LEVEL,
  	    "%s %s %s value '%s' is not an integer.", errorStr, routine,
            list[0], list[1] );
        }
        else
        {
    ...
    else
    {
        ...
        "method keyword %s.", errorStr, routine, _type, list[0] );
    }
...
int Lookup3::key_COLUMNVAR( char **list, int nlist, char *re_list )
{
    ...
    if ( nlist < 2 )
    {
        /*AVCP PrintError( routine, */
        logMessageWithArgsAndExitOnError( FATAL_LEVEL,
        "%s %s DATE, INPUT_TS or Component State "
        "sub-keyword required immediately after LOOKUP3 method "
        "keyword %s. (%s %s %s)", errorStr, routine, list[0], getType(),
       	_owner->getID(), _id );
        //return STATUS_FAILURE;
    }
    ...
    if( _colVar )
    {
        ...
        /*AVCP PrintError( routine,*/
        logMessageWithArgsAndExitOnError( FATAL_LEVEL,
        "%s %s '%s' is a duplicate usage of the '%s' keyword.", 
	errorStr, routine, re_list, list[0] );
    }
    ...
    else
    {
        ...
        //char *CompStatement = list[1];  -- CP update from cda 4/28/10
	...
	// to create one bigger string containing the entire expression.
        if( nlist > 3 )
        {
            ...
            {
                // Add a space
                CompStatement = strcat( CompStatement, " " );
                // Add the next piece
                CompStatement = strcat( CompStatement, list[i] );
            } CP update from cda 4/28/10 */
        ...
        if( !_colExpr )
        {
            ...
            /* AVCP PrintError( routine,*/
	    logMessageWithArgsAndExitOnError( FATAL_LEVEL,
	    "%s %s Could not interpret Component State on %s %s "
            "%s. Line: \"%s\".", errorStr, routine, getType(), 
	    _owner->getID(), _id, re_list );
        }
        ...
        else if( _colExpr->verify() )
        {
           ...
           // Not created correctly.
            ...
            /*AVCP PrintError( routine,*/
	    logMessageWithArgsAndExitOnError( FATAL_LEVEL,
  	    "%s %s Unable to verify constant values for \"%s\". "
            "(%s %s %s)", errorStr, routine, CompStatement, getType(),
	    _owner->getID(), _id );
        }
    ...
    if( _colVar != INDEX_TS &&  _colIndex )
    {
       ...
       {
        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
        "%s %s COLVAR parameterized to not use the "
        "specified TSINDEX COLUMNS time series. (%s %s %s)", 
	warnStr, routine, getType(), _owner->getID(), _id );
       }
    }
...
int Lookup3::key_INTERPOLATE( char **list, int nlist )
{
    ...
    if ( nlist < 2 )
    {
        /*AVCP PrintError( routine,*/
        logMessageWithArgsAndExitOnError( FATAL_LEVEL,
	"%s %s Direction sub-keyword 'ROWS', 'COLUMNS', ALL' or "
        "'BOTH'  required immediately after %s method keyword %s.", 
        errorStr, routine, getType(), list[0] );
        //return STATUS_FAILURE;
    }
...
int Lookup3::key_ROWVAR( char **list, int nlist, char *re_list )
{
	...
	// Otherwise, _rowOrigUnits remains as initialized (probably "\0").
        ...
        //char *CompStatement = list[1]; -- CP update code from cda 04/28/10
	char CompStatement[MAXC];
	strcpy( CompStatement, list[1] );
	...
	// to create one bigger string containing the entire expression.
        if( nlist > 3 )
        {
            ...
            /*for( i=3; i<nlist; i++ )
            {
                ...
                // Add the next piece
                CompStatement = strcat( CompStatement, list[i] );
            } CP update code from cda 04/28/10 */
	    ...
	    logMessageWithArgsAndExitOnError( FATAL_LEVEL,
	    "%s %s Could not interpret Component State on %s %s "
            "%s. Line: \"%s\".", errorStr, routine,
	    getType(), _owner->getID(), _id, re_list );
        }
        // Now check if it was created correctly.
	    ...
	    logMessageWithArgsAndExitOnError( FATAL_LEVEL,
	    "%s %s Unable to verify constant values for \"%s\".",
            errorStr, routine, CompStatement );
        }
        // Set the conversion factor that was, or could have been, used to
        // convert constants within the expression.  This conversion will be
        // appropriate also for column index values on the table.  This is not
    ...
    if( _rowVar != INDEX_TS &&  _rowIndex )
    {
       ...
       {
        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
        "%s %s ROWVAR parameterized to not use the "
        "specified TSINDEX ROWS time series. (%s %s %s)", warnStr, routine,
	getType(), _owner->getID(), _id );
       }
    }
...
int Lookup3::key_TABLEVAR( char **list, int nlist, char *re_list )
{
    ...
    if ( nlist < 2 )
    {
        /*AVCP PrintError( routine,*/
        logMessageWithArgsAndExitOnError( FATAL_LEVEL,
        "%s %s WITHDRAW(AL), RELEASE, DIVERSION or "
        "AUGMENTATION sub-keyword required immediately after "
        "LOOKUP3 method keyword %s.", errorStr, routine, list[0] );
        //return STATUS_FAILURE;
    }
            ...
            (_tableVar == TVAR_RELEASE || _tableVar == TVAR_WITHDRAWAL ) ) )
    {
        ...
        /*AVCP PrintError( routine,*/
       logMessageWithArgsAndExitOnError( FATAL_LEVEL,
       "%s %s '%s' not allowed on %s owned by %s %s.", errorStr, routine,
    ...
    switch( _tableVar )
    {
        ...
        case TVAR_RELEASE:
            if( _withdraw_obs || _augment_obs || _diversion_obs )
            {
                ...
                /*AVCP PrintError( routine, */
		logMessageWithArgsAndExitOnError( FATAL_LEVEL,
	        "%s %s %s %s not compatible with "
                "TSINPUT OBSERVEDWITHDRAWAL, OBSERVEDAUGMENTATION "
                "and / or OBSERVEDDIVERSION. (%s %s %s)", errorStr, routine,
        ...
        case TVAR_WITHDRAWAL:
            if( _release_obs || _diversion_obs )
            {
                ...
                /*AVCP PrintError( routine, */
		logMessageWithArgsAndExitOnError( FATAL_LEVEL,
	        "%s %s %s %s not compatible with TSINPUT "
                "OBSERVEDRELEASE and / or OBSERVEDDIVERSION. (%s %s %s)",
        ...
        case TVAR_DIVERSION:
	        ...
	        logMessageWithArgsAndExitOnError( FATAL_LEVEL,
	        "%s %s %s %s not compatible with TSINPUT "
                "OBSERVEDRELEASE and / or OBSERVEDWITHDRAWAL. (%s %s %s)",
                errorStr, routine, list[0], list[1], getType(), 
		_owner->getID(), _id );
            }
            break;
        ...
        case TVAR_AUGMENTATION:
	        ...
	        logMessageWithArgsAndExitOnError( FATAL_LEVEL, 
	        "%s %s %s %s not compatible with TSINPUT "
                "OBSERVEDRELEASE. (%s %s %s)", errorStr, routine,
                list[0], list[1], getType(), _owner->getID(), _id );
            }
            break;
        default:
...
int Lookup3::key_TOCOMP( char **list, int nlist )
{
    ...
    if ( nlist < 3 )
    {
        ...
        /*AVCP PrintError( routine,*/
        logMessageWithArgsAndExitOnError( FATAL_LEVEL,
	"%s %s Component ID and mode required "
        "immediately after keyword %s. (%s %s %s)", 
	errorStr, routine, list[0], getType(), _owner->getID(), _id );
        //return STATUS_FAILURE;
    }
    ...
    if( _receivingComp == NULL )
    {
        /*AVCP PrintError( routine,*/
        logMessageWithArgsAndExitOnError( FATAL_LEVEL,
	"%s %s ToComp %s not found in Component tree. "
        "(%s %s %s)", errorStr, routine,
    ...
    if( !strcasecmp( list[2], "INSTANTANEOUS" ) )
    {
		...
		//printf("ownNum = %d, CompNum = %d\n", ownNum, CompNum);
        if ( ownNum > CompNum )
        {
            //totErrs++;
            /*AVCP PrintError( routine, */
	    logMessageWithArgsAndExitOnError( FATAL_LEVEL,
	    "%s %s %s transfer to Component %s is unacceptable "
            "due to solution order of components. (%s %s %s)", 
	    errorStr, routine, list[2], list[1], getType(), 
	    _owner->getID(), _id );
        }
        _toCompMode = 0;
    }
    else if( strcasecmp( list[2], "NEXTSTEP" ) )
    {
        ...
        /*AVCP PrintError( routine,*/
	logMessageWithArgsAndExitOnError( FATAL_LEVEL,
       	"%s %s Keyword \"%s\" unacceptable for transfer ToComp. (%s %s %s)", 
	errorStr, routine, list[2], getType(), _owner->getID(), _id);
    }
    ...
    if( _Comp_ts.allocateDataSpace() )
    {
        /*AVCP PrintWarning( 1, routine,*/
        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	"%s %s Troubles allocating data space for transfer "
        "time series on %s.", errorStr, routine, _id );
        return( STATUS_FAILURE );
    }
    ...
    if( _receivingComp->setInflowTS( &_Comp_ts) )
    {
        ...
        /*AVCP PrintError( routine,*/
        logMessageWithArgsAndExitOnError( FATAL_LEVEL,
	"%s %s Troubles setting TOCOMP timeseries on %s. (%s %s %s)",
        errorStr, routine, _receivingComp->_id, getType(), _owner->getID(),_id);
        //return STATUS_FAILURE;
    }
...
int Lookup3::key_TSINDEX( char **list, int nlist, char *re_list )
{
    ...
    if ( nlist < 3 )
    {
        /*AVCP PrintError( routine, */
        logMessageWithArgsAndExitOnError( FATAL_LEVEL,
	"%s %s Sub-keyword ROWS or COLUMNS and timeseries "
    ...
    if( !strcasecmp( list[1], "ROWS" ) )
    {
        ...
        if( _rowIndex )
        {
            ...
            /*AVCP PrintError( routine,*/
	    logMessageWithArgsAndExitOnError( FATAL_LEVEL,
	    "%s %s '%s' is a duplicate usage of the '%s %s' keywords. "
	    "(%s %s %s)", errorStr, routine, re_list, list[0], list[1],
            getType(), _owner->getID(), _id );
        }
        ...
        if( _rowIndex == NULL )
        {
            ...
            /*AVCP PrintError( routine,*/
	    logMessageWithArgsAndExitOnError( FATAL_LEVEL,
	    "%s %s Troubles setting index timeseries \"%s\". (%s %s %s)", 
	    errorStr, routine, list[2], getType(), _owner->getID(), _id );
        }
    }
    else if( !strcasecmp( list[1], "COLUMNS" ) )
    {
        ...
        if( _colIndex )
        {
            ...
            /*AVCP PrintError( routine,*/
	    logMessageWithArgsAndExitOnError( FATAL_LEVEL,
	    "%s %s '%s' is a duplicate usage of the "
            "'%s %s' keywords. (%s %s %s)", errorStr, routine, re_list,
	    list[0], list[1], getType(), _owner->getID(), _id );
        }
...
int Lookup3::key_TSINPUT( char **list, int nlist, char *re_list )
{
    ...
    if ( nlist < 3 )
    {
        /*AVCP PrintError( routine,*/
        logMessageWithArgsAndExitOnError( FATAL_LEVEL,
        "%s %s Sub-keyword OBSERVEDWITHDRAW, OBSERVEDDIVERSION, "
    ...
    if( strncasecmp( list[1], "OBSERVED", 8 ) )
    {
       ...
       if( getFewsDebugFlag() > 0 )
       {
        ...
        "OBSERVEDAUGMENTATION or OBSERVEDRELEASE and timeseries alias "
        "required immediately after keyword %s. (%s %s %s)",
        warnStr, routine, list[0], getType(), _owner->getID(), _id );
       }
        ...
        if( testTS == NULL )
        {
            /*AVCP PrintError( routine,*/
	    logMessageWithArgsAndExitOnError( FATAL_LEVEL,
  	    "%s %s Troubles setting observed timeseries \"%s\". (%s %s %s)", 
	    errorStr, routine, list[2], getType(), _owner->getID(), _id );
        }
        return STATUS_FAILURE;
    }
    ...
    if( !strncasecmp( list[1], "OBSERVEDRELEASE", 15 ) )
    {
	    ...
	    logMessageWithArgsAndExitOnError( FATAL_LEVEL,
	    "%s %s '%s' is a duplicate usage of the "
            "'%s %s' keywords. (%s %s %s)", errorStr, routine,
	    re_list, list[0], list[1], getType(), _owner->getID(), _id );
        }
        ...
        if( _withdraw_obs || _augment_obs || _diversion_obs )
        {
            ...
            /*AVCP PrintError( routine,*/
	    logMessageWithArgsAndExitOnError( FATAL_LEVEL,
	    "%s %s %s %s not compatible with other "
            "%s OBSERVEDWITHDRAWAL,  OBSERVEDAUGMENTATION and / or "
            "OBSERVEDDIVERSION. (%s %s %s)", errorStr, routine, list[0], 
	    list[1], list[0], getType(), _owner->getID(), _id );
        }
        ...
        if( _ownerType != CM_RESERVOIR)
        {
            ...
            /*AVCP PrintError( routine,*/
	    logMessageWithArgsAndExitOnError( FATAL_LEVEL,
	    "%s %s %s %s compatible only with RESERVOIR "
            "components. (%s %s %s)", errorStr, routine,
	    list[0], list[1], getType(), _owner->getID(), _id );
        }
        ...
        if( _release_obs == NULL )
        {
            ...
            /*AVCP PrintError( routine,*/
	    logMessageWithArgsAndExitOnError( FATAL_LEVEL,
	    "%s %s Troubles setting observed timeseries \"%s\". "
            "(%s %s %s)", errorStr, routine, list[2], getType(), 
	    _owner->getID(), _id );
        }
    }
    else if( !strncasecmp( list[1], "OBSERVEDWITHDRAW", 16 ) )
    {
	    ...
	    "%s %s %s %s not compatible with other "
            ...
            errorStr, routine, list[0], list[1], list[0], 
	    getType(), _owner->getID(),_id );
        }
        ...
        if( _ownerType != CM_RESERVOIR)
        {
            ...
            /*AVCP PrintError( routine,*/
	    logMessageWithArgsAndExitOnError( FATAL_LEVEL,
	    "%s %s %s %s compatible only on RESERVOIR "
            "components. (%s %s %s)", errorStr, routine, list[0], list[1], 
	    getType(), _owner->getID(), _id );
        }
	    ...
	    "%s %s Troubles setting observed timeseries \"%s\". "
            "(%s %s %s)", errorStr, routine,
	    list[2], getType(), _owner->getID(), _id );
        }
    }
    else if( !strncasecmp( list[1], "OBSERVEDDIVERSION", 17 ) )
    {
        ...
        if( _ownerType != CM_NODE)
        {
            ...
            /*AVCP PrintError( routine,*/
	    logMessageWithArgsAndExitOnError( FATAL_LEVEL,
	    "%s %s %s %s compatible only on NODE "
            "components. (%s %s %s)", errorStr, routine, list[0], list[1], 
	    getType(), _owner->getID(), _id );
        }
    ...
    else if( !strncasecmp( list[1], "OBSERVEDAUGMENT", 15 ) )
    {
	    ...
	    logMessageWithArgsAndExitOnError( FATAL_LEVEL,
	    "%s %s %s %s not compatible with other "
            "%s OBSERVEDRELEASE. (%s %s %s)", errorStr, routine, list[0], 
	    list[1], list[0], getType(), _owner->getID(),_id );
        }
    ...
    switch( _tableVar )
    {
        ...
        case TVAR_RELEASE:
            if( _withdraw_obs || _augment_obs || _diversion_obs )
            {
                ...
                /*AVCP PrintError( routine,*/
	        logMessageWithArgsAndExitOnError( FATAL_LEVEL,
		"%s %s %s %s not compatible with TSINPUT OBSERVEDWITHDRAWAL "
                ", OBSERVEDAUGMENTATION and / or OBSERVEDDIVERSION. (%s %s %s)",
                errorStr, routine, list[0], list[1], 
		getType(), _owner->getID(), _id );
            }
            break;
        ...
        case TVAR_WITHDRAWAL:
            if( _release_obs || _diversion_obs )
            {
                ...
                /*AVCP PrintError( routine,*/
	        logMessageWithArgsAndExitOnError( FATAL_LEVEL,
	        "%s %s %s %s not compatible with TSINPUT OBSERVEDRELEASE "
		"and / or OBSERVEDDIVERSION. (%s %s %s)", errorStr, routine,
                list[0], list[1], getType(), _owner->getID(), _id );
            }
            break;
		...
		logMessageWithArgsAndExitOnError( FATAL_LEVEL,
	        "%s %s %s %s not compatible with TSINPUT OBSERVEDRELEASE "
		"and / or OBSERVEDWITHDRAWAL. (%s %s %s)", errorStr, routine,
                list[0], list[1], getType(), _owner->getID(), _id );
            }
            break;
        // _tableVar = TVAR_AUGMENTATION is incompatible with TSINPUT releases.
        ...
        case TVAR_AUGMENTATION:
            if( _release_obs )
            {
                ...
                logMessageWithArgsAndExitOnError( FATAL_LEVEL,
	        "%s %s %s %s not compatible with TSINPUT "
	        "OBSERVEDWITHDRAWAL. (%s %s %s)", errorStr, routine,
                list[0], list[1], getType(), _owner->getID(), _id );
            }
            break;
        default:
...
int Lookup3::key_VALUES( char **list, int n_items, char **re_list, int *rowCount )
{
    ...
    if( value_list == NULL || n_value == 0 )
    {
        ...
        /*AVCP PrintError( routine, */
	logMessageWithArgsAndExitOnError( FATAL_LEVEL,
        "%s %s Troubles forming sub_list beginning with %s. "
        "Does \"END%s\" exist? (%s %s %s)", errorStr, routine, list[0], 
    ...
    else if( !_colVar || !_rowVar || !_tableVar )
    {
        ...
        /*AVCP PrintError( routine,*/
       logMessageWithArgsAndExitOnError( FATAL_LEVEL,
       "%s %s Cannot understand VALUES section without "
       "prior knowledge of 'COLUMNVAR', 'ROWVAR' and 'TABLEVAR'. "
       "(%s %s %s)", errorStr, routine, getType(), _owner->getID(), _id );
    }
    ...
    else if( buildTable( value_list, n_value ) )
    {
        ...
        /*AVCP PrintError( routine,*/
        logMessageWithArgsAndExitOnError( FATAL_LEVEL,
        "%s %s Table not successfully read. (%s %s %s)",
        errorStr, routine, getType(), _owner->getID(), _id );
    }
...
int Lookup3::key_WEEKLYVARIATION( char **list, int nlist )
{
    ...
    if( nlist -1  != requiredSize )
    {
        /*AVCP PrintError( routine,*/
        logMessageWithArgsAndExitOnError( FATAL_LEVEL,
        "%s %s Keyword %s must preceed %d scaling "
        "values instead of %d. (%s %s %s)", errorStr, routine, list[0], 
	requiredSize, nlist-1, _type, _owner->_id, _id );
        //return STATUS_FAILURE; 
    }
...
{static char rcs_id1[] = "$Source: /fs/hseb/ob81/ohd/ofs/src/resj_topology/RCS/Lookup3_construct.cxx,v $";
 static char rcs_id2[] = "$Id: Lookup3_construct.cxx,v 1.1 2006/10/26 15:24:26 hsu Exp $";}
/*  ===================================================  */
