...
int Component :: initialize()
{
	...
	if( !_expr_list ){
		...
		"%s %s Unable to allocate memory for %d Expression*.", 
		warnStr, routine, MAX_METHOD );
	...
	if( !_method_list ){
		/*PrintWarning( 1, routine,*/
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		"%s %s Unable to allocate memory for %d Method*.", 
		warnStr, routine, MAX_METHOD );
		return( STATUS_FAILURE );
	}
	...
	if( _outflow_ts.allocateDataSpace() ) {
		...
		" for outflow time series on %s.", warnStr, routine, _id );
		return( STATUS_FAILURE );
	}
