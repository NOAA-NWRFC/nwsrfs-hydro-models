...
int RainEvap::construct ( char** re_list, int n_items )  
{
	...
	for( i = 0; i < n_items; i++ ) { 
		...
		if( list == NULL || nlist == 0 ) {
			...
			"%s (%s) - Troubles getting data for "
			"RainEvap %s on %s.", errorStr, routine, 
			_id, _owner->_id );
			/* AVCP 	continue; */
		}
		...
		if( !strcasecmp( list[0], "TSINPUT" ) ) {
			...
			if( !strcasecmp( list[1], "OBSERVEDPRECIP" ) ) {
				...
				"method.", errorStr, routine, list[1] );
				/* AVCP 	continue;*/
			}
			continue;
		}
		...
		if( !strcasecmp( list[0], "PRECIP" ) ) {
		   ...
		   "%s PRECIP keyword found, but not "
		   "needed in %s %s on %s. ", 
		   routine, _type, _id, _owner->_id );
		   }
/*
			...
			if( precip_list ) {
				...
				totErrs++;
			        ...
			        logMessageWithArgsAndExitOnError( FATAL_LEVEL,
				    "%s (%s) It appears you have "
					"two PRECIP sections for %s %s on %s. "
					"Confused.",errorStr, routine, _type, _id, 
					_owner->getID() );
				continue;
			}
			...
			if( precip_list == NULL || n_precip == 0 ) {
				...
				totErrs++;
			        ...
			        logMessageWithArgsAndExitOnError( FATAL_LEVEL,
				       "%s (%s) PRECIP -- ENDPRECIP "
					"section in %s %s on %s malformed. ", 
					errorStr, routine, _type, _id, _owner->_id );
				if( precip_list ) {
			...
			if( p_val_list == NULL || n_precip_val == 0 ) {
				...
				totErrs++;
					...
					"%s on %s malformed.",errorStr, routine, _type, _id,
					_owner->_id );
				if( p_val_list ) {
		...
		if( !strcasecmp( list[0], "EVAP" ) ) {
			...
			if( e_val_list == NULL || n_evap_val == 0 ) {
			   ...
			   /* AVCP  totErrs++;
			    ...
			    "ENDVALUES section but does not.  Look "
			    "for both keywords in your input.  %s "
			    "%s on %s malformed.", errorStr, routine,
...
*/
			   ...
			   /* AVCP totErrs++;
			    PrintError( routine, */
	                    logMessageWithArgsAndExitOnError( FATAL_LEVEL, 
	                    "%s (%s) - VALUES -- ENDVALUES "
			    "section should ONLY be within an EVAP -- ENDEVAP "
  			    "section but is not.  %s %s on %s "
			    "malformed.", errorStr, routine, 
			    _type, _id, _owner->_id );
/*
...
*/
		...
		if( !strcasecmp( list[0], "ENDPRECIP" ) ) {
		   ...
		   if ( getFewsDebugFlag() >= 3 )
	           {
		      logMessageWithArgsAndExitOnError( DEBUG_LEVEL, 
		        "%s ENDPRECIP keyword found, but not "
		        "needed in %s %s on %s. ", 
		         routine, _type, _id, _owner->_id );
		   }
		}
			...
			if( nlist > 2 ) {
				...
				if( nlist != 2 + ( 24 / _t_mult ) ) {
					...
					PrintError( routine, */
				        ...
				        FATAL_LEVEL,
				        "%s (%s) %s %s "
				        "RainEvap must have one and only one "
				        "distribution value for each timestep.",  			              errorStr, routine, _owner->getID(), _id );
					/* AVCP continue;*/
				}
					...
					|| dist_tally < 0.99999 ) 
				{
				     ...
				     PrintError( routine, */
				        ...
				        FATAL_LEVEL,
				        "%s (%s) - Distribution on" 
					"%s %s RainEvap does not sum to 1.0.", 
					errorStr, routine, 
					_owner->getID(),_id );
		        ...
		        {
			    logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
			    "(%s) - Date: %s Evap Value: %4.2f",
			    routine, list[0], _evap_ctl.getDataValue(date, 1) );
			}	
			list = FreeStringList( list );
			continue;
		}
...
*/	
	if( _evap_obs ) {
		...
		if( _n_precip == 0 ) {
		   ...
		   if( getFewsDebugFlag() > 0 )
		   {
			...
			warnStr, routine, _type, _id, _owner->_id );
		   }
		}
	}
	...
	if( _precip_obs || _evap_obs || _n_precip > 0 || _n_evap > 0 ) {
	   ...
	   PrintError( routine, */
	   logMessageWithArgsAndExitOnError( FATAL_LEVEL, 
	      "%s (%s) - %s %s on %s has no source of precip or "
	      "evap.", errorStr, routine, _type, _id, _owner->_id );
	}
