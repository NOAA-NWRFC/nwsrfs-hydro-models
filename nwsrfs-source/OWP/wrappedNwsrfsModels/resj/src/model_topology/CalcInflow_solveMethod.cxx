...
if( !strcasecmp( _owner->_id, "RESB" ) ){
	...
	sprintf( temp, "date=%s\nendStor=%f\nstartStor=%f\nnetLossVol=%f\n"
		...
		"startWith=%f\nendInfl=%f\nstartInfl=%f\n_meanToInstErr=%f\n"
		"origInfl=%f\n",
	       	cur_date.toString(), endStor, startStor, netLossVol, 
		_remainingVol, endRel, startRel,
	       	endWith, startWith, endInflAvg, startInfl, _meanToInstErr, 
		endInfl);
	/*AVCP int ipr = ResJSys::getIPR(); 
	int length = strlen( temp );
	ResJ_ccwrite( &length, temp, &ipr );*/
	logMessageWithArgsAndExitOnError( DEBUG_LEVEL, "%s %s", routine, temp );
}
	...
	if( _remainingVol < -997.5 && _remainingVol > -1000 ) {
		...
		"%s %s _remainingVol was adjusted to not "
		"appear to be missing for %s %s %s.", warnStr, routine,
