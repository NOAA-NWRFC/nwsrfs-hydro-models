...
TSDate _curDate; // bug #562 5/29/12
	...
	TSDate &cur_date, HourTS &inFromOwnRes )
{
	...
	if( this == ucp && ucp != NULL ) {
        ...
        _curDate = cur_date;  //bug #562 5/29/12
...
int Component :: buildSubTree_specialTieIn ( )
{
	...
	if( _specialTieIn ) {
		...
		for( i = 0; i < _in_count; i++ ) {
			if( _inflow_ts[i] == NULL ) {
				/*AVCP PrintError( routine,*/
			        ...
			        "%s %s Troubles checking "
				"inflow timeseries for MaxStage %s "
				"subtree.", errorStr, routine, _id );
		...
		if( numTies != _specialTieIn ) {
			...
			/*AVCP PrintError( routine,*/
		        logMessageWithArgsAndExitOnError( FATAL_LEVEL,
		        "%s %s Copied component %s is missing "
			...
			"ties were made: %s", errorStr, routine, _id, Ties );
			/*AVCP return ( STATUS_FAILURE );*/
		}
	}
