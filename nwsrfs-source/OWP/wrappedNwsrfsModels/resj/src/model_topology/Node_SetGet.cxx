...
int Node::buildMinimumTS( char** min, int nmin )
{
    ...
    for( i = 0; i < nmin; i++ )
    {
	    ...
	    "%s %s Troubles getting minimum flow time series "
            "for %s  %s.", errorStr, routine, getType(),_id );
        }
        ...
        if ( nlist != 2 )
        {
            ...
            /*AVCP PrintError( routine,*/
	    logMessageWithArgsAndExitOnError( FATAL_LEVEL,
	    "%s %s Two fields expected in MINDISCHARGE section "
            "of %s %s. (Line: %s).", errorStr, routine, getType(), getID(), 
	    min[i] );
        }
        else
        {
            ...
            if( i > 0 )
            {
                if( date <= prevDate )
                {
                    ...
                    /*AVCP PrintError( routine,*/
		    logMessageWithArgsAndExitOnError( FATAL_LEVEL,
		    "%s %s MinDischarge dates must be chrological. (%s %s).", 
		    errorStr, routine, getType(), _id );
                }
            }
            prevDate = date;
            ...
            if( !IsDouble( list[1] ) )
            {
                ...
                /*AVCP PrintError( routine,*/
	        logMessageWithArgsAndExitOnError( FATAL_LEVEL,
	        "%s %s MinDischarge value '%s' not in a "
                "recognizable format. (%s %s).", errorStr, routine, 
		list[1], getType(), _id );
            ...
            if( value < 0 )
            {
                ...
                /*AVCP PrintError( routine, */
	        logMessageWithArgsAndExitOnError( FATAL_LEVEL,
	        "%s %s MinDischarge value '%s' cannot be negative. (%s %s).", 
		errorStr, routine, list[1], getType(), _id );
            }
...
int Node :: setCOstring( )
{
    ...
    char temp_str[30+16+40+1];    // Labels, values, future space, end
    ...
    sprintf( temp_str, "%-12.12s%-12.12s%6.6s%16.16s%40.40s", _type, _id,
        "-99999", value_str, future_str );
    ...
    if( ResJSys::addCOString( temp_str ) )
    {
        ...
        "%s (%s) - Troubles adding carryover data to CO array.", 
	warnStr, routine );
...
int Node :: setStates( char** states, int n_items )
{
	...
	for( i = 0; i < n_items; i++ ) {
		...
		list = BreakStringList( states[i], " \n\t",
			...
			/*AVCP PrintError( routine, */
		        logMessageWithArgsAndExitOnError( FATAL_LEVEL,
			"%s %s Troubles getting %s state "
			"data values for %s (Line: \'%s\').", errorStr, routine,                        _type, _id );
			//continue;
		}
				...
				//totErrs++;
                                /*AVCP PrintError( routine, */
			        logMessageWithArgsAndExitOnError( FATAL_LEVEL,
				"%s %s Keyword RATING_CURVE required "
                                "immediately after MAXSTAGE "
                                "method keyword %s (Node %s).", 
				errorStr, routine, list[0], _id );
                                //list = FreeStringList( list );
                                //continue;
                        }
				...
				//totErrs++;
                                /*AVCP PrintError( routine,*/
			        logMessageWithArgsAndExitOnError( FATAL_LEVEL,
			        "%s %s Problems creating sub-list beginning "
				"with line \"%s\".  Perhaps "
                                "no values or no 'END%s'? (Node %s).",
                                errorStr, routine, states[i], "TABLE", _id );
                                //continue;
                        }
				...
				//totErrs++;
                                /*AVCP PrintError( routine,*/
			        logMessageWithArgsAndExitOnError( FATAL_LEVEL,
			        "%s %s Troubles filling rating curve for Node "
				"%s.", errorStr, routine, _id );
                                //continue;
                        }
				...
				/*AVCP PrintError( routine, */
			        logMessageWithArgsAndExitOnError( FATAL_LEVEL,
				"%s %s Rating TABLE found, as well"
				" as a RATINGCURVEID; both cannot"
				" be defined at the same node.", errorStr,
			        routine );
                                //continue;
			}
		...
		if( !strcasecmp( list[0], "RATINGCURVEID" ) ) {
			...
			idlen = strlen(list[1]);
				...
				//totErrs++;
                                /*AVCP PrintError(routine,*/
			        logMessageWithArgsAndExitOnError( FATAL_LEVEL,
			        "%s %s RATINGCURVEID string is longer "
                                "than 8 characters.", errorStr, routine );  
                                //continue;
                        }
			else {
				...
				/*AVCP PrintError( routine, */
			        logMessageWithArgsAndExitOnError( FATAL_LEVEL,
				"%s %s RATINGCURVEID found, as well"
		...
		if( !strcasecmp( list[0], "TSINPUT" ) ) {
			...
			if( strcasecmp( list[1], "INFLOW" ) ) {
				...
				/*AVCP PrintError( routine,*/
			        logMessageWithArgsAndExitOnError( FATAL_LEVEL,
				"%s %s Keyword INFLOW required "
				"immediately after reach keyword %s. "
				"%s is not valid.", errorStr, routine,
				list[0], list[1] );
				//list = FreeStringList( list );
				//continue;
			}
			...
			if( _n_son == 0 && _in_count > 0 ) {
				/*AVCP PrintWarning( 1, routine, */
			      ...
			      {
			        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
				"%s %s Applying non-primary flow \"%s\" at "
				"Node \"%s\".  At least one other flow "
				"has been defined as inflow to the "
				"node.", warnStr, routine, list[2], getID() );
			      }
			}
			...
			if( setInflowTS( (HourTS*)TSList::
				...
				/*AVCP PrintError( routine,*/
			        logMessageWithArgsAndExitOnError( FATAL_LEVEL,
			        "%s %s Troubles setting inflow "
				"timeseries \"%s\" on %s.", errorStr, routine,
				states[i], _id );
				//continue;
			}
			...
			if( !_inflow_ts[_in_count-1]->
				...
				/*PrintWarning( 2, routine, */
			     ...
			     {
			        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
				"%s %s Inflow time series %s for %s is not "
				"set as instantaneous.", warnStr, routine, 
				ts_id, _id );
			     }
			}
		list = FreeStringList( list );
		...
		// Handle keyword TSOUTPUT
			...
			if( !strcasecmp( list[1], "OUTFLOW" ) ) {
				...
				if( _output_ts[n_out] == NULL ) {
					...
					"%s as a valid Node %s timeseries.", 
					errorStr, routine, list[0], ts_id );
					//list = FreeStringList( list );
					//continue;
				}
			}
				...
				/*AVCP PrintError( routine, */
			        logMessageWithArgsAndExitOnError( FATAL_LEVEL,
			        "%s %s Keyword INFLOW or OUTFLOW required "
			...
			if ( !tempTS ) {
				...
				"'%s' in time series listing.", errorStr,
				routine, ts_id );
				//continue;
			}
...
//------------------------------------------------------------------------------
			   ...
			   {
			    logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
			    "%s %s Value required with keyword %s for "
			    "%s \"%s\". ", warnStr, routine, list[0], 
                            _type, _id );
			   }
                            ...
                            if( _prevDischarge < 0 )
                            {
                                ...
                                /*AVCP PrintError( routine, */
			       logMessageWithArgsAndExitOnError( FATAL_LEVEL,
			       "%s %s %s value must be non-negative. (%s %s)",
			   ...
			   if( getFewsDebugFlag() > 0 )
			   {
		        ...
		        {
		         logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
			 "%s %s Unrecognized keyword \"%s\" for Node line "
			 "\"%s\".", warnStr, routine, list[0], states[i] );
		        }
                ...
                else if( NULL != strstr( list[0], "DIVERSION" ) )
                {
                    ...
                    if( !strcasecmp( list[0], "PREVIOUSDIVERSION" ) )
                    {
                        if( nlist < 2 ) {
                            ...
                            /*AVCP PrintError( routine,*/
			    logMessageWithArgsAndExitOnError( FATAL_LEVEL,
			    "%s %s Keyword '%s' must have a value. (%s %s)", 
			    errorStr, routine, list[0], _type, _id );
                        }
                        else
                        {
                             ...
                             !strcasecmp( list[0], "INITIALDIVERSION" ) )
                    {
                        if( nlist < 2 )
                        {
                            ...
                            /*AVCP PrintError( routine,*/
			    logMessageWithArgsAndExitOnError( FATAL_LEVEL,
			    "Keyword '%s' must have a value. (%s %s)", 
			    errorStr, routine, list[0], _type, _id );
                        }
                        else
                        {
			...
			"\"%s\".", warnStr, routine, list[0], states[i] );
                ...
                else if( !strcasecmp( list[0], "MINREMAINDER" ) )
                {
                    ...
                    if( value_list == NULL || n_value == 0 )
                    {
                        ...
                        /*AVCP PrintError( routine,*/
		        logMessageWithArgsAndExitOnError( FATAL_LEVEL,
		        "%s %s Troubles handling %s %s.  Does "
                        "\"END%s\" exist?", errorStr, routine, _id, 
			list[0], list[0] );
                    }
                        ...
                        /*AVCP PrintError( routine, */
		        logMessageWithArgsAndExitOnError( FATAL_LEVEL,
			"%s %s Troubles building %S time series for %s %s.", 
			errorStr, routine, list[0], getType(), _id );
                    }
                    else
                    {
                        // We were successful.
	...
	if( _startDischarge == MISSING ) {
		...
		/*AVCP PrintError( routine,*/
	        logMessageWithArgsAndExitOnError( FATAL_LEVEL,
		"%s %s Keyword INITIALDISCHARGE and Value required but not "
	...
	if( _prevDischarge == MISSING ) {
		...
		/*AVCP PrintError( routine,*/
	        logMessageWithArgsAndExitOnError( FATAL_LEVEL,
		"%s %s Keyword PREVIOUSDISCHARGE and Value required but not "
		"found in %s \"%s\".", errorStr, routine, _type, _id );
	}
        ...
        if( _startInflow == MISSING ) {
	       ...
	       {
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	        "%s %s Calculated STARTINGINFLOW is negative.  Revising to 0. "
		"(%s %s)", warnStr, routine, getType(), _id );
	       }
                _startInflow = 0.0;
            }
        }
        if( _prevInflow == MISSING ) {
	       ...
	       {
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	        "%s %s Calculated PREVIOUSINFLOW is negative.  Revising to 0. "
		"(%s %s)", warnStr, routine, getType(), _id );
	       }
                _prevInflow = 0.0;
            }
...
{static char rcs_id1[] = "$Source: /fs/hseb/ob83/ohd/ofs/src/resj_topology/RCS/Node_SetGet.cxx,v $";
 static char rcs_id2[] = "$Id: Node_SetGet.cxx,v 1.13 2007/10/02 20:46:43 hsu Exp $";}
/*  ===================================================  */
