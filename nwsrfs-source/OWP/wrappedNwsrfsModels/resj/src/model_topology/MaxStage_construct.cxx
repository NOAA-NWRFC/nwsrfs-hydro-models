...
int MaxStage :: construct ( char** re_list, int n_items )  
{
	...
	for( i = 0; i < n_items; i++ ) {
		...
		if( list == NULL || nlist == 0) {
			...
			"%s %s Troubles getting data for MaxStage %s %s.",
		...
		if( !strcasecmp( list[0], "TABLE" ) ) {
			...
			if( _stage_flow_tbl.populate( &sub_list[1],
				...
				/*AVCP PrintError( routine,*/
			        logMessageWithArgsAndExitOnError( FATAL_LEVEL,	
			      	"%s %s Troubles filling rating curve for %s on"
				"%s.", errorStr, routine, _id, _owner->_id );
				//return( STATUS_FAILURE );
			}
			table_found = 1;
			list = FreeStringList( list );
		...
		// Handle keyword MAXIMUMSTAGE
			...
			if( nlist < 2 ) {
				...
				/*AVCP PrintError( routine,*/
			        logMessageWithArgsAndExitOnError( FATAL_LEVEL,
			        "%s %s Value required immediately after "
				"MAXSTAGE method keyword %s.", 
		...
		// Handle keyword MAXIMUMDISCHARGE
			...
			if( temp == NULL ) {
				...
				/*AVCP PrintError( routine,*/
			        logMessageWithArgsAndExitOnError( FATAL_LEVEL,
			        "%s %s Troubles getting downstream control "
				"NODE %s for %s on %s.", 
				errorStr, routine, list[1], _id, _owner->_id );
				//return( STATUS_FAILURE );
			}
			...
			_dcp = (Node*)temp;
			   ...
			   if( getFewsDebugFlag() > 0 )
			   {
			       ...
			       "following their correction.", warnStr, routine,
			       list[0], _id );
			   }
		}
	...
	if( _min_release == MISSING ) {
		...
		/*AVCP PrintError( routine, */
		logMessageWithArgsAndExitOnError( FATAL_LEVEL,
	        "%s %s Must specify a minimum release value for %s on %s.",
...
//		_criterion = 1.0;
		/*AVCP PrintError( routine,*/
	        logMessageWithArgsAndExitOnError( FATAL_LEVEL,
	        "%s %s Must specify a convergence criterion for %s on %s.",
	        errorStr, routine, _id, _owner->_id );
	}
	if( !_dcp ) {
		missingVital++;
		/*PrintError( routine,*/
	        logMessageWithArgsAndExitOnError( FATAL_LEVEL,
	        "%s %s Must specify a downstream control point for %s on %s.", 
		errorStr, routine, _id, _owner->_id );
	}
	if( (_max_stage == MISSING) && (_max_discharge == MISSING) ) {
		...
		/*AVCP PrintError( routine,*/
	        logMessageWithArgsAndExitOnError( FATAL_LEVEL,
	        "%s %s Neither MAXIMUMSTAGE nor MAXIMUMDISCHARGE is "
		"specified for %s on %s.", errorStr, routine, _id, _owner->_id );
	}

	/*AVCP if( missingVital ) {
		return( STATUS_FAILURE );
	}*/
	...
	if( (_max_stage != MISSING) && (_max_discharge != MISSING) ) {
		/* PrintError( routine, */
	        logMessageWithArgsAndExitOnError( FATAL_LEVEL,
	        "%s %s Both MAXIMUMSTAGE and MAXIMUMDISCHARGE are specified "
		"for %s on %s.", errorStr, routine, _id, _owner->_id );
		//return ( STATUS_FAILURE );
	}
	...
	// node); if MAXIMUMDISCHARGE is defined, no table or curve is required
        // (but it won't hurt if one is present, the table/curve will be
	// ignored)
	if (_max_discharge == MISSING) {
		// If no TABLE is present in this method, then a TABLE or 
	        // RATINGCURVEID must be defined at the downstream control 
	        // point (NODE).
		if( !table_found ) {
			...
			// no table at the NODE, and no RCID at the NODE either
				/*AVCP PrintError( routine,*/
			        logMessageWithArgsAndExitOnError( FATAL_LEVEL,
			        "%s %s Stage constraint used but no rating "
				"curve ID or table found at node or defined "
				"in method for %s on %s.", 
				errorStr, routine, _id, _owner->_id );
				/*AVCP missingVital++;*/
				/*AVCP PrintError( routine, "Must specify a "
				  "rating curve for %s on %s.", 
				  _id, _owner->_id );*/
			}
			else if ( (_dcp->_rating_table.getNRows() > 0) ) {
			// No TABLE at the method, but one is defined at the 
			// node;
			// populate the stage_flow_table from the node data
				_stage_flow_tbl.operator=( _dcp->_rating_table );
			// note that if both a table and a RCID are defined at 
			// the NODE, there will have been an earlier error and 
			// execution will never reach this point
			}
			// else there is a RCID defined at the node; the 
			// process db will be queried for stage/discharge 
			// relationships
		}
		else {
			// TABLE is present in the method; make sure that a 
		        // TABLE or RATINGCURVEID is not defined at the NODE
			if ( (strcmp(_dcp->_rating_curve_id,"NULL")) ||
		             (_dcp->_rating_table.getNRows() > 0) ) {
				/*AVCP PrintError( routine,*/
			        logMessageWithArgsAndExitOnError( FATAL_LEVEL,
			        "%s %s MAXSTAGE defines a Rating Table, and a "
				"Rating Table or Curve ID is specified "
				"at the Node as well. NODE and MAXSTAGE are "
				"for %s on %s.", errorStr, routine,
				_id, _owner->_id );
				//return( STATUS_FAILURE );
			}
		}
...
/*  ==============  Statements containing RCS keywords:  */
{static char rcs_id1[] = "$Source: /fs/hseb/ob83/ohd/ofs/src/resj_topology/RCS/MaxStage_construct.cxx,v $";
 static char rcs_id2[] = "$Id: MaxStage_construct.cxx,v 1.9 2007/10/09 13:39:24 hsu Exp $";}
/*  ===================================================  */
