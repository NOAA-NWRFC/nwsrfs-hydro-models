...
int Component :: solve( TSDate& date, int isPrimary )
{
	...
	int	i=0;
        ...
        int     numInterval = _totNumInterval; //bug #562 5/29/12
	...
	for( i=0; i<_n_meth; i++ ) {
		...
		if( _expr_list[i] == NULL ) {
		        ...
		        if ( getFewsDebugFlag() > 10 )
			{
		           ...
		           "%s Expr. is NULL, skipping %s.%s.%s \n", 
			   routine, _id, _method_list[i]->getType(), 
		...
		if( _expr_list[i]->evaluate() == 0.0 ){
			...
			/*AVCP PrintDebug( 15, routine,*/
			   logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		  	   "%s %s is FALSE, skipping %s.%s.%s...\n", 
			   routine, _expr_list[i]->toString(), _id, 
		...
		if( _method_list[i]->solveMethod( date, isPrimary ) ){
			/*AVCP PrintWarning( 1, routine,*/
		        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		        "%s %s Troubles solving method %s.", warnStr, routine,
		 	_method_list[i]->getID() );
	...
	if( finalizeSolution( date ) ) {
	    ...
	    {
	        logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
	        "%s %s Troubles finalizing solution for %s.", 
		warnStr, routine, _id );
	    }
	}

        //bug #562 5/29/12
	//Need to reset _totNumInterval to original value (numInterval) before pass to this
	//method because when recursively call the solve function or solveMethod
	//function then _totNumInterval will be changed.  
        if(_totNumInterval > numInterval) 
	   _totNumInterval = numInterval;
/********
