	...
	double** group_val )
{
	...
	if ( _withdraw_obs ) {
		...
		//	missing?
			...
			/* AVCP PrintDebug( 5, routine */
			    ...
			    "(%s) - Setting withdraw for %s to %f "
			    "on %s.", routine, _owner->_id, with_value, 
			    cur_date.toString() );
		        }
			return( STATUS_SUCCESS );
		}
	...
	// Unknown _mode value
		...
		logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		"%s (%s) - Unknown SetWithdraw INTERPOLATE "
	...
	if( target_value == MISSING ) {
		...
		/* AVCP PrintWarning( 1, routine, */
		logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		"%s (%s) - SetWithdraw %s unable to find "
		"target value for %s.  Last withdraw (%s) used.", 
		warnStr, routine, _id, cur_date.toString(), prev_withdraw );
	...
	if( with_value == MISSING ) {
		...
		/* AVCP PrintWarning( 1, routine, */
		logMessageWithArgsAndExitOnError( DEBUG_LEVEL,
		"%s (%s) - Value returned from a SetWithdraw"
		" time series is MISSING - Setting to 0.", warnStr, routine );
		with_value = 0.0;
	}
	...
	****/
	...
	{
	   logMessageWithArgsAndExitOnError( DEBUG_LEVEL, 
	   "(%s) - Setting outflow for %s to %f on %s.",
	   routine, _owner->_id, with_value, cur_date.toString() );
        }
	return( STATUS_SUCCESS );
