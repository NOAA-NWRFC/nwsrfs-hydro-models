...
C @PROCESS LVL(77)
C
CVR   SUBROUTINE PIN44 (P,LEFTP,IUSEP,C,LEFTC,IUSEC)
      SUBROUTINE PIN44 (P,C,FILENAME)
...
C    ================================= RCS keyword statements ==========
      ...
      DATA             RCSKW1,RCSKW2 /                                 '
     .$Source: /fs/hseb/ob83/ohd/ofs/src/fcinit_pntb/RCS/pin44.f,v $
     . $',                                                             '
...
C     CHECK TRACE LEVEL
CVR   CALL FPRBUG('PIN44   ',1,44,IBUG)
...
C     READ IN CARDS IN FREE FORMAT
 ...
 802     FORMAT('PIN44: CIDINS,CODINS: ',2A8,/,
     ...
     +       '         CIDOTE,CODOTE: ',2A8)
      ...
      IF (FEWSDEBUG.GE.1) THEN 
         WRITE(MESSAGESTRING,8025) NINTS
         call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
 8025    FORMAT('PIN44: NINTS: ',I5)
      ...
      IF (FEWSDEBUG.GE.1) THEN
         WRITE(MESSAGESTRING,8026) NOTTS
         call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
...
C                CIDOTE,CODOTE
      IF (NINTS.EQ.2) THEN
CVR      CALL CHEKTS (CIDINS,CODINS,NCOMP,1,'L3/T',0,1,IERFLG)
CVR      IF (IERFLG.NE.0) IERROR = 1

         CALL getDimensionAndUnitInFortran(CODINS, IDIM1, IUNIT)
         IF ((IDIM1.NE.'L3/T') .OR. (IUNIT.NE.'CMS')) THEN
            WRITE(MESSAGESTRING, *) 
     +      'FOR BEG INTERVAL INFLOW TS', CODINS, 
...
CVR   CALL CHEKTS (CIDINE,CODINE,NCOMP,1,'L3/T',0,1,IERFLG)
CVR   IF (IERFLG.NE.0) IERROR = 1

      CALL getDimensionAndUnitInFortran(CODINE, IDIM1, IUNIT)
      IF ((TRIM(IDIM1).NE.'L3/T ').OR.(TRIM(IUNIT).NE.'CMS ')) THEN
             WRITE(MESSAGESTRING, *) 
     +      'FOR END INTERVAL INFLOW TS ', CODINE, 
     +      ' L3/T IS EXPECTED INSTEAD OF ', IDIM1, 
     +      ' CMS IS EXPECTED INSTEAD OF ', IUNIT
             call logfromfortran(FATAL_LEVEL,MESSAGESTRING)
...
CVR   IF (NOTTS.EQ.2) THEN
CVR      CALL CHEKTS (CIDOTS,CODOTS,NCOMP,1,'L3/T',0,1,IERFLG)
CVR      IF (IERFLG.NE.0) IERROR = 1
CVR      CALL CHEKTS (CIDOTE,CODOTE,NCOMP,1,'L3/T',0,1,IERFLG)
CVR      IF (IERFLG.NE.0) IERROR = 1
CVR   ELSE
CVR      CALL CHEKTS (CIDOTE,CODOTE,NCOMP,1,'L3  ',0,1,IERFLG)
CVR      IF (IERFLG.NE.0) IERROR = 1
CVR   ENDIF
...
C     NPHASE SHOULD BE LESS THAN MAXPH
      IF (NPHASE.GT.MAXPH) THEN
         WRITE(MESSAGESTRING,
     +   FMT='(1H0,10X,
     +   ''**ERROR** NUMBER OF ROUTING PHASES '',
...
CVR      CALL ERROR
 ...
 811        FORMAT('PIN44: QI1: ',F10.2)
      ...
      DO 100 I=1,NPHASE
         ...
         IF (FEWSDEBUG.GE.1) THEN
            WRITE(MESSAGESTRING,812) I,QPHASE(I)
            call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
...
CVR   CALL CHECKP (IUSEP,LEFTP,IERFLG)
...
CVR   CALL CHECKC (IUSEC,LEFTC,IERFLG)
      IF (IERFLG.NE.0) IERROR = 1
