...
C-----------------------------------------------------------------------
      ...
      REAL *4   P(*),C(*),QINST(*),QINEN(*),QOUTST(*),QOUTEN(*)
      REAL   CTEMP(750)
...
C     COMMON /IONUM/IN,ITSDATA,IPR,IPU
C     COMMON /FCTIME/IDARUN,IHRRUN,LDARUN,LHRRUN,LDACPD,LHRCPD,NOW(5),
C    +               LOCAL,NOUTZ,NOUTDS,NLSTZ,IDA,IHR,LDA,LHR,IDADAT
...
C    ================================= RCS keyword statements ==========
      ...
      DATA             RCSKW1,RCSKW2 /                                 '
     .$Source: /fs/hseb/ob83/ohd/ofs/src/fcst_sarroute/RCS/ex44.f,v $
     . $',                                                             '
...
CVR   CALL FPRBUG ('EX44    ',1,44,IBUG)
...
C     AND SET NUMBER OF CARRYOVER VALUES TO SAVE
      ...
      NOTTS = NINT(P(33))
      IF (FEWSDEBUG.GE.1) THEN
        WRITE(MESSAGESTRING,1010) NINTS,NCO,NOTTS
        call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
...
C     LOCATE FIRST DATA VALUE IN THE TIME SERIES DATA ARRAY
      ...
      KHR = IHR
      KDT = NINT(P(38))
C     ITS = (KDA-IDADAT)*24/KDT + (KHR-1)/KDT + 1
      ITS = 1
      IF (FEWSDEBUG.GE.1) THEN
        WRITE(MESSAGESTRING,1002) KDA,KHR,KDT,IDADAT,ITS
        call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
 1002   FORMAT(' EX44: KDA,KHR,KDT,IDADAT,ITS: ',5I7)
      ENDIF

C     LOCATE LAST DATA VALUE IN THE TIME SERIES DATA ARRAY
C     LTS = (LDA-IDADAT)*24/KDT + (LHR-1)/KDT + 1
      LTS = (LDA-IDADAT)*24/KDT + (LHR-IHR)/KDT + 1
      IF (FEWSDEBUG.GE.1) THEN
        WRITE(MESSAGESTRING,1003) LDA,LHR,LTS
        call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
...
CVR
C     CALL FCONVT ('CMS ','L3/T',UNTOUT,RMF,RCF,IERFLG)
C     INSTEAD OF FCONVT USE INFO FROM MCP3
C     RMF= 35.31510  RCF= 0.000000  UNTOUT=CFS
CVR
...
C   CHECK THAT # OF CARRYOVER VALUES IS NOT GREATER THAN CTEMP ARRAY
      IF (NCO .GT. LCTEMP) THEN
         WRITE(MESSAGESTRING, 1016 ) LCTEMP, NCO
...
C        QOUTEN - (OUT) END PERIOD ROUTED OUTFLOW TIME SERIES
         ...
         IF (FEWSDEBUG.GE.1) THEN
            WRITE(MESSAGESTRING,2106) (CTEMP(L),L=1,NCO)
            call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
...
CVR      IF (IFILLC.EQ.1) THEN
CVR         IF (KDA.EQ.ICDAY(ICARY).AND.KHR.EQ.ICHOUR(ICARY)) THEN

CVR            CALL FCWTCO (KDA,KHR,CTEMP,NCO)
CVR            ICARY = ICARY + 1
CVR         ENDIF
CVR      ENDIF
...
1000  CONTINUE
 ...
 901     C(I) = CTEMP(I)
      ...
      IF (FEWSDEBUG.GE.1) THEN
        DO 999 I=1, NCO
          WRITE(MESSAGESTRING,*) 'CARRAY[',I,']=',C(I)
 999      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
      ENDIF
...
C     TO CFSD
 ...
 940     CONTINUE
      ...
      IF (FEWSDEBUG.GE.1) THEN
          WRITE(MESSAGESTRING,990) ICARY
          call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
 990      FORMAT (' EX44: ICARY:',I5)
      ENDIF

 992  IF (ITRACE.GE.1) THEN
