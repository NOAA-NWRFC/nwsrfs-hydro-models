...
C 
 ...
 1600   FORMAT('   *** ENTER XNIT51 ***')
      ...
      IF(FEWSDEBUG.GE.2)THEN
          WRITE(MESSAGESTRING,892) IDA,IHR,LDA,LHR,IDADAT,LDACPD,LHRCPD
          call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)
...
C      NUM = (LDA - IDA)*NTIM24 + (LHR-IHR)/MINODT + 1      
      NRUN = (LDACPD - IDA)*NTIM24 + LHRCPD/MINODT
C LC DON'T USE JUL DAY AND HOUR TO CALCULATE     
      NUM = NTIMESTEPS
ccav 10/30/08 use lstcmpdate input
ccav      NRUN = NOBSTIMESTEPS
      IF (NRUN.LT.0) NRUN = 0
...
ccav      CALL JULDA(IDABGN,IHRB,1,1,IYEAR,IHRM,NLSTZ,NOUTDS,TZCS)
...
C
      ...
      MIDNIT = 0      
      IF (FEWSDEBUG.GE.2) THEN
         WRITE(MESSAGESTRING,1610) JULDAT,JDAWEK,NODAYR,
     .  IDA,IDABGN
         call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)
