...
C
     ...
     & IRES,NRES,NRUN,NS2,NTIM24,NUM,ITME(1)
      COMMON/XCO51/ICOPOS(20),ISAVCO,NUMCOV
      COMMON/LTS51/
...
C
	...
	call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)
        WRITE(MESSAGESTRING,1613) NS2,(WK(LCW+L-1),L=1,NUMCOV)
	call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)
      ENDIF
...
C  CONVERT TO INPUT UNITS FOR TABLE LOOKUP
      ...
      FB(IP2) = QO2
      FB(IP2+1) = ELEV2
      IF (FEWSDEBUG.LT.2) GO TO 640
      IRTYP = ISTYP+1
      WRITE(MESSAGESTRING,1620) RESTYP(IRTYP)
      call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)
...
cc      IF(IS50.EQ.3) THEN
      ...
      IS51 = NPA(LPA+51)
      IA=LPA
C     CALL LKRT51(IS51,XH2,QI1,QI2,WK,NPA,NPA(LPA+1),FA,FB,CF,ELDS,
      CALL LKRT51(IS51,XH2,QI1,QI2,WK,NPA,NPA(IA+1),FA,FB,CF,ELDS,
     & NQTPWRN)
      LPA=IA
C
      IS52 = NPA(LPA+52)
...
C
      IF (FEWSDEBUG.GE.2) THEN
          WRITE(MESSAGESTRING,1691) NS2,(WK(LCW+L-1),L=1,NUMCOV)
	  call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)
 1691     FORMAT('  >>> SAVING CARRYOVER FOR PERIOD NO. ',I3 /
     &,(6F12.3))
      ENDIF
C
C LC commented out FCWTCO; this routine writes to an NWSRSF temp file
C we will write our carryover to our own file
C for now we write carryover for 1 day at end of run so
C no need to initialize common block variables
C in the future when we write for specific dates
C we will replace put our call to write states to our files
C below and need to have initialized IFILLC=1, NCSTOR=NUM_OF_DATES,
C and ICDA and ICHOUR to carryover date and time
C      CALL FCWTCO(ICDAY(IPOS),ICHOUR(IPOS),WK(LCW),NUMCOV)
      LASTCO = LASTCO + 1
C
 910  CONTINUE
C
