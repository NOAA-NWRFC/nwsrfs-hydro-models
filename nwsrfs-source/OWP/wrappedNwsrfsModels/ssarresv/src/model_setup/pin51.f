...
C
C
C      SUBROUTINE PIN51(PO,LEFTP,IUSEP,CO,LEFTC,IUSEC,WK,NWK)
      SUBROUTINE PIN51(PO,CO,WK, NWK)
C
...
C..................................
C  SET DEBUG SWITCH
C LC hard code FEWSDEBUG to 2

ccaivo turn on ITRACE for debugging
ccav      ITRACE = 1
C LC comment out call to FPRBUG; we can't retrieve
C a debug level (IPBUG) this way      
C      CALL FPRBUG(SNAME,1,NUMOP,IPBUG)
cav      IF (ITRACE.GT.0) FEWSDEBUG = 1
C LC part of above comment
C      IF (IPBUG.GT.0)  FEWSDEBUG = 2
      IF (FEWSDEBUG.GE.1) THEN
      WRITE(MESSAGESTRING,1698)
...
C
 ...
 1605 FORMAT(' *** POS. NOS. :',I4,' THRU ',I4)
      WRITE(MESSAGESTRING,1610) (WK(J),J=I,IE)
      call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)
  350 CONTINUE
 1610 FORMAT(8F10.2)
  390 CONTINUE
...
C
      IF (FEWSDEBUG.LT.2) GO TO 590
      WRITE(MESSAGESTRING,530)
      call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)
...
C
      ...
      IF (FEWSDEBUG.LT.2) GO TO 790
      WRITE(MESSAGESTRING,740)
      call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)
...
C
C LC
C comment out reference to LUSEP, LEFTP, LUSEC, and LEFTC
C      LUSEP = 19 + (NPINF+NPSAB+NPSAR) +
C     .        (NTINF+NTSAB+NTSAR+1)
C
C LC
C comment out reference to LUSEP, LEFTP, LUSEC, and LEFTC
C      LUSEC = NCINF + NCSAB + NCSAR
C LC
C comment out reference to LUSEP, LEFTP, LUSEC, and LEFTC
C      IF ( (LUSEP.GT.LEFTP) .OR. (LUSEC.GT.LEFTC) ) NOROOM = .TRUE.
C
C...................................................
...
C      METSAR=PO(8)
      ...
      CALL ETOM51(PO)
C      
C      IF (FEWSDEBUG.LT.2 .OR. METSAR.EQ.1) GO TO 9999
C      NTL = PO(14)
C      WRITE(IODBUG,1650)
...
C comment out reference to LUSEP, LEFTP, LUSEC, and LEFTC
C      DO 965 IJK=1,5
C      IF (NOROOM) WRITE(IPR,970) LEFTP,LUSEP,LEFTC,LUSEC
C  970 FORMAT(/10X,'*** ERROR ***',/
C     .10X,' MORE SPACE IS REQUIRED TO STORE THE PARAMETERS AND ',
C     .'CARRYOVER FOR THIS RESERVOIR OPERATION THAN IS AVAILABLE.'/,
C     .10X,' FOR STORING PARAMETERS IN THE P ARRAY, ',I4,' WORDS ARE ',
C     .'AVAILABLE; ',I4,' WORDS ARE NEEDED.'/,
C     .10X,' FOR STORING CARRYOVER IN THE C ARRAY, ',I4,' WORDS ARE ',
C     .'AVAILABLE; ',I4,' WORDS ARE NEEDED.')
C 965  CONTINUE
C
