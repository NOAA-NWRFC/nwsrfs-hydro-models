...
130   FORMAT (' FMT=',15A)
        ...
        IF (IERNME(1,I).EQ.IBLNK) GO TO 140
           NWORDS=3
           IF (N.EQ.93) NWORDS=1
           WRITE (MESSAGESTRING,FMT,ERR=150) (IERNME(K,I),K=1,NWORDS)
	   call logfromfortran(FATAL_LEVEL,MESSAGESTRING)
           CALL ERROR
           GO TO 170
140     WRITE (MESSAGESTRING,FMT,ERR=150)
        call logfromfortran(FATAL_LEVEL,MESSAGESTRING)
        CALL ERROR
        GO TO 170
150     IF (FMT(1).NE.'(''    '')') then
           WRITE (MESSAGESTRING,160) FMT
	   call logfromfortran(FATAL_LEVEL,MESSAGESTRING)
...
C
      IF (IERR.GT.0) THEN
         WRITE (MESSAGESTRING,180) IERR,VALUE1,VALUE2,IPT
	 call logfromfortran(FATAL_LEVEL,MESSAGESTRING)
