...
C
 ...
 109  Z(I)=PO(IZ+I)
      LPO=LPO+3*NVAL+1
C
      WRITE(MESSAGESTRING,1720)
      call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)
      IB=1
      IE=NVAL
      IF(IE .GT. 8)IE=8
 110  WRITE(MESSAGESTRING,1725) UNITL,(X(I),I=IB,IE)
      call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)
      WRITE(MESSAGESTRING,1726) UNITST,(Y(I),I=IB,IE)
      call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)
...
120   CONTINUE
 ...
 135  CONTINUE
C
      WRITE(MESSAGESTRING,'(/10X,''BACKWATER TABLE: '')')
      call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)
      WRITE(MESSAGESTRING,1820)
      call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)
 1820 FORMAT(/20X,'   RES RELS   RES ELEV D/S CONTRL',
     & '   RES RELS   RES ELEV  DS CONTRL',
     & '   RES RELS   RES ELEV  DS CONTRL')
      IF(IBWTYP.EQ.1) THEN
         WRITE(MESSAGESTRING,1821) UNITQ,UNITL,UNITQ,
     & UNITQ,UNITL,UNITQ,UNITQ,UNITL,UNITQ
         call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)
      ENDIF
      IF(IBWTYP.EQ.2) THEN
         WRITE(MESSAGESTRING,1821) UNITQ,UNITL,UNITL,
     & UNITQ,UNITL,UNITL,UNITQ,UNITL,UNITL
      ...
      IE=NVAL
      IF(IE .GT. 3) IE=3
 140  IF(IBWTYP.EQ.1) THEN
         WRITE(MESSAGESTRING,1824) (X(I),Y(I),Z(I),I=IB,IE)
	 call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)
      ENDIF
      IF(IBWTYP.EQ.2) THEN
          WRITE(MESSAGESTRING,1825) (X(I),Y(I),Z(I),I=IB,IE)
	  call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)
      ENDIF
...
C  PRINT MAXEL
      ELMAX=PO(LPO)
      WRITE(MESSAGESTRING,1730)UNITL,ELMAX
      call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)
...
C  PRINT MINEL
      ELMIN=PO(LPO)
      WRITE(MESSAGESTRING,1731)UNITL,ELMIN
      call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)
...
C  PRINT MINQREL
      QRELMN=PO(LPO)
      WRITE(MESSAGESTRING,1732)UNITQ,QRELMN
      call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)
...
C  PRINT SHUTRESV
      QSHUT=PO(LPO)
      WRITE(MESSAGESTRING,1733) UNITQ,QSHUT
      call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)
...
1745  FORMAT(/10X,20H       TIME SERIES:,13X,2HID,10X,4HTYPE,
     1 10X,8HTIME(HR)/)
      ...
      IT=PO(LTS+3)
      WRITE(MESSAGESTRING,1750) GTSKW(II),PO(LTS),PO(LTS+1),PO(LTS+2),IT
      call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)
