...
C LC : Comment out calles to CHEKTS and FDCODE
C
C  SEE IF TIME-SERIES EXISTS
C
      ICHK = 0
C      CALL CHEKTS(TSID(1,ID),DTYPE(ID),IDT(ID),ICHK,DCODE,0,1,IERCK)
...
C  SEE IF TIME-SERIES EXISTS
C
C COMMENT OUT CALLS TO CHEKTS, FDCODE AND DIMENSION CHECK
C      ICHK = 0
C      CALL CHEKTS(TSID(1,ID),DTYPE(ID),IDT(ID),ICHK,DCODE,0,1,IERCK)
C      IF (IBUG.GE.2) WRITE(IODBUG,1666) ICHK
C
