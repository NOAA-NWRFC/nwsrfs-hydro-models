...
!CP      WRITE(IODBUG,40) X,Y
         WRITE(MESSAGESTRING, 40) X,Y
	 call logfromfortran(DEBUG_LEVEL, MESSAGESTRING) 
...
C WRITE TRACE AND DEBUG IF REQUIRED.
C
!CP   102 IF (IBUG.EQ.1) GO TO 170
!CP   IF (IBUG.GE.5) GO TO 110
!CP   GO TO 190

 102  IF ( FEWSDEBUG.GE.5 ) THEN
!CP      110 WRITE(IODBUG,120) X,Y,Z
         WRITE(MESSAGESTRING, 120) X,Y,Z 
	 call logfromfortran(DEBUG_LEVEL, MESSAGESTRING) 
  120 FORMAT(1H0,43H X,Y, AND Z VALUES AT THE END OF F3WAYZ ARE,3F12.3)

!CP      WRITE(IODBUG,130) (YY(I),I=1,NY)
         WRITE(MESSAGESTRING, 130) 
         call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
  130 FORMAT(1H0,36H YY VALUES IN THE 3-WAY RELATION ARE) 
         WRITE(FORMATSTR, *) '%12.3f'
         call logonedimensionarrayfromfortran(DEBUG_LEVEL,NULLLINE,1,10,
     >        FORMATSTR,1,NY,1,YY)
         

!CP      WRITE(IODBUG,140) (ZZ(I),I=1,NZ)
         WRITE(MESSAGESTRING, 140) 
         call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
  140 FORMAT(1H0,14H ZZ VALUES ARE)
         WRITE(FORMATSTR, *) '%12.3f'
         call logonedimensionarrayfromfortran(DEBUG_LEVEL,NULLLINE,1,10,
     >        FORMATSTR,1,NZ,1,ZZ)
         ...
         DO 150 I=1,NZ
!CP      150 WRITE(IODBUG,160) ZZ(I),(YY(J),XX(J,I),J=1,NY)
  150    WRITE(MESSAGESTRING, 160) ZZ(I),(YY(J),XX(J,I),J=1,NY)
         call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
