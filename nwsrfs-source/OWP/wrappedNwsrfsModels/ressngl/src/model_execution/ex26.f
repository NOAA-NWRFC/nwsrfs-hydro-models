...
C
C
      SUBROUTINE EX26(PO,CO,D,IDPT,W,VM126,NHRYR)
C
C-----------------------------------------------------------------------
...
!CP   IF (IBUG.GE.1) WRITE(IODBUG,1698) ISEG
!CP   1698    FORMAT('    *** ENTER EX26 *** -- ')
      IF ( FEWSDEBUG.GE.4 ) THEN
         WRITE(MESSAGESTRING, '(A23)') '     *** ENTER EX26 ***' 
...
C
      NUM = 0
      CALL XNIT26(PO,NHRYR)
      NUMDAY = (NTIMESTEPS / NTIM24) + 1
C
C  DEBUG OUTPUT OF INFLOW TIME-SERIES (BOTH INST. AND MEAN)
C
      LOBSA = NRUN
      IOFF = IDOFST * NTIM24
!CP   IF (IBUG.GE.2) WRITE(IODBUG,1610) (D(IDPT(1)+IOFF+I-1),I=1,NUM)
!CP   IF (IBUG.GE.2) WRITE(IODBUG,1620) (D(IDPT(2)+IOFF+I-1),I=1,NUM)
      IF ( FEWSDEBUG.GE.4 ) THEN
         NPRINT=NUM
         IF ( NPRINT .GT. 100 ) NPRINT = 100
         WRITE(MESSAGESTRING, 1610) !(D(IDPT(1)+IOFF+I-1),I=1,NUM)
         call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
 1610    FORMAT(/'   ** INST. INFLOW TIME-SERIES :'//) !,(8F10.1))
     ...
     >        FORMATSTR,IDPT(1)+IOFF,IDPT(1)+IOFF+NPRINT-1,1,D)
   
         WRITE(MESSAGESTRING, 1620) !(D(IDPT(2)+IOFF+I-1),I=1,NUM)
         call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
 1620    FORMAT(/'   ** MEAN INFLOW TIME-SERIES :'//)!,(8F10.1))
         WRITE(FORMATSTR, *) '%10.1f'
         call logonedimensionarrayfromfortran(DEBUG_LEVEL,NULLLINE,1,8,
     >        FORMATSTR,IDPT(2)+IOFF,IDPT(2)+IOFF+NPRINT-1,1,D)
...
C
!CP      CALL FCONVT(4HM   ,4HL   ,IENGUN,XMULT,XADD,IERX)
! Convert meters to feet
      XMULT = 3.280840
      ELVMIN=ELVMIN*XMULT
C
!CP   345  WRITE(IPR,810) NERRD,ELVMIN,AUNIT(METRIC+1)
 345  WRITE(MESSAGESTRING, 810) NERRD,ELVMIN,AUNIT(METRIC+1)
...
!     1         (D(IDPT(LOCIDP)+IOFFEL+I-1),I=1,NPEL)
 ...
 1630 FORMAT(/'   ** OBSERVED POOL ELEVAT TIME-SERIES :'//)!,(8F10.2))
         ...
         call logonedimensionarrayfromfortran(DEBUG_LEVEL,NULLLINE,1,8,
     >   FORMATSTR,IDPT(LOCIDP)+IOFFEL,IDPT(LOCIDP)+IOFFEL+NPRINT-1,1,D)
...
!CP   IF (IBUG.GE.2) WRITE(IODBUG,1650) (CO(I),I=1,NUMCOV)
      IF ( FEWSDEBUG.GE.4 ) THEN
         WRITE(MESSAGESTRING, 1650) !(CO(I),I=1,NUMCOV)
         call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
 1650 FORMAT(/'   ** CARRYOVER VALUES AT START OF RUN :'//) !,(8F10.2))
         WRITE(FORMATSTR, *) '%10.2f'
         call logonedimensionarrayfromfortran(DEBUG_LEVEL,NULLLINE,1,8,
     >	      FORMATSTR,1,NUMCOV,1,CO)
      END IF
C
C  EXECUTE ANY PRE-COMPUTATION LOOP UTILITIES
C
      ELEV1 = CO(5)
      CALL XPRE26(PO,CO,W,D,IDPT,LOCWS,LOCOWS,LOBSA,NHRYR)
C
C  CALL COMPUTATIONAL LOOP SUPERVISOR (ACTUAL SIMULATION OCCURS WITHIN
C  CONTROL OF THAT ROUTINE.)
C
      CALL XQT26(PO,CO,D,W,IDPT,LOCWS,LOCOWS,LOBSA,VM126,NHRYR)
C
C----------------------------------------------------------------
...
C
      ...
      D(LOCERR(I))=ERRD(I)
1900  CONTINUE
!CP   IF (IBUG.GE.2) WRITE(IODBUG,1640)
      IF ( FEWSDEBUG.GE.4 ) THEN
         IF ( NPEL.GT.100 ) NPRINT=100
...
!CP   1699 FORMAT('    *** EXIT EX26 ***')
      IF ( FEWSDEBUG.GE.4 ) THEN
         WRITE(MESSAGESTRING,'(A23)') '      *** EXIT EX26 ***'
         call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
      END IF
ccc   ibug=0
      RETURN
