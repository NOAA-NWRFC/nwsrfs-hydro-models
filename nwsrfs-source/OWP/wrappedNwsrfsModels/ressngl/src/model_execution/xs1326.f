...
C
      SUBROUTINE XS1326(SUNUM,PO,W,LOCWS,LOCOWS,IDPT,NHRYR)
C---------------------------------------------------------------------
...
C
     ...
     .          DATMIN(30),DATEUP(30),DATELR(30),NHRYR(1)
C
...
!CP   1600 FORMAT('   *** ENTER XS1326 ***')
      IF ( FEWSDEBUG.GE.4 ) THEN
         WRITE(MESSAGESTRING, '(A23)') '   *** ENTER XS1326 ***'
...
C
C      LOCMTR = LOCWS(2) + NDD*NTIM24
      LOCMTR = LOCWS(2) + NTIMESTEPS
C      LOCSTR = LOCMTR + 2*NDD*NTIM24
      LOCSTR = LOCMTR + 2*NTIMESTEPS
...
C
  100 CONTINUE
C EJV MOD
C      LOCSMX = LOCOWS(11) + NDD*NTIM24
      LOCSMX = LOCOWS(11) + NTIMESTEPS
C      LOCSMN = LOCOWS(11) + 2*NDD*NTIM24
      LOCSMN = LOCOWS(11) + 2*NTIMESTEPS
C END EJV
      LOCNXT = LOCNXT + IADD
      NELVUP = PO(LOCNXT)
      ELVUPR = -999.0
...
!CP   1699 FORMAT('   *** EXIT XS1326 ***')
      IF ( FEWSDEBUG.GE.4 ) THEN
         WRITE(MESSAGESTRING, '(A23)') '   *** EXIT XS1326 ***'
	 call logfromfortran(DEBUG_LEVEL, MESSAGESTRING) 
      END IF
      RETURN
      END
