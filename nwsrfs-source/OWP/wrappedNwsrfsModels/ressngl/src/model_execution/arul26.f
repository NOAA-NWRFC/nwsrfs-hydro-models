...
C **********************************************************************
!CP   IF(IBUG-1)80,10,20
!CP   10 WRITE(IODBUG,30)
!     GO TO 80
!CP   20 WRITE(IODBUG,30)
   
      IF ( FEWSDEBUG.LT.4 ) GO TO 80

      WRITE(MESSAGESTRING, 30) 
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING) 
   30 FORMAT(1H0,10X,17H** ARUL26 ENTERED)
   
!CP   WRITE(IODBUG,40) NRUN,NTIMRL,NOMSG,QIMAX,DIFMAX,IBUG
      WRITE(MESSAGESTRING, 40) NRUN,NTIMRL,NOMSG,QIMAX,DIFMAX,IBUG
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
   40 FORMAT(1H0,39H    NRUN,NTIMRL,NOMSG,QIMAX,DIFMAX,IBUG/1X,3I6,2F12.
     $3,I6)
   
!CP   WRITE(IODBUG,50)(QIMHYD(I),I=1,NRUN)
      WRITE(MESSAGESTRING, 50) 
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
   50 FORMAT(1H0,12H INFLOWS ARE)
      WRITE(FORMATSTR, *) '%12.3f'
      call logonedimensionarrayfromfortran(DEBUG_LEVEL,NULLLINE,1,10,
     >     FORMATSTR, 1,NRUN,1,QIMHYD)
   
!CP   WRITE(IODBUG,60) (BDEV(I),I=1,NTIMRL)
      WRITE(MESSAGESTRING, 60)
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
   60 FORMAT(1H0,20H BACK DEVIATIONS ARE)
      WRITE(FORMATSTR, *) '%12.3f'
      call logonedimensionarrayfromfortran(DEBUG_LEVEL,NULLLINE,1,10,
     >     FORMATSTR,1,NTIMRL,1,BDEV)
   
!CP   WRITE(IODBUG,70) (OBSELV(I),I=1,NRUN)
      WRITE(MESSAGESTRING, 70) 
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
   70 FORMAT(1H0,41H OBSERVED ELEVATIONS THROUGH RUN TIME ARE)
      WRITE(FORMATSTR, *) '%12.3f'
      call logonedimensionarrayfromfortran(DEBUG_LEVEL,NULLLINE,1,10,
     >     FORMATSTR,1,NRUN,1,OBSELV)
...
C WRITE TRACE AND DEBUG IF REQUIRED.
C **********************************************************************
!CP   IF(IBUG-1)230,210,180
      IF ( FEWSDEBUG.LT.4 ) GO TO 230
!CP180WRITE(IODBUG,190)NTIMRL,AGBDEV,AVGDEV,LOBSTO,(DEVIAT(I),I=1,LOBSTO
      WRITE(MESSAGESTRING, 190) NTIMRL,AGBDEV,AVGDEV,LOBSTO
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING) 
  190 FORMAT(1H0,60H NO. OF TIME PERIODS USED FOR COMPUTING AVERAGE DEVI
     ...
     $VATIONS IS,F8.3,20H DEVIATIONS FOR LAST,I6,12H PERIODS ARE)

      WRITE(FORMATSTR, *) '%8.3f'
      call logonedimensionarrayfromfortran(DEBUG_LEVEL,NULLLINE,1,15,
     >     FORMATSTR,1,LOBSTO,1,DEVIAT)

!CP   WRITE(IODBUG,200) (SUMMSG(I),I=1,LOBSTO)
      WRITE(MESSAGESTRING, 200) 
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
  200 FORMAT(1H0,49H VALUES IN ARRAY OF SUM OF MISSING DEVIATIONS ARE)
      WRITE(FORMATSTR, *) '%12.3f'
      call logonedimensionarrayfromfortran(DEBUG_LEVEL,NULLLINE,1,10,
     >     FORMATSTR,1,LOBSTO,1,SUMMSG)

!CP   210 WRITE(IODBUG,220)
  210 WRITE(MESSAGESTRING, 220) 
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
