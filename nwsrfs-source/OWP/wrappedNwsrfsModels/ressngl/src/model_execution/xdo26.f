...
C DESC  EXECUTED.
C----------------------------------------------------------------------
      SUBROUTINE XDO26(SUNUM,PO,W,D,IDPT,LOCWS,LOCOWS,NHRYR)
C---------------------------------------------------------------------
...
C
C
!CP   IF (IBUG.GE.1) WRITE(IODBUG,1698)
!CP   1698 FORMAT('   *** ENTER XDO26 ***')
      IF ( FEWSDEBUG.GE.4 ) THEN
         WRITE(MESSAGESTRING, '(A23)') '   *** ENTER XDO26 ***'
...
C
!CP   IF (IBUG.GE.2) WRITE(IODBUG,1601) IBASE,LEVEL
      IF ( FEWSDEBUG.GE.3 ) THEN
         WRITE(MESSAGESTRING, 1601) IBASE,LEVEL
	 call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
      END IF
...
C
 1040 CONTINUE
      CALL XS0426(SUNUM,PO,W,LOCOWS,NHRYR)
      NSCHEX = NSCHEX + 1
      GO TO 9000
C--------------------------------
C  FILL AND SPILL
...
C
 1070 CONTINUE
      CALL XS0726(SUNUM,PO,W,LOCOWS,NHRYR)
      NSCHEX = NSCHEX + 1
      GO TO 9000
C---------------------------------------
...
C
 1080 CONTINUE
      CALL XS0826(SUNUM,PO,W,D,LOCWS,LOCOWS,IDPT,NHRYR)
      NSCHEX = NSCHEX + 1
      GO TO 9000
C---------------------------------------
...
C
 1090 CONTINUE
      !IF (NS2.LT.LOBSTO) GO TO 9000 !CP FB1354
      MQXQT = .TRUE. !HSU added for FB1354 10/14
      CALL XS0926(SUNUM,PO,W,LOCWS,LOCOWS,IDPT,NHRYR)
      NSCHEX = NSCHEX + 1
      GO TO 9000
C---------------------------------------
...
C
 1100 CONTINUE
      CALL XS1026(SUNUM,PO,W,D,LOCWS,LOCOWS,IDPT,NHRYR)
      NSCHEX = NSCHEX + 1
      GO TO 9000
C---------------------------------------
C  INDUCED SURCHARGE
C
 1110 CONTINUE
      SCHORG = SURCHG
      CALL XS1126(SUNUM,PO,W,LOCWS,LOCOWS,NHRYR)
...
C
 1130 CONTINUE
      CALL XS1326(SUNUM,PO,W,LOCWS,LOCOWS,IDPT,NHRYR)
      NSCHEX = NSCHEX + 1
      GO TO 9000
C
C--------------------------------
...
!CP   1699 FORMAT('    *** EXIT XDO26 ***')
      IF ( FEWSDEBUG.EQ.4 ) THEN
         WRITE(MESSAGESTRING, '(A23)') '    *** EXIT XDO26 ***'
	 call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
      END IF
      RETURN
      END
