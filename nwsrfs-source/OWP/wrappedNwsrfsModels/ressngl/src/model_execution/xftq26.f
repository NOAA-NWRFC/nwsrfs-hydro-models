...
!CP   1600 FORMAT('   *** ENTER XFTQ26 ***')
      IF ( FEWSDEBUG.GE.4 ) THEN
         WRITE(MESSAGESTRING, '(A23)') '   *** ENTER XFTQ26 ***'
...
C
      ...
      NRT = PO(LOCNLR)
C
!CP   IF (IBUG.GE.2) WRITE(IODBUG,1620) WHICH(1)
      IF ( FEWSDEBUG.GE.3 ) THEN
         WRITE(MESSAGESTRING, 1620) WHICH(1)
         call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
      END IF
...
!CP   IF (IBUG.GE.2) WRITE(IODBUG,1630) PO(LESELV+I-1),TQ,W(LOCFBQ+I-1)
      IF ( FEWSDEBUG.GE.3 ) THEN
         WRITE(MESSAGESTRING, 1630) PO(LESELV+I-1),TQ,W(LOCFBQ+I-1)
         call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
      END IF
...
C
      ...
      NSMR = PO(LOCSMR)
C
!CP   IF (IBUG.GE.2) WRITE(IODBUG,1620) WHICH(2)
      IF ( FEWSDEBUG.GE.3 ) THEN
         WRITE(MESSAGESTRING, 1620) WHICH(2)
         call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)  
      END IF
      DO 530 I=1,NSE
...
C
      LOCGTR = LOCGAT + 5
      NGTR = PO(LOCGTR)
!CP   IF (IBUG.GE.2) WRITE(IODBUG,1620) WHICH(3)
      IF ( FEWSDEBUG.GE.3 ) THEN
         WRITE(MESSAGESTRING, 1620) WHICH(3)
         call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
      END IF
      DO 550 I=1,NSE
...
!CP   IF (IBUG.GE.2) WRITE(IODBUG,1690) (W(LOCTQ+I-1),I=1,NQGEN)
!CP   IF (IBUG.GE.2) WRITE(IODBUG,1691) (W(LOCTQ+NQGEN+I-1),I=1,NQGEN)
      IF ( FEWSDEBUG.GE.3 ) THEN
         NPRINT=NQGEN
	 IF ( NPRINT.GT.100 ) NPRINT=100
         ...
         call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
 1690    FORMAT(/'   ** TOTAL Q VS. MAX GEN. Q CURVE :')
         WRITE(FORMATSTR, *) '%12.3f'
         call logonedimensionarrayfromfortran(DEBUG_LEVEL,NULLLINE,1,6,
     >        FORMATSTR,LOCTQ,LOCTQ+NPRINT-1,1,W)
         
         WRITE(FORMATSTR, *) '%12.3f'
	 call logonedimensionarrayfromfortran(DEBUG_LEVEL,NULLLINE,1,6,
     >        FORMATSTR,LOCTQ+NQGEN,LOCTQ+NPRINT+NPRINT-1,1,W)
      END IF
!1691 FORMAT((6F12.3))
 9500 CONTINUE
!CP   IF (IBUG.GE.1) WRITE(IODBUG,1699)
!CP   1699 FORMAT('    *** EXIT XFTQ26 ***')
      IF ( FEWSDEBUG.GE.4 ) THEN
         WRITE(MESSAGESTRING, '(A23)') '    *** EXIT XFTQ26 ***'
	 call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
      END IF
      RETURN
      END
