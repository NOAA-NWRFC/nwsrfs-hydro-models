C MODULE XS0426
C
      SUBROUTINE XS0426(SUNUM,PO,W,LOCOWS,NHRYR)
C--------------------------------------------------------------------
...
!CP   1600 FORMAT('   *** ENTER XS0426 ***')
      IF ( FEWSDEBUG.GE.4 ) THEN
         WRITE(MESSAGESTRING, '(A23)') '   *** ENTER XS0426 ***'
...
C
!CP   1650 IF (IBUG.GE.1) WRITE(IODBUG,1699)
!CP   1699 FORMAT('    *** EXIT XSO426 ***')
 1650 IF ( FEWSDEBUG.GE.4 ) THEN
         WRITE(MESSAGESTRING, '(A23)') '    *** EXIT XSO426 ***'
	 call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
      END IF
      RETURN
      END
