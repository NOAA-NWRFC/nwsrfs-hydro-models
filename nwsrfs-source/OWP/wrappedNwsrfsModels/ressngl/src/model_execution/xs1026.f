...
C
      SUBROUTINE XS1026(SUNUM,PO,W,D,LOCWS,LOCOWS,IDPT,NHRYR)
C--------------------------------------------------------------------
...
C
      DIMENSION PO(1),W(1),LOCWS(1),LOCOWS(1),IDPT(1),D(1),NHRYR(1)
C
...
!CP   1600 FORMAT('   *** ENTER XS1026 ***')
      IF ( FEWSDEBUG.GE.4 ) THEN
         WRITE(MESSAGESTRING, '(A23)') '   *** ENTER XS1026 ***'
...
C
      IFM = IFMSNG(ELVNRM)
      IF (IFM.EQ.1) CALL XFRU26(PO,LOCNXT+6,W,LOCOWS,NHRYR)
      IADD = 1
...
!CP   1699 FORMAT('    *** EXIT XS1026 ***')
      IF ( FEWSDEBUG.GE.4 ) THEN
         WRITE(MESSAGESTRING, '(A23)') '    *** EXIT XS1026 ***'
	 call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
      END IF
      RETURN
      END
