...
C
      DIMENSION CO(*),W(*),D(*),LOCOWS(*),IDPT(*),
     & QMD(NUMDAY),QWORK(NUMDAY),IERD(NUMDAY)
!BUG 280     & QMD(31),QWORK(31),IERD(31)
C
...
!CP   1600 FORMAT('   *** ENTER XRIN26 ***')
      IF ( FEWSDEBUG.GE.4 ) THEN
         WRITE(MESSAGESTRING, '(A23)') '   *** ENTER XRIN26 ***'
...
!CP   1699 FORMAT('    *** EXIT XRIN26 ***')
      IF ( FEWSDEBUG.GE.4 ) THEN
         WRITE(MESSAGESTRING, '(A23)') '    *** EXIT XRIN26 ***'
	 call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
      END IF
      RETURN
      END
