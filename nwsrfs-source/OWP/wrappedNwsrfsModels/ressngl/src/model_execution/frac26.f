...
!CP   COMMON/WHERE/ISEG(2),IOPNUM,OPNAME(2)
...
!CP   WRITE(IODBUG,40)DIFQI,DIFQOK,QI1,QO1,QOK1,QSFRAC,S1,SFILL,IBUG
      WRITE(MESSAGESTRING, 40)
     $      DIFQI,DIFQOK,QI1,QO1,QOK1,QSFRAC,S1,SFILL,IBUG
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
...
C NO SPILLWAY THIS TIME PERIOD
      ...
      CONTINUE
!CP   WRITE(IPR,90) X
C_fg1062      WRITE(MESSAGESTRING, 90) X
C_fg1062      call logfromfortran(WARNING_LEVEL, MESSAGESTRING)
C_fg1062 90    FORMAT(1H0,10X,'**WARNING** IN FRAC26-COMPUTATION OF FRACTION',
C_fg1062     $ 'OF TIME DAM IS IN SPILWAY MODE WASNT IN RANGE 0-1 FOR'
C_fg1062     $ /10X,'GIVEN TIME PERIOD.  SO FRACTION IS SET EQUAL TO',
C_fg1062     $ F4.1,'...ITS ALL OK, NO NEED TO WORRY BABY')
!CP   CALL WARN
C END EJV MODIFY
  110 FRAC26=X
!CP   IF(IBUG-1)160,140,120
      IF ( FEWSDEBUG.LT.4 ) GO TO 160
!CP   120 WRITE(IODBUG,130)X
      WRITE(MESSAGESTRING, 130) X
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
  130 FORMAT(1H0,27H FRACTION OF TIME PERIOD IS,F12.3)

!CP   140 WRITE(IODBUG,150)
