...
C--------------------------------------------------------------------
      SUBROUTINE XIF26(PO,IPTR,W,D,IDPT,LOCWS,LOCOWS,NHRYR)
C--------------------------------------------------------------------
...
!CP   INCLUDE 'common/fdbug'
      INCLUDE 'flogm'
C
      DIMENSION PO(1),W(1),D(1),IDPT(1),LOCWS(1),LOCOWS(1),NHRYR(1)
      LOGICAL TRUEIF
C
...
!CP   1600 FORMAT('   *** ENTER XIF26 ***')
      IF ( FEWSDEBUG.GE.4 ) THEN
         WRITE(MESSAGESTRING, '(A23)') '   *** ENTER XIF26 ***'
...
!CP   1699 FORMAT('    *** EXIT XIF26 ***')
      IF ( FEWSDEBUG.GE.4 ) THEN
         WRITE(MESSAGESTRING, '(A23)') '    *** EXIT XIF26 ***'
	 call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
      END IF
      RETURN
      END
