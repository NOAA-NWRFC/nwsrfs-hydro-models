...
C-----------------------------------------------------------------
      SUBROUTINE XPRE26(PO,CO,W,D,IDPT,LOCWS,LOCOWS,LOBSA,NHRYR)
C------------------------------------------------------------------
...
!CP   1600 FORMAT('   *** ENTER XPRE26 ***')
      IF ( FEWSDEBUG.GE.4 ) THEN
         WRITE(MESSAGESTRING, '(A23)') '   *** ENTER XPRE26 ***'
...
C
      I5 = 5
C      LPTAEL = LOCOWS(I5) + NDD*NTIM24
      LPTAEL = LOCOWS(I5) + NTIMESTEPS
      LPTARA = LPTAEL + NSE
...
C
      ...
      IF (DORULE) CALL XU1426(PO,CO,W,D,IDPT,LOCOWS,NHRYR)
C
C
C  DEBUG OUTPUT IF REQUESTED.
C
!CP   IF (IBUG.GE.2) WRITE(IODBUG,1690) DORULE,DOBACK,DOSUMF,DORAIN,
      IF ( FEWSDEBUG.GE.3 ) THEN
         WRITE(MESSAGESTRING, 1690) DORULE,DOBACK,DOSUMF,DORAIN,
     .               ADJUST
         call logfromfortran(DEBUG_LEVEL, MESSAGESTRING) 
...
!CP   1699 FORMAT('    *** EXIT XPRE26 ***')
      IF ( FEWSDEBUG.GE.4 ) THEN
         WRITE(MESSAGESTRING, '(A23)') '    *** EXIT XPRE26 ***'
	 call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
      END IF
      RETURN
      END
