...
!CP   1600 FORMAT('   *** ENTER XINQ26 ***')
      IF ( FEWSDEBUG.GE.4 ) THEN
         WRITE(MESSAGESTRING, '(A23)') '   *** ENTER XINQ26 ***'
...
C
      ...
      LBFQI = LOCOWS(1)
C      LBFQIM = LBFQI + NDD*NTIM24
      LBFQIM = LBFQI + NTIMESTEPS
C
C-----------------------------------------------------------------------
...
C
      IF ( LTSQI+NS2-2.GT.0) QI1 = D(LTSQI+NS2-2)
      QI2 = D(LTSQI+NS2-1)
...
!CP   1699 FORMAT('    *** EXIT XINQ26 ***')
      IF ( FEWSDEBUG.GE.4 ) THEN
         WRITE(MESSAGESTRING, '(A23)') '    *** EXIT XINQ26 ***'
	 call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
      END IF
      RETURN
      END
