...
C
      DIMENSION PO(*),RESNAM(2), NHRYR(*)
      INTEGER IDABGN, MONBGN, DAYBGN
C
...
!CP   1600 FORMAT('   *** ENTER XNIT26 ***')
      IF ( FEWSDEBUG.GE.4 ) THEN
         WRITE(MESSAGESTRING, '(A23)') '   *** ENTER XNIT26 ***'
...
C
C LEE      NUM  = (LDA - IDA)*NTIM24 + LHR/MINODT
      NRUN = (LDACPD - IDA)*NTIM24 + LHRCPD/MINODT
      NUM = NTIMESTEPS
!CP   NRUN = NOBSTIMESTEPS
      IF (NRUN.LT.0) NRUN = 0
      IF (NRUN.GT.NUM) NRUN = NUM 
      
c      IF ( FEWSDEBUG.GE.3 ) THEN
c         WRITE(MESSAGESTRING,*) 'ntimesteps***',NUM 
c         call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
c	 WRITE(MESSAGESTRING,*) 'nobstimesteps***',NOBSTIMESTEPS 
c         call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
c      ENDIF
	 
!CP   IF (IBUG.GE.2) WRITE(IODBUG,1605) MINODT,NTIM24,NUM,NRUN,IDOFST
      IF ( FEWSDEBUG.GE.3 ) THEN
         WRITE(MESSAGESTRING, 1605) MINODT,NTIM24,NUM,NRUN,IDOFST
         call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
...
C
C  BUG1199 11/28/13
C  COMPUTE THE NUMBER OF HOURS IN THE YEAR THEN STORE TO NHRYR() WHICH
C  IS USED BY ERUL26
C
      DO ITSTEP = 1, NUM
         IADD = ITSTEP/NTIM24
         IDSET = IDARUN + IADD
         NHRYR(ITSTEP) = (365 + LEAP26(IDSET, IHR)) * 24
      END DO
C
C  END BUG1199 11/28/13
C
...
C LC commented out JULDA; use code in common
C      CALL JULDA(IDABGN,IHRB,1,1,IYEAR,IHRM,NLSTZ,NOUTDS,TZCS)
      MONBGN = 1
      DAYBGN = 1
      call CALCULATEJULDAY( MONBGN, DAYBGN, IYEAR, IHRM, IDABGN)
...
!CP   IF (IBUG.LT.2) GO TO 110
      IF ( FEWSDEBUG.LT.3 ) GO TO 110
!CP   WRITE(IODBUG,1609) IDA,IHR,LDA,LHR,LDACPD,LHRCPD,MIDNIT,
      WRITE(MESSAGESTRING, 1609) IDA,IHR,LDA,LHR,LDACPD,LHRCPD,MIDNIT,
...
!CP   WRITE(IODBUG,1610) JULDAT,JDAWEK,NODAYR,IDA,IDABGN
      WRITE(MESSAGESTRING, 1610) JULDAT,JDAWEK,NODAYR,IDA,IDABGN
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
...
C
      JULBGN = IDA*24
!CP   IF (IBUG.GE.2) WRITE(IODBUG,1650)
      IF ( FEWSDEBUG.GE.3 ) THEN
...
!CP   IF (IBUG.GE.2) WRITE(IODBUG,1660) ICOPOS(I)
      IF ( FEWSDEBUG.GE.3 ) THEN
         WRITE(MESSAGESTRING, 1660) ICOPOS(I)
         call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
      END IF
 1660 FORMAT(I5)
  550 CONTINUE
...
!CP   1699 FORMAT('    *** EXIT XNIT26 ***')
      IF ( FEWSDEBUG.GE.4 ) THEN
         WRITE(MESSAGESTRING, '(A23)') '    *** EXIT XNIT26 ***'
	 call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
      END IF
      RETURN
      END
