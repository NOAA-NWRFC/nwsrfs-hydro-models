...
C DESC  RUN PERIOD.
C-------------------------------------------------------------------
      SUBROUTINE XFRU26(PO,LOCP,W,LOCOWS,NHRYR)
C------------------------------------------------------------------
...
!CP   1600 FORMAT('   *** ENTER XFRU26 ***')
      IF ( FEWSDEBUG.GE.4 ) THEN
         WRITE(MESSAGESTRING, '(A23)') '   *** ENTER XFRU26 ***'
...
C
      CALL ERUL26(W(LOCW),PO(LOCPT+1),PO(LOCPT+NRUL+1),NHRYR)
C
...
!CP   1699 FORMAT('    *** EXIT XFRU26 ***')
      IF ( FEWSDEBUG.GE.4 ) THEN
         WRITE(MESSAGESTRING, '(A23)') '    *** EXIT XFRU26 ***'
	 call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
      END IF
      RETURN
      END
