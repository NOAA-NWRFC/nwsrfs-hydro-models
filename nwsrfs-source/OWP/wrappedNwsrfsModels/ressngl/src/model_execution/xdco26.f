...
C
      DATA BLANK/4H    /
C
!CP   IF (IBUG.GE.1) WRITE(IODBUG,1600) IBUG,NSUDEF
...
C
 1020 CONTINUE
      ...
      W(LOCDCO) = PO(LOCPM+1)
C
!CP   IF (IBUG.GE.2) WRITE(IODBUG,1620) ISUNUM,LOCDCO,W(LOCDCO)
      IF ( FEWSDEBUG.GE.3 ) THEN
         WRITE(MESSAGESTRING, 1620) ISUNUM,LOCDCO,W(LOCDCO)
         call logfromfortran(DEBUG_LEVEL, MESSAGESTRING) 
      END IF
 1620 FORMAT('   FOR S/U NO.',I5,' WORK LOCATION AND VALUE IS :',I8,
     .       F12.3)
C
      GO TO 100
C
...
!CP   WRITE(IODBUG,1620) ISUNUM,LOCDCO,W(LOCDCO)
      IF ( FEWSDEBUG.LT.3 ) GO TO 100
      WRITE(MESSAGESTRING, 1620) ISUNUM,LOCDCO,W(LOCDCO)
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
      LCEND = LOCSPI+2
!CP   WRITE(IODBUG,1621) (W(L),L=LOCSPI,LCEND)
      WRITE(MESSAGESTRING, 1621) 
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
 1621 FORMAT(' ADD''L VALUES ARE:')
      NPRINT=LCEND
      IF ( NPRINT.GT.100) NPRINT=100
      
      WRITE(FORMATSTR, *) '%10.3f'
      call logonedimensionarrayfromfortran(DEBUG_LEVEL,NULLLINE,1,8,
     >     FORMATSTR,LOCSPI,NPRINT,1,W)
      GO TO 100
C
C---------------------------------------------------------
...
!CP   WRITE(IODBUG,1620) ISUNUM,LOCDCO,W(LOCDCO)
      ...
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
      LCEND=LOCSTQ
!CP   WRITE(IODBUG,1622)
      WRITE(MESSAGESTRING, 1622)
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
 1622 FORMAT(10X,'    IG ITRIB  LOC-LK-G1  RC-ID')
      DO IG=1,NGAGES
      LCST = LCEND+1
      LCEND = LCST+IG*2
!CP   WRITE(IODBUG,1623) IG,ITRIB(IG),W(LCST),(W(L),L=LCEND-1,LCEND)
      WRITE(MESSAGESTRING, 1623) IG,ITRIB(IG),W(LCST),
     $                           (W(L),L=LCEND-1,LCEND)
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
 1623 FORMAT(10X,I6,I6,F10.0,2X,2A4)
      ENDDO
      GO TO 100
C
C---------------------------------------------------------
...
!CP   WRITE(IODBUG,1620) ISUNUM,LOCDCO,W(LOCDCO)
      ...
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
      LCEND = LOCFBD+3
!CP   WRITE(IODBUG,1621) (W(L),L=LOCFBD,LCEND)
      NPRINT=LCEND
      IF ( NPRINT.GT.100 ) NPRINT=100
      
      WRITE(FORMATSTR, *) '%10.3f'
      call logonedimensionarrayfromfortran(DEBUG_LEVEL,NULLLINE,1,8,
     >     FORMATSTR,LOCFBD,NPRINT,1,W)
      GO TO 100
C
C-----------------------------------------------------------
...
!CP   1699 FORMAT('    *** EXIT XDCO26 ***')
      IF ( FEWSDEBUG.GE.4 ) THEN
         WRITE(MESSAGESTRING, '(A23)') '    *** EXIT XDCO26 ***'
	 call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
      END IF
      RETURN
      END
