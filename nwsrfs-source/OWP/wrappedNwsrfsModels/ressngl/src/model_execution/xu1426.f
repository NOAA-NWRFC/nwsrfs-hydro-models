...
C
      SUBROUTINE XU1426(PO,CO,W,D,IDPT,LOCOWS,NHRYR)
C---------------------------------------------------------------------
...
C
      DIMENSION PO(1),CO(1),W(1),LOCOWS(1),D(1),IDPT(1),NHRYR(1)
C
...
C
      IF ( FEWSDEBUG.GE.4 ) THEN
         WRITE(MESSAGESTRING, '(A23)') '   *** ENTER XU1426 ***'
	 call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
      END IF
!CP   IF (IBUG.GE.1) WRITE(IODBUG,1600)
!CP   1600 FORMAT('   *** ENTER XU1426 ***')
C
...
C
      CALL XFRU26(PO,LOCPM,W,LOCOWS,NHRYR)
C
...
C
      LOCRUL = LOCOWS(2)
      LOCMIS = LOCOWS(6)
C      LOCDEV = LOCMIS + NDD*NTIM24
      LOCDEV = LOCMIS + NTIMESTEPS
C
...
!CP   1699 FORMAT('    *** EXIT XU1426 ***')
      IF ( FEWSDEBUG.GE.4 ) THEN
         WRITE(MESSAGESTRING, '(A23)') '    *** EXIT XU1426 ***'
	 call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)  
      END IF
C
      RETURN
