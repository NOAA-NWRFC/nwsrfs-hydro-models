...
C
   ...
   20 CONTINUE
C
!CP   IF (IBUG.GE.2) WRITE(IODBUG,1610) (ELEV(I),I=1,NHALF)
!CP   IF (IBUG.GE.2) WRITE(IODBUG,1611) (AREA(I),I=1,NHALF)
!1611 FORMAT((6F12.3))

      IF ( FEWSDEBUG.GE.3 ) THEN
         WRITE(MESSAGESTRING, 1610)
         call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
 1610    FORMAT(/'   ** ELEVATION VS. AREA CURVE :') 

         WRITE(FORMATSTR, *) '%12.3f'
         call logonedimensionarrayfromfortran(DEBUG_LEVEL,NULLLINE,1,6,
     >        FORMATSTR,1,NHALF,1,ELEV)
         WRITE(FORMATSTR, *) '%12.3f'
         call logonedimensionarrayfromfortran(DEBUG_LEVEL,NULLLINE,1,6,
     >        FORMATSTR,1,NHALF,1,AREA)
      END IF
      RETURN
      END
