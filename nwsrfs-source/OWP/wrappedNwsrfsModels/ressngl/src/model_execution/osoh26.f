...
!CP   WRITE(IODBUG,40) NUMLG,ANUMLG,NUMSM,ANUMSM,NLGDN,NSMDN,NGATE,    N
      WRITE(MESSAGESTRING, 40) NUMLG,ANUMLG,NUMSM,ANUMSM,NLGDN,NSMDN,
     $NGATE,    NLGSOH,NSMSOH,NGASOH,NPOSOH,DISTLG,DISTSM,COEF,SUMFRA,
     $QQI1,QQI2,  QQIM,QQO1,QQO2,QQOK1,QQOK2,SS1,SS2,NTERPQ,NTERP,
     $RTQQIM,IBUG
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING) 
...
C WRITE TRACE AND DEBUG IF REQUIRED.
C
!CP   IF(IBUG-1)420,400,310
!CP310 WRITE(IODBUG,40) NUMLG,ANUMLG,NUMSM,ANUMSM,NLGDN,NSMDN,NGATE,N

      IF ( FEWSDEBUG.LT.4 ) GO TO 420
      WRITE(MESSAGESTRING, 40) NUMLG,ANUMLG,NUMSM,ANUMSM,NLGDN,NSMDN,
     $NGATE,    NLGSOH,NSMSOH,NGASOH,NPOSOH,DISTLG,DISTSM,COEF,SUMFRA,
     $QQI1,QQI2,  QQIM,QQO1,QQO2,QQOK1,QQOK2,SS1,SS2,NTERPQ,NTERP,
     $RTQQIM,IBUG
...
!CP   WRITE(IODBUG,320)(SIGELV(I),SIGSTO(I),I=1,NSIGEL)
      WRITE(MESSAGESTRING, 320) !(SIGELV(I),SIGSTO(I),I=1,NSIGEL)
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
  320 FORMAT(1H0,64H ALTERNATING VALUES OF SIGNIFICANT ELEVATIONS AND
     ...
     >     FORMATSTR,1,NSIGEL,1,SIGELV,SIGSTO)
   
!CP   WRITE(IODBUG,330)(ELVLG(I),QLG(I),I=1,NELVLG)
      WRITE(MESSAGESTRING, 330) !(ELVLG(I),QLG(I),I=1,NELVLG)
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
  330 FORMAT(1H0,101HALTERNATING VALUES OF POOL ELEVATION AND DISCHARGE
     $OVER LARGE-BOARD SPILLWAY WITH ALL BOARDS DOWN ARE) !/(1X,10F12.3))
      WRITE(FORMATSTR, *) '%12.3f'
      call logonedimensionarrayfromfortran(DEBUG_LEVEL,NULLLINE,2,5,
     >     FORMATSTR,1,NELVLG,1,ELVLG,QLG)

      IF(NUMSM.EQ.0) GO TO 350
!CP   WRITE(IODBUG,340)(ELVSM(I),QSM(I),I=1,NELVSM)
      WRITE(MESSAGESTRING, 340) !(ELVSM(I),QSM(I),I=1,NELVSM)
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING) 
  340 FORMAT(1H0,101HALTERNATING VALUES OF POOL ELEVATION AND DISCHARGE
     $OVER SMALL-BOARD SPILLWAY WITH ALL BOARDS DOWN ARE) !/(1X,10F12.3))
      WRITE(FORMATSTR, *) '%12.3f'
      call logonedimensionarrayfromfortran(DEBUG_LEVEL,NULLLINE,2,5,
     >     FORMATSTR,1,NELVSM,1,ELVSM,QSM)

  350 IF(NGATE.EQ.-999) GO TO 370
!CP   WRITE(IODBUG,360)(ELVFL(I),QFLUD(I),I=1,NELVFL)
      WRITE(MESSAGESTRING, 360) !(ELVFL(I),QFLUD(I),I=1,NELVFL)
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
  360 FORMAT(1H0,94H ALTERNATING VALUES OF POOL ELEVATION AND
     $DISCHARGE THROUGH FLOOD GATE FULL OPEN ARE) !/(1X,10F12.3))
      WRITE(FORMATSTR, *) '%12.3f'
      call logonedimensionarrayfromfortran(DEBUG_LEVEL,NULLLINE,2,5,
     >     FORMATSTR,1,NELVFL,1,ELVFL,QFLUD)

!CP   370 WRITE(IODBUG,380)(O(I),SOH(I),TMPSOH(I),I=1,NOSOH)
  370 WRITE(MESSAGESTRING, 380) !(O(I),SOH(I),TMPSOH(I),I=1,NOSOH)
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
  380 FORMAT(1H0,110HSETS OF VALUES (3 VALUES TO A SET) OF OUTFLOW,
     ...
     $FRACTION(1.-SUMFRA) OF THE ROUTING STEP ARE) !/(1X,10F12.3))
      WRITE(FORMATSTR, *) '%12.3f'
      call logonedimensionarrayfromfortran(DEBUG_LEVEL,NULLLINE,3,3,
     >     FORMATSTR,1,NOSOH,1,O,SOH,TMPSOH)

!CP   WRITE(IODBUG,390)(ELVSOH(I),STOSOH(I),I=1,NELSOH)
      WRITE(MESSAGESTRING, 390) !(ELVSOH(I),STOSOH(I),I=1,NELSOH)
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
  390 FORMAT(1H0,112HALTERNATING VALUES OF SPECIFIED ELEVATIONS FOR
     ...
     $UNITS OF MEAN FLOW FOR THE ROUTING STEP ARE) !/(1X,10F12.3))
      WRITE(FORMATSTR, *) '%12.3f'
      call logonedimensionarrayfromfortran(DEBUG_LEVEL,NULLLINE,2,5,
     >     FORMATSTR,1,NELSOH,1,ELVSOH,STOSOH)

!CP   400 WRITE(IODBUG,410)
