...
C
C
!CP   IF (IBUG .GE. 1) WRITE(IODBUG,1601)
!CP   1601 FORMAT('   *** ENTER XM126 ***')
!CP   IF (IBUG .GE. 2) WRITE(IODBUG,1602) VALUE
      IF ( FEWSDEBUG.GE.4 ) THEN
         WRITE(MESSAGESTRING, '(A23)') '   *** ENTER XM126 ***'
	 call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
         WRITE(MESSAGESTRING, 1602) VALUE
	 call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
      END IF
...
!CP   1699 FORMAT('    *** EXIT XM126 ***')
      IF ( FEWSDEBUG.GE.4 ) THEN
          WRITE(MESSAGESTRING, '(A23)')'    *** EXIT XM126 ***'
          call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
      END IF
      RETURN
      END
