...
C
      SUBROUTINE XQT26(PO,CO,D,W,IDPT,LOCWS,LOCOWS,LOBSA,VM126,NHRYR)
C-------------------------------------------------------------------
...
C
ccc	ibug=2
!CP   IF (IBUG.GE.1) WRITE(IODBUG,1650)
!CP   1650 FORMAT('   *** ENTER XQT26 ***')
      IF ( FEWSDEBUG.GE.4 ) THEN
         WRITE(MESSAGESTRING, '(A23)') '   *** ENTER XQT26 ***'
...
C
      LOCTR1 = LOCWS(2)
C      LOCTR2 = LOCTR1 + NDD*NTIM24
      LOCTR2 = LOCTR1 + NTIMESTEPS
C      LOCTR3 = LOCTR2 + NDD*NTIM24
      LOCTR3 = LOCTR2 + NTIMESTEPS
C      LOCTR4 = LOCTR3 + NDD*NTIM24
      LOCTR4 = LOCTR3 + NTIMESTEPS
      IF ( FEWSDEBUG.GE.4 ) THEN
         WRITE(MESSAGESTRING, *) 'LOCTR1 = ', LOCTR1
         call logfromfortran(DEBUG_LEVEL, MESSAGESTRING) 
         WRITE(MESSAGESTRING, *) 'LOCTR2 = ', LOCTR2
         call logfromfortran(DEBUG_LEVEL, MESSAGESTRING) 
      END IF
C
...
C DIVIDE CFS BY NUMBER OF PERIODS PER DAY(NNPPDD)
      ...
      IF ( VM126(IBOHIS) .NE. -888.0 ) THEN
         VM126(IBOHIS)=VM126(IBOHIS)/NNPPDD
      END IF
1225  CONTINUE
C *** ENDCHANGE***
C
!CP   IF (IBUG.GE.2) WRITE(IODBUG,1601) MJ126,IDAHR,MPDST,MPDEND,NVM126,
!CP                  (VM126(I),I=1,NVM126)
      IF ( FEWSDEBUG.GE.3 ) THEN
         WRITE(MESSAGESTRING, 1601) MJ126,IDAHR,MPDST,MPDEND,NVM126 
         call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
...
ccc1602FORMAT('   * STARTING TIME INTERVAL LOOP:'/10X,'ADJUST RUN? ',L4,
...
cc      IENTER = -999
      ...
      ISURG = 0
      ltsbfqx=ltsbfq
!CP   IF (IBUG.LT.2)  GO TO 20
      IF ( FEWSDEBUG.LT.3 )  GO TO 20
      LOOPNO= LOOPNO+1
!CP   WRITE(IODBUG,1602) LOOPNO,ADJRUN,DORAIN,DOBACK,DOSUMF,NUMSIM
      WRITE(MESSAGESTRING, 1602) LOOPNO,ADJRUN,DORAIN,DOBACK,
     $ DOSUMF,NUMSIM
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
...
!CP   WRITE(IODBUG,1603) (W(LPTADL+I-1),I=1,NUM)
      NPRINT=NUM
      IF ( NPRINT.GT.100 ) NPRINT=100
      
      WRITE(MESSAGESTRING, 1603) 
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
 1603 FORMAT(' EQUIVALENT INFLOW FROM DIRECT RAINEVAP:') 
      WRITE(FORMATSTR, *) '%10.1f'
      call logonedimensionarrayfromfortran(DEBUG_LEVEL,NULLLINE,1,8,
     >     FORMATSTR,LPTADL,LPTADL+NPRINT-1,1,W)
   
!CP   WRITE(IODBUG,1604) (D(LTSBFQ+I-1),I=1,NUM)
      WRITE(MESSAGESTRING, 1604) 
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
 1604 FORMAT(' INSTANTEOUS BACK ADJUSTED INFLOW:') 
      WRITE(FORMATSTR, *) '%10.1f'
      call logonedimensionarrayfromfortran(DEBUG_LEVEL,NULLLINE,1,8,
     >     FORMATSTR,LTSBFQ,LTSBFQ+NPRINT-1,1,D)

 15   IF(LOCOWS(13).EQ.0) GO TO 20
!CP   WRITE(IODBUG,1606) (W(LPTQI+I-1),I=1,NUM)
      WRITE(MESSAGESTRING, 1606) 
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
 1606 FORMAT(' INSTANTEOUS INFLOW FOR SUMINF:')
      WRITE(FORMATSTR, *) '%10.1f'
      call logonedimensionarrayfromfortran(DEBUG_LEVEL,NULLLINE,1,8,
     >     FORMATSTR,LPTQI,LPTQI+NPRINT-1,1,W)
   
!CP   WRITE(IODBUG,1607) (W(LPTQIM+I-1),I=1,NUM)
      WRITE(MESSAGESTRING, 1607) 
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
 1607 FORMAT(' MEAN INFLOW FOR SUMINF:')
      WRITE(FORMATSTR, *) '%10.1f'
      call logonedimensionarrayfromfortran(DEBUG_LEVEL,NULLLINE,1,8,
     >     FORMATSTR,LPTQIM,LPTQIM+NPRINT-1,1,W)
 20   CONTINUE
C
...
C DON't CARE if ESP (MAINUM.EQ.2)
C      IF (MAINUM .EQ. 2) FCST = .TRUE.
...
C
      ...
      LOCO = LOCWS(4)
C
!CP   IF (IBUG.LT.2) GO TO 60
      IF ( FEWSDEBUG.LT.3 ) GO TO 60
...
!CP   WRITE(IODBUG,1610) NS2
      WRITE(MESSAGESTRING, 1610) NS2
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
 1610 FORMAT(/2X,28(1H*)/2X,1H*,26X,1H*/2X,1H*,' TIME INTERVAL NO. ',I6,
     .2H */2X,1H*,26X,1H*/2X,28(1H*)/)
C
!CP   WRITE(IODBUG,1613) 
!CP   &  KHRPRX,IDAY,JDAWEK,JULDAY,(W(LOCO+L-1),L=1,NUMCOV)   

      WRITE(MESSAGESTRING, 1613) KHRPRX,IDAY,JDAWEK,JULDAY
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
 1613 FORMAT(/'   ** CARRYOVER AT START OF HOUR ',I3,
     & ' CALENDAY ',I3,' DAYOFWEEK ',I2,' DAYOFYEAR ',I4)
      WRITE(FORMATSTR, *) '%12.2f'
      call logonedimensionarrayfromfortran(DEBUG_LEVEL,NULLLINE,1,6,
     >     FORMATSTR,LOCO,LOCO+NUMCOV-1,1,W)
 60   CONTINUE
C
...
C
  ...
  410 CONTINUE
C
!CP   IF (IBUG.GE.2) WRITE(IODBUG,1630)
      IF ( FEWSDEBUG.GE.3 ) THEN
...
C
      CALL XRCL26(PO,W,D,IDPT,LOCWS,LOCOWS,NHRYR)
C
...
C
!CP   IF (IBUG.GE.2) WRITE(IODBUG,1690) W(LOCTR1+NS2-1),W(LOCTR2+NS2-1),
      IF ( FEWSDEBUG.GE.3 ) THEN
         WRITE(MESSAGESTRING, 1690) W(LOCTR1+NS2-1),W(LOCTR2+NS2-1),
...
C
      ...
      IF (MQXQT) GO TO 1500 !Hsu added for FB1354 10/14
C
...
C
      IF(LOBSTO.EQ.0.OR.ISAVCO.EQ.0) GO TO 1500

C LC comment out reference to MAINUM;
C ALWAYS DO; DON'T CARE IF NOT MAINUM.EQ.1 (FORECAST MODE)
C INSTEAD check if LOBSTO.EQ. NUM; like if MAINUM.NE.1
C     IF(MAINUM.NE.1) GO TO 1500
!     IF(LOBSTO.EQ.NUM) GO TO 1500
C
C
...
C
      ...
      W(LOCST+I-1) = -999.0
 1250 CONTINUE
C      LOCST = LOCST + NDD*NTIM24
      LOCST = LOCST + NTIMESTEPS
 1260 CONTINUE
C
...
C
 9000 CONTINUE
      IF ( FEWSDEBUG.GE.4 ) THEN
         WRITE(MESSAGESTRING, '(A23)') '    *** EXIT XQT26 ***'
	 call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
      END IF
!CP   IF (IBUG.GE.1) WRITE(IODBUG,1699)
!CP   1699 FORMAT('    *** EXIT XQT26 ***')
ccc	ibug=0
      RETURN
      END
