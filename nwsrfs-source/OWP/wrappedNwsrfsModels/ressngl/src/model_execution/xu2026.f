...
!CP   1600 FORMAT('   *** ENTER XU2026 ***')
      IF ( FEWSDEBUG.GE.4 ) THEN
         WRITE(MESSAGESTRING, '(A23)') '   *** ENTER XU2026 ***'
...
C
     ...
     . NDAMQ,IFLAG,NTRP,IBUG)
C
!CP   IF (IBUG.GE.2) WRITE(IODBUG,1690) ELEVT,QMAXT
      IF ( FEWSDEBUG.GE.3 ) THEN
         WRITE(MESSAGESTRING, 1690) ELEVT,QMAXT
	 call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
...
!CP   1699 FORMAT('    *** EXIT XU2026 ***')
      IF ( FEWSDEBUG.GE.4 ) THEN
         WRITE(MESSAGESTRING, '(A23)') '    *** EXIT XU2026 ***'
	 call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
      END IF
      RETURN
      END
