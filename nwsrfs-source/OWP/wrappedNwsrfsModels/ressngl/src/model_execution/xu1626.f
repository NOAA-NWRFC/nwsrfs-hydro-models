...
!CP   INCLUDE 'common/fdbug'
      ...
      INCLUDE 'fctime'
C
      DIMENSION PO(1),W(1),D(1),LOCWS(1),LOCOWS(1),IYDAY(12)
...
!CP   1600 FORMAT('   *** ENTER XU1626 ***')
      IF ( FEWSDEBUG.GE.4 ) THEN
         WRITE(MESSAGESTRING, '(A23)') '   *** ENTER XU1626 ***'
...
!CP   IF(IBUG.LT.2) GO TO 110
      IF ( FEWSDEBUG.LT.4 ) GO TO 110
      IF ( NS2.GT.1 ) GO TO 110
      IST=LOCRTS
      IEND=IST+NUM-1

!CP   WRITE(IODBUG,1602) (D(I),I=IST,IEND)
      IF ( IEND.GT.100 ) NPRINT=100
      WRITE(MESSAGESTRING, 1602) 
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING) 
 1602 FORMAT(5X,'RAINFALL DATA FROM TIME SERIES:') 
      WRITE(FORMATSTR, *) '%8.5f'
      call logonedimensionarrayfromfortran(DEBUG_LEVEL,NULLLINE,1,10,
     >     FORMATSTR,IST,NPRINT,1,D)
...
C bug624 04/11/12 this bug fixed for historical run 
      CALL MDYH1(IDA,IHR,IMON,IDAY,IYEAR,IHRM,NLSTZ,NOUTDS,TZCS)
      ISTART = IYDAY(IMON) + IDAY   
ccc      IDAYC = MOD(NDAY+ISTART-1,1461)
ccc      NDAYC = MOD(IDAYC,365)
C bug870 09/27/12 this bug correct error in bug624
      IDAYC=(NDAY+ISTART-1)/1461
      NDAYC=NDAY-IDAYC
C end bug870

      ADDHTE = -FACT * W(LRETS+NDAYC)*PCT
C end bug624
      GO TO 300
C
...
!CP   IF(IBUG.LT.2) GO TO 220
      IF ( FEWSDEBUG.LT.4 ) GO TO 220
      IF ( NS2.GT.1 ) GO TO 220
      IST=LOCETS
      NDUM=NUM/NTIM24
      IEND=IST+NDUM-1
!CP   WRITE(IODBUG,1605) (D(I),I=IST,IEND)
      IF ( IEND.GT.100 ) NPRINT=100
      WRITE(MESSAGESTRING, 1605) 
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
...
C
!CP   IF (IBUG.LT.2) GO TO 350
      IF ( FEWSDEBUG.LT.3 ) GO TO 350
...
C
!CP   WRITE(IODBUG,1630) NRAEV,ELEV1,ADDHT,ADDHTR,ADDHTE
      WRITE(MESSAGESTRING, 1630) NRAEV,ELEV1,ADDHT,ADDHTR,ADDHTE
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
 1630 FORMAT(5X,'> NRAEV',I2,
     .' AT ELEV. ',F7.2,' RAINEVAP HT = ',F7.3,
     .' RAINFALL AMT = ',F7.3,' EVAP HT = ',F7.3)
!CP   WRITE(IODBUG,1620) TSTADL,READLQ,ADDHTV,AREA1,FACT
      WRITE(MESSAGESTRING, 1620) TSTADL,READLQ,ADDHTV,AREA1,FACT
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
...
C
!CP   IF (IBUG.GE.2) WRITE(IODBUG,1680) NRAEV,READLQ,PCTDIF,HREA,REDORE
      IF ( FEWSDEBUG.GE.1 ) THEN
         WRITE(MESSAGESTRING, 1680) NRAEV,READLQ,PCTDIF,HREA,REDORE
     .       ,ELEV2
         call logfromfortran(DEBUG_LEVEL, MESSAGESTRING) 
      END IF
...
!CP   1699 FORMAT('    *** EXIT XU1626 ***')
      IF ( FEWSDEBUG.GE.4 ) THEN
         WRITE(MESSAGESTRING, '(A23)') '    *** EXIT XU1626 ***'
	 call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
      END IF
      RETURN
      END
