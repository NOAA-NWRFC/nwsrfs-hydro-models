...
C
C
!CP   IF (IBUG.GE.1) WRITE(IODBUG,900)
!CP   900 FORMAT(10X,'*** ENTER XDOL26 ***')
      IF ( FEWSDEBUG.GE.4 ) THEN
         WRITE(MESSAGESTRING, '(A23)') '   *** ENTER XDOL26 ***'
...
C
      IF ( FEWSDEBUG.GE.4 ) THEN
         WRITE(MESSAGESTRING, *) ' LWSUSE =', LWSUSE, ' NSUDEF =',NSUDEF
         call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
      END IF
      LOCWS(1) = 1
...
C
   11 CONTINUE
      LOCWS(2) = LWSUSE + 1
C      LWSUSE = LWSUSE + 4*NDD*NTIM24
      LWSUSE = LWSUSE + 4*NTIMESTEPS
C
...
C
      ...
      LOCOWS(1) = LWSUSE + 1
C      LWSUSE = LWSUSE + 2*NDD*NTIM24
      LWSUSE = LWSUSE + 2*NTIMESTEPS
C
C---------------------------------------------
...
C
      LOCOWS(2) = LWSUSE + 1
C      LWSUSE = LWSUSE + NDD*NTIM24
      LWSUSE = LWSUSE + NTIMESTEPS
C
C--------------------------------------------
...
C
      LOCOWS(5) = LWSUSE + 1
C LC - 2nd refernce to NDD does not use NTIM24 - use NTIMESTEPS/NTIM24 anyway
      LWSUSE = LWSUSE + NTIMESTEPS + 2*NSE + (NTIMESTEPS/NTIM24) + NUMCO
C
C-------------------------------------------------------------------
...
C
      LOCOWS(11) = LWSUSE + 1
C     LWSUSE = LWSUSE + 3*NDD*NTIM24
      LWSUSE = LWSUSE + 3*NTIMESTEPS
C
C---------------------------------------------------------------
...
C
      LOCOWS(12) = LWSUSE + 1
C     LWSUSE = LWSUSE + 8*NDD*NTIM24
      LWSUSE = LWSUSE + 8*NTIMESTEPS
C
C--------------------------------------------------------------------
...
C
      LPTQI = LOCOWS(13)
C     LPTQIM = LPTQI + NDD*NTIM24
      LPTQIM = LPTQI + NTIMESTEPS
      IOFF = IDOFST * NTIM24
C
...
C
      LWK = PO(16)
!CP   IF (IBUG.GE.2) WRITE(IODBUG,1690) LWSUSE,LWK,(LOCWS(K),K=1,5),
      IF ( FEWSDEBUG.GE.3 ) THEN
         WRITE(MESSAGESTRING, 1690) LWSUSE,LWK,(LOCWS(K),K=1,5),
     .                              (LOCOWS(K),K=1,13)
         call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
 1690 FORMAT(1H0,' NO. OF WORDS NEEDED FOR WORK SPACE = ',I8,' PO(16)='
     .,I5/,6X,'LOCATIONS FOR REQUIRED WORK SPACE = ',5I8,/
     .6X,'              OPTIONAL WORK SPACE = ',13I8)
      END IF
C
!CP   IF (ITRACE.GE.1) WRITE(IODBUG,901)
!CP   901 FORMAT(11X,'*** EXIT XDOL26 ***')
      IF ( FEWSDEBUG.GE.4 ) THEN
         WRITE(MESSAGESTRING, '(A23)') '    *** EXIT XDOL26 ***'
	 call logfromfortran(DEBUG_LEVEL, MESSAGESTRING) 
      END IF
C
      RETURN
