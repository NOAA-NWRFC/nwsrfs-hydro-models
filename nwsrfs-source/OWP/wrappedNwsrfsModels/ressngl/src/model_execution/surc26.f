...
!CP   WRITE(IODBUG,40) SURCHG,TENDCY,QIBAC1,QI1,QI2,QIMRAC,QIMSUR,FRACT,
      WRITE(MESSAGESTRING, 40) SURCHG,TENDCY,QIBAC1,QI1,QI2,QIMRAC,
     & QIMSUR,FRACT,ELBAC1,ELEV1,ELEV2,HUPPER,HCHECK,HLOWER,RULEL1,
     & RULEL2
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
   40 FORMAT(1H0,10X,
...
C Kwang said that if less than -900 that means missing value so we don't 
C need to do "HUPPER + 999.0"

   50 HCHECX=HCHECK
C     IF(HCHECX.LT.-900.0) HCHECX=RULEL1 + HCHECK + 999.0
      IF(HCHECX.LT.-900.0) HCHECX=RULEL1
      HLOWEX=HLOWER
C     IF(HLOWEX.LT.-900.0) HLOWEX=RULEL1 + HLOWER + 999.0
      IF(HLOWEX.LT.-900.0) HLOWEX=RULEL1
      HUPPEX=HUPPER
C     IF(HUPPEX.LT.-900.0) HUPPEX=RULEL1 + HUPPER + 999.0
      IF(HUPPEX.LT.-900.0) HUPPEX=RULEL1 
      TNDCY=TENDCY
...
CC      GO TO 170
 95   DIF=QI1-QIBAC1
  ...
  170 SURCHG=.TRUE.
  180 TENDCY=TNDCY
!CP   IF(IBUG-1) 250,230,190
! 190 WRITE(IODBUG,200)
      IF ( FEWSDEBUG.LT.4 ) GO TO 250
...
CC220 FORMAT(1H0,56H ALTERNATING VALUES OF ELEVATION AND REQUIRED INFLOW
CC     $ ARE/(1X,10F12.3))
