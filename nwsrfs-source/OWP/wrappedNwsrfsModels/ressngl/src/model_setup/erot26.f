...
C
      DO 100 I=1,NUMERR
         N = IERTYP(I)
!CP      WRITE (IPR,20) N,IERLN(I),IERFLD(I),IERPOS(I)
         WRITE(MESSAGESTRING, 20)  N,IERLN(I),IERFLD(I),IERPOS(I)
...
80          CONTINUE
         IF (IERNME(1,I).EQ.IBLNK) GO TO 90
            NWORDS = 3
            IF (N.EQ.93) NWORDS = 1
!CP         WRITE (IPR,FMT) (IERNME(K,I),K=1,NWORDS)
            WRITE(MESSAGESTRING, FMT) (IERNME(K,I),K=1,NWORDS)
            call logfromfortran(FATAL_LEVEL, MESSAGESTRING)
!CP         CALL ERROR
            GO TO 100
!CP90    WRITE (IPR,FMT)
 90      WRITE(MESSAGESTRING, FMT) 
         call logfromfortran(FATAL_LEVEL, MESSAGESTRING)
!CP      CALL ERROR
100   CONTINUE
C
      IF (IERR.GT.0) THEN
!CP      WRITE (IPR,110) IERR,VALUE1,VALUE2,IPT
         WRITE(MESSAGESTRING, 110) IERR,VALUE1,VALUE2,IPT
         call logfromfortran(FATAL_LEVEL, MESSAGESTRING)
