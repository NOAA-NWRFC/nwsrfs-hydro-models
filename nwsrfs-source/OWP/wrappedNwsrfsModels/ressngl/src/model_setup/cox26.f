...
C
!CP   SUBROUTINE COX26(POLD,COLD,PNEW,CNEW,D,MD)
      SUBROUTINE COX26(POLD,COLD,PNEW,CNEW)
C
...
C
!CP   COMMON BLOCKS
!CP   COMMON/FENGMT/METRIC
!CP   COMMON/IONUM/IN,IPR,IPU
!     REAL*8 SUBNAM
!     LOGICAL CONV3
...
C
C
      CHARACTER*5 DIMN, DIMO, UCODE
      DATA BLANK/4H    /
!CP   DATA SUBNAM/8HCOX26   /
!CP   DATA DIML3T/4HL3/T/
C
C----------------------------
...
!     IF (IPBUG .GT. 0) IBUG=2
      IF ( FEWSDEBUG.GE.3 ) THEN
          WRITE(MESSAGESTRING, '(A23)') '    *** ENTER COX26 ***'
      ...
      MDTO = POLD(7)
      NTO = 24/MDTO
!CP   IF(IBUG.EQ.0) GO TO 20
      IF ( FEWSDEBUG.LT.1 ) GO TO 20
C
C  DEBUG OUTPUT - NEW AND OLD PARAMETERS AND CARRYOVER
!CP   WRITE(IODBUG,800)
      WRITE(MESSAGESTRING, 800)
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
!CP  800 FORMAT(/1X,'RES-SNGL DEBUG -- OLD AND NEW PARAMETERS AND',
  800 FORMAT(/1X,'RES-SNGL DEBUG -- OLD AND NEW CARRYOVER')
!CP   WRITE(IODBUG,801) NPTO,NPTN,NCTO,NCTN,NTO,NTN
      WRITE(MESSAGESTRING, 801) NPTO,NPTN,NCTO,NCTN,NTO,NTN 
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
  801 FORMAT(1X,'NPO,NPN,NCO,NCN,NTO,NTN= ',6I5)
  
!CP   WRITE(IODBUG,802) (POLD(I),I=1,NPTO)
!CP   WRITE(IODBUG,802) (PNEW(I),I=1,NPTN)
!CP   WRITE(IODBUG,802) (COLD(I),I=1,NCTO)
!CP   WRITE(IODBUG,802) (CNEW(I),I=1,NCTN)

C     WRITE(FORMATSTR, *) '%8.2f'
C     call logonedimensionarrayfromfortran(DEBUG_LEVEL,NULLLINE,1,15,
C     >     FORMATSTR,1,NPTO,1,POLD)
      
C      WRITE(FORMATSTR, *) '%8.2f'
C      call logonedimensionarrayfromfortran(DEBUG_LEVEL,NULLLINE,1,15,
C     >     FORMATSTR,1,NPTN,1,PNEW)
      
      WRITE(FORMATSTR, *) '%8.2f'
      call logonedimensionarrayfromfortran(DEBUG_LEVEL,NULLLINE,1,15,
     >     FORMATSTR,1,NCTO,1,COLD)
      
      WRITE(FORMATSTR, *) '%8.2f'
      call logonedimensionarrayfromfortran(DEBUG_LEVEL,NULLLINE,1,15,
     >     FORMATSTR,1,NCTN,1,CNEW)
   20 CONTINUE
C
...
C  END OF CHANGE OF 1/24/91
   ...
   35 CONTINUE
      ...
      GO TO 44
!CP   36 WRITE(IPR,803) NTO,NTN
  36  WRITE(MESSAGESTRING,803) NTO,NTN
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
  803 FORMAT(10X,'OPERATION TIME INTERVAL CHANGED FROM ',I5,' TO',I5)
      GO TO 39
!CP   37 WRITE(IPR,804) NPTO,NPTN
  37  WRITE(MESSAGESTRING,804) NPTO,NPTN
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
  804 FORMAT(10X,'PARAMETER NO. CHANGED FROM ',I5,' TO',I5)
      GO TO 39
!CP   38 WRITE(IPR,805) NCTO,NCTN
  38  WRITE(MESSAGESTRING,805) NCTO,NCTN
      call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)  
  805 FORMAT(10X,'CARRYOVER NO. CHANGED FROM ',I5,' TO',I5)
   39 I1=2
      I2=6
!CP   WRITE(IPR,806) (PNEW(I),I=I1,I2)
      WRITE(MESSAGESTRING,806) (PNEW(I),I=I1,I2)
  ...
  806 FORMAT(10X,'**WARNING** FOR OPERATION= RES-SNGL ',2X,'NAME= ',5A4)
      ...
      call logfromfortran(WARNING_LEVEL, MESSAGESTRING)
  807 FORMAT(/10X,'NO CARRYOVER TRANSFER WILL BE PERFORMED DUE TO ',
...
C
 1020 CONTINUE
      ...
      NCOLD = POLD(LPO+1)
 1102 CONTINUE
!CP   IF (IBUG .GE. 1) WRITE(IODBUG,1190)LEVEL
      IF ( FEWSDEBUG.GE.3 ) THEN
         WRITE(MESSAGESTRING,1190) LEVEL 
         call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
 1190    FORMAT(/17X,'PRESCRIBED DISCHARGE SCHEME -- SETQ(',I1,1H))
      END IF
!CP   IF (IBUG .LE. 1) GO TO 1191
      IF ( FEWSDEBUG.LT.3) GO TO 1191
      IEND = LPO+NPOLD-1
!CP   IF (NPOLD .GE. 1) WRITE(IODBUG,901) (POLD(I),I=LPO,IEND)
      IF (NPOLD .GE. 1) THEN
         WRITE(FORMATSTR, *) '%10.2f'
	 call logonedimensionarrayfromfortran(DEBUG_LEVEL,NULLLINE,1,10,
     >        FORMATSTR,LPO,IEND,1,POLD)
      END IF
      IEND = LPN+NPNEW-1
!CP   IF (NPNEW .GE. 1) WRITE(IODBUG,901) (PNEW(I),I=LPN,IEND)
      IF (NPNEW .GE. 1) THEN
         WRITE(FORMATSTR, *) '%10.2f'
	 call logonedimensionarrayfromfortran(DEBUG_LEVEL,NULLLINE,1,10,
     >        FORMATSTR,LPN,IEND,1,PNEW)
      END IF
 1191 CONTINUE
      DTYPE = PNEW(LTN+2)
!CP   CALL FDCODE(DTYPE,X,DIMN,IX,IX,X,IX,IX)
      CALL getDimensionAndUnitInFortran(DTYPE, DIMN, UCODE)
      DTYPE = POLD(LTO+2)
!CP   CALL FDCODE(DTYPE,X,DIMO,IX,IX,X,IX,IX)
      CALL getDimensionAndUnitInFortran(DTYPE, DIMO, UCODE)
 ...
 1108 CNEW(I) = DUMY
      GO TO 1150
 1105 CONTINUE
!CP   IF (NCOLD .GE. 1) CALL TSCV26(COLD,X,NTO,NTN,LCO,NCNEW,NCOLD,D)
      IF (NCOLD .GE. 1) CALL TSCV26(COLD,X,NTO,NTN,LCO,NCNEW,NCOLD)
...
C     SAME UNITS FOR OLD AND NEW TIME SERIES
 ...
 1114 CNEW(I) = X(J)
      GO TO 1150
 1116 IF (TRIM(DIMO) .EQ. "L3/T") GO TO 1119
...
C     OLD TIME SERIES = INSTANTANEOUS DISCHARGE
 ...
 1150 CONTINUE
!CP   IF (IBUG .LE. 1) GO TO 500
      IF ( FEWSDEBUG.LE.3 ) GO TO 500
      IEND = LCO+NCOLD-1
!CP   IF (NCOLD .GE. 1) WRITE(IODBUG,901) (COLD(I),I=LCO,IEND)
      IF (NCOLD .GE. 1) THEN
         WRITE(FORMATSTR, *) '%12.6f'
         call logonedimensionarrayfromfortran(DEBUG_LEVEL,NULLLINE,1,10,
     >        FORMATSTR,LCO,IEND,1,COLD)
      END IF
      IEND = LCN+NCNEW-1
!CP   IF (NCNEW .GE. 1) WRITE(IODBUG,901) (CNEW(I),I=LCN,IEND)
      IF (NCNEW .GE. 1) THEN
         WRITE(FORMATSTR, *) '%12.6f'
         call logonedimensionarrayfromfortran(DEBUG_LEVEL,NULLLINE,1,10,
     >        FORMATSTR,LCN,IEND,1,CNEW)
      END IF
!CP   WRITE(IODBUG,900) LPO,LTO,LCO,NPOLD,NCOLD,LPN,LTN,LCN,NPNEW,NCNEW
      IF ( FEWSDEBUG.GE.4 ) THEN
         WRITE(MESSAGESTRING,900) LPO,LTO,LCO,NPOLD,NCOLD,LPN,LTN,LCN,
     >                            NPNEW,NCNEW
         call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
      END IF
      GO TO 500
...
C
 1030 CONTINUE
      ...
      NCOLD = POLD(LPO+1)
 1202 CONTINUE
!CP   IF (IBUG .GE. 1) WRITE(IODBUG,1290)LEVEL
      IF ( FEWSDEBUG.GE.3 ) THEN
         WRITE(MESSAGESTRING,1290) LEVEL
	 call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
      END IF
 1290 FORMAT(/17X,'PRESCRIBED ELEVATION SCHEME -- SETH(',I1,1H))
      IEND = LPO+NPOLD-1
!CP   IF (IBUG .GE. 2) WRITE(IODBUG,901) (POLD(I),I=LPO,IEND)
      IF ( FEWSDEBUG.GE.3 ) THEN
         WRITE(FORMATSTR, *) '%10.2f'
         call logonedimensionarrayfromfortran(DEBUG_LEVEL,NULLLINE,1,10,
     >        FORMATSTR,LPO,IEND,1,POLD)
      END IF
      IEND = LPN+NPNEW-1
!CP   IF (IBUG .GE. 2) WRITE(IODBUG,901) (PNEW(I),I=LPN,IEND)
      IF ( FEWSDEBUG.GE.3 ) THEN
         WRITE(FORMATSTR, *) '%10.2f'
         call logonedimensionarrayfromfortran(DEBUG_LEVEL,NULLLINE,1,10,
     >        FORMATSTR,LPN,IEND,1,PNEW)
...
C
 1050 NCOLD=0
      NCNEW=1
!CP   IF (IBUG .GE. 1) WRITE(IODBUG,1490)LEVEL
      IF ( FEWSDEBUG.GE.3 ) THEN
         WRITE(MESSAGESTRING,1490) LEVEL
	 call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
      END IF
...
!CP   IF (IBUG .LE. 1) GO TO 500
      IF ( FEWSDEBUG.LE.3 ) GO TO 500
!CP   IF (NCOLD .GE. 1) WRITE(IODBUG,901) COLD(LCO)
      IF (NCOLD .GE. 1) THEN
         WRITE(MESSAGESTRING,'(F12.6)') COLD(LCO)
         call logfromfortran(DEBUG_LEVEL, MESSAGESTRING) 
      END IF
!CP   IF (NCNEW .GE. 1) WRITE(IODBUG,901) CNEW(LCN)
      IF (NCNEW .GE. 1) THEN
         WRITE(MESSAGESTRING,'(F12.6)') CNEW(LCN)
         call logfromfortran(DEBUG_LEVEL, MESSAGESTRING) 
      END IF
!CP   WRITE(IODBUG,900) LPO,LTO,LCO,NPOLD,NCOLD,LPN,LTN,LCN,NPNEW,NCNEW
      IF ( FEWSDEBUG.GE.4 ) THEN
...
C
 10600 NCOLD=0
      NCNEW=1
!CP   IF (IBUG .GE. 1) WRITE(IODBUG,1590) LEVEL
      IF ( FEWSDEBUG.GE.3 )THEN
         WRITE(MESSAGESTRING,1590) LEVEL
         call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
      END IF
...
C
!CP 1080 IF (IBUG .GE. 1) WRITE(IODBUG,1790) LEVEL
 1080 IF ( FEWSDEBUG.GE.3 ) THEN
         WRITE(MESSAGESTRING,1790) LEVEL
         call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
      END IF
 1790 FORMAT(/17X,'D/S H OR Q AND POOL H CONTROLLED POOL RELEASE ',
     &'SCHEME -- STPOOLQ(',I1,1H))
      CALL ISUN26(POLD,COLD,LPO,LTO,LCO,NPOLD,IFIND,NUMN)
!CP   IF (IBUG .LE. 1) GO TO 1791
      IF ( FEWSDEBUG.LT.3 ) GO TO 1791
      IEND = LPO+NPOLD-1
!CP   IF (NPOLD .GE. 1) WRITE(IODBUG,901) (POLD(I),I=LPO,IEND)
      IF (NPOLD .GE. 1) THEN
         WRITE(FORMATSTR, *) '%10.2f'
...
C   BRING POINTER TO "STPOOLQ" CARRYOVER IN OLD ARRAY
 ...
 1732 CONTINUE
 1736 CONTINUE
!CP   IF (IBUG .GE. 2) WRITE(IODBUG,900)
      IF ( FEWSDEBUG.GE.4 ) THEN
         WRITE(MESSAGESTRING,900) NGN,NLKN,LPND,LCND,NGO,NLKO,LPOD,LCOD
         call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
      END IF
      ITSN = 0
...
C
!CP 1100 IF (IBUG .GE. 1) WRITE(IODBUG,930)LEVEL
 1100 IF ( FEWSDEBUG.GE.3 )THEN
         WRITE(MESSAGESTRING,930) LEVEL
         call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
 930     FORMAT('MINH(',I1,1H))
!CP   IF (IBUG .GE. 1) WRITE(IODBUG,982)CO(LC)
         WRITE(MESSAGESTRING,982) CO(LC)
         call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
      END IF
 982  FORMAT(3HUPQ,F11.2)

      LC=LC+1
!CP   IF (IBUG .GE. 1) WRITE(IODBUG,984)CO(LC),CO(LC+1)
      IF ( FEWSDEBUG.GE.3 ) THEN
         WRITE(MESSAGESTRING,984) CO(LC),CO(LC+1)
         call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
 984     FORMAT(7HUPSTAGE,2(F11.2))
!CP   IF (IBUG .GE. 1) WRITE(IODBUG,985)
         WRITE(MESSAGESTRING,985)
         call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
...
C
!CP 1110 IF (IBUG .GE. 1) WRITE(IODBUG,2090)LEVEL
 1110 IF ( FEWSDEBUG.GE.3 ) THEN
         WRITE(MESSAGESTRING,2090) LEVEL
	 call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
      END IF
...
C
!CP 1120 IF (IBUG .GE. 1) WRITE(IODBUG,2190)LEVEL
 1120 IF ( FEWSDEBUG.GE.3 ) THEN
         WRITE(MESSAGESTRING,2190)LEVEL
         call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
      END IF
...
C
 ...
 2140 NCNEW = LCND-LCN+1
      ...
      DO 2144 I=LCN,LCND
 2144 CNEW(I)=0.0
!CP 2145 IF (IBUG .LE. 1) GO TO 500
 2145 IF ( FEWSDEBUG.LE.3 ) GO TO 500
      IEND = LCO+NCOLD-1
!CP   IF (NCOLD .GE. 1) WRITE(IODBUG,901) (COLD(I),I=LCO,IEND)
      IF (NCOLD .GE. 1) THEN
         WRITE(FORMATSTR, *) '%12.6f'
...
C
 1130 CONTINUE
      NCOLD=0
      NCNEW=NTN
!CP   IF (IBUG .GE. 1) WRITE(IODBUG,2290)LEVEL
      IF ( FEWSDEBUG.GE.3 ) THEN
         WRITE(MESSAGESTRING,2290)LEVEL
         call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
      END IF
...
!CP 2230 IF (IBUG .LE. 1) GO TO 500
 2230 IF ( FEWSDEBUG.LE.3 ) GO TO 500
      IEND = LCO+NCOLD-1
!CP   IF (NCOLD .GE. 1) WRITE(IODBUG,901) (COLD(I),I=LCO,IEND)
      IF (NCOLD .GE. 1) THEN
         WRITE(FORMATSTR, *) '%12.6f'
...
C
!CP 260  IF (IBUG .GE. 1) WRITE(IODBUG,2690)LEVEL
 260  IF ( FEWSDEBUG.GE.3 ) THEN
         WRITE(MESSAGESTRING,2690) LEVEL
	 call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
      END IF 
 2690 FORMAT(/17X,'RULECURVE ADJUST UTILITY -- RULADJDJ(',I1,1H))
      CALL ISUN26(POLD,COLD,LPO,LTO,LCO,NPOLD,IFIND,NUMN)
!CP   IF (IBUG .LE. 1) GO TO 2691
      IF ( FEWSDEBUG.LE.3 ) GO TO 2691
      IEND = LPO+NPOLD-1
!CP   IF (NPOLD .GE. 1) WRITE(IODBUG,901) (POLD(I),I=LPO,IEND)
      IF (NPOLD .GE. 1) THEN
         WRITE(FORMATSTR, *) '%10.2f'
...
C
 ...
 2620 CONTINUE
      ...
      NCNEW = NDEVN+1
      LCOP1 = LCO+1
!CP   IF (NDEVO .GE. 1) CALL TSCV26(COLD,X,NTO,NTN,LCOP1,NDEVN,NDEVO,D)
      IF (NDEVO .GE. 1) CALL TSCV26(COLD,X,NTO,NTN,LCOP1,NDEVN,NDEVO)
      LCNP1 = LCN+1
      CALL CNCO26(CNEW,X,NDEVN,NDEVN,LCNP1)
!CP 2630 IF (IBUG .LE. 1) GO TO 500
 2630 IF ( FEWSDEBUG.LE.3 ) GO TO 500
!CP   IF (NDEVO .GE. 1) WRITE(IODBUG,901) (X(I),I=1,NDEVO)
      IF (NDEVO .GE. 1) THEN
         WRITE(FORMATSTR, *) '%12.6f'
	 call logonedimensionarrayfromfortran(DEBUG_LEVEL,NULLLINE,1,10,
     >        FORMATSTR,1,NDEVO,1,X)
      END IF
      IEND = LCN+NCNEW-1
!CP   IF (NCNEW .GE. 1) WRITE(IODBUG,901) (CNEW(I),I=LCN,IEND)
      IF (NCNEW .GE. 1) THEN
...
!CP   IF (IBUG .GE. 1) WRITE(IODBUG,2990)LEVEL
      IF ( FEWSDEBUG.GE.3 ) THEN
         WRITE(MESSAGESTRING,2990)LEVEL
         call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
      END IF
...
C
  300 NCOLD=0
      NCNEW=1
!CP   IF (IBUG .GE. 1) WRITE(IODBUG,3090)LEVEL
      IF ( FEWSDEBUG.GE.3 ) THEN
         WRITE(MESSAGESTRING,3090)LEVEL
	 call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
      END IF
...
C
 500  CONTINUE
!CP   IF(IBUG.EQ.0) GO TO 510
      IF ( FEWSDEBUG.LT.1 ) GO TO 510
!CP   WRITE(IODBUG,902) (CNEW(I),I=1,NCTN)
