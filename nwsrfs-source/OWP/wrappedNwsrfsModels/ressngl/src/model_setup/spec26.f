...
!CP   IF (IBUG.GE.1) WRITE (IPR,*) 'IN SPEC26 - NSUIN=',NSUIN,
!CP  *   ' DEFCDE(NSUIN)=',DEFCDE(NSUIN)
      IF ( FEWSDEBUG.GE.4 ) THEN
         WRITE(MESSAGESTRING, *)'IN SPEC26 - NSUIN= ',
     >	        NSUIN,' DEFCDE(NSUIN)= ',DEFCDE(NSUIN)
...
C
  100 CONTINUE
      ASU=SUNO(1)
!CP   IF (IBUG .GE. 2) WRITE(IPR,890) ASU
      IF ( FEWSDEBUG.GE.3 ) THEN
         WRITE(MESSAGESTRING, 890) ASU
	 call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
      END IF
...
C
 9999 CONTINUE
      ISUIN = NSUIN
!CP   IF (NUMERR.EQ.0.AND.NUMWRN.EQ.0.AND.IBUG.LT.2) RETURN
      IF (NUMERR.EQ.0.AND.NUMWRN.EQ.0.AND.FEWSDEBUG.LT.3) RETURN
