...
C
!CP   IF (ITRACE.GE.3) WRITE(IODBUG,*) 'ENTER STOR26'
      IF ( FEWSDEBUG.GE.4 ) THEN
         WRITE(MESSAGESTRING, '(A12)') 'ENTER STOR26'
...
!CP      IF (IBUG.GE.1) WRITE (IODBUG,*) 'IN STOR26 - I=',I,
         IF ( FEWSDEBUG.GE.4 ) THEN
            WRITE(MESSAGESTRING, *) 'IN STOR26 - I=',I,
...
!               ENDIF
            IF ( FEWSDEBUG.GE.4 ) THEN 
               WRITE(MESSAGESTRING, *)
     >         'IN STOR26 - (WORK(N),N=LOCPSP,LOCPSP+NPAR)=', 
     >         (WORK(N),N=LOCPSP,LOCPSP+NPAR)
               call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)

               WRITE(MESSAGESTRING, *)'IN STOR26 - NEXTP=',NEXTP
               call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
            END IF
         ...
         IF (NTS.GT.0) THEN
!CP         IF (IBUG.GE.1) WRITE (IODBUG,*) 'IN STOR26 - NEXTTS=',NEXTTS
            IF ( FEWSDEBUG.GE.4 ) THEN
               WRITE(MESSAGESTRING, *) 'IN STOR26 - NEXTTS=',NEXTTS
               call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
            END IF
         ...
         IF (NCO.GT.0) THEN
!CP         IF (IBUG.GE.1) WRITE (IODBUG,*) 'IN STOR26 - NEXTCO=',NEXTCO
            IF ( FEWSDEBUG.GE.4 ) THEN
               WRITE(MESSAGESTRING, *) 'IN STOR26 - NEXTCO=',NEXTCO
               call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
            END IF
...
C
      ...
      LOCRC = LOCNRC + 1
!CP   IF (IBUG.GE.1) WRITE(IODBUG,1688) RCID26
      IF ( FEWSDEBUG.GE.4 ) THEN
          WRITE(MESSAGESTRING, 1688) RCID26
	  call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
 1688     FORMAT(' IDS OF ALL STORED RATING CURVES:' / (2X,2A4,2X,2A4))
      END IF
      DO 820 I=1,3
!CP       IF (IBUG.GE.1) WRITE(IODBUG,1689) I,NRC26(I)
          IF ( FEWSDEBUG.GE.4 ) THEN
	     WRITE(MESSAGESTRING, 1689) I,NRC26(I)
             call logfromfortran(DEBUG_LEVEL, MESSAGESTRING) 
 1689        FORMAT(' FOR TYPE ',I2,' RC, NO. DEFINED = ',I2)
         ...
         DO 810 J=1,NRSTO
            JST = (J-1)*2 + 1
!CP         IF (IBUG.GE.1) WRITE( IODBUG,1690) RCID26(JST,I),
!CP  *         RCID26(JST+1,I),LOCRC
            IF ( FEWSDEBUG.GE.4 ) THEN
	       WRITE(MESSAGESTRING, 1690) RCID26(JST,I),
 ...
 1690 FORMAT(' ID OF RC STORED = ',2A4,' LOC IN PO ARRAY =',I4)
  ...
  820    CONTINUE
C
!CP 9000 IF (IBUG.GE.1) WRITE(IODBUG,1695)
!CP      IF (ITRACE.GE.3) WRITE(IODBUG,*) 'EXIT STOR26'
 9000    IF ( FEWSDEBUG.GE.4 ) THEN
            WRITE(MESSAGESTRING, 1695) NPARG,NTSG,NCOG,NPARSU,
     >	                               NTSSU,NTSPAC,NCOSU
            call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
 1695       FORMAT('NPARG,NTSG,NCOG,NPARSU,NTSSU,NTSPAC,NCOSU=',7I10)

         END IF
	 IF ( FEWSDEBUG.GE.4 ) THEN
            WRITE(MESSAGESTRING, '(A11)') 'EXIT STOR26'
            call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
         END IF
C
      RETURN
