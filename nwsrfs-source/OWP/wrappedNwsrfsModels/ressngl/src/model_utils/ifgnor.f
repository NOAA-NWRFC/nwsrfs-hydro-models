...
C
      IBUG = IFBUG(4HIGNO)
!CP   IF (IBUG.GE.1) WRITE(IODBUG,1610) NIGNOR,(IGNOPT(K),IGSDAT(K),
      IF ( FEWSDEBUG.GE.5 ) THEN
         WRITE(MESSAGESTRING, 1610) NIGNOR,(IGNOPT(K),IGSDAT(K),
     .      IGEDAT(K),K=1,NIGNOR)
...
C
  999 CONTINUE
!CP   IF (IBUG.GE.1) WRITE(IODBUG,1690) ITYPE,JULHR,ISUTYP,IFGNOR
      IF ( FEWSDEBUG.GE.5 ) THEN
         WRITE(MESSAGESTRING, 1690) ITYPE,JULHR,ISUTYP,IFGNOR
	 call logfromfortran(DEBUG_LEVEL, MESSAGESTRING)
