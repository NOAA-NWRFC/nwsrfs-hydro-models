...
void readModelArgs()
{
     ...
     for (i = 0; i < numOfProperties; i++)
     {
		 ...
		 else if (!strcmp(properties[i].name, OUTPUT_HTML_FILE_NAME))
         {
             strcpy(modelArgs.outputHTMLFile, properties[i].value);
         }
		 else if (!strcmp(properties[i].name, HTML_CSS_FILE_NAME))
		 {
			strcpy(modelArgs.HTMLCSSFile, properties[i].value);
		 }
...
void setInformationFromArguments(char *argsFileName)
{
   ...
   logMessageWithArgsAndExitOnError(DEBUG_LEVEL,
   "Using OHD Model Version: %s%s %s\n", VERSION_NAME, VERSION_NUMBER, VERSION_DATE);
...
/*****************************************************************************
 *   Module Name: getOutputHTMLFileName()
 *  
 *   Original Author: CHPS/FEWS Team
 *  
 *   Module Creation Date: 05/05/08
 *  
 *   Description:
 *      Returns a copy output HTML file name.
 *  
 *   Calling Arguments:
 *  
 *      Name       Input/Output     Type       Description
 *  
 *   Return Value: the output HTML file name 
 *             
 *   Local Variables:
 *      Type       Name                        Description
 *       
...
char *getOutputHTMLFileName()
{
   return modelArgs.outputHTMLFile;

} /* getOutputHTMLFileName() ---------------------------------------------- */

/*****************************************************************************
*   Module Name: getHTMLCSSFileName()
*  
*   Original Author: CHPS/FEWS Team
*  
*   Module Creation Date: 05/05/08
*  
*   Description:
*      Returns a copy output HTML CSS file name.
*  
*   Calling Arguments:
*  
*      Name       Input/Output     Type       Description
*  
*   Return Value: the output HTML CSS file name 
*             
*   Local Variables:
*      Type       Name                        Description
*       
...
char *getHTMLCSSFileName()
{
	return modelArgs.HTMLCSSFile;
}
/*****************************************************************************
...
int readOptions(int argc, char**argv)
{
   ...
   while ( (c = getopt(argc, argv, "Vh?")) != EOF )
   {
      switch(c)
      {
	    ...
	    break;
         case 'V':
            printf("\n\tUsing OHD Model: %s - Version: %s%s %s\n\n",
	  	   argv[0], VERSION_NAME, VERSION_NUMBER, VERSION_DATE);
	    break;
         case '?':
            err = -1;
            break;
