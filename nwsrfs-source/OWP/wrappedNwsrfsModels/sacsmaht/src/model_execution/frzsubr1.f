...
C ----------------------------------------------------------------------
     ...
     +         SMCMAX, PSISAT, B, DT, K,STYPE,QUARTZ,IVERS,CK) 
...
100   FREE=FRH2O(TAVG, SMC, SH2O, SMCMAX, B, PSISAT, CK )
...
77    RETURN
 ...
 42   GF=H  
  39  RETURN  
      ...
      FUNCTION FRH2O(T,SMC,SH2O,SMCMAX,B,PSIS,CK)
...
CC  WILL BE ESTIMATED FROM FLERCHIGER EQ.
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      ...
      FRH2O = AMAX1 ( (SMC - SWL*0.01 ), 0.02 )

c  ***        Flerchinger equation             *******      
cc      fk=(((-HLICE/(GS*psis))*((TR-T0)/TR))**(-1/b))*smcmax
cc      frh2o = AMIN1 ( fk, smc )
