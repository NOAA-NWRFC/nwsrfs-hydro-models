...
C
cCP   SUBROUTINE CHCO24(KDA,KHR,NOUTZ,NOUTDS,N,IFRZE,IPRINT,IOUT,
cCP   1  APIX,SMIX,IBUG)
      SUBROUTINE CHCO24(KDA,KHR,NOUTZ,NOUTDS,N,IFRZE,IPRINT,
     1   APIX,SMIX)
C**************************************************************
...
cCP   IF (IBUG.GT.0) WRITE(IOUT,903) N,IPRINT,IFRZE
      IF (FEWSDEBUG.GT.3) THEN
        WRITE(MESSAGESTRING, 903) N,IPRINT,IFRZE
...
C     PRINT CHANGES IF REQUESTED
      IF (IPRINT.EQ.0) RETURN  !!!!! CP ?????????
      CALL MDYH1(KDA,KHR,MO,ID,IY,IH,NOUTZ,NOUTDS,TZ)
cCP   WRITE(IOUT,900) MO,ID,IY,IH
      WRITE(MESSAGESTRING,900) MO,ID,IY,IH
      call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)
...
cCP   WRITE(IOUT,901) API1,SMI1,BFSC1,BFI1,FI1,FEI1
      WRITE(MESSAGESTRING,901) API1,SMI1,BFSC1,BFI1,FI1,FEI1
      call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)
  901 FORMAT(1H ,38X,'INITIAL VALUES',4F10.2,F10.1,F10.2)
  
cCP   WRITE(IOUT,902) API,SMI,BFSC,BFI,FI,FEI
      WRITE(MESSAGESTRING,902) API,SMI,BFSC,BFI,FI,FEI
      call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)
