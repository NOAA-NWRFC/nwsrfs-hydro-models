...
C
C @PROCESS LVL(77)
C UPDATED BY G WALTERS NWRFC FOR AUTOCALIBRATION TOOL NOV 2021
CGW      SUBROUTINE EX57 (P,C,T,PE,QNAT,QADJ,QDIV,QRFIN,QRFOUT,QOL,QCD,CE)
      SUBROUTINE EX57 (NDT,AREA,EFF,MFLOW,IRFSTOR,ACCUM,DECAY,
     1 ETD,QNAT,QADJ,QDIV,QRFIN,QRFOUT,QOL,QCD,CE,RFSTOR)
...
C     RFSTOR(*)  - RETURN FLOW STORAGE ARRAY
...
C     MFLOW     - MINIMUM FLOW
      ...
      INTEGER NDT, I, OPTION
      REAL PI, A, QSUM, QADD
      REAL AREA, EFF, MFLOW, IRFSTOR, ACCUM, DECAY
CGW      REAL * 4  P(*),C(*),T(*),PE(*),QNAT(*),QADJ(*)
      REAL * 4  ETD(NDT),QNAT(NDT)
      REAL * 4  QADJ(NDT),QDIV(NDT),QRFIN(NDT),QRFOUT(NDT)
CGW      REAL * 4  QDIV(*),QRFIN(*),QRFOUT(*),QOL(*),QCD(*)
      REAL * 4  QOL(NDT),QCD(NDT),CE(NDT),RFSTOR(NDT)
Cf2py intent(out) QADJ,QDIV,QRFIN,QRFOUT,QOL,QCD,CE
CGW      REAL * 4  CTEMP(1)
CGW      REAL K
CGW      REAL PI,RLAT,DEC,SSANGLE,H,HYEAR
CGW      REAL MATC,MATF,PERC
CGW      REAL A,AREA,EFF,ACCUM,DECAY,QSUM,QADD
CGW      REAL RFSTOR,MFLOW
CGW   CHARACTER *5 IDIMI
CGW      CHARACTER *5 IUNIT

CGW      INTEGER OPTION,W,X,Y,Z,DAY,LDAY,YDAY,BASE
...
CGW      COMMON /IONUM/IN,ITSDATA,IPR,IPU

C     TIMING INFORMATION COMMON

CGW      COMMON /FCTIME/IDARUN,IHRRUN,LDARUN,LHRRUN,LDACPD,LHRCPD,NOW(5),
CGW     +               LOCAL,NOUTZ,NOUTDS,NLSTZ,IDA,IHR,LDA,LHR,IDADAT
...
CGW      include 'flogm'
CGW      COMMON /FCARY/IFILLC,NCSTOR,ICDAY(20),ICHOUR(20)
...
CGW     .$Source: /fs/hseb/ob72/rfc/ofs/src/fcst_ex/RCS/ex57.f,v $
CGW     . $',                                                             '
CGW     .$Id: ex57.f,v 1.2 2004/02/05 19:46:22 edwin Exp $
CGW     . $' /
C    ===================================================================

C     CHECK THE TRACE LEVEL AND WHETHER DEBUG OUTPUT IS NEEDED
CSR   CALL FPRBUG ('EX57    ',1,57,FEWSDEBUG)
...
CGW         RLAT = P(51)*2*PI/360
CGW         HYEAR = P(57)

C  DEBUG OUTPUT FOR THE FLOW COMPUTATIONS

CGW       IF (FEWSDEBUG.GE.1) THEN
CGW          WRITE (MESSAGESTRING,1003)
CGW          call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)
CGW 1003       FORMAT (/,' EX57: CROP ET: OPTION 0: DATE',8X,'DAY',2X,
CGW     +              '6HR MATs(4) ',16X,'MATC',3X,'MATF',2X,'PERC',
CGW     +              2X,'K',4X,'CE')
CGW      END IF
...
CGW     call getDimensionAndUnitInFortran('SQME ', IDIMI, IUNIT)
...
CGW       BASE = KDA + 1
CGW            LDAY = MOD(BASE,1461)
CGW            IF (LDAY.GT.366) THEN
CGW               YDAY = LDAY - 366
CGW            ELSE
CGW               YDAY = LDAY
CGW            ENDIF
CGW            DAY = MOD(YDAY,365)
CGW            IF (DAY.EQ.0) THEN
CGW               DAY = 365
CGW            ENDIF

C  CALCULATE DAILY PERCENTAGE OF ANNUAL DAYTIME HOURS -- H(1) = H(366)

CGW            DEC = 0.4093*SIN(2*PI*(284+DAY)/365)
CGW            SSANGLE = ACOS(-1*TAN(RLAT)*TAN(DEC))
CGW            H = SSANGLE*24/PI
CGW            PERC = 100*H/HYEAR

C  SCS BLANEY CRIDDLE ET ESTIMATION EQUATION

CGW            IF (FEWSDEBUG.GE.1) THEN
CGW          WRITE (MESSAGESTRING,*) 'DAY IS' ,DAY
CGW          call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)
CGW       ENDIF
CGW       K = P(DAY+57)
CGW            CE(I) = 25.4*K*MATF*PERC/100

C  DEBUG OUTPUT FOR CROP ET COMPUTATIONS USING TEMPERATURE INPUT

CGW   IF (FEWSDEBUG.GE.1) THEN
CGW       CALL MDYH1(KDA,KHR,ILM,ILD,ILY,ILH,NOUTZ,NOUTDS,NTZCD)
CGW       WRITE (MESSAGESTRING,1004) ILM,ILD,ILY,DAY,T(W),T(X),
CGW     +                  T(Y),T(Z),MATC,MATF,PERC,K,CE(I)
CGW       call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)
CGW 1004       FORMAT (' EX57: CROP ET: OPTION 0: ',I2,I3,2I5,6F7.2,
CGW     +                 2F5.2,F6.2)
CGW   END IF
...
CGW   IF (FEWSDEBUG.GE.1) THEN
CGW      WRITE (MESSAGESTRING,1005)
CGW      call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)
CGW 1005      FORMAT (' EX57: CROP ET: OPTION 0: ',/,
CGW     +              30X,'DATE        - Month  Day  Year',/,
CGW     +              30X,'DAY         - Day of the Year (1-365)',/,
CGW     +              30X,'6HR MATs(4) - Input temperatures (deg C)',/,
CGW     +              30X,'MATC        - Average temperature (deg C)',/,
CGW     +              30X,'MATF        - Average temperature (deg F)',/,
CGW     +              30X,'PERC        - Daily percent annual daylight ',
CGW     +              'hours',/,
CGW     +              30X,'K           - Daily empirical coefficient',/,
CGW     +              30X,'CE          - Crop evapotranspiration (mm)',/)
CGW   END IF
      ELSE
CGW      write(*,*) 'IN ELSE'
...
CGW         IF (FEWSDEBUG.GE.1) THEN
CGW           WRITE (MESSAGESTRING,1006)
CGW      call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)
CGW 1006      FORMAT (' EX57: CROP ET: OPTION 1: DATE',8X,'DAY',2X,
CGW     +              'K',4X,'PE',4X,'CE')
CGW         END IF
...
CGW   IF (FEWSDEBUG.GE.1) THEN
CGW      CALL MDYH1(KDA,KHR,ILM,ILD,ILY,ILH,NOUTZ,NOUTDS,NTZCD)
CGW      WRITE (MESSAGESTRING,1007) ILM,ILD,ILY,DAY,K,PE(I),CE(I)
CGW      call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)
CGW 1007      FORMAT (' EX57: CROP ET: OPTION 1: ',I2,I3,2I5,F5.2,2F6.2)
CGW   END IF
...
CGW         IF (FEWSDEBUG.GE.1) THEN
CGW            WRITE (MESSAGESTRING,1008)
CGW       call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)
CGW 1008       FORMAT (' EX57: CROP ET: OPTION 1: ',/,
CGW     +              30X,'DATE   - Month  Day  Year',/,
CGW     +              30X,'DAY    - Day of the Year (1-365)',/,
CGW     +              30X,'K      - Daily empirical coefficient',/,
CGW     +              30X,'PE     - Input potential ',
CGW     +              'evapotranspiration (mm)',/,
CGW     +              30X,'CE     - Crop evapotranspiration (mm)',/)
CGW         END IF
CGW      ENDIF
...
CGW      IF (FEWSDEBUG.GE.1) THEN
CGW         WRITE (MESSAGESTRING,1009)
CGW    call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)
CGW 1009    FORMAT (/,' EX57:   FLOWS: DATE',10X,'CE',5X,'QCD',4X,'QDIV',
CGW     +           4X,'QSUM',4X,'QNAT',4X,'QADJ',4X,'QRFIN',3X,
CGW     +           'QRFOUT',3X,'QOL',4X,'RFSTOR')
CGW      END IF

C  LOOP TO PERFORM PRIMARY CONSUMPTIVE USE CALCULATIONS
CGW         write(*,*) 'STARTING DO LOOP'
CGW      DO I = ITS,LTS
         DO I = 1,NDT
CGW             write(*,*) 'I',I
             CE(I)=ETD(I)
             QCD(I) = CE(I)*AREA*A
             QDIV(I) = QCD(I)/EFF
             QRFOUT(I) = RFSTOR(I)*AREA*A*DECAY
...
C         CHECK TO SEE IF WATER IS AVAILABLE FOR DIVERSION
             ...
             QADJ(I) = QSUM-QDIV(I)
             IF (I.LT.NDT) THEN
                RFSTOR(I+1) = ((RFSTOR(I)*AREA*A)+QRFIN(I)
     +                         -QRFOUT(I))/(AREA*A)
             END IF
...
CGW         IF (IFILLC.EQ.1 .and. ncstor .ne. 0) THEN
c         IF (IFILLC.EQ.1) THEN
CGW            IF (KDA.EQ.ICDAY(ICARY).AND.KHR.EQ.ICHOUR(ICARY)) THEN
CSR              CALL FCWTCO (KDA,KHR,CTEMP,NCO)
CGW               ICARY = ICARY + 1
CGW            ENDIF
CGW         ENDIF
...
CGW   IF (FEWSDEBUG.GE.1) THEN
CGW      CALL MDYH1(KDA,KHR,ILM,ILD,ILY,ILH,NOUTZ,NOUTDS,NTZCD)
CGW      WRITE(MESSAGESTRING,1010)ILM,ILD,ILY,CE(I),QCD(I),QDIV(I),
CGW     +     QSUM,QNAT(I),QADJ(I),QRFIN(I),QRFOUT(I),QOL(I),CTEMP(1)
CGW      call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)
CGW 1010      FORMAT (' EX57:   FLOWS: ',I2,I3,I5,10F8.2)
CGW   END IF
...
CGW         KDA = KDA + 1
         END DO
CGW         write(*,*) 'Out of DO LOOP'
C  DEBUG OUTPUT DESCRIPTOR INFORMATION
CGW      IF (FEWSDEBUG.GE.1) THEN
CGW         WRITE (MESSAGESTRING,1011)
CGW    call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)
CGW 1011    FORMAT (' EX57:   FLOWS: ',/,
CGW     +           20X,'DATE    - Month  Day  Year',/,
CGW     +           20X,'CE      - Crop evapotranspiration (mm)',/,
CGW     +           20X,'QCD     - Crop Demand (cmsd)',/,
CGW     +           20X,'QDIV    - Diversion Flow (cmsd)',/,
CGW     +           20X,'QSUM    - Natural + Return Flow Out (cmsd)',/,
CGW     +           20X,'QNAT    - Natural Runoff (cmsd)',/,
CGW     +           20X,'QADJ    - Adjusted Runoff (cmsd)',/,
CGW     +           20X,'QRFIN   - Return Flow In (cmsd)',/,
CGW     +           20X,'QRFOUT  - Return Flow Out (cmsd)',/,
...
CGW      IF (IFILLC.EQ.1) THEN
CGW         C(1) = CTEMP(1)
CGW      ENDIF

CGW      IF (FEWSDEBUG.GE.1) THEN 
CGW   WRITE(MESSAGESTRING,990) ICARY
CGW   call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)
CGW 990  FORMAT (' EX57: ICARY:',I5)
CGW      END IF

CGW 992  IF (ITRACE.GE.1) THEN
CGW   WRITE(MESSAGESTRING,991)
CGW   call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)
CGW      END IF

CGW 991  FORMAT (//,' EX57: EXITED:')
CGW      write(*,*) 'THE END'
      RETURN
      END
