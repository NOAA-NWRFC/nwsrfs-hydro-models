...
CFC      IF (NP.GT.0) WRITE (IODBUG,900) NP,(P(I),I=1,NP)
      ...
      do 908 I=1,NP
CGW         WRITE(MESSAGESTRING,900) NP,P(I)
CGW	call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)
  908 continue
      END IF
CGW  900 FORMAT (' 'I5,' VALUES IN THE P ARRAY:' /
CGW     *   1X,F12.3,2(1X,2A4,1X,A4,1X,F12.3) /
CGW     *   (10(1X,F12.3)))
C   
CFC      IF (NC.EQ.0) WRITE( IODBUG,901) NC,(C(I),I=1,NC)
      IF(FEWSDEBUG .GE. 1) THEN
      do 909 I=1,NC
CGW      WRITE(MESSAGESTRING,901) NC,C(I)
CGW	call logfromfortran(DEBUG_LEVEL,MESSAGESTRING)
  909 continue  
      end if    
CGW  901 FORMAT (' ',I5,' VALUES IN THE C ARRAY' /
CGW     *   (10(1X,F12.3)))
C
      RETURN
C
